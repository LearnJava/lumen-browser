...
- `UA_DEFAULTS` deliberately carries only `html{user-select:none}` (CC-CSS-6) and NO
  `font-family`. The chrome document is laid out by the ordinary engine, so it starts from
  `lumen_layout::ComputedStyle::root()`, whose default family became `serif` in BUG-128 ч.2;
  chrome is insulated from that solely because the asset itself declares
  `body{font-family:var(--font-ui)}` (Inter → -apple-system → Segoe UI → Roboto → sans-serif)
  and every chrome element inherits from `<body>`. A UA pin here would be dead weight — an
  `html` rule loses to the asset's `body` rule. Regenerating the asset without that `body`
  declaration would silently switch all chrome text to Times New Roman; the test
  `ua_defaults_need_no_font_family_because_the_asset_sets_its_own` exists to catch exactly that.
  Note this also means chrome does NOT use the bundled Golos Text face for most of its text —
  the reserved-name path in paint (`font_family: vec![]` → Golos Text) covers the *imperative*
  shell overlays (tooltips, panels, omnibox), not this document.
