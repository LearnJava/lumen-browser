...
- **WebAssembly MVP interpreter** (`crates/js/src/wasm/`, U-4 stage 1). 2026-06-18.
  ...
  - Boundaries (documented): exported `Memory.buffer` is a snapshot copy (no live aliasing — emscripten `HEAP32` pattern incoherent); host imports can't touch instance memory mid-call; no JS-level `SharedArrayBuffer`/`Atomics` objects; `memory.init` / unknown `0xFE` sub-opcodes rejected at decode (graceful `CompileError`).
