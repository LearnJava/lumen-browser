...
- **`new CSSStyleSheet()`/`.replaceSync()`/`.replace()`/`document.adoptedStyleSheets`/`shadowRoot.adoptedStyleSheets`** (CSSOM-5 срез 1, [BUG-897](../bugs/BUG-897-OPEN.md), 2026-09-04) — the write half of CSSOM-1 above.
  ...
  - **No cascade wiring.** `refresh_dynamic_css` (`crates/shell/src/relayout.rs`) rebuilds the page's cascade exclusively from `<style>`/`<link>` DOM text; there is no channel carrying a constructed/adopted sheet's content to the shell, and building one crosses the ADR-016 engine thread (a synchronous cross-thread call from the wrong place already hung the process silently once, BUG-976) — deliberately deferred, not forgotten.
  - Tests: `crates/js/src/dom/tests/v8_cssom5_constructed_stylesheets.rs` (9 tests, `V8JsRuntime` directly).
  - Out of scope: `insertRule`/`deleteRule`, a writable `CSSStyleDeclaration` on a `CSSStyleRule`, `@import`/`@font-face`/`@supports` as `CSSRule`, cascade application of a constructed/adopted sheet.
