...
- **`[Exposed=Worker]` members come from one place: `dom::install_worker_exposed_v8`
  ...
  `ABORT_SHIM`, `STREAMS_SHIM`, `HEADERS_SHIM`, `FETCH_BODY_SHIM`, `FORM_DATA_SHIM`,
  `FILE_API_SHIM`, `WEBSOCKET_SHIM`) plus
  ...
  errors, срез 4). A native that needs a
  ...
  natives come from one builder shared with the page (`install::net::websocket_natives`).
- **A dedicated/shared worker's `fetch()` returns the page's `Response` (WORKER-1 срез 5).**
  `crate::worker_net` registers natives of the page's shape over a per-scope body store —
  `_lumen_worker_net_fetch` leaves the body in a "last body" slot, `_lumen_stream_alloc`/
  `_lumen_fetch_body_*` read it — so `FETCH_BODY_SHIM`'s network factory
  `_lumen_response_from_fetch_cache` works unchanged, and `_lumen_document_base_url` is the
  worker's script URL. Read the body **before** the next `_lumen_worker_net_fetch`: the slot is
  single, exactly like the page's `FetchCache`. The worker `XMLHttpRequest`
  (`shim/worker_net_shim.js`) is still its own, smaller class than the page's `xhr.rs` (that
  one runs on the page-only `_lumen_fetch_async_*` natives); an async request does its
  synchronous network call in a `setTimeout(0)` task, so handlers assigned after `send()` fire.
  The service worker keeps its own `Request`/`Response` (`sw_worker.rs`), evaluated over the
  slice's — their shape is part of the `respondWith` round trip.
- **A worker's sockets are polled by its task loop (WORKER-1 срез 3, BUG-1071).** A
  `WebSocket` event lands on the provider's threads with nothing to wake the worker thread,
  so `worker::run_worker_tasks` pumps `_lumen_pump_websockets()` once per turn (its own
...
- **Fetch API JS shim** (`install_dom_api`, `crates/js/src/dom.rs`). 2026-05-22.
  ...
  - **TRUSTEDTYPES-1 срез 1 (2026-09-12):** first enforced sink — `setTimeout`/`setInterval` string handlers (TT L2 §4.1.1 "Get Trusted Type compliant string", script subset). `globalThis._lumen_tt_get_compliant_script(input, sink)` unwraps an already-`TrustedScript` value as-is; otherwise, when the shell has flipped `_lumen_tt_set_require_script(true)` (document CSP carries `require-trusted-types-for 'script'` — `crates/shell/src/scripts.rs`, pushed once per navigation, same spot as the `parse_time_layout`/stylesheet snapshots), routes the stringified value through `defaultPolicy.createScript(value, 'TrustedScript', sink)` or throws `TypeError` when there is no default policy. Without the CSP directive the value passes through unchanged (existing Phase-0 behaviour for pages that never opt in). Called from `_lumen_timer_string_handler` (`crates/js/src/shim/web_api_shim_mid_b.js`) synchronously at `setTimeout`/`setInterval` call time — matches HTML LS's synchronous timer-initialisation steps while keeping the lazy fire-time compile from BUG-831. Verified against WPT `trusted-types/Window-setTimeout-setInterval.html` and `block-string-assignment-to-Window-setTimeout-setInterval.html` (both declare `require-trusted-types-for 'script'` via `<meta>`) with a live-window probe. Every other TT L2 §4.4 sink (`innerHTML`/`outerHTML`, `Range.createContextualFragment`, `document.write`/`writeln`, `<script>.src`/`.textContent`, `eval`/`new Function`, event-handler attributes) is still unenforced — see `bugs/BUG-811-FIXED.md` срез 3. 4 new unit tests (`dom/tests/v8_trusted_types.rs`).
- **Web Storage API** (`install_dom_api`, `crates/js/src/dom.rs`). 2026-05-25.
  ...
  - `_lumen_make_storage` JS factory + `localStorage`/`sessionStorage` globals in shim.
  - **BUG-773 ([P1] 2026-08-24) — a real WebIDL legacy platform object.** The factory used to return a plain object literal with five own methods and a `length` accessor, so `storage.foo = 'x'` created an ordinary JS property on the wrapper: invisible to `getItem`/`length`/`key()` and lost on the next page load. There is now a global `Storage` interface (non-constructible; operations and the `length` accessor on a shared `Storage.prototype`, `Symbol.toStringTag`), and each area is a `Proxy` whose target's `[[Prototype]]` is that prototype. Three things worth knowing before touching it: (1) *visibility* — `Storage` has no `[LegacyOverrideBuiltIns]`, so a name the object or its prototype chain already answers **hides** the same-named storage key, which is exactly what keeps `storage.length`/`storage.clear` working after `setItem('length', …)`; (2) the named **setter** is not subject to that hiding — `storage[k] = v` reaches `setItem` for every string name, and `set.window.js` asserts a same-named setter on the prototype is never invoked, so `get` and `set` are deliberately asymmetric; (3) `Object.defineProperty` with a data descriptor is the same setter spelled a third way, while an accessor descriptor is rejected. The object→natives binding is a `WeakMap` (`_lumen_storage_impl`) rather than a field, because any own property would both be page-visible and shadow a storage key of that name; `ownKeys` appends `Reflect.ownKeys(target)` so a non-configurable symbol property a page defined does not trip the Proxy invariant. Only *string* names go through the hooks — a symbol stays an ordinary own property. Category `webstorage` went 63/1270 → 1229/1277 subtests; 14 unit tests, one per WPT file. Not fixed by it: a lone surrogate in a key or value still becomes `U+FFFD` crossing into Rust ([BUG-901](../bugs/BUG-901-OPEN.md)).
...
- **URL / URLSearchParams / performance / queueMicrotask** (`crates/js/src/dom.rs`). 2026-05-25.
  ...
  - `performance` (W3C HR Time L2): `now()` (DOMHighResTimeStamp, time origin captured at `install_dom` call), `timeOrigin`, stub `mark/measure/getEntriesByName/getEntriesByType/clearMarks/clearMeasures`. Exposed on `window.performance`. **BUG-400 ([P3] 2026-08-11):** was a flat object literal; now a real `interface Performance : EventTarget` (HR Time L3 §4) — non-constructible interface object on `window.Performance`, operations on `Performance.prototype`, singleton built with `Object.create` + `EventTarget.call`, `timeOrigin` a getter-only accessor. Moving the operations off the instance is what makes the WebIDL default `toJSON()` correct: it must serialise attributes only, and an instance with no own enumerable properties can only report `{timeOrigin}`. The legacy Navigation Timing attributes `timing`/`navigation` (`PerformanceTiming`/`PerformanceNavigation`) are real since [BUG-767](../bugs/BUG-767-FIXED.md) (fixed 2026-09-07, unblocked by [BUG-640](../bugs/BUG-640-FIXED.md)): both are getters on `Performance.prototype` (`performance_shim.js`) derived from the same navigation-milestone snapshot the L2 `PerformanceNavigationTiming` entry carries (`_perf_last_navigation_entry`, one conversion function `_perf_l2_to_l1` rather than a second source), so they are real where that snapshot is real and honestly `0`/`TYPE_NAVIGATE` where it is honestly stubbed. `Performance.prototype.toJSON` now includes both.
...
- **Async setTimeout / setInterval / clearTimeout / clearInterval + scheduler.postTask** (`crates/js/src/dom.rs`). 2026-05-25.
  ...
  - JS timer queue (`_lumen_timers`) — plain JS array `{id, fn, deadline, interval}`. `setTimeout`/`setInterval` append; `clearTimeout`/`clearInterval` splice; `_lumen_tick_timers()` drains expired entries, reschedules intervals, runs callbacks, and calls `_lumen_request_wakeup` for the next timer. **BUG-831 ([P1] 2026-08-25):** a handler that is not a `Function` is no longer thrown away (both entry points opened with `if (typeof fn !== 'function') return 0;`, which also handed back the id the spec reserves for «no timer created»). `_lumen_timer_string_handler` wraps it in a closure that runs `(0, eval)(src)` when the timer fires — **indirect** eval, because HTML LS §8.6 wants a *classic script*, i.e. global scope: `new Function(src)` would lose a `var` the string declares and a direct `eval(src)` would evaluate the code inside `setTimeout`'s own scope. Compilation happens per firing, so an interval recompiles each tick and a broken string queues normally and fails inside the timer loop (reported via `_lumen_report_exception`, BUG-591). The same hole existed in `worker.rs`'s `WORKER_TIMERS_SHIM::_schedule` and is closed the same way — §8.6 is `WindowOrWorkerGlobalScope` — with `args` emptied there, since §8.6 step 8 hands the trailing arguments to a Function handler only. **BUG-909 ([P3] 2026-09-19):** the page queue now carries `args` too (`Array.prototype.slice.call(arguments, 2)`, empty for a string handler), the interval-record rebuild in `_lumen_tick_timers` copies it forward, and the callback runs via `fn.apply(globalThis, args)` — mirroring `WORKER_TIMERS_SHIM`, which already did this. Still open around it: `requestAnimationFrame` keeps the silent `return 0` where WebIDL wants a `TypeError`; the service-worker stub (`sw_worker.rs`) calls `fn()` synchronously, so a string throws there.
  - **BUG-847 ([P1] 2026-08-25) — both arguments are WebIDL `long`, and neither was converted.** `var ms = (typeof delay === 'number' && delay > 0) ? delay : 0` implemented neither half of the conversion §8.6 declares (ToNumber, then ToInt32, *then* step 5 «negative → 0»), so it was wrong for four separate inputs, not one: `Math.pow(2, 32)` → 49 days, `Math.pow(2, 31)` → 24.8 days (the same modulo from the negative side), `Infinity` → an unreachable deadline, and any delay that was not already a number (`'100'`, an object with a `valueOf`) → 0, i.e. fired immediately instead of being converted. `_lumen_timer_delay` is the conversion, and it is `WORKER_TIMERS_SHIM::_toDelay` verbatim — §8.6 is `WindowOrWorkerGlobalScope`, and the shims drifting apart is what BUG-831 was. The *handle* of `clearTimeout`/`clearInterval` is a `long` too and was compared raw on **both** shims, so `clearTimeout(String(id))` cancelled nothing; `_lumen_timer_handle` (page) and an inline `Number(id) | 0` (worker, which also covers `SharedWorkerGlobalScope`) fix that. `cancelAnimationFrame` had done `id | 0` all along — the trick existed in the file, twenty lines below, and had simply never been carried up.
  ...
  - `scheduler` (W3C Prioritized Task Scheduling API): superseded — the live implementation is `crates/js/src/scheduler.rs` (installed after `WEB_API_SHIM`, overwrites this stub). BUG-665 (2026-09-26): a single priority queue drained one task per `_lumen_timers` engine task, scheduling state carried in V8 continuation-preserved embedder data (`yield()` inherits across `await`/`queueMicrotask`), `TaskSignal.any()`; see the module docs.
...
- **MutationObserver / ResizeObserver / IntersectionObserver + getBoundingClientRect** (`crates/js/src/dom.rs`). 2026-05-26.
  ...
  - `MutationObserver` (WHATWG DOM §4.3.2): `observe(target, options)` with full options normalization (`childList`, `attributes`, `attributeFilter`, `attributeOldValue`, `characterData`, `characterDataOldValue`, `subtree`); `disconnect()`; `takeRecords()`. `_mo_notify(nid, type, ...)` fires from primitive wrappers, delivers via `_lumen_flush_mutation_observers()` (sync) and `queueMicrotask` (async production path). **BUG-317 ([P3] 2026-07-20):** `MutationRecord` exposed as a non-constructible interface global (top-level `function` + `window.MutationRecord`, BUG-314 pattern); each record built in `_mo_notify` gets `MutationRecord.prototype` via `Object.setPrototypeOf` so `record instanceof MutationRecord` holds (DOM §4.3.3). **BUG-318 ([P3] 2026-07-20):** record accounting made spec-correct — `observe()` re-registers the observer in `_mo_observers` after a `disconnect()` (only the constructor did before); `subtree:true` is scoped via `_lumen_mo_in_subtree` (parent-chain walk) instead of matching every document mutation; `element.textContent` emits a `childList` record (removed old children + added text node) not `characterData`; live text nodes gained `.data`/`.nodeValue` setters routing through `_lumen_set_text_content` (→ `characterData` with `oldValue`); each record's `target` is the mutated node, `addedNodes`/`removedNodes` are node wrappers, `attributeNamespace` present. **BUG-827 ([P1] 2026-08-25):** nodes the *parser* wrote produce records too, and the mechanism is a replay rather than a hook — there is no streaming parser to hook into (the shell parses the whole document, then walks the finished tree, then runs the scripts), so `ParserInsertLog` in `crates/shell/src/main.rs` snapshots the `(parent, child)` pairs in tree order before the document moves into its `Arc`, cuts them at each executable classic `<script>`'s own subtree end, and hands each segment to `_lumen_mo_parser_inserted(pairs)` right before that script's `eval` — which feeds them through the same `_mo_notify` `appendChild` uses. The tail goes out in one segment after the last classic script (deferred modules run when parsing is over, HTML LS §8.1.3.1). `flush_parser_inserts` gates on a `_lumen_mo_observing()` round-trip, so a page with no observer pays one tree walk and never sees the argument built — a record queued before `observe()` is dropped by spec anyway. Known deviations from a real parser: delivery lands *before* the script closing the segment (the microtask checkpoint is the end of the segment's own `eval`), and a text node arrives as one whole `childList` record instead of a stream of `characterData` ones.
  - `ResizeObserver` (W3C): `observe(target, {box})`, `unobserve(target)`, `disconnect()`. `_lumen_deliver_resize_observers()` delivers only if the observed box changed by >0.5 px, or if the observation has never been reported. Shell calls it after `relayout_page`. **BUG-661 ([P1] 2026-08-23):** the relayout call is no longer the only trigger — an observation that has never been reported schedules its own delivery pass as an event-loop task (`_ro_initial_pass`, written straight into `_lumen_timers` with `nesting: 0`, the BUG-842 pattern), so an `observe()` on a page that then sits still still gets its spec-guaranteed first notification. The pass waits (bounded by `_RO_INITIAL_MAX_ATTEMPTS`) until `_lumen_layout_published()` sees a rect on `documentElement` (named `_ro_layout_published` until BUG-807 gave it a second caller), because a parse-time `observe()` runs before the shell's first snapshot, where every element reads back as «no box» and a 0×0 first entry would be *wrong* rather than merely missing. Same slice: `observe()` throws `TypeError` on a non-`Element` (`nodeType !== 1`, so `document` and text nodes are caught too) and the constructor on a non-function; `contentBoxSize`/`contentRect` are the real content box (border box − border widths − padding, read from the published computed styles by `_ro_content_geometry`, with `contentRect.x/y` the padding offset inside the border box per §5.1) while `borderBoxSize` keeps the border box and `devicePixelContentBoxSize` scales by `devicePixelRatio`; and detaching an observed element resets its last reported size, hooked by wrapping the single `_lumen_remove_child` native **by assignment** (a hoisted `function` declaration would overwrite the native before the alias is taken and recurse), which covers every removal in the shim including the implicit one inside a reparenting `appendChild`.
  - `IntersectionObserver` (WICG): `observe(target)`, `unobserve(target)`, `disconnect()`. `_lumen_deliver_intersection_observers()` intersects element rect with root expanded by `rootMargin` (`_parse_root_margin` supports `px` shorthand 1–4 values), delivers `IntersectionObserverEntry` instances with threshold crossing semantics — **BUG-1131:** the interface is global (`window.IntersectionObserverEntry`, WebIDL constructor); delivery builds entries with `Object.create(IntersectionObserverEntry.prototype)` + `_io_entry_slot`, fields in the non-enumerable `_ioe` slot behind readonly prototype getters, rects are `DOMRectReadOnly` (`_io_dom_rect`). Shell calls it after `relayout_page`. **BUG-807 ([P1] 2026-08-24):** and, as for `ResizeObserver` one entry above, that call is no longer the only trigger — `observe()` queues the initial notification Intersection Observer §3.2 requires (`_io_initial_pass` in `_lumen_timers`, `nesting: 0`), so an observe-and-wait page no longer depends on someone else dirtying the layout. The wait-for-the-first-snapshot condition is shared with `ResizeObserver`, hence `_ro_layout_published` → `_lumen_layout_published`. The delivery loop no longer skips a target with no box outright: §3.2.1 reports such a target as an empty box with `isIntersecting: false`, and §3.2 makes the *first* notification unconditional, so the skip now applies only once the target has been reported at least once (a box that existed and disappeared is delivery *content* — BUG-626/627/628 — not this bug). Argument validation stays deliberately absent: the internal lazy-image observer observes a `{__nid__}` proxy rather than an `Element`, so an §3.2 `TypeError` would break `loading=lazy` (BUG-626).
  - `Selection` (Selection API §3, `web_api_shim_mid.js`) — **BUG-671:** a global interface (`new Selection()` → `TypeError`, `Symbol.toStringTag` `"Selection"`); the document selection `_lumen_selection` is `Object.create(Selection.prototype)`, its attributes/operations sit on the prototype, brand-checked against that singleton, with IDL `length`s (`setBaseAndExtent` 4, `collapse`/`extend`/… 1) and the `setPosition` alias of `collapse`. Not done: `direction`, `containsNode`/`getComposedRanges`/`modify` stay stubs, `removeRange` does not throw `NotFoundError`.
...
- **IndexedDB (W3C Indexed Database API 3.0)** (`WEB_API_SHIM`, `crates/js/src/dom.rs`). 2026-05-29.
  ...
  - Event delivery: `success`/`error`/`upgradeneeded`/`complete`/`abort` fire via `_lumen_idb_flush()`, scheduled by `queueMicrotask` and callable directly by tests and by the service-worker scope (mirrors the raf / MutationObserver pattern). An unhandled request error (no `preventDefault`) aborts its transaction.
  - **Budgeted flush (2026-08-23, BUG-842):** one flush dispatches at most `_IDB_FLUSH_BUDGET` (1024) requests, counting the version change transaction inside `_idb_process_open`; a transaction that outruns the budget stays *unfinished* and resumes in the next event-loop turn, which is what keeps WPT's `keep_alive` (a request re-armed from its own `onsuccess`) from starving the page. Two non-obvious halves: the continuation is written straight into `_lumen_timers` with `nesting: 0` rather than scheduled via `setTimeout`, because the §8.6 4 ms clamp (BUG-271) would otherwise throttle a long cursor walk to 250 steps/s; and `_idb_schedule_flush` is a no-op while `_idb_flushing`, because a re-armed request would otherwise chain a microtask from inside a microtask and never return to the loop, budget or no budget. `_idb_has_task_queue()` false (service-worker scope, whose `setTimeout` stub runs synchronously) → the drain keeps its original unbounded form with the `1e6` backstop.
  ...
  - **Transaction lifetime (2026-08-25, [BUG-841](../bugs/BUG-841-FIXED.md)):** a transaction is queued by `IDBDatabase.prototype.transaction` itself, not by its first request — §3.1.7 commits a transaction as soon as control returns to the event loop with nothing of its own left, so an *empty* one must commit too, and queueing at creation is also what makes `_idb_active_txns` order commits by creation rather than by first request. Two flags where there used to be one: `_finished` is the spec state, set **synchronously** by `abort()`/`commit()` (so `objectStore()`, a new request and a second `abort()` all refuse at once), while `_settled` records that the terminal event has been delivered — `_idb_flush_txn` keys its early-out on `_settled`, and `_idb_process_open`, which settles a versionchange transaction in its own code, sets it too, or an `abort()` inside `onupgradeneeded` would fire the terminal event twice. A request still queued when the abort lands is settled by `_idb_abort_txn_requests` (§3.4.5 step 3: `AbortError` + `error` event, `readyState` `done`) instead of being dropped — deliberately not through `_idb_dispatch_request`, whose job includes running the action and re-aborting on an unhandled error. `commit()` exists; `mode` goes through the IDBTransactionMode enum check before every algorithm step (WebIDL converts first) while `'versionchange'` is refused *after* the store-name `NotFoundError` (§3.3.4 step 5); `durability` is accepted and reflected. Not covered: `error`/`abort` do not bubble to `IDBTransaction`/`IDBDatabase` ([BUG-914](../bugs/BUG-914-OPEN.md)).
  - **Connection queues (2026-08-25, [BUG-843](../bugs/BUG-843-FIXED.md)):** `_idb_connections` (name → live `IDBDatabase`s) is the registry an upgrade or a delete broadcasts `versionchange` to; while any of them is still open the request fires `blocked` at itself and is **parked** (`_idb_park_open`/`_idb_unpark`) together with the whole rest of that name's queue, since §3.3.1 processes a queue in order. Parking rather than re-queueing is deliberate: putting the request back into `_idb_pending_opens` would cost an event-loop turn per spin, i.e. the unbounded drain BUG-842 fixed — only `close()` can wake it. Everything an open/delete decides (version comparison, `VersionError`, the `IDBDatabase`, the deletion itself) is now resolved when the queue reaches the request, not when it was called, or a request queued behind a delete would see a database that no longer exists. `close()` is «close pending» (§3.3.9): it refuses further transactions at once but keeps blocking until the connection's own transactions have settled, and the connection of an aborted version change transaction is closed too — it is never handed to the page. Cost measured on the WPT category: a page that leaks a connection (because of unrelated pre-existing defects) now blocks forever where it used to barge through, and every dirty flush re-serializes a database set that no longer shrinks ([BUG-917](../bugs/BUG-917-FIXED.md), fixed).
  - **Abort reverts (2026-08-25, [BUG-913](../bugs/BUG-913-FIXED.md)):** `_idb_txn_snapshot`/`_idb_revert_txn` — an undo snapshot taken once per transaction, restored before the requests are settled so a handler already reads the reverted database. Three shapes worth knowing: the snapshot is restored **into the same store object** (`_idb_restore_store` writes its fields in place) because every `IDBObjectStore`/`IDBIndex` the page holds keeps a direct reference to it, and a deleted store therefore comes back by its original reference; record wrappers are cloned while the stored values are shared, since `_write` assigns into an existing wrapper's `value`; and a versionchange transaction snapshots **eagerly** in `_idb_process_open`, before the version is bumped, while an ordinary one snapshots at its first request (schema mutations bypass the request queue and take it themselves). The same path serves the emergency abort on an unhandled request error, so a half-applied batch is rolled back too. Fixed with it: an aborted versionchange used to answer the open request with `success` — it now fails with `AbortError` (§3.3.1).
  - 33 tests (open+upgrade, keyPath/autoIncrement CRUD, put-overwrite, duplicate→abort, getAll ordering + key range, delete/clear, index get/getAll, unique-index violation, cursor forward/reverse/update/delete, IDBKeyRange.includes, cmp, version downgrade error, deleteDatabase, second-connection persistence; + persistence: reload round-trip, version restore, Date round-trip, delete-database restore, read-only no-persist; + BUG-842: a self-rearming request yields between turns; + BUG-841: empty transaction commits, creation-order commit of three transactions, synchronous finish after `abort()`, queued request settled with `AbortError`, `commit()` refusing further requests, invalid mode/durability; + BUG-913: abort reverts writes, key generator, upgrade schema+version).
...
- **Functional WebGL context** (`crates/js/src/webgl_canvas.rs`, §7F, task #28). 2026-06-02.
  ...
  - **Present framebuffer into the page `<canvas>` (ph3-webgl2 срез 5, 2026-09-21).** `CONTEXTS` now stores a `WebGlEntry { gl, nid }` — `nid` is the backing `<canvas>` element's `__nid__` (`-1`/absent → `None`, e.g. contexts built without a DOM node in tests), captured once at `getContext('webgl'|'webgl2')` time and passed through the new 3-arg `_lumen_webgl_create(nid, w, h)`. `_lumen_webgl_clear`/`_lumen_webgl_draw_arrays` call `present(id)` after mutating the framebuffer: `present` flips the buffer from WebGL's bottom-left to top-left origin (`flip_rows_rgba`, same flip `readPixels` already did per-rect) and forwards it to `canvas2d::present_rgba(nid, w, h, rgba)` — the exact hook WebGPU's `_lumen_webgpu_canvas_present` already uses, so a WebGL canvas now composites as `canvas:{nid}` like every other canvas source. Residual: this exposes the same unnoised-pixel gap `readPixels` already had to `drawImage(webglCanvas, …)` too (`CanvasNoiseGenerator` still isn't wired into the software GL path — see CAPABILITIES.md). 2 new tests (`clear_presents_to_page_canvas`, `context_without_nid_does_not_present`) — the assertion must call `V8JsRuntime::flush_canvas_updates()`, not `canvas2d::flush_dirty()` directly: the natives run on the runtime's JS thread, which owns its own `thread_local!` `CANVASES`/`DIRTY` instance distinct from the test's calling thread.
  - **Parser-built `<canvas>` never got a functional WebGL context (found ph3-webgl2 срез 6, 2026-09-21, writing the first real graphic-test page).** Unlike Canvas 2D — whose `getContext('2d')` lives on `HTMLCanvasElement.prototype` (`web_api_shim_mid.js`) and therefore works for any canvas regardless of how it was created — the WebGL surface is added per-*instance* by `_addCanvasStubs`, wired only through a `document.createElement` interception (line above). A `<canvas>` straight out of the HTML source never goes through `document.createElement`, so its `getContext('webgl'|'webgl2')` fell through to the prototype method, which only knows `'2d'`/`'bitmaprenderer'`/`'webgpu'` and returned `null` — every prior WebGL unit test built its canvas with `document.createElement`, so this never surfaced. Fixed by having `install_webgl_canvas_v8` additionally walk `document.getElementsByTagName('canvas')` once, at install time — `install_dom` always builds the DOM-core `document` from the already-parsed tree before this install runs, so every canvas already in the page exists as a real element by then. Test: `preexisting_parser_canvas_gets_webgl_stub`.
  - **`gl.TEXTURE_2D`/`gl.TEXTURE0..7` were never defined on the context object (found same session).** `bindTexture`/`activeTexture`/`texImage2D` were already correctly forwarded to `SoftwareWebGl`, but nothing named the enums a real page must pass — `gl.bindTexture(gl.TEXTURE_2D, tex)` silently bound nothing (`gl.TEXTURE_2D === undefined`, coerced to `0`, compared against the real `0x0DE1` internally), so a GLSL ES 3.00 fragment shader's `texture(u_tex, …)` always sampled the no-texture fallback instead of the uploaded pixel. Added `TEXTURE_2D`/`TEXTURE0`..`TEXTURE7` (matches the already-declared `MAX_COMBINED_TEXTURE_IMAGE_UNITS: 8`). Test: `named_texture_constants_reach_texture_sampling`.
...
- **Functional Canvas 2D context** (`crates/js/src/canvas2d.rs` + `dom.rs`, HTML LS §4.12.4, task 5A.2). 2026-06-02.
  ...
  - Forwarded surface: `fillRect`/`clearRect`/`strokeRect`, `beginPath`/`moveTo`/`lineTo`/`closePath`/`arc`/`fill`/`stroke`, `rect`/`ellipse` (ellipse≈circle in Phase 0), `fillStyle`/`strokeStyle` (via `CanvasColor::from_css_str`), `lineWidth`/`globalAlpha` (spec-validated ranges), `getImageData` (applies per-session fingerprint noise via `Context2D::get_image_data`). (transforms/text/shadows/gradients/patterns/drawImage/clip were stubs in Phase 0 — wired in later phases; see the dated bullets below). ~~`toDataURL`/`toBlob` stay blank (ADR-007 anti-fingerprint)~~ — **fixed 2026-08-31** ([BUG-454](../bugs/BUG-454-FIXED.md)): they now encode the real (noised) bitmap at its real size, honestly reporting `image/png` for any requested type. The noise generator itself (`lumen_canvas::CanvasNoiseGenerator`) was also redesigned that day from a sequential XOR stream to a positional one (`channel_delta(seed, x, y, channel)`, pure — the same pixel noises identically whatever rectangle read it), and is now armed with a real per-session+origin seed on every 2D context (element canvas, `OffscreenCanvas`, `transferControlToOffscreen`'s snapshot) rather than never being set at all. The seed must be captured by value into the registering native's `move` closure, not read from a `thread_local!` at call time — the installer and the native's actual invocation run on different OS threads in this V8 compat layer; see CLAUDE.md's Known gotchas for the measured proof.
...
- **Canvas 2D object model: five real interfaces instead of five object literals** ([BUG-449](../bugs/BUG-449-FIXED.md), `crates/js/src/shim/web_api_shim_mid.js`). 2026-08-30.
  - `CanvasRenderingContext2D`/`ImageData`/`TextMetrics`/`CanvasGradient`/`CanvasPattern` are global classes; every member sits on the prototype and every factory returns `Object.create(X.prototype)`. The whole per-instance state is one **non-enumerable** slot (`__canvas2d__`, `__image_data__`, `__text_metrics__`, `__gid__`, `__patid__`), which doubles as the WebIDL brand check — that check is not cosmetic: without it `CanvasRenderingContext2D.prototype.createImageData.call(null, …)` would paint through a nid borrowed from a foreign object, and `putImageData` accepted any `{width, height, data}` literal.
  - `new ImageData(…)` implements both WebIDL overloads. The **error type identifies the chosen overload**: argument 0 picks the buffer form only when it is a `Uint8ClampedArray`, so `new ImageData(new Uint8Array(100), 25)` is an `IndexSizeError` for width 0, not a `TypeError` for the wrong buffer; and the `ImageDataSettings` dictionary converts before any constructor step, which is why `new ImageData(self, 4, 4)` is a `TypeError`. `width`/`height`/`data` are readonly getters, `colorSpace`/`pixelFormat` exist, and a `rgba-float16` request is refused aloud rather than backed by an 8-bit buffer.
  - `TextMetrics` reports all twelve IDL attributes from the font, through the new `_lumen_canvas2d_text_metrics` native (`canvas2d.rs`): the ink extents come from glyph bounding boxes (a blank glyph contributes advance only), the vertical ones from `hhea`, the em square from the ascent's share of the em — all measured from the **alignment point** and the `textBaseline` line, using the very baseline table `render_text_to_canvas` draws with.
  - Also landed here: `reset()`, `isContextLost()`, `getContextAttributes()`, `roundRect()` (quarter-ellipse corners, one shared shrink factor, negative extents mirror the rect together with its corners) and the seven text-shaping state attributes, which ignore an out-of-enum value as §4.12.5.1.12 asks. `setTransform()` with no arguments now resets to the identity instead of sending six `NaN` into the native, and `addColorStop` rejects an offset outside `[0, 1]`.
  - 21 regression tests in `dom/tests/v8_core/canvas_object_model.rs` (its own file: `selectors_canvas_window.rs` is 1 874 lines against the 2 000 ceiling).
...
- **HTML Popover API** (`crates/js/src/dom.rs`, WHATWG HTML §6.12). 2026-06-03.
  ...
  - `beforetoggle` / `toggle` events with `oldState`/`newState` fired synchronously on show and hide. Both are real `ToggleEvent` instances since 2026-08-25 ([BUG-578](../bugs/BUG-578-FIXED.md)) — the class lives next to `HashChangeEvent` and is shared with `<details>`; `beforetoggle` is cancelable and cancelling it aborts the show/hide (it used to be `cancelable: false`, so `preventDefault()` there did nothing).
...
- **Frame bridge: `<iframe>` browsing context from JS** (`crates/js/src/frame_bridge.rs`, BUG-480 slices 2–10, P3 2026-08-23…26).
  ...
  - `contentWindow.location` write (GAP-NAVCTX slice 8, BUG-884, 2026-09-13): `.href=`/`.assign()`/`.replace()`/whole-property assignment all forward to `navigateFrameHost(bid, hostNid, url)`, which writes the host's `src` attribute through the same native the element's own `src=` setter uses (`_lumen_f_set_attr` for an ancestor bid, `_lumen_make_element(hostNid).setAttribute` for an ordinary one) — `frame_dynamic.rs::poll_dynamic_frames` picks the change up as `delta.changed` next tick, same path slices 6–7 wired for `.src=`. Compromise: the URL resolves against the HOST'S OWNER document's base (like a plain `src=`), not the reading document's base, so it can diverge from spec when parent and child sit at different bases.
  - **The `frameElem` Element facade has no `.attributes` (`NamedNodeMap`) at all** — reading it off a
    ...
    `TypeError` ([BUG-970](../bugs/BUG-970-FIXED.md)). `getAttribute`/`setAttribute`/`hasAttribute`/
...
- **`<video>` FFmpeg-container queueing landed behind `ffmpeg-video` (GAP-MEDIADECODE срез 6, 2026-09-20), decode still not wired.** `startFetch` in `shim/video_element.js` now tries `startFfmpegLoad` (extension-sniffed `.mp4`/`.webm`/`.ogg`/`.ogv`, mirrors `startGifLoad`'s polling of `__lumen_video_ready`) before falling back to the dedicated media source failure steps. It reuses `VideoGifStore` rather than a new store type — a new `pending_ffmpeg_loads: Mutex<Vec<(u32, String)>>` field, drained by nobody yet — and the same `playback` map/natives (`__lumen_video_play`/`pause`/`seek`/…) once an entry exists, since `VideoPlaybackState` carries no GIF-specific field. New native `__lumen_video_ffmpeg_load(nid, src)` and `canPlayType`'s "maybe" answer for `video/mp4|webm|ogg` are both gated on the new `lumen-js` feature `ffmpeg-video` (marker only, no dependency — `lumen-js` still never links FFmpeg; decode stays shell-side per `VideoGifStore`'s design). Feature is off by default: turning it on without also wiring the shell's decode tick (срез 7, `lumen-media-ffmpeg`'s `FfmpegVideoDecoder`/`tick_video_ffmpegs` mirroring `tick_video_gifs`) makes any `.mp4`/`.webm`/`.ogg` source poll `__lumen_video_ready` forever instead of failing — five pre-existing tests whose assertions were specifically about the no-FFmpeg default (`can_play_type_mp4_empty`, `assigning_src_runs_resource_selection_and_reports_the_failure`, `load_reruns_the_algorithm_with_abort_and_emptied`, `source_children_report_failure_on_the_source_element`, `play_rejects_once_the_resource_is_known_unsupported`) are `#[cfg(not(feature = "ffmpeg-video"))]` for exactly this reason. Tests: `can_play_type_ffmpeg_mimes_maybe_with_feature`, `native_video_ffmpeg_load_registers_pending` (feature-gated on), `can_play_type_webm_ogg_empty_without_feature` (feature-gated off).
...
- **`el.checked` is document-backed too (BUG-444, 2026-08-30) — and the `checked` content attribute is no longer where it lives.** The `checked` accessor pair in `_LUMEN_WRAPPER_MEMBERS` (`web_api_shim_mid.js`) goes through `_lumen_get_dirty_checked`/`_lumen_set_dirty_checked` into `Document::dirty_checkedness`, for the same reason `value` does: checkbox painting, `:checked`/`:indeterminate` matching, form submission and constraint validation all read it from Rust, and a native click writes it from Rust. Consequences inside the shim: `defaultChecked` became one plain reflection row (`['defaultChecked', 'checked', 'bool']`) and the BUG-383 `_lumen_default_checked` snapshot map with its `_lumen_capture_default_checked` helper is **gone** — do not reintroduce a JS-side default-checkedness shadow, it could never see the native-click path that made it necessary. `_lumen_radio_select` unticks siblings via `_lumen_set_dirty_checked(…, false)`, not `_lumen_remove_attr`: writing the attribute on a sibling that already has a dirty flag would be a silent no-op. `form.reset()` clears the entry (`_lumen_clear_dirty_checked`), and so does `_lumen_gc_collect` for a released nid.
- **Cross-boundary GC (`docs/tasks/ph3-gc-js-dom.md`, срезы 1-4, Path A leak sweep, not a tracing cycle collector).** `_lumen_element_wrappers` (`dom.rs`) stores `WeakRef`s, not the wrapper objects themselves — a strong cache would permanently root every wrapper it ever built, so `Document::js_refs` (`lumen-dom`) could never see a real decrement. `_lumen_dom_acquire_ref`/`_lumen_dom_release_ref` natives (`v8_runtime/install/dom_core.rs`) call `Document::acquire_js_ref`/`release_js_ref` from the wrapper-cache get/set path; a `FinalizationRegistry` (`_lumen_node_wrapper_finalizer`, `web_api_shim_mid.js`) calls `release_ref` once V8 actually collects a wrapper. **The `FinalizationRegistry` callback is a `Platform` task, not a microtask** — no `setTimeout`/promise loop runs on `lumen-js`'s dedicated V8 thread to pump that queue, so срез 1/2's refcount wiring alone would have leaked every detached-with-dropped-JS-ref node in a live tab forever (found while building срез 2's test harness; `tc39_proposals.rs`'s `atomics_wait_async_notify_resolves_ok` doc comment independently confirms the same queue sits unpumped elsewhere). `V8JsRuntime::run_gc_pass(level)` (`v8_runtime/runtime.rs`) closes that gap for real code paths: `Isolate::low_memory_notification()` then `Platform::pump_message_loop()`, called from the existing `hibernation.rs` `BackgroundRecent`/`BackgroundOld` tier-transition sites (no new call site added) — `level == 0` (foreground tab) stays a no-op, matching `GcLevel::Soft`'s "never stall a foreground tab" contract. `#[cfg(test)]` `force_gc_for_testing` loops several forced-GC + pump rounds (not the theoretical minimum of one each) to absorb V8's conservative stack scanner occasionally over-retaining an object for one extra pass; `--expose-gc` is gated on `cfg!(test)` (`named_access::apply_v8_test_gc_flag`) so a live page never sees a `gc()` global. See `subsystems/dom.md` for the DOM-side half (`dead_node_ids`/`reclaim_dead_nodes`).
...
- **The five-site checklist above is not hypothetical — `getRootNode` was the third `Node` member found missing on the `document` literal ([BUG-599](../bugs/BUG-599-FIXED.md), 2026-09-09), after `hasChildNodes` (BUG-327) and `compareDocumentPosition` (BUG-732), and it cost more than WPT points: react-dom calls it on the app container, which under a Next.js App Router *is* `document`, so hydration died with `Minified React error #446`. Two things generalise. **(1)** The gap is invisible to `grep "getRootNode"` — the method existed twice (wrappers, `DocumentFragment` literal), so the only reliable check for "does the live `document` answer X" is `typeof document.X` in a running isolate, not a source search; a bug report saying "missing entirely" can be two-thirds wrong. **(2)** A member whose answer depends on the *tree walk* (not just on `this`) needs a third site nobody lists: the walk itself. `_LUMEN_WRAPPER_MEMBERS.getRootNode` climbs `_lumen_get_parent` and returns `_lumen_make_element` for whatever it stops on — but `Document::attach_shadow` allocates the `ShadowRoot` **with no parent** (host→root lives only in the `shadow_roots` map, and the one binding exposed for the reverse direction, `_lumen_get_shadow_root_host`, reads that same absent `parent`), so a node inside a shadow tree stops on the shadow root and gets a plain `Element` wrapper with `host === undefined` instead of a `ShadowRoot`. That half is [BUG-1045](../bugs/BUG-1045-FIXED.md) and is not a one-liner: it needs a nid → shadow-root/host binding first.
- **The live `document`'s children are arena edges — the *opposite* of the detached document — and the walk that reaches them dies one shape further out than the checklist names ([BUG-557](../bugs/BUG-557-FIXED.md), 2026-09-09).** Three things worth carrying forward. **(1)** `_lumen_root_nid` is `lumen_dom::Document::root()`, a real arena node, so `document.removeChild`/`insertBefore`/`replaceChild` are one native call each on that id — do **not** reach for `_lumen_build_detached_document`'s `_children` array (BUG-415 note below): that array exists because a *detached* document has no arena backing for the document→child edge, and mirroring it on the live document would give one tree two disagreeing views. The two shapes look alike and share almost no implementation; that is correct, not duplication to be refactored away. **(2)** The five-site checklist in the BUG-377 note undercounts by one for anything a *walk* touches. Completing the `document` literal was not enough for `for (n = document.firstChild; n; n = n.nextSibling)` — the walk stopped on step one, because in standards mode `document.firstChild` is the **doctype** and `_lumen_make_doctype` had no `nextSibling`. That wrapper is reachable only through the document, so every `typeof document.X` audit reports the document complete while the walk through it is still broken; audit the *first child*, not just the receiver. **(3)** The live `document` still deliberately has **no** `__nid__` — ~60 sites in the shim read `x.__nid__ !== undefined` as "this is an element-shaped node" and would start treating the document as one if it grew one. `TreeWalker`/`NodeIterator` rooted at `document` used to read `root.__nid__` directly and traverse nothing as a result ([BUG-1046](../bugs/BUG-1046-FIXED.md), fixed 2026-09-21 by routing `_root_nid`/`_cur_nid`/`_NodeIterator._ensure` through the existing `_lumen_tree_nid(...)` helper instead of adding the property).
...
- **`<details>` state and the `toggle` event** (`crates/js/src/dom.rs`, HTML LS §4.11.1). 2026-08-25, [BUG-851](../bugs/BUG-851-FIXED.md).
  ...
  - The hook is a pair of wrappers over `_lumen_set_attr`/`_lumen_remove_attr` (the same shape `_mo_notify` uses one thousand lines above), narrowed by `name === 'open' && tag === details`. Writing the attribute through any *new* helper therefore cannot silently bypass the event — which is what went wrong before: the only dispatch site was a `click` listener on `document` that also flipped the attribute, so a scripted `summary.click()` flipped it twice and a script write to `open` reported nothing.
  - `toggle` is a **queued task**, written straight into `_lumen_timers` with `nesting: 0` (the `_ro_schedule_initial`/`_lumen_fire_hashchange` pattern — the §8.6 4 ms clamp is about timer nesting, not about a task the engine queues on the page's behalf). Nothing is delivered by the statement after a write; a unit test must call `_lumen_tick_timers()`.
  - A second change before the task runs **replaces** it rather than queueing another (`_details_toggle_pending` keeps the pending record's `oldState` and takes the new `newState`), so `open = true; open = false` in one turn yields one `closed` → `closed` event.
  - Three paths need their own entry point, and each exists for a reason the others cannot serve: `_lumen_details_open_scan()` runs from `_lumen_apply_ready_state('interactive')` because markup never passes through the write hook (the [BUG-826](../bugs/BUG-826-FIXED.md)/[BUG-838](../bugs/BUG-838-FIXED.md) shape); `_lumen_details_native_toggled(nid, wasOpen)` is called by the shell *after* it has flipped the attribute itself, because the flip must be in the document before the `relayout_form()` that follows (ADR-016 M2.2c-3); `_details_known_open` records the state the machinery has accounted for, which is what keeps the scan from re-reporting an element a script already moved.
  - Related boundary, one function over: `_lumen_activation_target` treats `<a>`/`<area>` as activatable **only with an `href`** — HTML LS §4.6.1 makes an href-less anchor a placeholder, neither a hyperlink nor interactive content — so a click inside `<summary><a>…</a></summary>` reaches the summary. The old document listener walked ancestors itself and never consulted that table, so removing it exposed the gap.
  - Not covered: `<details>` dispatches no `beforetoggle` at all; a `preventDefault()` on a native click does not stop the shell's flip (the native path never reads the dispatch result).

- **`HTMLFrameElement` and its sub-document accessors sit on the prototype, not on the element ([BUG-854](../bugs/BUG-854-FIXED.md), 2026-08-25).** The interface is generated by the same block as `HTMLAreaElement`/`HTMLTrackElement`, `FRAME` is mapped in `_lumen_html_tag_prototypes`, and all eight IDL attributes of HTML LS §16.3.3 are ordinary `_lumen_install_reflection` rows. `contentDocument`/`contentWindow` read the same bridge registry `<iframe>` uses (`frame_bridge.rs`, BUG-480 slice 2) but are defined on `HTMLFrameElement.prototype` rather than patched onto each element the way `iframe_element.rs` does: a `<frame>` written by the parser never passes through a `createElement` hook, so a per-element patch would cover exactly the case that matters least. That structural difference used to have a visible consequence — `frame.src` returned the resolved absolute URL `[ReflectURL]` asks for while `iframe.src` returned the raw attribute, because `iframe_element.rs`'s per-element own property shadowed the (correct) prototype row; fixed by removing the duplicate own property ([BUG-920](../bugs/BUG-920-FIXED.md)).

- **Разбор числового значения содержимого атрибута живёт в ДВУХ зеркалах, и они обязаны отвечать одинаково** (`_lumen_parse_integer`, `crates/js/src/shim/web_api_shim_tail_b.js` ↔ `lumen_dom::attr_int`, `crates/engine/dom/src/attr_int.rs`). 2026-08-30, [BUG-452](../bugs/BUG-452-FIXED.md).
  - Через JS-зеркало читаются `<canvas>`-размеры (`_lumen_canvas_dims`), вся таблица отражения `long`/`ulong` (`_lumen_define_reflection`) и рукописный `tabIndex`; через Rust-зеркало — intrinsic-размер `<canvas>` в layout (`box_tree.rs`). Расхождение видно не как ошибка, а как несогласованная страница: `<canvas width="100.999">` укладывался боксом 300 CSS px и одновременно отвечал `canvas.width === 100`. Поэтому таблица ожиданий вендоренных WPT `2d.canvas.host.size.attributes.*` продублирована в тестах ОБОИХ зеркал — это не дублирование, а единственное, что ловит их дрейф.
  - **Хвост игнорируется, а не отвергается.** §2.4.4.1 шаг 8 собирает ASCII-цифры и возвращает, поэтому `'100em'`/`'100.999'`/`'100#!?'` → 100, а `'0x100'` → **0**. Прежний комментарий в шиме («deliberately stricter than `parseInt`, which would accept '12px'») записывал поведение самой функции как правило — строже `parseInt` здесь и есть дефект. Эталон, с которым стоит сверяться: `tests/wpt/html/dom/reflection.js::ReflectionTests.parseInt` — реализация тех же шагов в самом WPT.
  - **Пропуск пробелов — только ASCII** (TAB/LF/FF/CR/SPACE, Infra §4.6). `String.trim()` и `str::trim()` шире: они едят U+00A0, на котором спека обязана дать ошибку разбора.
  - **Границы диапазона — правило ОТРАЖЕНИЯ (§2.6.2), а не разбора.** Сам разбор верхнего предела не имеет; вне 0…2147483647 (`ulong`) отражается ДЕФОЛТ, а не насыщение. Два следствия: границу надо ставить в каждом месте отражения отдельно (в таблице её не было, и `tabIndex` — рукописный аксессор — мимо неё), и `<div tabindex="2147483648">` при этом остаётся ФОКУСИРУЕМЫМ (атрибут разобрался успешно, флаг фокуса ставит разбор, а не диапазон), поэтому его `tabIndex` — 0, а не −1.
  - **У сеттера и у атрибута правила РАЗНЫЕ.** Аргумент сеттера идёт через приведение WebIDL (`ToNumber` + усечение), значение атрибута — через §2.4.4.1: `el.tabIndex = '3px'` → 0, тогда как `tabindex="3px"` → 3. Разбор строки в сеттере делает их одинаковыми и этим ломает обе стороны.
  - Нативный бэкинг-стор canvas сохраняет свой `clamp(1, MAX_CANVAS_DIM)` (`canvas2d.rs`) — нулевой холст легален и просто не имеет пикселей, но растеризатор нулевого протяжения не видит. Это деталь аллокации, и она **не должна** быть наблюдаемой из JS; клампы `< 1` в `_lumen_canvas_dims` были попыткой выразить её на уровне IDL и стоили `width="-100"` ответа `1` вместо дефолта.

- **`document.styleSheets`/`<style>`/`<link>.sheet`/`CSSStyleSheet`/`CSSRule`-иерархия, только чтение** (CSSOM-1, `docs/tasks/p1-cssom-1-stylesheets.md`, 3 среза, 2026-09-03; closes the read half of [BUG-471](../bugs/BUG-471-FIXED.md)/[BUG-746](../bugs/BUG-746-FIXED.md)).
  - `V8JsRuntime::stylesheet_nodes: Arc<Mutex<Vec<lumen_css_parser::StylesheetNodeEntry>>>` mirrors the `computed_styles` pattern: `update_stylesheet_nodes()` fills it in `run_scripts_with_dom` right after `install_dom` (before the first script, BUG-443 order) and again when the cascade rebuilds post-script. Frames/bfcache/docking/hibernation get an empty registry, same fallback `LayoutSource::stylesheet_nodes` already uses.
  - The registry type is canonical in `lumen_css_parser` (not shell-local as the срез 2 plan assumed) — `crates/js` cannot depend on `crates/shell` (layering), and `lumen-css-parser` is the shared sibling of both.
  - Rust natives live in `crates/js/src/v8_runtime/install/stylesheets.rs` (6 natives: `_lumen_stylesheet_owner_nids`/`_disabled`/`_rule_count`/`_rule_json`/`_media_child_count`/`_media_child_json`), serializing each rule/media-rule to JSON via `serde_json::json!`.
  - JS side introduces `_lumen_make_indexed_list` (`web_api_shim_mid.js`, next to `_lumen_make_range`) — a shared Proxy helper for index-addressed object lists, used by both `CSSRuleList` and `StyleSheetList`; each JS object re-reads Rust state by index on every access, so object identity is **not** stable across reads (`sheet.cssRules[0] !== sheet.cssRules[0]`, a deliberate simplification, no wrapper cache).
  ...
  - **BUG-493 (P6, 2026-09-25): the registry is reconciled with the DOM on read, and CSSOM edits reach the painted cascade.** `v8_runtime/sheet_sync.rs::SheetSync::sync` walks the tree (shadow roots included — their owners are kept out of `document.styleSheets` via `shadow_owned`) whenever `DomTouched::epoch` (bumped by `record_dom_touch*`) moved; a `<style>` whose text is unchanged keeps its entry so `insertRule` edits survive, a changed one is re-parsed and its `CssomDeltaLog` entries dropped (`cssom_epoch` bumped). `.sheet` goes through `index_for_owner` (no walk when the node is already registered, connected and text-equal) and is cached per element wrapper — CSS-in-JS compares `tag.sheet` by identity. The `CSSStyleSheet` wrapper is keyed by owner `nid`, not registry index. `patched_cascade` lays the edit log onto the shell's text-built cascade (a `<style>` filled only by `insertRule` merges whole); the shell reads it through `persistent_js::CascadeFeed`, cached UI-side by `set_js_ctx` in both engine-thread modes — **under the engine thread (default) `Lumen::js_ctx` is `None` on the UI thread**, so anything `refresh_dynamic_css` pulls from the page must come through that handle, not `js_ctx` (adopted sheets were silently skipped for the same reason until this fix). Tests: `crates/js/src/dom/tests/v8_bug493_sheet_registry.rs`.
  - Out of scope, срез 1 of CSSOM-5 below covers part of this: `new CSSStyleSheet()`, `stylesheet.disabled` writes. Still out of scope: `insertRule`/`deleteRule`, a writable `CSSStyleDeclaration` on a `CSSStyleRule`, `@import`/`@font-face`/`@supports` as `CSSRule`.

- **`new CSSStyleSheet()`/`.replaceSync()`/`.replace()`/`document.adoptedStyleSheets`/`shadowRoot.adoptedStyleSheets`/`.insertRule()`/`.deleteRule()`** (CSSOM-5, done 2026-09-06, [BUG-897](../bugs/BUG-897-FIXED.md)) — the write half of CSSOM-1 above.
  - **Separate registry, not a `stylesheet_nodes` entry.** A constructed sheet has no owning DOM node, so it cannot share that registry's index space (rebuilt from document order). `V8JsRuntime::constructed_stylesheets: Arc<Mutex<Vec<ConstructedStylesheet>>>` (`install/constructed_stylesheets.rs`) is indexed by construction order instead; the JS wrapper carries that index as a hidden own property (`_lumenSheetIdx`, non-enumerable, set once by `_lumen_make_constructed_style_sheet`).
  - **`adoptedStyleSheets` is a THIRD registry** — `V8JsRuntime::adopted_stylesheets: Arc<Mutex<HashMap<u32, Vec<u32>>>>` keyed by an opaque scope id the JS side alone interprets (`_LUMEN_ADOPTED_DOCUMENT_SCOPE` sentinel `u32::MAX` for `document`, a shadow root's own `nid` for its own list). Rust never distinguishes the two; it just stores whatever key it is given.
  - **This is what makes `shadowRoot.adoptedStyleSheets` survive BUG-877** (`_lumen_make_shadow_root` rebuilds a fresh wrapper object literal on every `host.shadowRoot` read) — for this one property only, because the state lives in Rust keyed by `nid`, not on the wrapper itself. Every other shadow-root expando still resets on rebuild.
  - **Validation, not `instanceof` alone.** `document.styleSheets[i]`/`element.sheet` (CSSOM-1, read-only) objects pass `x instanceof CSSStyleSheet` — same global constructor, same `.prototype` — but were never constructed, so they carry no `_lumenSheetIdx`. `_lumen_adopted_sheet_ids` (`web_api_shim_mid.js`) checks both; a read-only sheet handed to `adoptedStyleSheets =` throws `TypeError` same as `null`/a plain object would.
  - **Cascade wiring (срез 2).** `V8JsRuntime::document_adopted_fingerprint`/`document_adopted_stylesheet` (`constructed_stylesheets.rs`) let the shell pull the document scope's enabled sheets in list order; `refresh_dynamic_css` (`crates/shell/src/relayout.rs`) and `page_pipeline.rs::parse_and_layout` compare the fingerprint each relayout and rebuild only on change, the same shape as `inline_style_fingerprint` (BUG-743). Plain `Mutex` reads, no cross-thread `Isolate` spawn — not the BUG-976 shape. Shadow-root scopes are **not** read here: Lumen has no shadow-scoped cascade at all (`extract_style_blocks` doesn't descend into shadow trees either), so a shadow root's own `<style>` is in the same boat as its `adoptedStyleSheets`.
  - **`insertRule`/`deleteRule` (срез 3).** The actual splice is [`lumen_css_parser::Stylesheet::insert_rule`]/`delete_rule` (`crates/engine/css-parser/src/parser.rs`) — every rule-list mutation is confined to that one file by a workspace-scanning gate (`every_stylesheet_mutation_in_the_workspace_announces_itself`), so the natives here only resolve the registry entry (`Arc::make_mut` — cheap, no other owner holds the `Arc` past a transient read) and translate `Result<_, CssomRuleMutationError>` into the sentinel-return convention this module already used for `_lumen_set_adopted_stylesheets` (no exception path through `reg!`'s natives). `insertRule`'s text must parse to exactly one style rule or `@media` block and nothing else, or it is `SyntaxError` — other rule kinds (`@font-face`, `@import`, …) are not yet `cssom_rules()`-representable, so `insertRule` cannot add them either. Reaches the cascade through the same fingerprint as replaceSync/disabled, no new wiring needed.
  - Tests: `crates/js/src/dom/tests/v8_cssom5_constructed_stylesheets.rs` (25 tests, `V8JsRuntime` directly); rule-splice edge cases (index shifting, media interleaving, syntax/index rejection) are unit-tested directly on `Stylesheet` in `crates/engine/css-parser/src/parser/tests/revision.rs`.
  - Out of scope: a writable `CSSStyleDeclaration` on a `CSSStyleRule`, `@import`/`@font-face`/`@supports` as `CSSRule`, `document.styleSheets`' own read-only sheets still lack `insertRule`/`deleteRule` (separate registry, CSSOM-1/2), shadow-scoped cascade.
...
- **`_LUMEN_WRAPPER_MEMBERS` (`crates/js/src/dom.rs`) sits on a prototype BELOW the interface
  ...
  shadowing shape (a module's own `Object.defineProperty` outranking a correct `_lumen_install_reflection`
  row) previously hit `iframe.src`, fixed by deleting the duplicate own property ([BUG-920](../bugs/BUG-920-FIXED.md)).
