...
- **Fetch API JS shim** (`install_dom_api`, `crates/js/src/dom.rs`). 2026-05-22.
  ...
  - ⬜ **Request headers never reach the network** ([BUG-749](../bugs/BUG-749-OPEN.md)): no `_lumen_fetch_*` binding and no `JsFetchProvider` method takes a headers parameter, so `init.headers`/`Request.headers` are parsed only for the body's `Content-Type` and then dropped. Response headers do come back (`_lumen_fetch_get_headers`) — the hole is one-directional.
...
- **URL / URLSearchParams / performance / queueMicrotask** (`crates/js/src/dom.rs`). 2026-05-25.
  ...
  - `PromiseRejectionEvent` (HTML LS §8.1.7.5) + `window.onunhandledrejection`/`onrejectionhandled`. **BUG-702 ([P3] 2026-08-09):** the interface exists for construction and feature detection only — nothing dispatches the two events yet ([BUG-716](../bugs/BUG-716-OPEN.md), needs `v8::Isolate::set_promise_reject_callback`). Defining it is load-bearing anyway: core-js reads a missing `PromiseRejectionEvent` as proof that the native `Promise` is untrustworthy and swaps in its own polyfill, so before this every core-js site ran on a polyfilled `Promise` layered over our microtask machinery.
