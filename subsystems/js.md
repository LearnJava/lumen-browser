...
- **WebAssembly MVP interpreter** (`crates/js/src/wasm/`, U-4 stage 1). 2026-06-18.
  ...
  - Host imports: JS functions stored as `Persistent<Function>`, called from the interpreter via the `HostImports` trait (`JsHost`). Args/results marshal as `f64`. Re-entrant calls into a *busy* instance return a trap (instance is removed from the registry during a call) instead of panicking.
  - Boundaries (documented): `f64` numeric boundary (i64 > 2^53 lossy); exported `Memory.buffer` is a snapshot copy (no live aliasing — emscripten `HEAP32` pattern incoherent); host imports can't touch instance memory mid-call; SIMD (`0xFD`) / threads / `memory.init` rejected at decode (graceful `CompileError`).
  - Bytes cross JS→Rust as `TypedArray<u8>` (the engine's `Vec<u8>` `FromJs` requires a real `Array`, which the shim does not pass).
  - 20 tests: 14 engine (add/factorial-loop/if-else/memory/import/div-trap/f64/nested-branch + parse/validate) + bridge/JS-integration (instantiate-and-call, exports introspection).
