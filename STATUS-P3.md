...
- **(б-2) Расширить покрытие** на остальные страницы по мере роста `cpu_raster`. Текущий растеризатор пропускает текст/картинки/градиенты — такие страницы дают почти-пустые эталоны, поэтому добавлять их в `PAGES` (`snapshot_cpu.rs`) только после реализации соответствующих примитивов в `crates/engine/paint/src/cpu_raster.rs`.
