...
- graphic-tests-migration (8A.6): каждый из 22 текущих HTML-тестов получает (а) Rust-тест в crates/driver/tests/ со structural-ассертами по COVERAGE.md, (б) PNG-эталон в graphic_tests/snapshots/. Старый run.py остаётся для уровня 4 (cross-browser vs Edge), но перестаёт быть основным gate.
...
- bench-ram-axis (9G.5): расширить lumen-bench: добавить peak_rss / steady_state_rss / tier_transition_rss замеры; CI gate фейлит PR при >5% регрессе RAM или >20% регрессе любого tier-transition. Binding по ADR-008 §«Performance gate». Делать **рано** — критично для всех последующих T0-экономия задач.
