...
Queue (Wave 2 — приоритет на тестирование изнутри, см. lumen-plan.md §6.11 / §15 / ADR-006):
...
- tinyskia-cpu-raster (8A.5): tiny-skia опционально под cfg(test) — для детерминизма пикселей на Windows/macOS/Linux CI. Не попадает в release-бинарь.
...
Queue (Wave 3 — Automation Phase 1 + Anti-detection Phase 1, см. ADR-006 + ADR-007):
...
Queue (Wave 4 — Automation Phase 2 + Anti-detection Phase 2):
