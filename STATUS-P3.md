...
- BUG-183 (TEST-26, 17.74%) — `mask-image` gradient mask
- BUG-210 (TEST-92, 15.59%) — system color keywords
- BUG-186 (TEST-33, 14.89%) — multi-column layout
- BUG-202 (TEST-83, 14.02%) — `scroll-behavior` регресс (G-3 помечена ✅ но FAIL)
- BUG-193 (TEST-64, 13.89%) — table `border-spacing`/`col_span`/`row_span`
- BUG-180 (TEST-18, 21.21%) — `<img>` rendering
- BUG-196 (TEST-67, 16.41%) — `attr()` typed substitution

**Средний diff (5–10%):**
- BUG-200 (TEST-80, 9.89%) — `border-collapse`
