...
- **BUG-226** (2026-06-21) — SVG stroke центрирован на кромке (TEST-47 2.27% → 1.20%
  ...
  на `rect`/`circle`/`ellipse` рисовался целиком ВНУТРИ бокса
  (`DrawBorder { rect: b.rect }`, border-box-модель), тогда как SVG 2 §13.7 центрирует
  штрих на кромке геометрии (½ ширины наружу, ½ внутрь). Видимый orange-core 79×59 vs
  Edge 89×69 при stroke-width 10 (±5px = w/2). Фикс в `emit_svg_shape`
  (`display_list.rs`), армы `Rect` и `Circle/Ellipse`: `b.rect` надувается на
  `stroke_w/2` по всем сторонам (`stroke_rect`), внешние радиусы = `r + w/2` (square-углы
  без радиуса остаются square); `DrawBorder` рисует внутрь на полную ширину `w` →
  внутренняя кромка на `r − w/2`, центрлайн на исходной кромке `b.rect`. Fill остаётся
  на исходном `b.rect`; even-odd-кольцо (BUG-175) строит внутренний радиус для
  скруглённых. Прогон gdigrab dev-release: TEST-47 2.27%→1.20%, TEST-70 1.63%
  (без изменений), TEST-82 2.38%→2.31% — **регрессий нет** (TEST-54/60/119 — только
  `<path>`, другая ветка). Регресс-тест `svg_rect_stroke_is_centred_on_edge`
  (display_list.rs, paint 779 lib-тестов). TEST-47 переатрибутирован BUG-226 → BUG-176
  (kappa-AA эллиптических дуг). **NB:** при merge замечены 2 красных на main теста
  `color_management` (ICC, домен P1) — заведён BUG-227, к BUG-226 не относится.
...
- **BUG-199** (2026-06-20) — `@starting-style` (TEST-71 7.03% → KNOWN_DEBTORS). Дефекта движка
  ...
  centred flex+gap; `@starting-style` корректно НЕ просачивается в статический каскад). Пиксельный
  замер Edge-эталона (y=360): box-a красный полоса 107px центр 391 = `transform:scale(0.5)`
  применён; box-b синий левый край 458 vs нормальные 532 = `translateX(-80px)` применён; обе при
  полной непрозрачности. Edge headless `--screenshot` (без `--virtual-time-budget`) ловит
  entry-transition в полёте: transform у START-значения (~0% прогресса), opacity у END-значения
  (~100%) — взаимно несогласованный кадр для синхронных 0.4s переходов = артефакт тайминга захвата
  Edge, не дефект движка. Совпасть требует проводки `@starting-style` в каскад (домен P4) + engine
  entry-переходов + воспроизведения невозможного кадра. Тот же класс, что TEST-77/BUG-126. TEST-71
  → KNOWN_DEBTORS (baseline 7.03%), регресс-тест `starting_style_does_not_leak_into_static_cascade`
  (style.rs) фиксирует спек-корректный инвариант. BUG-199 OPEN (DEBTOR) — deferred entry-transition
  engine. Без правок production-кода (layout 2935 зелёные).

- **BUG-198** (2026-06-20) — `object-fit` SVG (TEST-70 7.82% → 1.63% → KNOWN_DEBTORS BUG-176).
  Пиксельный анализ Edge-эталона: inline `<svg>` НЕ replaced element, поэтому CSS
  `object-fit`/`object-position` к нему не применяются — Edge фитит viewBox только
  по `preserveAspectRatio` (SVG §7.8); все 5 «режимов» теста Edge рисует одинаково
  как `meet`/contain. BUG-110 ошибочно завёл маппинг через object-fit (растягивал
  fill, кропал cover, не масштабировал none). Фикс: `compute_object_fit_transform`
  → `compute_preserve_aspect_ratio_transform` (box_tree.rs; object-fit остаётся для
  `<img>`-SVG). Вторая причина: femtovg `draw_fill_rounded_rect` + `CornerRadii::clamped_to_box`
  схлопывали эллиптические углы `min(w/2,h/2)` → SVG `<ellipse>` рисовался «стадионом»;
  переписано на CSS §5.5 (по-осевой scale-factor, сохраняет rx≠ry). Тесты:
  `preserve_aspect_ratio_*` (5, box_tree.rs), `clamped_to_box_preserves_wide_ellipse`
  (display_list.rs). Остаток 1.63% = kappa-AA эллиптических дуг vs точная дуга Edge →
  KNOWN_DEBTORS (BUG-176). SVG-регрессий нет (TEST-47/54/60/82/119 на baseline).

- **BUG-181** (2026-06-20) — `object-fit` basic (TEST-19 9.05% → KNOWN_DEBTORS, BUG-219).
  Расследование: геометрия placement верна во всех 5 режимах. Пиксельный анализ
  Edge-эталона vs Lumen: средние RGB ячеек совпадают (cover-center agi
  65.5/85.5/87.1 vs 65.6/85.7/87.1), лучший сдвиг = (0,0) для всех 9 ячеек
  (поиск ±3px не нашёл уменьшения diff → нет смещения), letterbox contain
  perceptron (852×725→141×120, бары 19.5px) и cover-кроп agi (1024²→180²,
  object-position) совпадают с Edge. Остаток 9.05% = image-resampling AA на
  высокочастотном контенте (тонкие линии perceptron-диаграммы + rusty-текстура
  agi): `resize_area_avg` box-фильтр расходится с Edge downscale kernel →
  54–63% пикселей agi-ячеек отличаются на >30/255 при идентичном среднем цвете.
  Та же причина, что TEST-18 (BUG-219). Регресс-тесты `bug181_*`
  (display_list.rs) фиксируют геометрию `fit_image_rect` на реальных размерах
  теста; TEST-19 → KNOWN_DEBTORS (BUG-219, baseline 9.05%). Без правок
  production-кода (геометрия уже корректна), paint 754 теста зелёные.
