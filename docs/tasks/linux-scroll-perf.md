...
### Срез — scroll-copy: резать ЦЕНУ MISS (2026-07-13, ОТРИЦАТЕЛЬНЫЙ, НЕ влит)

Реализован и **измерен** приём Шага 1 ниже (scroll-copy). Вывод: **корректен, но
net-выигрыша по перфу на Intel HD 530 нет** — код откачен, в main не влит (ветка
`p1-scroll-copy` осталась в git-истории). Ниже — что сделано, замер и почему не
взлетело, чтобы следующая сессия НЕ переизобретала это на той же iGPU.

**Что реализовано (правильно):** ping-pong двух текстур полосы в `PageBandCache`;
при промахе с texel-выровненным вертикальным перекрытием того же ключа/размера —
`copy_texture_to_texture` перекрытия FRONT→BACK + рерастеризация только strip-а в
BACK + swap. Ключевой урок, стоивший одного неверного захода: **strip надо
ОТСЕКАТЬ (cull), а не скрайзить (scissor).** Первый заход сеял `clip_stack`
slice-прямоугольником — draw-ops клиппились scissor'ом, но `level_bounds` видел
полные вершины, поэтому **все blur/filter-пассы всё равно бежали по полному bbox**
(профиль: `ops 183 layers 2`, encode 33 мс — как без среза). Рабочий вариант:
`leaf_outside_band_y` отсекает листья ЦЕЛИКОМ вне `strip ± slop` (slop 64 CSS px
≥ потолок blur-паддинга 34 px) → их offscreen-уровни остаются `LevelBounds::Empty`
→ существующий viewport-cull выбрасывает их filter-пассы (`ops 183→93`). Листья,
задевающие strip, рисуются ПОЛНОСТЬЮ (без клипа → blur со всей окрестностью), их
«лишняя» часть в overlap затирается copy. Пиксельно бит-идентично (band_top
целочисленны + dpr=1 ⇒ старый и новый рендеры контента имеют те же субпиксельные
позиции + целый сдвиг, который texel-copy учитывает точно).

**Пиксельная A/B (корректность):** spectacle `-a` по активному окну на 10 стопах
(вниз 5 промахов + вверх, top/bottom strip), diff `magick compare -metric AE`,
ON (default) vs OFF (`LUMEN_NO_SCROLL_COPY=1`). Worst **0.13 %** (< порог 0.5 %;
большинство стопов 0.0000 %) — мелкие ненулевые = шум захвата AA/тайминга, не шов.
**Приём корректен.**

**Перф-замер (`miss_probe.py`, self-A/B, один release-бинарник, bench-static-scroll
step 120, 250 скроллов):**

| Конфиг | MISS median | mean | p95 | max | MISS-доля времени |
|---|---|---|---|---|---|
| scroll-copy ON (cull) | 6.15 мс | 14.01 | **38.7** | 80.9 | 29.7 % |
| OFF (полная полоса) | 5.95 мс | 13.21 | **48.6** | 88.6 | 28.2 % |

p95 упал ~20 %, но median/mean/сумма чуть выросли → **net ≈ ноль, в пределах
шума прогонов** (max скачет 70–88 в обе стороны).

**Почему не взлетело (диагноз):**
1. **MISS-median уже дёшев (~6 мс)** — оптимизировать особо нечего. Дорогой хвост
   (p95/max) — это кадры, где strip (~864 CSS px при band-bias, ~½ полосы) попадает
   на плотный блюр, а его **всё равно надо отрисовать** для strip-а. Band-раст стал
   бимодальным (`10 мс` когда strip пуст ⇄ `30 мс` когда плотный) — cull режет
   лёгкие кадры, но не тяжёлые, а именно тяжёлые формируют хвост.
2. **Overhead copy съедает экономию fill.** `copy_texture_to_texture` полосы
   1024×~945 (~4 МБ GPU→GPU) + whole-band `Clear(bg)` BACK + ОТДЕЛЬНЫЙ `queue.submit`
   (sync-точка) стоят на слабой iGPU столько же, сколько экономит вырезанный fill.
   Ровно предупреждение из «Срез 2» ниже: *«полноэкранный blit полосы 1024×1800 на
   Intel HD 530 сам стоит ~10 мс… потолок здесь раза в 2, не 6–8× как на Windows»*.

**Вывод для следующей сессии:** на этой iGPU резать ЦЕНУ MISS через scroll-copy
**не стоит** — упираемся в bandwidth копии, а не в fill. Полезнее: (а) снизить
band-bias-strip (меньше strip = реже дорогой band-раст, но чаще MISS — искать
оптимум); (б) запасная задача №2 ниже (хвост `bench-anim-scroll`); (в) вернуться к
scroll-copy только на более мощном GPU, где copy дёшев относительно fill. Метод
пиксельной A/B (spectacle `-a` + `magick compare -metric AE` по стопам) —
переиспользуемый для любой wgpu-компоновщик-правки на Wayland.

## СЛЕДУЮЩАЯ СЕССИЯ — что делать (подробно)

> **ОБНОВЛЕНО 2026-07-13:** «Шаг 1 (scroll-copy)» ниже **уже опробован и дал
> отрицательный результат** на этой iGPU (см. раздел «Срез — scroll-copy» выше:
> корректен, но net-выигрыша нет — упирается в bandwidth copy, не в fill). **НЕ
> переделывать его на Intel HD 530.** Актуальные направления — запасные задачи
> ниже + пункт «Вывод для следующей сессии» в том разделе. Шаг 1 оставлен как
> справочник механики (и на случай более мощного GPU).
...
сдвиг (срез выше) снял ~⅓ MISS через их ЧАСТОТУ. Резать **ЦЕНУ** одного MISS через
scroll-copy на этой iGPU **не окупается** (проверено). Ниже — исходный план Шага 1
(справочно) и запасные задачи (актуальны).
