...
Медиана уже отличная (200 fps, 8× экономия CPU vs Chromium) — дальнейший ROI
scroll-компоновщика на этой iGPU падает. **Единственный оставшийся зазор — хвост
(p95):** band-MISS-кадры при непрерывном скролле (полоса = вьюпорт + ≤768 px →
re-raster каждые ~768 px) и `bench-anim-scroll` p95 22 мс (> 16.7 мс → редкий
джанк). Кандидаты, по убыванию отдачи именно для хвоста:

1. **Инкрементальный band-raster (scroll-copy):** при рецентрировании полосы
   копировать перекрывающийся регион внутри текстуры и растеризовать только
   вновь открывшуюся тонкую полосу — режет цену каждого MISS с полной полосы до
   ~1 экрана. Прямо бьёт в хвост непрерывного скролла (bench-профиль).
2. **Тайловый кэш полос** (в координатах scroll-root, «финальная форма» урока
   ...
   = HIT, не MISS. Больше помогает реальному туда-сюда-скроллу, чем bench-у.
...
Замерять ЛЮБОЙ из них тем же `bench_scroll.py` back-to-back (default vs новый
kill-switch), сравнивая p95/mean, не только медиану.
