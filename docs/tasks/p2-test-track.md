...
**Первый же прогон дал находку — [BUG-787](../../bugs/BUG-787-OPEN.md).**
...
Репро в `fuzz/regressions/` намеренно **не** закоммичен: replay-шаг CI
проигрывает этот каталог на каждом прогоне, и до фикса это повесило бы джоб
вместо отчёта о находке. Вместо этого `fuzz_image` внесён в `KNOWN_FAILING`
воркфлоу — таргет продолжает гоняться и собирать репро, но его падение не
красит джоб (тот же размен, что `KNOWN_DEBTORS` в `graphic_tests/run.py`:
вечно красный джоб перестают читать). Убрать из списка — в коммите с фиксом.
