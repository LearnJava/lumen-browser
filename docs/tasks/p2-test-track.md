...
**Обходной приём для гейта, стоит знать заранее:** `classify_one()` в
`expectations.py` сравнивает актуальный статус только с **первым** элементом
списка `expected: [...]` (`SUBTEST_GOOD_STATUSES = {"PASS"}` проверяется
только против `expected[0]`) — остальные элементы списка `wptreport`
не подавляют ключ `"expected"` сами по себе и этим скриптом не читаются.
Чтобы подтест с флипающимся результатом никогда не читался как REGRESSION,
первым в списке должен стоять статус НЕ из `GOOD_STATUSES` (обычно `FAIL`),
а не `PASS` — иначе `--check` продолжит валить гейт при каждом флипе в
FAIL, даже если `FAIL` формально есть в списке. Тот же приём уже негласно
использован в `css/css-position/sticky/position-sticky-scrolled-remove-
sibling.html.ini`, но не был описан как контракт скрипта. `--check`
подряд ×2 после исправления порядка — 0 регрессий. Baseline на момент этого
среза был 237 → 238 (до пересчёта после мержа среза 32, см. ниже).
