...
**Второй прогон — вторая находка, [BUG-788](../../bugs/BUG-788-FIXED.md).**
...
### TEST-3: срез 55 (2026-09-22) — `fetch`: детерминированный дефект в редиректе на не-HTTP(S)-схему (BUG-1098), известный TIMEOUT-флап (BUG-1022) на `orb/tentative`/`metadata/generated`

**Выбор кандидата.** Из списка «Дальше» среза 54 взята `fetch` (985 файлов, 166 `.https.`) —
вторая из двух отложенных опций, без cross-origin-DNS-доминирования `referrer-policy`. Число
категорий 259 → 260.

**Baseline.** `--update-expected --all --root fetch --recursive --processes 7
--binary target/dev-release/lumen.exe`: 906 id (985 файлов минус нетестовые
ресурсы/хелперы), **387/906 harness OK, 2 050/7 051 подтестов**, 436 `.ini`.
...
конкретные сабтесты РАЗНЫЕ между прогонами) — тот же почерк, что уже задокументированный класс
[BUG-1022](../../bugs/BUG-1022-FIXED.md) (заметка добавлена туда с деталями обоих прогонов),
baseline по этому кластеру принят как записан первым `--update-expected`, не перегенерирован.

**Что нашлось (движковый дефект, не флап).** Единственная регрессия, повторившаяся
**одинаково во всех трёх** прогонах (включая невалидный по причине обрыва раннера — этот
конкретный тест не попал в число 111 потерянных) и трижды воспроизведённая изолированно
`run_smoke.py` (без соседних категорий, детерминированно 3/3):
`fetch/api/redirect/redirect-schemes.any.html` [`Fetch: handling different schemes in
redirects 1`, редирект на `mailto:`] — промис `fetch()` резолвится `undefined` вместо
отклонения `TypeError`; попутно обнаружено, что редирект на `data:` вовсе не логирует
«unsupported scheme» (5 строк лога вместо ожидаемых 6). Baseline изначально фиксировал этот
файл как полный `ERROR` (harness не долетел до конца) — по всей видимости, разовая случайность
самого первого прогона; `.any.worker.html`/`.any.sharedworker.html`-варианты уже были
...
**Не разобрано.** Остальные ~500 файлов `harness != OK` (не `ERROR`/`TIMEOUT` из двух известных
классов) не расклассифицированы по первопричине — `fetch/http-cache/*`, `fetch/fetch-later/*`,
`fetch/local-network-access/*` не в скоупе или требуют отдельного среза на разбор кластеров
(в частности `fetch-later`/`local-network-access` — незавершённые/tentative спеки, возможно
`WONTFIX (Phase N+)` кандидаты, не проверено).

**Окружение этой сессии.** Тот же бинарь `dev-release`, без изменений в `crates/` между
началом среза и текущим `HEAD`. Windows 10 19045, `tests/wpt/.venv/Scripts/python.exe`,
`MSYS_NO_PATHCONV=1`, `--processes 7`.

Дальше: `referrer-policy` полный `--check` (перенесено из среза 54, всё ещё не выполнено),
разбор ~500 нерасклассифицированных `fetch`-провалов выше, следующая по размеру
некатегоризированная категория WPT-VENDOR (не выбрана — не исследовано этим срезом).

### TEST-3: срез 56 (2026-09-22) — `service-workers`: категория целиком объясняется BUG-1069, новых движковых находок нет

**Выбор кандидата.** Список «Дальше» среза 55 не называл конкретную категорию (пункт 3 —
«не выбрана, не исследовано этим срезом»). Пересчитан список оставшихся: `WPT-VENDOR`
`done`-строк в ROADMAP.md — 291, из них 43 без каталога в `tests/wpt/metadata/` (часть —
`html-*`/`dom-rest` — ложные срабатывания метода: вендорены под `html/`/`dom/`, не под
собственным именем каталога, и уже частично покрыты другим путём — см. TEST-3 состояние
2026-08-18). Из реальных кандидатов `service-workers` (795 файлов в вендоренном дереве) —
крупнейший. Число категорий 260 → 261.

**Baseline.** `--update-expected --all --root service-workers --recursive --processes 7
--binary target/dev-release/lumen.exe`: 328 отобранных id (после фильтра по
`RUNNABLE_ITEM_TYPES` — testharness/test262 — остальные 467 файлов вендоренного дерева не
...
требуют secure context по спецификации, поэтому весь отобранный поднабор категории —
`.https.`-only: это первая категория, где влияние бага измерено не частично (как в
предыдущих срезах — `workers` 25/337, `editing` 10/700, `pointerevents` 24/258), а на
**100% файлов**. Заметка добавлена в файл бага.

**Два `--check` подряд — оба чистые.** 0 регрессий, 0 неожиданных PASS, 0 прочих
отклонений в обоих прогонах — стабильно, флапа нет.

**Не разобрано.** Реализованы ли service workers в движке вообще — этот baseline не
даёт никакого сигнала в любую сторону, поскольку каждый тест гибнет на TLS ещё до
исполнения кода воркера. После починки BUG-1069 категория потребует полной
перегенерации; сегодняшний baseline — заглушка, и первый же `--check` после фикса
сертификата ожидаемо покажет «стену» из 291 unexpected-pass на одной этой категории —
это ожидаемо и желаемо, не повод откатывать фикс.

**Окружение этой сессии.** Тот же `dev-release`, пересобран в начале сессии (инкрементально,
2м06с, изменений в `crates/` относительно среза 55 не было — единственный коммит между
ними merge среза 55 самого). Windows 10 19045, `tests/wpt/.venv/Scripts/python.exe`,
`MSYS_NO_PATHCONV=1`, `--processes 7`.

Дальше: масштаб этого среза (100% категории — один тулинговый баг) делает починку
BUG-1069 самой высокоценной оставшейся задачей дорожки TEST-3 — её собственный раздел
«Ожидание» описывает фикс как чисто тулинговый (перевыпуск сертификата, без правок
Rust), а даст реальное покрытие сразу по ~2000 `.ini` в уже «закрытых» категориях, а не
...
общего знаменателя, тот же класс, что [BUG-1022](../../bugs/BUG-1022-FIXED.md) на `fetch`
...
(1→2→1→5), тот же класс, что [BUG-1022](../../bugs/BUG-1022-FIXED.md).** Прогон 1 — 2
...
55. Инстанс задокументирован в [BUG-1022](../../bugs/BUG-1022-FIXED.md) (новая секция).
...
регрессий, тот же класс, что [BUG-1022](../../bugs/BUG-1022-FIXED.md).** Прогон 1 — 5
...
`fetch`/`signed-exchange`. Инстанс задокументирован в [BUG-1022](../../bugs/BUG-1022-FIXED.md)
