...
- **[BUG-1065](../../bugs/BUG-1065-OPEN.md)** — найден при проверке утверждения для BUG-1063: тот же
  ...
  Заметка о [BUG-622](../../bugs/BUG-622-OPEN.md): `docs/engine-gaps.md` предупреждает, что вызовы
...
### TEST-3: срез 45 (2026-09-21) — `navigation-api`: 70 из 475 id падают на относительном URL в `navigation.navigate()`, найден BUG-1074
...
| Причина | id | Доказательство |
|---|---|---|
| `navigation.navigate()` с относительным URL, [BUG-1074](../../bugs/BUG-1074-OPEN.md) | 67 | `browsingContext.navigate(…) failed: navigation failed: invalid url: "#frag": relative URL without a base`; в логе 146 строк `Reload: #…`/`Reload: ?…` |
...
(`TypeError: … is not a function` в логе). BUG-1074 маскирует остальное: 70 id не доходят даже до этого.
...
**Ограничение записанного.** 70 + 10 + 4 + 1 = 85 id записаны как `ERROR` — нижняя планка; после починки BUG-1074 baseline регенерируется, сдвиг
