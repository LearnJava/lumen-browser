...
- [x] `_lumen_pip_request_window` зарегистрирован (`pip_bindings.rs`) и
      enqueue-ит `PipRequest::OpenDocument { width, height }`.
- [x] Document PiP открывает реальное OS floating-окно нужного размера
      (`open_pip_os_document` в `main.rs`, `PipOsConfig::sized`).
- [x] `PictureInPictureWindow` унифицирован между video- и document-PiP
      (`document_pip.rs` определяет класс; `video_pip.rs` переиспользует его
      через `globalThis.PictureInPictureWindow`, fallback только в изоляции).
- [x] Resize OS-окна доставляет `_lumen_pip_deliver_resize` в JS (обе ветки —
      общий `globalThis.__lumen_pip_active_window`; шелл зовёт `deliver_pip_resize`
      на `WindowEvent::Resized`/`ScaleFactorChanged` pip-окна).
- [x] Ограничение «DOM-контент внутри document-PiP окна» задокументировано как
      follow-up (в коде `document_pip.rs`/`open_pip_os_document`, в этом файле и
      в `CAPABILITIES.md`).
- [x] `CAPABILITIES.md` + `subsystems/js.md`/`shell.md` обновлены (Document PiP 🟡 —
      реальное окно есть, DOM-контент отложен).

## Остаток (follow-up, вне этого среза)

- Форвардинг реального DOM-контента запрашивающего документа внутрь
  document-PiP окна (сейчас окно показывает только фон). Требует отдельного
  рендер-пайплайна для под-документа PiP-окна.
- `PictureInPictureWindow.document` — всё ещё in-memory заглушка (не настоящий DOM).
