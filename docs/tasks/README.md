...
```
ROADMAP.md (строка, status ≠ done)   ← главный список задач
   │
   ├─ STATUS-PN.md "Next": строка-указатель `ROADMAP.md:NN (<id>) → docs/tasks/<id>.md`
   │
...
- **Источник правды по структуре — `ROADMAP.md`** (`grep "| <id> " ROADMAP.md`). `STATUS-PN.md` —
  только голые строки-указатели `ROADMAP.md:NN` на открытые задачи (приоритет сверху вниз), без
  заголовков/описаний/завершённых (история = `git log`).
- **Переиндексация при вставке (обязательно).** Вставил `K` строк в `ROADMAP.md` на строке `L` →
  все указатели `ROADMAP.md:NN` во всех `STATUS-PN.md` с `NN ≥ L` сдвинуть на `+K`. После любой
  правки ROADMAP, меняющей нумерацию, проверить, что каждый указатель попадает в нужную строку
  (`sed -n 'NNp' ROADMAP.md`). По возможности дописывать строки в конец блока фазы — меньше сдвиг.
...
| **Ссылка в STATUS** | Каждая задача = строка в `STATUS-PN.md "Next"` вида `ROADMAP.md:NN (<id>) → docs/tasks/<id>.md` |
