...
## Схема задач — не здесь

Схема учёта (`ROADMAP.md` → `STATUS-PN.md` → `docs/tasks/<id>.md`), правила указателей и
**переиндексация после сдвига строк** описаны в одном месте:
[`docs/dev-roles.md`](../dev-roles.md) §«Task tracking schema (invariant)».

Этот файл описывает только формат самого брифа — что положить в `docs/tasks/<id>.md` и когда его
удалить.

> Здесь до 2026-09-03 лежала вторая копия схемы. Она разошлась с оригиналом в самом дорогом месте:
> переиндексацию предписывала делать руками (`NN ≥ L → +K`), не упоминая
> `scripts/remap_status_pointers.py --apply`. Сейчас в STATUS-файлах 183 указателя, из них 145 в
> `STATUS-P3.md`, и перенос одной строки в `BUGS-FIXED.md` сдвигает всё, что ниже. Копия делала
> ручной работой то, для чего есть одна команда.
...
| [p1-monolith-split-queue.md](p1-monolith-split-queue.md) | Очередь SPLIT — разрезание монолитных файлов >2000 строк (батчи по сессии, P1) |
