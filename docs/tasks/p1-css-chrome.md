...
|---|---|---|
| Парсинг HTML/CSS хрома | **compile-time-валидация** + 1 раз при старте | Полная сериализация распарсенного дерева (CC-16) измерена и отклонена: 3.2–4.6 мс парсинга против 22.5 мс layout того же дерева и 4.3–4.9 с старта окна |
...
- **[BUG-333](../../bugs/BUG-333-FIXED.md)** — FIXED 2026-07-29. `.tab-row` (строки списка
  вкладок в `.sb-tabs`) рендерились с высотой 0 вместо 28px (текст соседних вкладок налезал
  друг на друга). Приписанная здесь причина — «`height:var(--tab-h)` не резолвится» —
  **опровергнута**: дословный `height:28px` давал ту же нулевую высоту, а `.toolbar` работал
  из-за `flex:none`, а не из-за `var()`. Настоящая причина — общий flex-дефект
  ([BUG-343](../../bugs/BUG-343-FIXED.md)): `lay_out_flex` передавал used-размер item'а
  записью в его же `style.height` и не откатывал её, поэтому пробный Step-1-проход `.sidebar`
  с неопределённой высотой выжигал `height:0px` в строки до настоящего прохода.
- **[BUG-334](../../bugs/BUG-334-FIXED.md)** — SVG `<use href="#i-...">` без явных `width`/
...
4. Побочно найдены 2 живых бага флипа CC-14, не пойманных чек-листом паритета: [BUG-403](../../bugs/BUG-403-FIXED.md)
   (профиль-поповер невидим под дефолтным хромом) и [BUG-404](../../bugs/BUG-404-FIXED.md)
...
* `strip::hit_test` / `TabHit` — их читает **живой, негейтированный** обработчик правого клика по
  вкладке (контекстное меню вкладки). Он же — второй сайт класса [BUG-404](../../bugs/BUG-404-FIXED.md)
...
удалены): [BUG-408](../../bugs/BUG-408-FIXED.md) — панель авто-архива вкладок недостижима;
[BUG-409](../../bugs/BUG-409-FIXED.md) — группы вкладок не отображаются;
[BUG-410](../../bugs/BUG-410-FIXED.md) — строка dropdown потеряла текстовый тег типа.

## Этап E — компайл-тайм оптимизации (опционально, после флипа)

### CC-16 (M): Пред-парсинг при сборке — **закрыто без реализации 2026-07-28 (P1): профиль старта не оправдал**
Предложение: сериализация `Document`+`Stylesheet` хрома в build.rs (serde-фича на dom/css-parser типах или кодогенерация конструкторов) + `include_bytes!` → пропуск парсинга при старте. Строка задачи была условной («делать только если профиль старта это оправдает»), поэтому срез свёлся к переписи. Инструмент оставлен в репозитории: `cc16_parse_cost_census` (`crates/chrome/src/lib.rs`, `#[ignore]`).

**Измерено** (dev-release, min из 20 сэмплов после 3 прогревочных; ассет — 102 487 байт, 2160 узлов, 372 CSS-правила):

| Величина | Значение |
|---|---|
| `parse_document` целиком | 3.2–4.6 мс (HTML 2.2–3.1, CSS 0.9–1.5) |
| Восстановление `Document` из bincode-блоба | 0.7–0.8 мс |
| `Document::clone` — пол любого восстановления | 0.6–0.7 мс |
| Размер bincode-блоба | 193 891 байт (×1.9 от исходных 102 КБ) |
| `lumen_layout::layout` того же дерева, 1280×800 | **22.5 мс** |
| Старт до первого непустого кадра (`[bench]`, окно, `samples/page.html`, DX12) | 4332 / 4458 / 4881 мс |

**Почему не оправдано.**
1. **Выигрыш — это разность, а не вся стоимость парсинга.** Пред-парсинг не отменяет восстановление: те же 2160 узлов и их строки кто-то обязан аллоцировать. Потолок выигрыша — `парсинг − восстановление` ≈ **2.4–3.8 мс**, при полу 0.6 мс (`Document::clone`), ниже которого не опустится ни bincode, ни кодогенерация конструкторов.
2. **Знаменатель.** 3.8 мс из 4.3–4.9 с старта окна — **≈0.08 %**. Даже против самого благосклонного знаменателя (весь CPU-старт headless-прогона `--dump-layout`, ~280–310 мс) это ~1 %. Реальный вес старта — компиляция wgpu-пайплайнов, уже заведённая как [BUG-406](../../bugs/BUG-406-OPEN.md) (3.0–7.0 с на DX12 против 0.26–0.40 с на Vulkan), то есть в ~1000 раз больше того, что снимает CC-16.
3. **Соседняя стадия дороже в 5 раз и пред-парсингом не снимается.** Тот же ассет сразу после парсинга проходит `layout` за 22.5 мс. Пред-парсинг убирает 3.9 мс из 26.4 мс работы над хромом на старте, оставляя 22.5 мс нетронутыми — цель выбрана не там, где стоимость. Ускорение этой стадии — предмет [BUG-341](../../bugs/BUG-341-OPEN.md)/CC-17, а не CC-16.
4. **Цена реализации несоразмерна.** `lumen_dom::Document` уже несёт `serde` (T3-гибернация), а вот `lumen_css_parser` — нет ни одного `Serialize` на **50 публичных типов** (`parser.rs`), и `Stylesheet::revision` устроен как «минтуется, никогда не выбирается» (иначе рукописное значение молча узаконило бы устаревший кэш) — десериализация обязана этот инвариант обойти. Плюс ~194 КБ блоба в бинарнике и новый инвариант «блоб синхронен ассету».

Вторая заявленная ценность — «отсутствие парсера в критическом пути первого кадра» — не материализуется: оба парсера всё равно нужны для страниц, из бинарника они не уходят, и первый кадр всё равно ждёт парсинга страницы.

**Что могло бы пересмотреть вердикт:** падение старта до сотен мс (закрытие BUG-406) *одновременно* с удешевлением стадии layout хрома — тогда 3.8 мс перестают быть шумом. До тех пор перепись пересчитывается одной командой:
`cargo test -p lumen-chrome --profile dev-release cc16_parse_cost_census -- --ignored --nocapture`.
...
акцент (`BAR_ERR`) не перенесён — [BUG-419](../../bugs/BUG-419-FIXED.md) (закрыт 2026-07-31).
...
|---|---|
| [BUG-419](../../bugs/BUG-419-FIXED.md) | цветовое выделение `ERR` в `#findCount` (косметика) — FIXED 2026-07-31 |
| [BUG-420](../../bugs/BUG-420-FIXED.md) | `#printOverlay` не печатает (обе кнопки — `close-modal`) и не связан с настройками печати |
| [BUG-421](../../bugs/BUG-421-FIXED.md) | `#view-settings` не изменяет ни одной настройки (`ToggleSwitch` — no-op с CC-9) |
| [BUG-422](../../bugs/BUG-422-FIXED.md) | нет действий над записями истории/закладок (переход, удаление, очистка, фильтр по папке, drag-refile) |
