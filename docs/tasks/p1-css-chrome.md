...
| [BUG-420](../../bugs/BUG-420-OPEN.md) | `#printOverlay` не печатает (обе кнопки — `close-modal`) и не связан с настройками печати |
| [BUG-421](../../bugs/BUG-421-OPEN.md) | `#view-settings` не изменяет ни одной настройки (`ToggleSwitch` — no-op с CC-9) |
| [BUG-422](../../bugs/BUG-422-OPEN.md) | нет действий над записями истории/закладок (переход, удаление, очистка, фильтр по папке, drag-refile) |
