...
`run_report.py --all --root webmcp --recursive --processes=4` (~6:10, 50 отобранных id): **1/50 harness OK, 1/1 сабтестов**. Единственный исполнившийся тест — `imperative/non-secure.html` (не-`.https.`, проверяет отсутствие API на небезопасном origin — тривиально верно). Остальные 49 — TIMEOUT на TLS-гэпе `UnknownIssuer` ([BUG-657](../../bugs/BUG-657-FIXED.md)), нулевой функциональный сигнал.
