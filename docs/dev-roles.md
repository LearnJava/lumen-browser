...
ROADMAP.md (one line per task, status ≠ done)   ← master task list for P1/P2 (grep "| <id> " ROADMAP.md)
   ...
   │   ROADMAP.md also carries a live-computed summary of CSS-SPECS.md (rows `css-specs-t0`…`t4`,
   │   note = `AUTO:CSS-SPECS:T<N>`, regenerated by gen_roadmap.py) — visibility only, not a work
   │   list. P4 still works directly off CSS-SPECS.md per-property rows, never off these summary rows.
   │
