...
- **`InProcessSession` renders (driver `screenshot_cpu_rgba`, headless MCP `screenshot`, the `cases::snapshot_cpu` gate)**: the session installs a V8 runtime with DOM bindings but never executes the page's own `<script>` elements, and hands `render_to_image_cpu` an unconditionally empty image set — so nothing a script draws (Canvas 2D included) and no decoded `<img>` pixels appear. The committed `graphic_tests/snapshots/cpu/57-canvas-2d.png` is a blank frame for exactly this reason and would stay green through a total Canvas 2D breakage (BUG-429). Not the same path as `--screenshot`/`--ipc-server`, which run scripts and composite canvas (BUG-428).
