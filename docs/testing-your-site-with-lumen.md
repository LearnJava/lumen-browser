...
- `type` вводит текст ровно как пользователь: значение попадает в поле, поле
  ...
  честно отвечает ошибкой (`Element is not a mutable text field` /
  `Element not found`), а не «успехом» без эффекта.
- `resource://screenshot` рендерит через **тот же движок**, что и живое окно
  ...
  `wait("document_ready")` вернул успех. **Но после мутации DOM** (`type`,
  `click`, `eval`) перевёрстка при включённом движковом потоке (ADR-023,
  дефолт) коммитится асинхронно, через несколько кадров, — снимок, сделанный
  сразу следом, ещё покажет прежний кадр. Здесь пауза (или `wait`) нужна.
