...
**вендорена и гоняется только одна** — `dom/nodes/` (168 файлов, движок исполнения — свой
минимальный executor `tests/wpt/browsers/lumen.py` поверх `wptrunner` + WebDriver BiDi: одно окно,
без `test_driver.*`, без iframes/multi-window — см. `docs/tasks/p2-wpt-integration.md`). Остальные
276 категорий **не вендорены** — таблица ниже перечисляет их все, чтобы было видно полное поле
...
поменять «Вендорено» на ✅/🟡 и добавить детальную таблицу по образцу `dom/nodes` (для этого,
вероятно, стоит обобщить `gen_status_md.py` на произвольную категорию, а не хардкодить `dom/nodes` —
пока не требовалось, категория одна). Скоуп/заметку в категорийном индексе можно и нужно
