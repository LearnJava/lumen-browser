...
python graphic_tests/run.py --per-test-process       # opt out of the live window: kill+relaunch lumen.exe per test (old default)
python graphic_tests/run.py --paint-bisect 100        # DEVX-4: baseline + one run per LUMEN_NO_* paint flag; prints a diff% table
```

**Preflight (running from a fresh worktree).** `run.py` checks its external tools before the first test instead of failing one test at a time 20 minutes in:

- `utils/ffmpeg.exe` and `graphic_tests/screenshots/` are **gitignored**, so a fresh worktree has neither. `ffmpeg.exe` is now resolved from the main working tree automatically (the worktree's `.git` file points at it); if it is missing there too, the run aborts in a second with a copy-paste `cp` line.
- A missing `msedge.exe` aborts the same way — without it every test would fail with "Edge screenshot missing".
- `LUMEN_PROFILE` defaults to `release`, while iteration builds are usually `dev-release`. If the requested profile has no `lumen.exe` but another profile does, the run aborts and prints the `LUMEN_PROFILE=<built> …` line instead of silently starting a ~10-minute cold build of the wrong profile. Passing `--build` still builds explicitly; if *no* profile is built, the cold build happens as before, with a warning up front.
- `--only NN` with no cached crop offset pulls in TEST-00 (calibration) automatically, like `--bisect`/`--recheck` already did — a single test no longer forces a full run.

**Live window is the default (SDC-4, formerly opt-in `--live` from SDC-3).** `run.py` keeps a single `lumen.exe --mcp-live-port N` window open for the entire run instead of killing and relaunching a process per test — the process-per-test model was the main source of wall-clock cost and focus-race flakiness ("magenta marker not found"). Navigation is driven through MCP (`LiveWindowSession`, SDC-2): `tools/call navigate` + `tools/call wait{condition:document_ready}` give a real load-complete signal instead of a blind `time.sleep`. The pixel capture itself is still gdigrab against the real femtovg-rendered window (not MCP's `resource://screenshot`, which renders through the CPU path and isn't at parity with femtovg — same gap as `--ipc`), so the live-window default is safe for real-JS tests (57, 129–138) unlike `--ipc`. TEST-00 magenta calibration still runs once and the offset is reused for the rest of the run. `--live` is kept as a no-op alias for old commands/scripts.

`--per-test-process` opts out of the live window and goes back to kill+relaunch per test — use it as a diagnostic fallback if the live window misbehaves. `--paint-bisect` always uses kill+relaunch itself, regardless of this flag (see below).

`--paint-bisect <id>` (DEVX-4): automates the manual `LUMEN_NO_*` A/B bisection described in `docs/automation.md` — runs the given test once as baseline, then once more per paint-optimization kill-switch (`LUMEN_NO_FRAME_SKIP`/`NO_SCROLL_COMPOSITOR`/`NO_ANIM_SPLIT`/`NO_BBOX_SCISSOR`/`NO_BBOX_BACKDROP`/`NO_IMAGE_MIPS`/`NO_BAND_BIAS`, `crates/engine/paint/src/renderer.rs`), each in a fresh `lumen.exe` process (the env var must be set before the paint pipeline reads it, so it always uses its own kill+relaunch, incompatible with `--ipc`). Prints a diff% table with each flag's delta from baseline, so you can see at a glance which optimization moves the picture. Reuses the cached crop offset (calibrates via TEST-00 first if none is cached).
...
- **A whole run can ERROR out on TEST-00 and report 156/156 failures — that is the capture failing, not the engine.** TEST-00 is the calibration page, and when it cannot find its magenta marker every later test reports `FAIL (no crop offset — run TEST-00 first)`, so the tail of the log reads `156/156 tests ERRORED (снимок/ffmpeg/IPC)` and the delta section says «Изменений нет» — a shape easy to misread as "my change broke everything". The live-window default removed the per-test focus race (see «Live window is the default» above) but not this one: on 2026-09-10 the first run errored wholesale and a plain re-run, with nothing changed, was clean. Read the *first* line of the log before the last one: `TEST-00: FAIL (magenta marker not found)` means re-run, and only a run whose TEST-00 passed is a measurement at all.
- **"Backgrounding fails loudly at TEST-00" understates it — the run keeps going and blames your change for 155 failures.** Observed 2026-09-19: launched as a background task, the window presented only the page background (a solid blue frame, page titles loading normally), TEST-00 errored, and the run then compared all 155 remaining tests against a crop offset it never obtained — `1/156 ERRORED` followed by `155/156 FAILED` at 92–100 % diff, including a dozen `KNOWN_DEBTORS РЕГРЕССИЯ: 99.93% > baseline 3.02%` lines that read as a ratchet catching a real regression. The same binary from a foreground process gave `TEST-00: PASS (0.00%)` and the normal `12/156 FAILED`. Until [BUG-1062](../bugs/BUG-1062-OPEN.md) lands, treat any run whose TEST-00 errored as no measurement at all and discard the whole log — do not read its debtor-ratchet claims. `--ipc` has no such failure mode (the snapshot arrives at (0,0), no calibration).
- **A single hung `ffmpeg` call kills the entire run, and `--continue-on-fail` does not save it.** `ffmpeg_diff`/`ffmpeg_crop` call `subprocess.run(timeout=15)` with no `try`, so `TimeoutExpired` propagates out of `run_one` and ends the process mid-set — two runs on 2026-09-19 died at TEST-01 and TEST-21. The hang does not reproduce in isolation (6/6 clean), so it reads as random; with two ffmpeg calls per test a 156-test set rarely finishes. Passing `stdin=subprocess.DEVNULL` went ~300 calls without a hang, which points at ffmpeg blocking on an inherited stdin pipe it reads for interactive commands. Tracked as [BUG-1061](../bugs/BUG-1061-OPEN.md); if a run dies with a `TimeoutExpired` traceback, nothing is wrong with your change — just re-run.
