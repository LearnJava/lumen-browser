...
- **An `.xhtml`/`.xht`/`.svg` page still has no real XML path** — it is HTML-parsed with point-fixes, not a conforming XML parser ([BUG-786](../bugs/BUG-786-OPEN.md)/[BUG-685](../bugs/BUG-685-OPEN.md)). All three measured `<script>` shapes now work (GAP-XMLDOC срезы 1/2/5): CDATA-wrapped inline bodies, a self-closing `<script src="…"/>`, and a prefixed `<h:script src>` (namespace resolved, request fires). What is still genuinely unhandled: self-closing `<table>`/`<select>`/`<button>` (0 hits in the vendored corpus, unmeasured because absent) and a `<title>`/`<script>` whose RAWTEXT/RCDATA state the tokenizer picks by bare tag name, not tree namespace.
