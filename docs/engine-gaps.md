...
- **An MCP `click` that misses its target answers `success` anyway** ([BUG-1044](../bugs/BUG-1044-OPEN.md)): the selector resolves to a point, the point hit-tests into whatever box actually covers it, and the caller is told the click landed. A probe then reads "the handler never ran" when the click never reached the element. Assert on the event *target*, not on the handler's side effect. (The `<button>`-shaped instance of this — a control with no CSS width laying out zero-wide, so every click fell through to the parent — is gone since [BUG-926](../bugs/BUG-926-FIXED.md), 2026-09-10.)
