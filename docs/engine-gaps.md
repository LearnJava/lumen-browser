...
- **`javascript:` URLs never execute** anywhere ([BUG-884](../bugs/BUG-884-OPEN.md)); `window.close()` is a no-op and `window.closed`/`name` are `undefined` ([BUG-887](../bugs/BUG-887-FIXED.md)); `document.open()`/`close()` do not exist ([BUG-888](../bugs/BUG-888-OPEN.md)); an entry made by `history.pushState(state, "")` (no URL argument) fires no `popstate` on traversal ([BUG-886](../bugs/BUG-886-OPEN.md)).
