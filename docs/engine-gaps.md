...
- **The live global `document` is not a `Node`** — the literal carries `appendChild`/`contains`/`childNodes` but no `removeChild`/`insertBefore`/`replaceChild`/`cloneNode`/`firstChild`/`lastChild`/`parentNode`/`textContent`, and `document instanceof Node` is `false` ([BUG-557](../bugs/BUG-557-OPEN.md), same root as BUG-1046). A framework that treats `document` as a mutation container hangs rather than fails: React 18's `hydrateRoot(document, …)` — what every Next.js App Router page does — retries forever on `document.removeChild is not a function`, 130 k repeats in 30 s, and `document_ready` never arrives. Root a hydration probe at a `<div>` container; that path is measured good (`samples/e2e4-hydration`).
- **A `<button>` with no CSS width lays out zero-wide** ([BUG-926](../bugs/BUG-926-OPEN.md)), and an MCP `click` aimed at it silently lands on the parent instead of failing ([BUG-1044](../bugs/BUG-1044-OPEN.md)). A probe that drives a button therefore reads "the handler never ran" when the click never reached the element. Give the button an explicit `width`/`display:block`, or assert on the event target rather than on the handler's side effect.
