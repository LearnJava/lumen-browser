...
- Punycode (RFC 3492) и homograph-проверки IDN — часть решения «что показать в адресной строке», см. `lumen-core::idn`. **Сам разбор URL закрыт `LIB-6`** (2026-09-01): scheme/host/path/query/fragment splitting, percent-encoding, dot-segment removal и `resolve()` теперь конечный автомат WHATWG через крейт `url` (см. Permanent exceptions ниже) — своя ~370-строчная реализация проходила 41.95% `urltestdata.json` (`LIB-0`), новая — 81.86%;
...
- TrueType-парсинга (`lumen-font`: cmap/glyf/CFF/COLR/CPAL/fvar/gvar — variable и color fonts), подбора шрифтов и растеризации глифов. **Text shaping закрыт `rustybuzz`** (LIB-1, 2026-08-31, production default с LIB-2, 2026-08-31; см. таблицу Permanent exceptions ниже) — свой GSUB/GPOS движок (GSUB 1/4 из 8, GPOS 1/2 из 9, без диакритики и сложных письменностей) удалён целиком в `LIB-3` (2026-09-01), `rustybuzz` — единственный `TextShaper` за trait-anchor `lumen_core::ext::TextShaper`;
...
| **`rustls-pemfile`** | PEM-декодирование X.509-сертификатов и приватных ключей — используется `tls::trusted_root_store()` (`crates/network/src/tls/mod.rs`) для загрузки дополнительного CA из файла, названного `LUMEN_EXTRA_CA_CERT` ([BUG-785](../../bugs/BUG-785-FIXED.md)) | `TlsBackend` (тот же, что `rustls`) | Partner-crate к `rustls` (тот же upstream org). Своя реализация base64/PEM-разбора сертификатных данных — тот же «не пишите свой crypto» антипаттерн |
...
| **`unicode-bidi`, `unicode-bidi-mirroring`** | UAX #9 Unicode Bidirectional Algorithm (правила P2–I2) и таблица `Bidi_Mirroring_Glyph` (правило L4) — свойства `direction` / `unicode-bidi` в `lumen-layout::bidi`. `unicode-bidi` уже был в дереве через `lumen-encoding` (`Icu4xUnicodeProvider::bidi_runs`), `unicode-bidi-mirroring` — транзитивно через text-стек | нет (чистые Unicode-таблицы, не заменяемая подсистема) | Алгоритму нужно свойство `Bidi_Class` **каждой** кодовой точки Unicode; своя таблица — это генератор из UCD и её ежегодная синхронизация, без какой-либо ценности для идентичности Lumen. Крейты — pure-Rust данные + ~1500 строк правил, без `unsafe` и без C |
| **JS engine** (`v8` v150.1.0 — **дефолт с cutover S12, ADR-018, 2026-07-14, единственный движок с S12b-F1, 2026-08-04; `rquickjs` полностью удалён из Cargo.toml/Cargo.lock срезом S12b-F4, 2026-08-04**) | Исполнение JavaScript | `JsRuntime` | V8 — 15 лет, миллиарды долларов, сотни инженеров. QuickJS был движком Phase 0–2; S0 go/no-go PASS 2026-07-13, cutover S12 2026-07-14. Новый код — только против V8-пути или движко-независимого шима (`WEB_API_SHIM`). |
| **`rustybuzz`** (0.14.1, `crates/engine/font`, обязательная зависимость с `LIB-3`) | Text shaping — OpenType `GSUB`/`GPOS` layout engine: mark attachment (GPOS 4), Arabic joining (GSUB 5/6/8), Indic reordering. Единственный `TextShaper`: свой GSUB/GPOS движок (`U-2`, GSUB 1/4 из 8, GPOS 1/2 из 9) удалён в `LIB-3` (2026-09-01) после того, как `LIB-2` (2026-08-31) продержал его продакшн-дефолтом через полный цикл пересъёмки graphic-tests/CPU/paint снимков без дрейфа. Уже был в дереве транзитивно через `femtovg` (её собственный, неиспользуемый text-путь) той же версии `0.14.1` — подключение не добавило новых версий зависимостей ни `rustybuzz`, ни `ttf-parser` | `lumen_core::ext::TextShaper` | Таблицы OpenType, решение принимает спека (ADR-027) |
| **`ttf-parser`** (0.21.1, транзитивно через `rustybuzz`) | Парсер таблиц шрифта для `rustybuzz`. Свой парсер (`lumen-font::face`) остаётся — растеризация, метрики, variable/color fonts читает он; оба парсера ленивые и заимствуют одни и те же байты шрифта без копирования, второй набор офсетных структур в памяти — не второй набор байтов шрифта | нет отдельного (внутренняя зависимость `rustybuzz`) | Уходит только вместе с `rustybuzz` |
| **`resvg`/`usvg`** (**=0.44.0** точно, не последняя 0.48 — `crates/engine/image`, обязательная зависимость с `LIB-4`, 2026-09-01) | Растеризация внешнего SVG (`<img src=*.svg>`, `background-image: url(*.svg)`) — `lumen_image::decode_svg` (`src/svg.rs`), диспетчеризуется из `decode_raw` по сигнатуре наравне с PNG/JPEG/GIF/WebP/AVIF. Свой SVG-движок (`box_tree/svg.rs`/`svg_path.rs`) рисует ноль `linearGradient`/`radialGradient`/`clipPath`/`mask`/`filter`/`pattern`/`marker` (измерение ADR-027) — растровый логотип с градиентом выходил плоским. Версия зафиксирована точно, потому что её `tiny-skia` — 0.11.4, та же, что уже несёт `lumen-paint`'s `cpu-render`: `cargo tree -i tiny-skia` держит одну версию при обоих фичах разом. Фича `raster-images` включена (вложенный `<image>`), `text`/`system-fonts` выключены (свои шрифты — детерминизм графгейта важнее рендера текста внутри SVG) | нет отдельного (`decode_raw`'s единая точка диспетчеризации по сигнатуре) | Таблицы SVG 1.1/2, решение принимает спека (ADR-027) |
| **`url`** (2.5.8, `crates/core`, обязательная зависимость с `LIB-6`, 2026-09-01) | Scheme/host/path/query/fragment splitting, percent-encoding normalization, dot-segment removal, relative-reference resolution — весь WHATWG URL Standard state machine, вызывается из `lumen_core::url::Url` (`inner: url::Url`). Своя ~370-строчная реализация проходила 41.95% `urltestdata.json` (`LIB-0`), сосредоточенных в двух целиком отсутствующих ветках алгоритма (IPv6-литералы `[...]`, forbidden/control host code points) — новая проходит 81.86%. `Url::host()` НЕ берёт значение из `url`: WHATWG-парсер всегда ASCII/IDNA-нормализует host (Punycode + lowercase), а адресная строка (`crates/shell/src/address_bar.rs::guard_display_text`, DS-6) ищет в тексте ТОЧНУЮ подстроку, которую ввёл пользователь — `host()` остаётся своей raw-экстракцией; `host_ascii()`/IDN homograph-проверки остаются на `lumen-core::idn`/`lumen-core::punycode`, как и решено в `LIB-6` | нет отдельного (`crate::url::Url` — сам swap-point) | Конечный автомат WHATWG, решение принимает спека (ADR-027) |
...
| `image` (feature `avif-native`, за cargo-фичей `avif` в `lumen-image`, не в default; GAP-avif срез 1, 2026-09-21) | Декодирование AVIF в RGBA через её `avif-native` (dav1d C + mp4parse) | `ImageDecoder` (`AvifImageDecoder`) | 1 | Едва ли. `dav1d-sys` требует системный `dav1d` через `pkg-config` — без него `cargo build --features avif` падает на build-скрипте `dav1d-sys` (проверено: нет `pkg-config` в тулчейне). Чистого-Rust AV1-декодера с готовой ISOBMFF-обвязкой нет (`rav1d` публикует только unsafe C-ABI) — своя реализация непропорциональна выгоде, пока AVIF не станет обязательным форматом |
| `jxl-oxide` (`lumen-image`, не за фичей — в default с GAP-avif срез 3, 2026-09-21) | Декодирование JPEG XL в RGBA (`jxl.rs::decode_jxl`) | `ImageDecoder` (`JxlImageDecoder`) | 1 | Едва ли. Чистый Rust, без cmake/meson/nasm/pkg-config (проверено `cargo tree`) — та же категория цены/выгоды, что у JPEG/WebP выше, но декодер CMYK/CMYKA и анимацию не покрывает; расширить их своими силами, если появится реальный спрос |
| `weezl` (прямая зависимость `lumen-image` с 2026-08-20, до этого — транзитивная через `gif`) | LZW-распаковка пикселей GIF-кадра собственным циклом (`lumen_image::gif::lzw_decode_into`) вместо `gif::FrameDecoder`, который на кадре с оборванным LZW-потоком не возвращает управление ([BUG-787](../../bugs/BUG-787-FIXED.md)) | `ImageDecoder` (тот же, что у GIF-декодера) | 2 | Уйдёт только вместе с `gif`: пока GIF-контейнер разбирает он, `weezl` в дереве всё равно, и отдельная замена ничего не убирает. Свой LZW (спек GIF §22, ~200 строк) осмыслен лишь если понадобится побитовый контроль сверх того, что даёт `decode_bytes` |
...
| Ручной FFI-слой (opaque-указатели) поверх FFmpeg shared-dev, БЕЗ `ffmpeg-next`/`ffmpeg-sys-next` (**статус: crate landed под feature `ffmpeg` (off by default), не подключён к `crates/js`**, решение — [ADR-030](../decisions/ADR-030-media-codec-strategy-ffmpeg.md), 2026-09-19) | Декодирование `video/mp4`/`video/webm`/`video/ogg` (GAP-MEDIADECODE) — сегодня декодируется только анимированный GIF. Пользователь выбрал «FFmpeg целиком» из трёх вариантов кодек-стратегии (альтернатива — узкий набор кодеков или внешний процесс-декодер) | `lumen_core::ext::VideoDecoder`/`VideoDecodeSession` реализованы в `crates/engine/media-ffmpeg` (`FfmpegVideoDecoder`/`FfmpegSession`), срез 4 | Таблицы видео-кодеков (H.264/VP8/VP9/AV1/…), решение принимает спека (ADR-027). `ffmpeg-sys-next`'s `bindgen`-генерация FFI на целевой `x86_64-pc-windows-msvc` даёт несовпадающий layout для набора структур FFmpeg И `libc`'s `struct tm` (см. ADR-030) — этот путь остаётся отклонён. Ревизии 2026-09-19 (срезы 2-3) подтвердили живым прогоном в scratch-PoC (`D:\Temp\ffmpeg-ffi-poc`, не коммитился), что обходной путь работает целиком, включая декодирующий цикл и `sws_scale`. Срез 4 перенёс это в реальный крейт `crates/engine/media-ffmpeg`: hand-rolled FFI (`src/ffi.rs`), `VideoDecoder`/`VideoDecodeSession` impl поверх custom `AVIOContext` (декодирует `&[u8]` в памяти, не путь на диске — обязательное условие для сетевого/Blob-ответа `<video>`), `av_seek_frame`+`avcodec_flush_buffers` для `frame_at(secs)`, `build.rs` резолвит `FFMPEG_DIR`. Крейт спрятан за feature `ffmpeg` (default off): без неё `cargo build --workspace` не требует FFmpeg вовсе ни на одной машине. Живые тесты (`cargo test -p lumen-media-ffmpeg --features ffmpeg`, нужен `FFMPEG_DIR` + DLL на `PATH`) декодируют `2x2-green.webm` (VP8/9) и `test.mp4` (H.264) из `tests/wpt/`. Не сделано: подключение к `crates/js/src/video_bindings.rs`'s resource-selection алгоритму (следующий срез), лицензионная заметка про GPL-компоненты FFmpeg-дистрибутива (не `cargo-deny`-проблема — в `Cargo.lock` не появилось ни одной новой лицензируемой Rust-зависимости, поскольку FFI ручной; риск — в связанных DLL, а не в графе Cargo) |
