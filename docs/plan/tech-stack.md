...
| `ffmpeg-next` + `ffmpeg-sys-next` (**статус: blocked, не в `Cargo.toml`**, решение — [ADR-030](../decisions/ADR-030-media-codec-strategy-ffmpeg.md), 2026-09-19) | Декодирование `video/mp4`/`video/webm`/`video/ogg` (GAP-MEDIADECODE) — сегодня декодируется только анимированный GIF. Пользователь выбрал «FFmpeg целиком» из трёх вариантов кодек-стратегии (альтернатива — узкий набор кодеков или внешний процесс-декодер) | `lumen_core::ext::VideoDecoder` (планируется, по образцу `ImageDecoder`) | Таблицы видео-кодеков (H.264/VP8/VP9/AV1/…), решение принимает спека (ADR-027) — но подключение уже упёрлось в живой блокер: `ffmpeg-sys-next`'s `bindgen`-генерация FFI на целевой `x86_64-pc-windows-msvc` даёт несовпадающий layout для набора структур FFmpeg И `libc`'s `struct tm` (компилятивная ассерция `size_of` падает антифлоу-вычитанием) — воспроизведено на FFmpeg 9.0.1 и 7.1 (обе — shared-dev дистрибутивы с headers+import libs, без сборки FFmpeg из исходников), с INCLUDE/LIB от `vcvars64.bat` и без. Не проблема версии FFmpeg, не проблема отсутствующих заголовков (`avfft.h` пришлось довезти из тега `n7.1` — сам FFmpeg 8+ его больше не поставляет), не проблема MSVC-окружения (прямой `clang -target x86_64-pc-windows-msvc -Xclang -ast-dump` на `<time.h>` корректно даёт `sizeof(struct tm) == 36`) — это дефект именно `bindgen`'а на этапе AST→Rust-структура. Подробности и следующий шаг (написать минимальный ручной FFI-слой с opaque-указателями вместо `bindgen`-структур, или откатиться на более старый `ffmpeg-sys-next`) — ADR-030 |
