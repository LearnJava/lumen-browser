...
- TrueType-парсинга (`lumen-font`: cmap/glyf/CFF/COLR/CPAL/fvar/gvar — variable и color fonts), подбора шрифтов и растеризации глифов. **Text shaping закрыт `rustybuzz` (LIB-1, 2026-08-31; см. таблицу Permanent exceptions ниже)** — свой GSUB/GPOS движок (`crate::shape::Shaper`, GSUB 1/4 из 8, GPOS 1/2 из 9, без диакритики и сложных письменностей) остаётся вторым `TextShaper` за trait-anchor `lumen_core::ext::TextShaper` и активен по умолчанию до `LIB-2` (пересъёмка эталонов после смены шейпера) — удаляется в `LIB-3`;
...
| **`rustybuzz`** (0.14.1, `crates/engine/font`, feature `rustybuzz-shaping`, default on) | Text shaping — OpenType `GSUB`/`GPOS` layout engine: mark attachment (GPOS 4), Arabic joining (GSUB 5/6/8), Indic reordering, всё, чего нет у своего движка (LIB-1, 2026-08-31). Уже был в дереве транзитивно через `femtovg` (её собственный, неиспользуемый text-путь) той же версии `0.14.1` — подключение не добавило новых версий зависимостей ни `rustybuzz`, ни `ttf-parser` | `lumen_core::ext::TextShaper` | Свой движок закрыт задачей `U-2` как done, но покрывает лишь GSUB 1/4 из 8, GPOS 1/2 из 9 — таблицы OpenType, решение принимает спека (ADR-027). Не активен по умолчанию до `LIB-2` (пересъёмка graphic-tests/CPU/paint снимков, которые смена шейпера двигает); включается раньше через `LUMEN_RUSTYBUZZ_SHAPING=1`. Измерено (release, bundled Inter, EN+RU предложение ~90 символов): ~32 мкс/вызов у своего движка против ~199 мкс/вызов у `rustybuzz` (~6×) — ожидаемо, `rustybuzz` прогоняет весь OT-конвейер, а не только Type 1/2/4 lookup'ы |
