...
| Ручной FFI-слой (opaque-указатели) поверх FFmpeg shared-dev, БЕЗ `ffmpeg-next`/`ffmpeg-sys-next` (**статус: blocked на самом FFI-слое, не в `Cargo.toml`**, решение — [ADR-030](../decisions/ADR-030-media-codec-strategy-ffmpeg.md), 2026-09-19) | Декодирование `video/mp4`/`video/webm`/`video/ogg` (GAP-MEDIADECODE) — сегодня декодируется только анимированный GIF. Пользователь выбрал «FFmpeg целиком» из трёх вариантов кодек-стратегии (альтернатива — узкий набор кодеков или внешний процесс-декодер) | `lumen_core::ext::VideoDecoder`/`VideoDecodeSession` (trait-anchor landed, `crates/core/src/ext.rs`) | Таблицы видео-кодеков (H.264/VP8/VP9/AV1/…), решение принимает спека (ADR-027). `ffmpeg-sys-next`'s `bindgen`-генерация FFI на целевой `x86_64-pc-windows-msvc` даёт несовпадающий layout для набора структур FFmpeg И `libc`'s `struct tm` (см. ADR-030) — этот путь остаётся отклонён. Ревизия 2026-09-19 (срез 2) подтвердила живым прогоном, что обходной путь сам работает: ручной `extern "C"` FFI (opaque `#[repr(C)] struct AVFormatContext { _private: [u8; 0] }`, БЕЗ `bindgen`) линкуется против того же FFmpeg 7.1 shared-dev (`avformat.lib`/`avcodec.lib`/`avutil.lib`/`swscale.lib`) и живьём открывает реальный `.webm`-файл (`avformat_open_input` → `avformat_find_stream_info` → `avformat_close_input`, все три успешны) — доказательство было сделано как одноразовый scratch-проект вне репозитория (`D:\Temp\ffmpeg-ffi-poc`, не коммитился), сам код в дереве Lumen не landed. Следующий шаг — реальный крейт (`crates/media-ffmpeg` или аналог) с декодирующим циклом (`avcodec_send_packet`/`avcodec_receive_frame`/`sws_scale` в RGBA8) и реализацией `VideoDecoder` — не начат, это отдельная по объёму задача (декодирующий цикл требует ABI-стабильных полей `AVFrame`/`AVCodecContext`, которые ручной слой должен объявить сам, а не через `bindgen`) |
