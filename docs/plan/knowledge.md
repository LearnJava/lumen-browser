...
**Архитектурная основа:** кастомизация и сменяемость интерфейсов опираются на абстракцию
`ChromeView` за контрактом `BrowserController` — см. [ADR-015](../decisions/ADR-015-swappable-chrome-view.md).
Поддерживаются **два бэкенда** (нативный Rust-UI и web-chrome, отрисовываемый самим движком),
выбираемые через `LUMEN_CHROME`/feature-флаг; выбор «победителя» сознательно отложен. Состояние
живёт в модели → смена интерфейса = переинстанцировать вид над той же моделью; новый интерфейс =
новый бандл/профиль/плагин без правок движка.

**Фаза:** 2-3 (тема/токены — сейчас; полная сменяемость интерфейсов — Phase 3+, ADR-015).
