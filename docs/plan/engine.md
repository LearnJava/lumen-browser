...
- **v1.0:** **V8** через `rusty_v8` (Deno-style) или **SpiderMonkey** через `mozjs`. V8 быстрее, SpiderMonkey ближе по духу. **Рекомендация: V8** — больше документации, тесты Deno как референс. **Сделано: V8 — дефолтный движок с cutover S12 (ADR-018, 2026-07-14); rquickjs — rollback-путь, удаляется срезами S12b.**
