...
- **Базовая пригодность shell** — без этого «открыть Habr-статью» невозможно как демо:
  ...
  - ✅ **Progressive / streaming rendering pipeline (PH1-2).** Реализовано: окно создаётся первым + троттлинг ~60 Гц + параллельный fetch CSS (PH1-2), TCP-level streaming HTML body (PH1-2a), инкрементальный dirty-subtree layout (PH1-2b), прогрессивная подгрузка картинок (PH1-2c). Детали — `git log --grep="PH1-2"`, текущее состояние — `CAPABILITIES.md`.
...
### Фаза 2 — v0.5 «Interactive» (18–24 месяца) ✅ завершена (app v0.5.0)
- QuickJS интеграция.
- Tier 1 Web APIs.
