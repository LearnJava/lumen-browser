...
> Точка входа для следующей сессии с готовым планом на три независимые
> задачи (ускорить `compute_style`, `LUMEN_ENGINE_THREAD` по умолчанию,
> частичный пересчёт стилей поддерева) — [docs/tasks/p3-cascade-perf.md](../tasks/p3-cascade-perf.md).
...
6. **`content-visibility: auto` во время скролла вызывает ПОЛНЫЙ
   `self.relayout()`, не облегчённый путь (обнаружено 2026-07-15, маршрутизация
   исправлена в [BUG-286](../../bugs/BUG-286-OPEN.md)).** Реальный тест колесом
   мыши на ria.ru поймал `[engine] relayout 1041.99ms` прямо посреди скролла —
   именно это ощущается как «задумался на секунду». `maybe_expand_cv_relevant`
   (`shell/src/main.rs`) звала `self.relayout()` напрямую, а не
   `relayout_raf_dirty()`, которым уже пользуются другие триггеры relayout
   (rAF-грязь, zoom-дебаунс) — BUG-286 переключил её на `relayout_raf_dirty()`.
   Под `LUMEN_ENGINE_THREAD=1` это уводит relayout на движковый поток
   (не блокирует UI-поток); на дефолтной сборке (флаг выключен) поведение и
   стоимость на UI-потоке не меняются — `relayout_raf_dirty()` падает обратно
   на тот же incremental-затем-full путь. Даже incremental не спас бы много —
   ...
   `compute_style`). Реальный фикс стоимости всё ещё упирается в пункты 1-4
   выше (ускорить сам каскад) — BUG-286 закрыл только рассинхронизацию
   маршрутизации, не корневую стоимость.
