...
| **Legacy deletion (CC-15, done 2026-07-28)** | Slice-by-slice removal of `toolbar.rs`'s builder, `tabs/strip.rs`'s renderer, `panels/*`'s `DisplayCommand` code, and the orphan `Palette` fields, by the same pattern S12b used to delete `rquickjs` after ADR-018. Closed by CC-15-6, which deleted the `LUMEN_LEGACY_CHROME` flag itself — the rollback window lasted one day, exactly as long as it took to prove the flip and cut the old path. Chrome surfaces the mock-up doesn't reach — reader view, source view, split view, real DevTools panels, the profile popover, ~30 panels total — stay legacy overlays indefinitely per the brief's risk #5; they were never behind the flag. Parity gaps the deletion surfaced are tracked as BUG-408…BUG-415, not as reasons to keep a rollback path |
