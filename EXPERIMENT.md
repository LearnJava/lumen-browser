...
2. **Ярус 1 — не рисовать лишнее** (главный рычаг; приёмы — см. «Карта
   ...
   - ~~вынос загрузки face-ов/ресайзов картинок с render-пути~~ **СДЕЛАНО
     ...
     Открытые хвосты:
     - тёплый остаток фазы faces = `hash_display_list` skip-frame хэша
       (Debug-fmt по 1062 командам, 1.2–2.5 мс/кадр). Кандидат:
       поколение DL на стороне шелла (шелл знает, когда пересобрал
       paint_ordered/патчил скролл) вместо тотального хэша; или быстрый
       структурный хэш без fmt-машинерии;
     - imgpre можно добить GPU-путём: рисовать оригинальную текстуру с
       ...
       (память), и остаток 24–32 мс холодного кадра;
