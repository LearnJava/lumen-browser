...
Fallback-цепочка: DX12 → Vulkan → GL. Открытый вопрос пользователю: сделать ли
GL дефолтом на этой машине (это тот же wgpu-рендер поверх GL-HAL, не femtovg).

## Очередь работ (по приоритету, цель 100–1000×)

1. **Ярус 0 — авто-проба бэкенда**: при старте пробный кадр + readback;
   Vulkan если пиксели не пустые (быстрейший API), иначе GL, DX12 — резерв.
   Инфраструктура readback уже есть (`render_to_image`). Часы работы.
