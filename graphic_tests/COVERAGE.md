...
| 32-list-markers.html | list markers | 21 | display:list-item · ::marker box · list-style-type: disc/circle/square/decimal/lower-alpha/lower-roman · list-style-position: outside/inside · list-style-type:none · ::marker { color } CSS override · ::marker { content } CSS override · **@counter-style** custom (alphabetic+prefix, numeric+prefix) · **list-style-type: \<custom-ident\>** lookup in CounterStyleRegistry · **list-style-image: url()** (image replaces bullet, CSS Lists L3 §2.3) |
...
| 51-scrollbar-rendering.html | Scrollbar rendering (DrawScrollbar) | 6 | overflow-y:scroll vertical scrollbar · overflow-x:scroll horizontal scrollbar · overflow:scroll both axes · no scrollbar when content fits · CC-CSS-1 `::-webkit-scrollbar`/`-thumb`/`-track` mapped onto scrollbar-width/scrollbar-color vs. the equivalent standard-property box |
...
| 1000000-final.html (initial-letter demo) | `initial-letter` drop cap (CSS Inline Layout L3 §5) | 2 | `::first-letter { initial-letter: 3 }` three-line drop cap (size only, sink = floor(3)) · `initial-letter: 3 2` sunken cap (size 3, sink 2 reserved lines). Letter promoted to inline-start float spanning `size × line-height`, text wraps beside it. Same text-parity debtor class as TEST-58 (BUG-100: Inter↔Edge metrics + large-glyph AA); dedicated unit-test wiring deferred to a measured graphic run. |
...
| 115-empty-cells.html | CSS Tables L2 §17.6.1.1 `empty-cells` | 2 | `empty-cells: hide` — empty `<td></td>` draws neither border nor background (separated-borders model) · `empty-cells: show` (initial) — every cell keeps border + background · checkerboard of filled/empty cells, no text |
| 116-gradient-interpolation.html | CSS Images L4 §3.1 — gradient `<color-interpolation-method>` (`in <space>`) | 5 | Same red→blue `linear-gradient(to right in <space>, …)` per strip: srgb · srgb-linear · oklab · lab · hsl — Lumen subdivides the stop list via color-mix maths, renderer lerps dense stops in sRGB; polar oklch/lch excluded pending BUG-154 |
| 117-quotes.html | CSS Generated Content L3 §3.2 — `quotes` + `content: open-quote/close-quote` | 4 | `auto` curly quotes “ ” · nested `<q>` → outer primary “ ”, inner secondary ‘ ’ (depth tracked in document order) · custom pairs « » / ‹ › via `quotes:` · `quotes: none` emits no marks |
| 118-media-hover-pointer.html | Media Queries L4 §5.3-5.6 — `hover`/`any-hover`/`pointer`/`any-pointer` | 6 | desktop (mouse) matches `(hover: hover)` · `(any-hover: hover)` · `(pointer: fine)` · `(any-pointer: fine)` → green swatches · `(hover: none)` and `(pointer: coarse)` do not match → red swatches |
| 119-paint-order.html | CSS Fill & Stroke L3 §6 / SVG 2 §13.7 — `paint-order` | 4 | thick-stroked closed `<path>` squares (stroke centred on the outline) · top row `normal` → full stroke band painted over the fill · bottom row `paint-order: stroke` → fill painted over the stroke, hiding its inner half (visible stroke ~half as thick, fill area larger) · two colour pairs (yellow/black, teal/crimson) |
| 120-media-contrast-data.html | Media Queries L5 §5.5-5.6 — `prefers-contrast`/`prefers-reduced-data` | 6 | no user preference matches `(prefers-contrast: no-preference)` · `(prefers-reduced-data: no-preference)` → green swatches · `(prefers-contrast: more/less/custom)` and `(prefers-reduced-data: reduce)` do not match → red swatches |
| 121-supports-selector.html | CSS Conditional L4 §4.2 — `@supports selector()` | 6 | recognised selectors `selector(:has(*))`/`selector(:is(...))`/`selector(::slotted(*))` apply the block → green swatches · unknown pseudos `selector(:totally-unknown-pseudo)`/`selector(::made-up-element)` do not apply → red swatches · `not selector(<unknown>)` → green |
| 122-line-height-step.html | CSS Rhythmic Sizing L1 §2 — `line-height-step` | 4 | natural column (no step) → 24px line bands · `line-height-step: 48px` column → 48px line bands at the stepped rhythm · single short line rounded up to a 60px band · child without its own step inherits the parent's 40px rhythm · coloured inline `<span>` backgrounds fill the stepped line box (non-text signal) |
| 123-supports-font-tech-format.html | CSS Conditional L4 §4 / Fonts L4 §4.3 — `@supports font-tech()`/`font-format()` | 7 | implemented `font-tech(variations)`/`font-tech(features-opentype)` and decodable `font-format(woff2)`/`font-format(truetype)` apply the block → green swatches · `font-tech(features-graphite)`/`font-format(embedded-opentype)` unsupported in both Lumen and Edge → red swatches · `not font-tech(features-graphite)` → green |
| 124-prefers-reduced-transparency.html | Media Queries L5 §5.7 — `prefers-reduced-transparency` | 3 | no user preference matches `(prefers-reduced-transparency: no-preference)` → green swatch · `(prefers-reduced-transparency: reduce)` does not match → red swatch · invalid `low` → Unsupported → red swatch · Edge also defaults to no-preference |
| 125-media-scripting.html | Media Queries L5 §6.2 — `scripting` | 4 | Lumen ships QuickJS so default is `scripting: enabled` → green swatch · `(scripting: none)` / `(scripting: initial-only)` do not match → red swatches · invalid `sometimes` → Unsupported → red swatch · Edge also reports scripting enabled |
| 126-media-inverted-colors.html | Media Queries L5 §5.8 — `inverted-colors` | 3 | no colour inversion matches `(inverted-colors: none)` → green swatch · `(inverted-colors: inverted)` does not match → red swatch · invalid `maybe` → Unsupported → red swatch · Edge also defaults to inverted-colors:none |
| 127-svg-text-anchor.html | SVG 2 §11.6 / §11.10.2 — `text-anchor` as a CSS property | 5 | magenta reference line at the anchor x · `text-anchor="start/middle/end"` attribute rows shift text right/centred/left of the line · `.css-mid { text-anchor: middle }` row (no attribute) → centred · `.css-end` overrides a conflicting `text-anchor="start"` attribute → left · glyph rasterisation diverges from Edge (rule #3) so the gate is the deterministic CPU snapshot + `--dump-display-list`, not the Edge pixel pipeline |
| 128-baseline-shift.html | SVG 1.1 §10.9.2 / CSS Inline L3 §5.2 — `baseline-shift` as a CSS property | 5 | magenta horizontal reference lines at each `y` baseline · attribute `baseline-shift="super"` row raises text above line · attribute `sub` row lowers text · CSS `baseline-shift: 24px` row raises by 24 px · CSS `super` overrides a conflicting `baseline-shift="sub"` attribute · glyph rasterisation diverges from Edge (rule #3) so the gate is the deterministic CPU snapshot + `--dump-display-list`, not the Edge pixel pipeline |
...
| 129-svg-dynamic-createelementns.html | BUG-243 — dynamic SVG built at runtime via `document.createElementNS` (no text) | 5 | `createElementNS(svg-ns, …)` creates NATIVE arena nodes (with `__nid__`) so layout/paint see them · `setAttribute` drives geometry/fill · `appendChild` + variadic `append()` attach children · `<g transform=translate>` group · solid `rect`/`circle`/filled `path` only (no `<text>` → no font-parity noise, holds ≤0.5%) · `firstChild`+`removeChild` clear loop · REQUIRES the JS capture path (gdigrab), not `--ipc`/CPU |
| 130-svg-dynamic-nested-transforms.html | BUG-243 — dynamic SVG, nested `<g>` transform composition (no text) | 2 | 16-deep spiral of nested squares (cumulative `rotate(11) scale(0.86)`) · 4×4 catalogue of a motif at increasing `translate()+scale()` · stresses `parse_svg_transform.compose` depth · JS capture (gdigrab) only |
| 131-svg-dynamic-opacity-layering.html | BUG-243 — dynamic SVG, alpha compositing (no text) | 4 | translucent RGB Venn discs (`fill-opacity:0.5`) · fill-opacity ramp 0.1→1.0 · stacked same-colour squares deepening tint · translucent concentric strokes (`stroke-opacity`) · straight sRGB alpha must match Edge · JS capture (gdigrab) only |
| 132-svg-dynamic-use-symbol.html | BUG-243 — dynamic SVG, `<defs>/<symbol>/<use>` instancing (no text) | 2 | multi-shape `badge`/`ring` symbols defined once · 5×8 grid of `<use>` at varying sizes · 12-instance rotated ring (`<use>` + `rotate(a cx cy)`) · shadow-clone path built from JS · JS capture (gdigrab) only |
| 133-svg-dynamic-fill-rule.html | BUG-243 — dynamic SVG, `fill-rule` nonzero vs evenodd (no text) | 3 | self-intersecting pentagram `<path>` columns (nonzero solid vs evenodd hollow) · concentric square-ring path (evenodd shows holes) · non-intersecting `<polygon>` chevron strip control · JS capture (gdigrab) only |
| 134-svg-dynamic-stroke-styles.html | BUG-243 — dynamic SVG, stroke presentation (no text) | 4 | concentric stroked `<rect>` frames (widths 4→19) · dashed `<line>`s with butt/round/square caps + varying dasharray · stepped `<polyline>` miter/round/bevel joins · `stroke-dashoffset` pair · JS capture (gdigrab) only |
| 135-svg-dynamic-paint-order.html | BUG-243 — dynamic SVG, document-order paint / z-stack (no text) | 4 | cascading overlapped squares · overlapping circle chain · woven vertical×horizontal bar grid · fanned rotated cards · later siblings paint over earlier, overlaps must match Edge · JS capture (gdigrab) only |
| 136-svg-dynamic-ellipse-radial.html | BUG-243 — dynamic SVG, `<circle>/<ellipse>` geometry (no text) | 4 | concentric wide ellipses (alt-colour rings) · circle bullseye · radial cluster of varying-radius circles · ellipse row with growing `ry` · curved edges align with Edge · JS capture (gdigrab) only |
| 137-svg-dynamic-transform-pinwheel.html | BUG-243 — dynamic SVG, `rotate/scale/matrix/skew` transforms (no text) | 4 | two blade pinwheels via `rotate(a cx cy)` · ring of rotated rounded rects · mirrored `matrix(±1.4 …)` chevrons · `skewX(-20)` bar grid · covers every `parse_svg_transform` branch · JS capture (gdigrab) only |
| 138-svg-dynamic-dashboard.html | BUG-243 — dynamic SVG, kitchen-sink dashboard (no text) | 4 | nested `<g>` panels · `<defs>/<use>` KPI dots · bar chart · polygonal-sector "pie" `<path>` · area+line chart (`<polyline>` stroke + `fill-opacity` area) · `<circle>/<ellipse>` gauge with rotated marker · broadest BUG-243 composition · JS capture (gdigrab) only |
| 139-float-flow.html | RP-4 — general float flow into nested blocks (CSS 2.1 §9.5) | 3 | `clear:left` in a non-BFC wrapper clears the enclosing context's float (blue block drops below the red float, full width) · `overflow:hidden` BFC block shifts past the float's right edge instead of overlapping · float nested in a non-BFC wrapper stacks to the right of the outer float (inherited float context) · solid boxes only, no overlaps (text ignored, rule 3) |
...
| 149-nested-overflow-scroll.html | CC-CSS-2 — INTERACTION: overflow:auto вложенность × border-radius × z-index (deps: 14, 36, 38) | 3 | outer+inner overflow:auto — независимые скроллбары (роутинг wheel к innermost-контейнеру покрыт unit-тестами lumen-layout, не диффом) · вложенный border-radius clip 16px/10px · z-index бейдж внутри вложенного scroll-слоя остаётся в своём scroll-layer (BUG-159 класс) |
| 150-font-variant-caps.html | CSS Fonts L4 §6.2 — font-variant-caps: весь набор значений на одном тексте | 6 | font-variant-caps: normal · small-caps · all-small-caps · petite-caps · all-petite-caps · unicase. Капитель синтезируется в layout-е (bundled-Inter без smcp/c2sc/pcap); цифры и пробелы не затрагиваются. titling-caps — не покрыт диффом (фича `titl` отсутствует в шрифте → no-op, проверен unit-тестом) |
| 151-unicode-bidi.html | CSS Writing Modes L4 §2.2 + UAX #9 — unicode-bidi × direction на одном латинском тексте | 6 | ltr/normal (база) · rtl/normal (порядок слов сохранён, выравнивание вправо) · rtl/bidi-override (разворот и слов, и букв) · ltr/bidi-override (контроль-«ноль») · вложенный span с rtl/bidi-override (развёрнуто одно слово) · вложенный span с rtl/embed (второй контроль-«ноль»). Латиница взята намеренно — глифы есть в bundled-Inter, поэтому расхождение нельзя списать на tofu. `isolate`/`isolate-override`/`plaintext` — не покрыты диффом (на чистой латинице визуально совпадают с `normal`), проверены unit-тестами `bidi::tests` |
| 152-grid-place-content.html | CSS Box Alignment L3 §5 + CSS Grid L1 §12.3 — распределение свободного места grid-контейнера между треками | 14 | justify-content: start · center · end · space-between · space-around · space-evenly (inline-ось) · align-content: center · end · space-between (блочная ось) · place-content: `end center` (два значения) · place-content: `space-evenly` (одно значение на обе оси) · align-content: normal → stretch auto-строк при заданной высоте · item со `grid-column: 1 / 4` поверх раздвинутых space-between зазоров (ячейка span-а считается по офсетам треков, а не по сумме размеров) · контроль переполнения: треки выше панели центрируются *unsafely* (§5.3 — сдвиг за start-край, как в Edge). Только solid-боксы, без текста |
| 153-zoom.html | CSS Viewport L1 §5 — zoom: множитель вложен в вычисленные длины, бокс реально занимает меньше места | 6 | эталон без zoom · тот же бокс под `zoom: 0.5` · `zoom: 80%` (та же величина другой записью) · перемножение по дереву `0.8 × 0.5` · три обязанные совпасть полосы (нет zoom / `normal` / невалидное значение — отбрасывается по CSS Syntax) · строка фиксированной ширины 440px с min-width/gap/margin, влезающая в 234px-панель только после масштабирования (форма tbank.ru). Все длины абсолютные: относительные резолвятся против уже отмасштабированных базисов и покрыты юнит-тестами `zoom_*` в style.rs. Не покрыто диффом: `line-height`, `rem` и `border-width` в `em` под zoom (известные ограничения, см. CSS-SPECS.md). Только solid-боксы, без текста |
| 155-svg-inline-gradients.html | LIB-5 (ADR-027) — inline `<linearGradient>`/`<radialGradient>` для `fill="url(#id)"` на живых SVG-элементах, дерево/каскад/хит-тест остаются свои (в отличие от LIB-4's внешнего SVG через resvg) | 6 | горизонтальный linearGradient на rect · radialGradient на circle · вертикальный 3-стопный linearGradient на path (клип через `ResolvedClipShape::Polygon`) · `gradientUnits="userSpaceOnUse"` на скруглённом rect · `stop-opacity` (красный затухает поверх синего фона) · наследование `fill:url()` через `<g transform="rotate(...)">`. Реализация: `SvgPaint::Url` резолвится в `SvgPaint::Gradient` при построении box tree (`box_tree/svg.rs` — единственное место с доступом к `Document`), paint клипует форму (`PushClipRect`/`PushClipRoundedRect`/`PushClipPath`) и рисует `DrawLinearGradient`/`DrawRadialGradient` внутри — переиспользует существующий CSS-градиентный рендер без нового бэкенд-кода. Не покрыто: `mask`/`clipPath`/`pattern`/`marker`, градиент на stroke (падает на плоский цвет первого stop'а), путь с несколькими subpath (клипуется только по первому контуру) |
| 156-svg-mask.html | LIB-9 (остаток LIB-5) — SVG `<mask>` элемент через `PushMaskLayer`/`PopMaskLayer` (CSS Masking L1 §5); инфраструктура была реализована во всех трёх бэкендах, но её не строил ни один emit-путь | 6 | rect замаскирован в левую половину · circle замаскирован меньшим circle · составная маска из трёх rect ("H") · luminance по умолчанию — серый `#808080` даёт полупрозрачность · `mask-type="alpha"` — тот же серый даёт полную видимость (прямое сравнение двух режимов на одной заливке) · path замаскирован через CSS `mask:` (не только presentation-атрибут). Реализация: `mask="url(#id)"`/CSS `mask` резолвится в `BoxKind::SvgShape::svg_mask` (`box_tree/svg.rs::resolve_svg_mask`, тот же паттерн LIB-5 для градиентов); paint эмитит изолирующий `PushOpacity{1.0}` + собственную заливку формы + `PushMaskLayer` + контент маски + `PopMaskLayer` + `PopOpacity` (`svg_text_decoration.rs::emit_svg_shape_masked`). `cpu_raster.rs` получил настоящую реализацию `PushMaskLayer`/`PopMaskLayer` (`LayerComposite::MaskLayer`) вместо отсутствовавшей — обязательное условие для детерминированного CPU-снапшот-гейта. Не покрыто: `mask` на `<g>`-группах (падает на прежнее поведение), реальная (не scissor-заглушка) реализация в femtovg-бэкенде (ADR-017: фолбэк, не дефолт) |
...
| `zoom` (число / проценты / normal / reset) | 153 | `line-height`, `rem` и `border-width` в `em` под zoom не масштабируются (CSS-SPECS.md); `reset` не «сбрасывает предков», а даёт 1.0 |
...
- **anchor()** — ✅ реализован (CSS Anchor Positioning L1 §3.1): `anchor(<anchor-el>? <anchor-side>, <fallback>?)` в `top`/`right`/`bottom`/`left` (включая `inset` shorthand); перехватывается в `apply_declaration` (`set_inset_side`/`parse_anchor_func`, style.rs) до обычного length-парсинга и хранится как `ComputedStyle.anchor_top/right/bottom/left: Option<AnchorFunc>`; резолвится в `lay_out_abs_children` (локальный registry, `resolve_inset`) и корректируется глобальным `apply_anchor_positions`-проходом (`resolve_inset_scoped`, учитывает `anchor-scope`) — та же двухфазная схема, что у `inset-area`; отсутствующий anchor без fallback → свойство считается `auto`; 5 unit-тестов style.rs (парсинг) + 8 unit-тестов anchor.rs (резолв, включая scope); тест 144
- **anchor-size()** — ✅ реализован (CSS Anchor Positioning L1 §4): `anchor-size(<anchor-el>? <anchor-size>)` в `width`/`height`, перехватывается в `apply_declaration` до обычного sizing-парсинга (`parse_anchor_size_func`, было готово раньше); демо-покрытие графическим тестом добавлено в этом срезе; тест 144
...
- **color-contrast()** — ✅ реализован (CSS Color L5 §11): `color-contrast( <color> vs <color>#{2,} [ to <target> ]? )` в `parse_color_contrast()` (style.rs, диспетчер в `parse_function_color`); выбирает из списка кандидатов цвет с максимальным контрастом к базовому по формуле WCAG 2.1 (`wcag_relative_luminance` + `wcag_contrast_ratio`); при указании `to <target>` (ключевые слова `AA`=4.5 / `AA-large`=3 / `AAA`=7 / `AAA-large`=4.5 или голое число-ratio) возвращается первый кандидат в порядке списка, дотягивающий до цели, иначе — наиболее контрастный; `vs`/`to` ищутся как самостоятельные токены на глубине скобок 0 (`find_keyword_at_depth0`), поэтому кандидаты сами могут быть цвет-функциями; 6 unit-тестов style.rs; graphic-теста нет (чистая value-функция, резолвится в `parse_color` и течёт в существующий paint — как `color-mix()`)
...
- **@color-profile / color(--name ...)** — 🟡 частично реализован (CSS Color L5 §4): `@color-profile --name { src: url(...); rendering-intent: ...; }` парсится и хранится (`ColorProfileRule`, `Stylesheet.color_profiles` в css-parser); `color(--name c1 c2 c3 [/ a])` распознаётся в `parse_css_color_fn()` (style.rs) вместо ошибки разбора. Реальная ICC-трансформация и проверка, что имя действительно объявлено `@color-profile`, отложены — каналы трактуются как уже sRGB (та же ветка, что `srgb`); 5 unit-тестов css-parser + 2 unit-теста style.rs; тест 142 (KNOWN_DEBTOR BUG-282 10.16% — Edge не поддерживает фичу вовсе, весь `color()` невалиден на его стороне)
...
- **contain-intrinsic-size** — ✅ реализован (CSS Box Sizing L4 §5): `ComputedStyle.contain_intrinsic_width/height: Option<Length>` non-inherited, initial None; parse longhands `contain-intrinsic-width/height`, логические `contain-intrinsic-inline-size/block-size` и shorthand `contain-intrinsic-size` (1–2 значения, каждое `auto? [none | <length>]` — `auto` принимается и игнорируется, `none`→None) в `apply_declaration()`; в `lay_out` (box_tree.rs) флаг `size_contained` (`contain: size` ∨ `content-visibility: hidden` ∨ auto-skipped) переключает auto-высоту блока/flex/grid на `contained_content_height()` (contain-intrinsic-height или 0), а ширину size-contained inline-block — на contain-intrinsic-width; 7 unit-тестов style.rs + 3 unit-теста box_tree.rs; тест 114
...
- **font-variant-caps** — ✅ реализован (CSS Fonts L4 §6.2): `FontVariantCaps` enum из 7 значений (normal/small-caps/all-small-caps/petite-caps/all-petite-caps/unicase/titling-caps), **наследуемое**, initial Normal; parse longhand `font-variant-caps` + caps-компонента shorthand `font-variant` (сбрасывает caps в initial, CSS Cascade L4 §3.1) и `font: … small-caps …`; рендер — `caps_synthesis()` в box_tree.rs режет InlineSegment на прогоны «капитель/обычный текст» (заглавные × SMALL_CAPS_SCALE=0.8 + компенсация базовой линии через vertical-align: length) до wrap/measure/paint, потому что bundled-Inter не содержит smcp/c2sc/pcap; запрет переноса по разрезу внутри слова; `titling-caps` уходит OpenType-фичей `titl` через `text_font_features()`; 9 unit-тестов box_tree.rs + 2 style.rs + 6 lib.rs; тест 150
...
- **Внешний SVG (`<img src=*.svg>`, `background-image: url(*.svg)`) через resvg** — ✅ реализован (LIB-4, ADR-027): `lumen_image::decode_svg` (`crates/engine/image/src/svg.rs`) рисует SVG-байты через `resvg`/`usvg` 0.44 (пин под `tiny-skia` 0.11.4, тот же, что уже несёт `lumen-paint`'s `cpu-render`) вместо прежнего пути «обернуть в HTML → свой layout/paint pipeline» (RP-5), который рисует ноль `linearGradient`/`radialGradient`/`clipPath`/`mask`/`filter`/`pattern`/`marker` (нулевые вхождения в `box_tree/svg.rs`/`svg_path.rs`, измерение ADR-027). Диспетчеризуется из `decode_raw` по сигнатуре `is_svg` наравне с PNG/JPEG/GIF/WebP/AVIF — оба call site в `crates/shell/src/subresources.rs`, раньше особый случай через удалённый `svg_image.rs`, теперь просто `decode_to`. Интринзик-размер: `width`/`height`/`viewBox` резолвит `usvg`, полное отсутствие обоих — CSS Images §5.1 дефолт 300×150 (не `usvg`'s собственный дефолт вьюпорта 100×100 — разные вопросы, `has_root_sizing_hint` различает случаи по тексту корневого тега). Вложенный `xlink:href` — только `data:` URL (файловый/сетевой href не резолвится, у `lumen-image` нет сетевого доступа по архитектуре). `text`/`system-fonts` фичи `resvg` выключены (кросс-машинная нестабильность через `fontdb`). 5 unit-тестов `svg.rs` (включая регрессионный на реальный градиент); тест 154
...
- **::placeholder** — ✅ реализован (CSS Pseudo-Elements L4 §4.10): `PseudoElementKind::Placeholder` в css-parser (селектор `input::placeholder { ... }`); `compute_pseudo_element_style(doc, node, "placeholder", ...)` вызывается при построении `FormControlKind::Input` (box_tree.rs) и сохраняется как `placeholder_style: Option<Box<ComputedStyle>>` рядом с `placeholder`-строкой; без совпавших правил остаётся `None` (UA-дефолт, без изменений в поведении); `emit_input_placeholder_text()` (paint/display_list.rs) резолвит `color`+`opacity` (компонуются в альфа-канал) и `font-*` из override, иначе UA-серый `#757575` как раньше; ограниченный набор свойств (color/opacity/font-family/font-weight/font-style), как у `::selection`; 4 unit-теста style.rs + 2 unit-теста display_list.rs; тест 140
- **appearance: none** — ✅ реализован (CSS Basic UI L4 §4.2): `ComputedStyle.appearance: Appearance` (`Auto`/`None`/`Compat`, **не наследуется**); parse в `apply_declaration()` (`none`→None, `auto`→Auto, прочее→Compat; `-webkit-`/`-moz-` алиасы); `apply_ua_appearance()` (layout/style.rs) снимает UA-рамку/паддинг/фон; wiring в `emit_form_control_indicator()` (paint/display_list.rs) — при `Appearance::None` индикатор не рисуется вообще (чекбокс-тик, radio-точка, range-трек/ползунок, progress-бар, select-стрелка подавлены), чтобы автор мог полностью переопределить контрол; 4 unit-теста display_list.rs; тест 111

### Трансформации (Transforms)

- **backface-visibility culling** — ✅ реализован (CSS Transforms L2 §5.1): парсинг (`ComputedStyle.backface_visibility: BackfaceVisibility`, не наследуется) был готов раньше (p4-backface-visibility 2026-07-04); этот срез добавляет paint-culling — `is_backface_hidden()` (paint/display_list.rs) берёт `forward_box_transform(b)` (уже используется для depth-sort, `child_z_depth`) и проверяет знак `m[10]` — z-компоненту трансформированной локальной нормали `(0,0,1)` (перенос не влияет на направление); при `backface-visibility:hidden` и `m[10] < 0` (грань повёрнута от зрителя) бокс и его поддерево пропускаются целиком в `walk()`/`walk_with_anim()`, до всех Push*-команд, чтобы не оставить несбалансированные PushOpacity/PushTransform/BeginStickyLayer; 3 unit-теста display_list.rs; тест 141

### At-rules

- **@function** — 🟡 частично реализован (CSS Functions and Mixins L1): `@function <name>(<params>) [returns <type>]? { decls }` парсится в css-parser (`FunctionRule`/`FunctionParameter`, `Stylesheet.function_rules`) — позиционные параметры с опциональными default-значениями, `returns` хранится сырой строкой без типизации; вызов `<name>(<args>)` в любом значении свойства резолвится в layout (`expand_custom_functions`, style.rs) — позиционный биндинг аргументов (включая default при отсутствующем аргументе), локальные `--x: ...;` декларации применяются по порядку в свою scope-карту, `result:` разворачивается через уже существующие `expand_vars`/`calc()` и подставляется на место вызова; вызов, достижимый через цепочку custom-property (`--gap: --double(5px); width: var(--gap);`), тоже резолвится — `var()` разворачивается до сканирования вызовов; неизвестное имя / отсутствующий обязательный аргумент без default / превышение глубины рекурсии (циклы) делают декларацию invalid-at-computed-value-time (как непойманный `var()`), а не паникуют. Отложено: `returns`-типизация, conditional group rules в теле (`@media`/`@container`), именованные аргументы; 14 unit-тестов (7 css-parser + 7 style.rs); тест 143

### Writing modes (вертикальный текст)

- **writing-mode: vertical-rl/vertical-lr + text-orientation** — ✅ реализован (CSS Writing Modes L4 §3–4): layout уже давал block-axis-swap (`vertical.rs::lay_out_vertical_block`) и вертикальный inline-поток (`lay_out_vertical_inline_run`/`wrap_inline_run_vertical`, текст переносится top→bottom по inline-size); этот цикл (P3-vertical, срезы 1–4) добавил paint — реальный поворот глифов. CPU-рендерер (`cpu_raster.rs`) и wgpu-рендерер (`renderer.rs`, live default бэкенд, ADR-017) оба честно поворачивают: `sideways` — весь run на 90° CW; `mixed` (initial) — per-glyph split через `display_list::split_mixed_runs`/`lumen_layout::vertical::is_cjk` (CJK upright, латиница повёрнута); `upright` — без поворота (текущий горизонтальный путь). femtovg (fallback-бэкенд) по-прежнему игнорирует `text_orientation` — вне скоупа. 9 unit-тестов vertical.rs (layout) + unit-тесты cpu_raster.rs/display_list.rs/renderer.rs (paint, требуют `cpu-render`/`backend-wgpu`); тест 145 (текстовый — ожидаем font-parity debtor, rule 3)

### ImageBitmap / OffscreenCanvas

- **createImageBitmap + ImageBitmapRenderingContext (bitmaprenderer)** — ✅ реализован (HTML LS §4.12.5, §4.12.14), для обоих движков (QuickJS и V8 — `offscreen_canvas.rs`/`canvas2d.rs` оба портированы под `v8-backend`, до этого `OffscreenCanvas`/`createImageBitmap` вовсе не существовали под дефолтным V8-движком). Форма ImageBitmap унифицирована для всех источников: `{width, height, __canvas_id__, close()}`. `createImageBitmap` поддерживает ImageData, OffscreenCanvas (снимок без detach источника — раньше ошибочно переиспользовал destructive-native `transferToImageBitmap`), `<img>` (пиксели из `img_bitmap_store`, уже decoded шеллом) и `Blob` (декодирование через `lumen_image::decode`, PNG/JPEG/WebP/GIF/AVIF — новая зависимость `lumen-js → lumen-image`); опции обрезки `sx/sy/sw/sh` реализованы пост-процессингом (crop поверх уже созданного bitmap, переиспользует существующие `get_image_data`/`from_image_data` нативы). `canvas.getContext('bitmaprenderer')` возвращает `ImageBitmapRenderingContext` (обычный объект, тот же паттерн, что `getContext('2d')`); `transferFromImageBitmap(bitmap)` забирает пиксели bitmap (нейтрализуя его) и презентует на страничный `<canvas>` через `canvas2d::present_rgba` (уже существовавший WebGPU-present путь, resize + put_image_data). `ImageBitmap.close()` теперь реально освобождает пиксели (было no-op). Ограничение: одна и та же `Document`-модель — ImageBitmap не переживает cross-realm postMessage-клонирование (структурированное клонирование ImageBitmap не реализовано); 4 новых/обновлённых unit-теста offscreen_canvas.rs; тест 146
