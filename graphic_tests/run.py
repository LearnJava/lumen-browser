...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '51': ('BUG-124', 7.48),    # scrollbar rendering: float-wrapper shrink-to-fit фикснут BUG-178 (9.91% → 1.09%); остаток = дробные layout Y-координаты vs пиксельное округление Edge. Paint-time снэппинг ЭМПИРИЧЕСКИ исключён (2026-06-25, 3 раунда: снэп fill/border/clip в femtovg → 1.09→1.17→1.13, не помогло): бокс у Edge стоит на 1px ВЫШЕ (бордер 195 vs 196), т.е. расхождение позиции в layout, не AA-кайма от paint. Корень = PS-1 (единая политика pixel-snapping в layout, домен P1). DS-9 (2026-07-23, P1): ратчет 1.09%→4.06% — тулбар удвоил высоту хрома (CHROME_H=72), TEST-00/03 калибруются на 0.00%; на неизменённом main этот же тест уже дрейфовал 1.09%→2.52% независимо от DS-9 (PS-1 сам по себе нестабилен), DS-9 добавляет к этому дрейфу свой сдвиг sub-pixel snapping — не новый класс дефекта. CC-CSS-1 (2026-07-24, P1): ратчет 4.06%→7.48% — добавлена пара демо-боксов `::-webkit-scrollbar`/`-thumb`/`-track` vs эквивалентный `scrollbar-width`/`scrollbar-color`; Lumen рисует flat-rect scrollbar (без нативных Edge-скруглений/оверлея), новая пара боксов добавляет площадь этого же класса расхождения (PS-1/BUG-124), не новый дефект — сама трансляция webkit-псевдоэлементов на стандартные поля подтверждена unit-тестами `style::tests::webkit_scrollbar_*`
