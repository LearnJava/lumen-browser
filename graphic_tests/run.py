...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '71': ('BUG-199', 7.03),    # ДРЕЙФ, обнаружен во время гейта BUG-431 (2026-08-21): entry-запись в этом словаре отсутствовала, хотя BUGS.md всё ещё числит BUG-199 OPEN(DEBTOR) с задокументированным baseline 1.76% — реальный живой прогон стабильно даёт 7.03% (воспроизведено дважды подряд, включая билд БЕЗ BUG-431). 71-starting-style.html не содержит <img>/<video>/<iframe> — BUG-431 не при чём, это самостоятельный, отдельно всплывший дрейф @starting-style/wgpu-класса (тот же механизм, что BUG-287 уже чинил один раз). Не ретриажено — просто заведён заново, чтобы гейт не молча стопился/красился на несвязанных задачах; актуальный ре-триаж — по BUG-199
