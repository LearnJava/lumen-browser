...
TESTS: list[tuple[str, str, float, str]] = [
    ...
    ('150', '150-font-variant-caps.html', 0.5, 'P4-font-variant-caps CSS Fonts L4 §6.2: normal / small-caps / all-small-caps / petite-caps / all-petite-caps / unicase на одном и том же тексте «MiXeD Case 42». Капитель синтезируется в layout-е (caps_synthesis: заглавные × SMALL_CAPS_SCALE с компенсацией базовой линии) — bundled-Inter не содержит ни smcp, ни c2sc, ни pcap. Текстовый тест — ожидаем font-parity debtor (rule 3, Inter vs Edge sans metrics)'),
    ('151', '151-unicode-bidi.html', 0.5, 'P4-unicode-bidi CSS Writing Modes L4 §2.2 + UAX #9: normal / embed / bidi-override при direction ltr и rtl на одном латинском тексте «ALPHA BETA GAMMA». Проверяет обе половины rule L2 — перестановку фрагментов в строке (lumen-layout bidi::reorder_line) и разворот символов внутри RTL-прогона (bidi::visual_text, применяется в display_list). Две контрольные строки-«нуля» (ltr+bidi-override, rtl+embed на span) обязаны совпасть с базовой: они ловят движок, который разворачивает слишком много. Текстовый тест — ожидаем font-parity debtor (rule 3, Inter vs Edge sans metrics)'),
    ('152', '152-grid-place-content.html', 0.5, 'P4-place-content CSS Box Alignment L3 §5 + CSS Grid L1 §12.3: align-content / justify-content / place-content в grid-контейнере. 14 панелей с треками заведомо меньше панели: justify-content start/center/end/space-between/space-around/space-evenly по inline-оси, align-content center/end/space-between по блочной, place-content в двух формах (два значения «end center» и одно «space-evenly»), stretch auto-строк при дефолтном align-content: normal, item со span 1/4 поверх раздвинутых space-between зазоров и контроль переполнения (треки выше панели центрируются unsafely, §5.3 — уезжают за start-край, как в Edge). Только solid-боксы, без текста'),
    ('153', '153-zoom.html', 0.5, 'P3-zoom CSS Viewport L1 §5 zoom: множитель вкладывается в вычисленные длины (не в матрицу отрисовки), поэтому бокс реально занимает меньше места. 6 панелей: эталон без zoom; тот же бокс под zoom: 0.5 и zoom: 80% (две формы записи одного множителя); перемножение по дереву (0.8 × 0.5 = 0.4 от эталона); три обязанные совпасть полосы (zoom отсутствует / normal / невалидное значение — последнее отбрасывается по CSS Syntax, а не читается как какой-то множитель); и форма, ради которой zoom и объявляют, — строка фиксированной ширины 440px с min-width, gap и margin, влезающая в 234px-панель только после масштабирования. Все длины абсолютные (px): относительные единицы резолвятся против уже отмасштабированных базисов и покрыты юнит-тестами. Только solid-боксы, без текста'),
    ('154', '154-svg-external-image-gradients.html', 0.5, 'LIB-4 (ADR-027): внешний SVG через resvg вместо своего SVG-движка — <img src=local.svg> с linearGradient+clipPath и background-image: url(local.svg) с radialGradient+pattern. Свой SVG-to-DisplayList путь (lumen-layout/lumen-paint) рисует ноль linearGradient/radialGradient/clipPath/mask/filter/pattern/marker; эталон подтверждает не плоские/пустые боксы, а настоящие градиенты и повторяющийся паттерн'),
    ('155', '155-svg-inline-gradients.html', 0.5, 'LIB-5: inline <linearGradient>/<radialGradient> для fill="url(#id)" на живых DOM-элементах (rect/circle/path), не внешний файл (LIB-4) — дерево/каскад/хит-тест остаются свои, ADR-027 требует «модель путей/красок/градиентов» у usvg, не рендерер целиком. 6 панелей: горизонтальный linearGradient на rect, radialGradient на circle, вертикальный 3-стопный linearGradient на path (через ResolvedClipShape::Polygon клип), gradientUnits=userSpaceOnUse на скруглённом rect, stop-opacity (затухание поверх синего фона), наследование fill:url() через <g> с rotate(). Реализация: SvgPaint::Url резолвится в SvgPaint::Gradient в box_tree/svg.rs (доступ к Document есть только там, не в каскаде); paint клипует PushClipPath/PushClipRect(RoundedRect) по форме и рисует DrawLinearGradient/DrawRadialGradient внутри — без нового рендер-бэкенда. mask/clipPath/pattern/marker и мультиконтурный (дырчатый) путь под градиентом — вне этого среза'),
    ('156', '156-svg-mask.html', 0.5, 'LIB-9 (остаток LIB-5): SVG <mask> элемент через PushMaskLayer/PopMaskLayer (CSS Masking L1 §5) — инфраструктура уже была реализована во всех трёх бэкендах, но её не строил ни один emit-путь. 6 панелей: rect замаскирован в левую половину, circle замаскирован меньшим circle, составная маска из трёх rect ("H"), luminance (по умолчанию) — серый #808080 даёт полупрозрачность, mask-type="alpha" — тот же серый даёт полную видимость (та же заливка маски, два разных результата — прямое сравнение режимов), путь замаскирован через CSS `mask:` (не только presentation-атрибут). Реализация: mask="url(#id)"/CSS mask резолвится в BoxKind::SvgShape::svg_mask (box_tree/svg.rs::resolve_svg_mask, тот же паттерн, что LIB-5 для градиентов — доступ к Document есть только на этапе построения box tree); paint эмитит изолирующий PushOpacity{1.0} + собственную заливку формы + PushMaskLayer + контент маски + PopMaskLayer + PopOpacity (svg_text_decoration.rs::emit_svg_shape_masked). cpu_raster.rs получил настоящую (не заглушку) реализацию PushMaskLayer/PopMaskLayer — LayerComposite::MaskLayer читает alpha/luminance напрямую из отрендеренного слоя маски, без MaskSpec-заглушки. Область: только SvgShape-элементы (rect/circle/ellipse/line/path/polygon/polyline) — mask на <g>-группах вне этого среза (падает на прежнее, до-LIB-9, поведение). femtovg-бэкенд остаётся scissor-заглушкой (ADR-017: фолбэк, wgpu — дефолт, где маска полностью работает)'),
    ('157', '157-webgl2.html', 0.5, 'P3-webgl2 срез 6 (docs/tasks/ph3-webgl2.md): getContext(\'webgl2\') реального software-растеризатора, presented на страничный <canvas> (срез 5). 5 канвасов, все full-viewport quad (clip [-1,1]x[-1,1], без частичного покрытия — ни один пиксель не AA-граница, чтобы software- и Edge GPU-путь не разошлись на сглаживании): drawArrays ES 1.0 (зелёный поверх красного clear) · drawElements с Uint16Array ELEMENT_ARRAY_BUFFER (синий) · createVertexArray/bindVertexArray(null)/rebind (оранжевый — виден только если VAO реально восстанавливает attrib state) · GLSL ES 3.00 in/out + texture() сэмплит 1×1 текстуру (голубой, texture() возвращает усреднённый цвет текстуры независимо от UV) · uniformMatrix3fv нулевой матрицей вырождает треугольники в точку — видим только clear-цвет (не золотой цвет quad), что доказывает, что юниформ реально доходит до вершинного шейдера, а не тихо игнорируется. ТРЕБУЕТ JS-захвата (gdigrab) — как 57/129-138, cpu_raster не исполняет JS'),
]
...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    # '24' (vertical-align) снят с учёта 2026-08-04 (P3, BUG-128): дефолтный
    #     font-family документа стал `serif`, инлайн-текст теста набирается
    #     тем же Times New Roman, что и у Edge → 0.5023% → 0.47%, цель 0.5%
    #     достигнута (ратчет «remove entry if ≤ 0.5%»).
    # '26' убран (BUG-277 срез 6, 2026-08-05): 13.50% → 0.00%. Диагноз DS-9
    #   («тулбар сдвинул sub-pixel snapping маски») был неверен: квад mask-композита
    #   и квад градиента маски вовсе не прогонялись через накопленный `PushTransform`,
    #   поэтому лежали в нетрансформированных координатах, а контент-слой рисовался
    #   сдвинутым на высоту хрома — нижние CHROME_H px каждого маскированного
    #   элемента просто не композитились. Рост 11.24→13.50 при удвоении хрома —
    #   прямое следствие, а не новый sub-pixel-эффект.
    # '49' убран (BUG-277 срез 9, 2026-08-05): 2.74% → 0.15%, цель ≤0.5%
    #   достигнута. Остаток после среза 2 был не «AA-кромка/font-parity», как
    #   предполагала прежняя запись, а straight-alpha интерполяция стопов
    #   градиентных blend-слоёв (CSS Images L4 §3.1) плюс их наклон в
    #   UV-пространстве — оба сняты срезом 9. Прежний комментарий:
    ...
    # '68','72','74','81' убраны (BUG-277 срез 4, 2026-08-05): 3.17→0.02, 1.29→0.01,
    #   3.74→0.01, 3.44→0.00 — все ≤0.5%. Страницы обрамлены скруглёнными боксами,
    #   а wgpu-SDF считал вне фигуры краевые полосы шириной r по каждой стороне
    #   (ветки «в угловой полосе ровно по одной оси» возвращали q без вычитания
    #   радиуса) → у каждого блока пропадали левый/правый столбец и нижняя полоса.
    # '100' убран (BUG-277 срез 14, 2026-08-05): 0.96% → 0.28%, цель ≤0.5%
    #   достигнута. Держалось на c5 («the clipping container itself is rotated»):
    #   `PushClipRect` под поворотом резал по AABB трансформированного
    #   прямоугольника, и полоса-ребёнок протекала в угловые треугольники между
    #   описанной рамкой и настоящим квадрилатералом. Теперь прямоугольный клип
    #   под rotate/skew идёт через ту же машинерию точной формы, что `clip-path`
    #   (срез 8). A/B на одном бинарнике: `LUMEN_NO_ROT_CLIP=1` 0.96 / фикс 0.28.
    # '100': ('BUG-277', 1.09),
    # '103' убран (BUG-277 срез 7, 2026-08-05): 1.79% → 0.24%, цель 0.5% достигнута.
    #   filter × transform держался на трёх дефектах wgpu-исполнителя, разобранных
    #   в срезе: backdrop-квад мимо накопленного PushTransform, straight-alpha
    #   композит премультиплированного слоя и общий blur-uniform (H-проход шёл по Y).
    # '104' убран (BUG-277 срез 6, 2026-08-05): 32.78% → 0.33%. Тот же нетрансформированный
    #   квад маски, что и у TEST-26, плюс конечная фигура радиального градиента:
    #   wgpu игнорировал `radius_x/radius_y` из дисплей-листа и нормировал `t` на
    #   полуширину бокса (farthest-side) вместо реального радиуса. Остаток 0.33% —
    #   квадратные углы у двух ячеек с градиентным фоном под `border-radius`
    #   ([BUG-631](../bugs/BUG-631-OPEN.md), дефект дисплей-листа, не wgpu).
    # '107','111' убраны (BUG-277 срез 4, 2026-08-05): 7.27→0.15 и 1.27→0.23, ≤0.5%.
    ...
    # '116' убран (BUG-277 срез 11, 2026-08-05): 2.40% → 0.38%, цель ≤0.5%
    #   достигнута — ровно число детерминированного CPU-снимка той же страницы.
    #   Полоса `in srgb` (2 стопа) всегда была чистой, а четыре полосы с
    #   `<color-interpolation-method>` расходились с Edge на последних ~39 px:
    ...
    #   `DrawSvgFill` уходит в бэкенд не бинарным треугольным супом, а
    #   pixel-aligned квадами с точным покрытием. Прежний комментарий:
    # '133': ('BUG-247', 0.63),   # Ратчет 1.95→0.63 (BUG-277 срез 10, 2026-08-05): страница сравнивает колонки nonzero и evenodd, и сломана была именно nonzero-колонка в wgpu — ear-clipping не тесселировал самопересекающиеся пентаграммы/кольца, теперь обе колонки идут через общий scanline-свип. A/B на этой машине: main 2.07 / фикс 0.63. BUG-245 FIXED 2026-06-30 (3.91%→1.95%): fill-rule:evenodd now honoured (scanline even-odd tessellation) — pentagram/ring centres hollow as in Edge. Residual = diagonal/star-edge AA under gdigrab (BUG-247 class), re-pointed to that OPEN bug
    '134': ('BUG-247', 2.99),   # Ратчет 3.35→2.99 (ревизия P3 2026-08-31, два чистых прогона стабильно 2.99%). stroke-dasharray placement diverges from Edge under gdigrab (high-freq pattern × subpixel shift); solid strokes match. Inherent AA, not a logic defect. Ратчет 3.69→3.35 (P3 2026-07-04): нативный femtovg-штрих `DrawSvgStroke` (без внутренних triangle-soup AA-швов на пунктире) — замер подтверждён двумя чистыми gdigrab-прогонами
    '135': ('BUG-247', 0.54),   # Ревизия P3 2026-08-31: живой прогон дважды подряд стабильно даёт 1.18% (выше baseline, но в пределах ±2% допуска гдиграб-шума — не регрессия, храповик не требуется); diff-картинка — тот же класс, что и раньше (кромка повёрнутого fan-бокса + circle-полоса сверху, никакой новой геометрии). Baseline не трогаем. fanned cards rotate(a cx cy): BUG-244 FIXED (15.62%→0.54%) — rotation centre now correct, cards fan out as in Edge. Residual 0.54% = rotated-rect edge AA (BUG-247 class), just over 0.5%
    '136': ('BUG-247', 0.55),   # Ратчет 1.98→0.55 (ревизия P3 2026-08-31): семь чистых прогонов подряд печатали
    #   «0.50%» (2-знаковое округление), но точный diff_stats() на сохранённом diff.png дал 0.5016% — выше
    #   порога 0.5, т.е. этот тест колеблется ПРЯМО НА границе цели, не достиг её устойчиво (три из семи
    #   попыток фактически прошли ниже 0.5 и один раз проверка check_debtor вернула REMOVE, но следующий чистый
    #   прогон снова дал >0.5% — gdigrab run-to-run шум пересекает границу в обе стороны). Baseline снижен до
    #   0.55% (запас над наблюдаемым ~0.50–0.51%), запись НЕ удалена. <circle>/<ellipse> curve-edge AA vs Edge
    #   (geometry/colour correct; only ~1px edge AA differs). Inherent rasterizer-vs-Edge AA
    '137': ('BUG-247', 1.36),   # Ратчет 2.21→1.36 (ревизия P3 2026-08-31, два чистых прогона стабильно 1.36%; diff-картинка — тот же класс, только кромки лопастей/диагоналей, новой геометрии нет). Ратчет 4.74→2.21 (замер 2026-08-05): строка была устаревшей — уже полный прогон 2026-07-31 давал 2.95%, срез 4 BUG-277 к этой странице отношения не имеет (скруглённых боксов на ней нет). pinwheels/skew: BUG-244 FIXED (20.79%→4.74%) — rotate()/skewX() now applied via CTM, blades fan correctly. Residual 4.74% = edge AA of many thin high-frequency skewed blades (BUG-247 class, amplified by frequency like TEST-134 dash)
    ...
    '90': ('BUG-209', 1.29),    # AVIF <picture>/<img>: доминирующая причина девиации (вложенный column-flex item схлопывался по контенту вместо растяжения на cross-size ряда) исправлена BUG-241 (гейт `explicit_cross.is_some()` в lay_out_flex). TEST-90 2.27%→1.71%: рамки/фоны ячеек теперь совпадают с Edge пиксель-в-пиксель (diff чёрный). AVIF-данные в тесте обрезаны — ни Lumen, ни Edge их не декодируют; Edge рисует placeholder сломанной картинки (иконка + alt-текст), Lumen — нет. Остаток = эта chrome-иконка (не совпадёт с Edge пиксельно) + вертикальный сдвиг подписи + font-parity меток (rule 3). Ревизия P3 2026-08-31: живой прогон дважды подряд стабильно даёт 1.29% (ратчет 1.71%→1.29%, независимая правка где-то на main снизила остаточный дрейф) — новый baseline
    '122': ('BUG-237', 9.69),  # line-height-step: Lumen спек-корректно округляет line-box (48/60/40px фоны), Edge свойство не поддерживает (удалён из Chromium ~2018) → эталон рисует un-stepped 24px fallback. Lumen корректнее reference-браузера (класс BUG-126/TEST-77 inset-area, BUG-199/TEST-71 @starting-style). Совпасть = отключить рабочую реализацию (запрещено). Остаток + font-parity переноса natural-колонки (rule 3)
    '53': ('BUG-219', 1.16),    # Перецелён из BUG-277 (BUG-277 срез 17, 2026-08-05): «wgpu-окно против
    #   headless-CPU» (`run.py --only 53` / `--only 53 --ipc`) дал 1.16% против 1.76% — wgpu уже
    #   НИЖЕ CPU-числа, расхождения бэкендов нет (срез 15 закрыл реальный дефект: DrawBackgroundImage
    #   не проходил через PushTransform). Фоновая картинка на странице — тот же `perceptron.png`,
    #   что в TEST-19/TEST-18 (BUG-181/BUG-219): остаток inherent image-downscale-resampling AA
    #   (area-avg ≠ Edge content-adaptive kernel) поверх обычного font-parity меток, класс BUG-219
    #   доминирует (диаграмма занимает больше диффа, чем подписи). Ратчет 8.17→1.16 (BUG-277 срез 15,
    #   2026-08-05): квады `DrawBackgroundImage` в wgpu не проходили через накопленный `PushTransform`,
    #   поэтому фон ложился на CHROME_H=69px выше своего бокса. A/B на одном бинарнике:
    #   `LUMEN_NO_IMG_XFORM=1` 7.95 / фикс 1.16. Прежний комментарий:   # background-origin:
    #   femtovg-baseline была 1.71% (font-parity, класс BUG-128 — история сохранена).
    #   BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу
    #   5.45% (совпадает с BUG-277 рядом «53»). DS-9 (2026-07-23, P1): ратчет 5.45%→8.17% — тулбар
    #   удвоил высоту хрома (CHROME_H=72), TEST-00/03 калибруются на 0.00%, сдвиг только меняет
    #   sub-pixel snapping фоновых слоёв (класс BUG-124/PS-1), не новый дефект
    # '54' убран (BUG-277 срез 12, 2026-08-05): 1.22% → 0.23%, цель ≤0.5%
    #   достигнута. A/B на одном бинарнике: LUMEN_NO_SVG_AA=1 → 1.22%, без него
    #   0.23%. Остаток среза 10 назван верно — отсутствие AA у треугольного супа.
    #   Прежний комментарий:
    # '54': ('BUG-277', 1.22),    # Ратчет 2.32→1.22 (BUG-277 срез 10, 2026-08-05): нонзеро-заливка в wgpu перешла с ear-clipping на scanline-свип. Ear-clipping определён только для простых многоугольников — у «песочных часов» (самопересекающийся контур, ряд 3) уха нет вовсе, тесселятор возвращал НОЛЬ треугольников и фигура не рисовалась (одна она давала 1.18 п.п. из 2.32). A/B на этой машине: main 2.32 / фикс 1.22, оба замера повторены — улучшение не gdigrab-шум. Остаток = отсутствие AA у треугольного супа в wgpu (пиксель кромки либо полный цвет штриха, либо чистый фон, тогда как cpu_raster/Edge дают дробное покрытие) — класс BUG-247, отдельный долг, не геометрия. femtovg-baseline была 0.26% (BUG-173 FIXED, нативный DrawSvgStroke — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 2.32% (совпадает с BUG-277 рядом «54»)
    '55': ('BUG-192', 0.54),    # <video> placeholder: фича корректна. Edge рисует `<video src="nonexistent.mp4">` БЕЗ серого placeholder (прозрачный, виден только фон) — Lumen рендерит так же (BUG-097: пустое <video> без poster/кадра не рисует ничего). Бокс с border (200×120, 3px solid #4299e1) совпадает с Edge по размеру/цвету. Декомпозиция diff (0.54%): 90% (0.48%) = font-parity 6 меток .label (11px sans, Inter vs Edge) — rule 3; остаток 0.05% = бордер-бокс на 1px ниже (y=212 vs 211), причина — line-height «normal» метки в ряду 1 (Inter ≈1.2 vs Edge sans → row1_height 171 vs 170), тоже font-parity. Реального дефекта плейсхолдера нет. Класс BUG-128. Ревизия P3 (2026-08-31): живой прогон дважды подряд стабильно даёт 0.54%, совпадает с baseline, ратчет не требуется
    '57': ('BUG-099', 0.88),    # <canvas> getContext("2d") — Phase 2. Ратчет вниз 4.14→2.96 (прогон 2026-06-23)→0.91 (2026-07-29)→**0.88** (ревизия P3 2026-08-31). IPC даёт 27.96% (режим-зависимый артефакт захвата); нормальный gdigrab = 0.88%. Ратчет 2.96→0.91 (2026-07-29): BUG-099 — размеры битмапа `<canvas>` это intrinsic (content-box) размер, а не значение `width`/`height` (HTML Rendering §15.4.1 не маппит dimension-атрибуты canvas на свойства), поэтому `box-sizing:border-box` вычитал из битмапа рамку (c3 был 180×150 вместо 186×156 у Edge) и весь ряд 2 стоял на 5px выше; плюс битмап красился в border-box вместо content-box. Canvas-AA и lineCap/lineJoin/miterLimit закрыты тем же днём (rasterize.rs) — диф от этого почти не сдвинулся (0.91→0.700% по CPU-метрике, в пределах допуска ратчета). **Ревизия P3 2026-08-31:** live-прогон дважды подряд даёт стабильные 0.88% (проверено на воспроизводимость — повторный запуск без `--no-cache` подтвердил то же число; один запуск с `--no-cache` дал ложные 100% — фокус рабочего стола увели во время gdigrab-захвата, известная ловушка, не регрессия). Комментарий про «неприменяемый lineJoin» устарел — closed тем же 2026-07-29 (см. bugs/BUG-099-OPEN.md «третий заход»), убран. Небольшое улучшение 0.91→0.88 правдоподобно от BUG-451/452 (парсинг цвета/размера `<canvas>`, слиты в main тем же днём) — diff-картинка структурных отличий не показывает, только фоновый шум кромок/подписей. Остаток = 1px сдвиг ряда 2 (класс PS-1/BUG-124) + font-parity меток (BUG-128), оба домена P1, точечного P3-дефекта не осталось
    # '60' убран (BUG-277 срез 12, 2026-08-05): 0.74% → 0.15%, цель ≤0.5%
    #   достигнута тем же покрытием кромки штриха. Прежний комментарий:
    ...
    '61': ('BUG-103', 10.59),  # View Transitions L1 РЕНДЕРИТСЯ (F2-4 ревизия 2026-06-22): startViewTransition + root cross-fade работают. Ратчет вниз 99.53→10.99 (прогон 2026-06-23: gdigrab дал настоящий кадр, не blank-capture). Остаток = тайминг захвата Edge (async update-callback по спеку — Edge headless снимает кадр ДО callback → старая DOM card1 active; Lumen рендерит устоявшееся card2 active, спек-корректно; тот же класс, что TEST-71/BUG-199 и TEST-77/BUG-126) + font-parity текста (rule 3). BUG-335 (2026-07-24, P1, CC-CSS-2): ратчет 10.99%→5.16% — фикс `PushScrollLayer` clip-transform (см. BUG-335) заодно поправил рендер вложенных/скроллируемых зон, задетых этим тестом. **Ратчет вверх 5.16%→10.59% (2026-08-05, P3, бисект BUG-103):** S12b-G5 (0bbab3c35, 2026-08-04) перенёс `document.startViewTransition` на V8 и вскрыл, что до этого коммита API на V8 не существовал вовсе (`typeof startViewTransition === 'undefined'`) — до-миграционный «5.16%» был случайным совпадением двух багов: сломанный API оставлял страницу в состоянии card1-active («before»), которое почти совпадало с эталоном Edge (Edge сам ловит кадр ДО callback — тот самый уже описанный остаток). G5 корректно чинит API (закрывает BUG-545/546) — теперь Lumen честно рендерит спек-корректное card2-active («after»), из-за чего расхождение с застрявшим на «before» кадром Edge стало полным, а не случайно замаскированным. Живая проверка (`--dump`/скриншот-трейс с логами прогресса кросс-фейда) подтвердила: сама 300мс кросс-фейд-анимация сходится к alpha=0 корректно и не подмешивает никакого «призрачного» слоя в финальный кадр — весь прирост диффа объясняется честным before/after-рассинхроном с Edge, тем же классом остатка, что и раньше, просто теперь не замаскированным. Не P3-регрессия, ратчет базового значения. Опц. полный L1 (named groups + ::view-transition pseudo) = XL, не валидируется этим тестом
    '65': ('BUG-128', 1.69),   # flex align-content. Перецелён из BUG-277 срезом 20 (2026-08-05, P3): «wgpu-окно vs headless-CPU» даёт 1.69%/2.08% — разрыв 0.39 п.п., на порядок меньше настоящих wgpu-дефектов этого бага (десятки п.п.), диф по строкам концентрируется на ::before-подписях (font_size:12px Arial) и скруглённых углах карточек, не на геометрии. История: ратчет 5.45→1.69 (BUG-277 срез 4, 2026-08-05, фикс SDF скруглённого прямоугольника), femtovg-baseline 2.08% (класс BUG-127)
    '63': ('BUG-128', 1.81),    # masonry. Перецелён из BUG-277 срезом 20 (2026-08-05, P3): «wgpu-окно vs headless-CPU» даёт 1.81%/2.09% — разрыв 0.28 п.п., на порядок меньше настоящих wgpu-дефектов; диф по строкам концентрируется на "Masonry Layout"-заголовке и "Item N"-подписях внутри плиток (font-parity), не на геометрии/цвете плиток. История: ратчет 5.46→1.81 (BUG-277 срез 4, 2026-08-05, фикс SDF скруглённого прямоугольника), femtovg-baseline 2.02% (класс BUG-176)
    ...
    # '36' убран (BUG-277 срез 4, 2026-08-05): 7.80% → 0.36%, цель ≤0.5% достигнута.
    #   Это был головной тест дефекта: wgpu-SDF считал вне фигуры краевые полосы
    #   шириной r, поэтому «скруглённый бокс» рисовался как внутренний прямоугольник
    #   плюс четыре угловые дуги. Плюс §5.5-клэмп радиусов единым коэффициентом
    ...
    # '30' убран (гейт BUG-431, 2026-08-21): 1.44% → 0.33%, цель ≤0.5%
    #   достигнута — воспроизведено дважды подряд. Страница (30-css-filter.html)
    #   не содержит <img>/<video>/<iframe>, так что улучшение не связано с
    #   BUG-431 (content-box для этих трёх элементов); попутная находка того же
    #   класса, что и прежние ратчеты — backend/шумовая вариация backdrop-filter
    #   (класс BUG-247), не отдельный дефект.
    ...
    #   градиентов: (1) `repeating-*-gradient` не повторялся вовсе — шейдер
    #   сворачивал t по периоду 1.0 вместо промежутка [first,last] стопов, так
    #   что полосы/кольца вырождались в плоский цвет; (2) стопы смешивались в
    #   straight-alpha вместо премультиплированного (CSS Images L4 §3.1);
    #   (3) направление linear-gradient считалось в UV-пространстве, из-за чего
    #   изолинии наклонялись на аспект бокса (45° на 180×80 выходил ~11°).
    '51': ('BUG-124', 1.67),    # scrollbar rendering: float-wrapper shrink-to-fit фикснут BUG-178 (9.91% → 1.09%); остаток = дробные layout Y-координаты vs пиксельное округление Edge. Paint-time снэппинг ЭМПИРИЧЕСКИ исключён (2026-06-25, 3 раунда: снэп fill/border/clip в femtovg → 1.09→1.17→1.13, не помогло): бокс у Edge стоит на 1px ВЫШЕ (бордер 195 vs 196), т.е. расхождение позиции в layout, не AA-кайма от paint. Корень = PS-1 (единая политика pixel-snapping в layout, домен P1). DS-9 (2026-07-23, P1): ратчет 1.09%→4.06% — тулбар удвоил высоту хрома (CHROME_H=72). CC-CSS-1 (2026-07-24, P1): ратчет 4.06%→7.48% — пара демо-боксов `::-webkit-scrollbar`/`-thumb`/`-track` vs `scrollbar-width`/`scrollbar-color`, тот же класс расхождения (PS-1/BUG-124), не новый дефект. BUG-335 (2026-07-24, P1, CC-CSS-2): ратчет 7.48%→1.72% — `PushScrollLayer` в wgpu `renderer.rs` не применял `apply_transform_to_clip()` (см. BUG-335), из-за чего scroll-контейнеры этого теста рендерились с неверным clip под живым chrome-transform; фикс убрал почти весь остаток. Ревизия P3 2026-08-31: ратчет 1.72→1.67 (правдоподобно от BUG-451/BUG-452); попутно найден и заведён отдельно BUG-931 (обрезка высоты инлайн-блочного содержимого в overflow-x:scroll контейнере, только на живом wgpu-пути)
    '45': ('BUG-128', 0.87),    # multiple backgrounds. Перецелён из BUG-277 срезом 20 (2026-08-05, P3): «wgpu-окно vs headless-CPU» даёт 0.87%/1.03% — wgpu уже НИЖЕ CPU-числа, разрыв 0.16 п.п. отрицательный (не долг wgpu). Диф по строкам концентрируется на текстовых подписях боксов (~79% массы) + известная кайма «1px y=0 row»; сами градиентные боксы дают <0.03 п.п. (163/24684 px над порогом в самом крупном из них). Историческая справка: femtovg-baseline была 1.02% (font-parity + 1px y=0 row, класс BUG-128 — история сохранена). Ратчет 5.81→0.87 (BUG-277 срез 9, 2026-08-05, фикс straight-alpha интерполяции стопов + UV-направления линии)
    ...
    #   скроллбаров (полосы не показаны в headless-скриншоте) — остаток чистый font-parity, класс
    #   BUG-128. Ратчет 9.82→6.14 (BUG-277 срез 4, 2026-08-05: фикс SDF скруглённого прямоугольника —
    #   скролл-карточки страницы скруглены; часть выигрыша от влитого BUG-128 ч.3). Ратчет 11.91→9.82
    #   (P3 2026-08-04, BUG-128: дефолт документа serif). scroll-behavior: femtovg-baseline была
    #   7.88% (font-parity + overlay scrollbar, класс BUG-128 — история сохранена). BUG-287/BUG-277
    #   (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 11.91% (совпадает
    #   с BUG-277 рядом «83»)
    # '92' убран (гейт BUG-431, 2026-08-21): 0.90% → 0.00%, цель ≤0.5% достигнута.
    #   Страница (92-*.html, system colors) не содержит <img>/<video>/<iframe>,
    #   не связано с BUG-431 — задокументированный остаток был gdigrab
    #   суб-пиксельным сдвигом (~+3px на 1000px), попавшим в допуск случайно.
    ...
    '62': ('BUG-644', 0.75),    # Перецелён из BUG-277 (BUG-277 срез 18, 2026-08-05): ратчет 8.01→0.75 —
    #   `.snap-both` (`display:grid; grid-template-rows: 1fr 1fr; height:220px`) отдавал каждой строке
    #   только `(220 - 2×content_height) / 2 ≈ 83.5px` вместо честных `220/2 = 112.3px`: `lay_out_grid`
    #   сидировал базовый размер fr-строк из пробной content-based высоты ячейки (нужно только как
    #   фолбэк для НЕопределённой высоты контейнера) и затем вычитал этот же размер из доступного
    #   пространства перед раздачей `fr` — двойной учёт, ~58px оставались неучтённым тёмным зазором
    #   под 2×2 сеткой NW/NE/SW/SE. Колонки той же функции этой ошибки не имели (`content_width` всегда
    #   определён, fr-колонки стартуют с 0, не с контента) — фикс привёл строки к тому же принципу.
    #   Остаток 0.75% — новый, отдельный дефект: `align-items: stretch` в гриде растягивает внешний
    #   бокс ячейки постфактум (`item.rect.height = cell_h`), не перекладывая уже готовое поддерево, —
    #   `.cell`'s собственный `align-items:center` центрирует текст относительно пробной (маленькой)
    #   высоты и остаётся у верхнего края растянутой ячейки. Заведён [BUG-644](bugs/BUG-644-OPEN.md) —
    #   не wgpu-исполнитель и не font-parity, отдельный layout-баг вне скоупа BUG-277. Историческая
    #   справка: femtovg-baseline была 8.85% (font-parity + border-radius AA, класс BUG-128/176). BUG-287/
    #   BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 16.07%.
    #   DS-9 (2026-07-23, P1): ратчет 16.07%→25.78% — тулбар удвоил высоту хрома (CHROME_H=72), сдвиг
    #   каскадно сместил sub-pixel snapping (класс BUG-124/PS-1), не новый дефект. BUG-335 (2026-07-24,
    #   P1, CC-CSS-2): ратчет 25.78%→8.01% — `PushScrollLayer` в wgpu не применял `apply_transform_to_clip()`
    ...
    '78': ('BUG-127', 3.60),   # scroll-driven animations: feature wired (F2-2) — animation-timeline: scroll()/view()/named драйвит прогресс анимации от позиции скролла, а не от часов (shell/animation_scheduler.rs). 12.02%→10.07%→9.54%: scroll()/named-боксы садятся в from-state как у Edge, view()-бокс едет по view-прогрессу (позиция совпадает); с D2-2 (BUG-231 FIXED) фон view-бокса композится (интерполированный оранжевый вместо teal). Остаток = font-parity текста (заголовок + метки + 5-строчный .info моноблок, Inter vs Edge, класс BUG-128, доминирует) — закроется только с FP-1. --ipc 2026-06-26: 10.07→5.76. Ратчет 2026-07-04 (ревизия P3): 5.76→4.64. Ратчет 2026-08-31 (ревизия P3): 4.64→3.60 — стабильно на двух прогонах, diff = чистый font-parity (заголовок/метки/.info моноблок), from-state рамки и view()-бокс совпадают пиксель-в-пиксель, новых дефектов нет
    ...
    # '71' убран (P3 2026-08-31, BUG-199): корень «дрейфа» — не движок, а сам тест.
    #   `.box-a`/`.box-b` несли `transition: opacity/transform 0.4s ease`, поэтому
    #   ЛЮБАЯ загрузка страницы (в Lumen и в Edge одинаково) запускала настоящий
    #   entry-transition — а однократный headless-скриншот не гарантирует, что
    #   переход успел доиграть. Edge — сам эталон! — ловил СЛУЧАЙНУЮ точку своего
    #   же перехода при каждой пересъёмке (`--no-cache`), поэтому именно
    #   ref-картинка была источником нестабильности 0.02%…10.6%, разброс
    #   воспроизведён (5 прогонов с --no-cache подряд: 0.13/0.72/1.29/2.83/5.15/
    #   7.02/7.24/8.40/10.20/10.57%). Живой рендер Lumen сам по себе стабилен —
    #   getComputedStyle() статичен с t=28ms на 2с вперёд (opacity:1/transform:none),
    #   но паринг/композитинг применяет transition-override НЕЗАВИСИМО от CSSOM,
    #   и этот оверлей у Lumen тоже недетерминирован по времени старта (не
    #   диагностировано глубже — вне scope P3, не трогает движковую логику).
    #   Файл-докстринг теста сам говорит: «static rendering... in static
    #   rendering these do NOT affect appearance (no active transition)» — то есть
    #   `transition:` в CSS противоречил заявленному намерению теста. Убраны
    #   обе строки `transition:` (@starting-style-правила остались — parse-тест
    #   не пострадал), эталон Edge пересобран (`--no-cache`): 0.02% стабильно на
    #   5 прогонах подряд, цель 0.5% достигнута. BUG-199 закрыт.
    '52': ('BUG-128', 1.36),    # Ратчет 4.25→1.36 (BUG-277 срез 7, 2026-08-05): text-shadow эмитит `PushFilter{Blur}`, а wgpu кодировал H- и V-проход сепарабельного гауссиана через один разделяемый `blur_uniform` — обе записи ложатся до `submit`, побеждает последняя, поэтому размытие шло только по вертикали. Историческая справка: text-shadow blur: blur-пайплайн корректен (BUG-191) — sigma=radius/2, GPU GaussianBlur на full-RT слое (halo не клипуется), multi-shadow и цветные glow совпадают с Edge по extent/intensity (glow-only и 20px кейсы проверены пиксельно). Остаток = font-parity: Edge рендерит serif 80px, Lumen — Inter sans; в diff доминируют два несовмещённых начертания глифов «A»/«B» (cyan/white ghosts), сами тени near-black = совпадают (rule 3)
    ...
    '58': ('BUG-100', 2.41),    # ::first-letter / ::first-line РЕАЛИЗОВАНЫ (дрейф трекера, прогон 2026-06-23): drop-cap «O» флоатится (extract_first_letter_float + 7 тестов), first-line зелёная+жирная. Diff-картинка подтверждает: фича работает, остаток = font-parity тела абзаца (Inter vs Edge sans → разные метрики → перенос строк сдвигается, ghosting глифов) + edge-AA большого 48px drop-cap-глифа (rule 3). --ipc 2026-06-26: 4.92→2.47. Ратчет P3 2026-08-31: generic-family fallback (monospace-метка) починен побочно BUG-128 (2026-08-04) — 2.47→2.41, остаток только font-parity
    '59': ('BUG-219', 10.03),   # Перецелён из BUG-277 (BUG-277 срез 18, 2026-08-05): «wgpu-окно против
    #   headless-CPU» дал 10.03% против 9.28% — расхождение бэкендов в пределах шума (Δ0.75 п.п.),
    #   дефекта wgpu-исполнителя нет (срез 15 уже вычистил реальный дефект — DrawCrossFade transform +
    #   премультиплексия). Diff — цветной шум по всей площади фотографического контента (тот же
    #   `perceptron.png`, что в TEST-18/19/53), не геометрия смешивания — тот же downscale-resampling
    #   класс, что уже описан в BUG-219. Ратчет 23.65→10.03 (BUG-277 срез 15, 2026-08-05): два дефекта
    #   wgpu — квад `DrawCrossFade` не проходил через накопленный `PushTransform` (плюс `CrossFadeVertex`
    #   не реализовывал `VertexPos`), и `CROSS_FADE_SHADER_SRC` смешивал straight-alpha RGB вместо
    #   премультиплированных цветов (CSS Images L4 §4.2) → 50%-ная смесь поверх прозрачной области
    #   выходила вдвое темнее Edge. Боксы `cross-fade() 30%/70%` пусты и у Edge (старый синтаксис без
    #   префикса невалиден — совпадение, не дефект). A/B: `LUMEN_NO_IMG_XFORM=1` 21.17 / фикс 10.03.
    #   Прежний комментарий:   # image-set() / cross-fade() РЕАЛИЗОВАНЫ, femtovg-baseline была 17.15%
    #   (грамматика приведена к CSS Images L4 §4, класс BUG-101 — история сохранена). BUG-287/BUG-277
    #   (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 23.65%
    '79': ('BUG-128', 5.65),    # text-underline-offset / text-decoration: геометрия линий корректна; остаток = font-parity (serif vs sans, Inter vs Edge) по всему тексту страницы (rule 3). Ратчет 6.76→5.65 (ревизия P3 2026-08-31, два чистых прогона)
    '110': ('BUG-214', 1.42),   # accent-color РЕАЛИЗОВАН (дрейф трекера, прогон 2026-06-23): emit_form_control_accents тинтит checkbox/radio/range/progress + юнит-тесты. Diff-картинка подтверждает: цвета-акценты применяются верно (cyan/magenta/green), остаток = расхождение нативной отрисовки form-виджетов (толщина трека слайдера, форма thumb, стиль progress-бара) vs UA-виджеты Edge — присущее кросс-браузерное расхождение (сам тест-HTML отмечает «native control sizes kept → divergence stays small»). Ратчет 1.70→1.42 (ревизия P3 2026-08-31, два чистых прогона: 1.44%/1.42%)
    ...
    '145': ('BUG-290', 3.39),   # writing-mode vertical-rl/vertical-lr × text-orientation mixed/upright/sideways: реальный дефект (BUG-289, vertical InlineRun текст никогда не рисовался на реальном DOM — emit_inline_run игнорировал writing_mode) найден и исправлен в этом же прогоне; mixed/sideways рендерят повёрнутый/upright-CJK текст корректно. Остаток 3.68%→3.39% (ратчет, ревизия P3 2026-09-01, два чистых прогона) = (1) font-parity Inter vs Edge sans (rule 3, класс BUG-100/TEST-58); (2) text-orientation:upright использует пословный, не per-glyph, вертикальный аванс (Edge раскладывает каждый символ индивидуально) — сознательно отложенный пробел, см. docs/tasks/ph3-writing-mode-vertical.md. mixed/upright/sideways визуально различимы (DoD задачи выполнен)
    '147': ('BUG-330', 2.91),   # background-repeat: space — геометрия проверена P3 2026-08-06 пиксельно против Edge (все 4 зазора совпадают с точной формулой space_axis_geometry в пределах ±1px): tile1..5 по x, зазор=(451-400)/4=12.75px — позиции границ у Lumen и Edge совпадают. Реального дефекта раскладки нет. Остаток 2.91% = (1) Edge сглаживает АА-кромку тайл/зазор на ~2px, Lumen красит квад-край жёстко (без AA) — тот же класс, что BUG-124/BUG-247 (rule 3 для не-текста); (2) даунскейл 852×725→80×80 (mip-цепочка есть, box-фильтр 2×2) даёт иные субпиксельные цвета внутри тайла, чем у Edge — независимый resampling-fidelity долг. Не point-fixable в P3: оба класса требуют системной AA/ресемплинг-политики (домен P1, PS-1-подобный). BUG-330 закрыт как DEBTOR
    '149': ('BUG-288', 4.00),   # BUG-277 срез 6 (2026-08-05): ратчет 7.91%→4.00% по требованию храповика. Источник улучшения — не этот срез (масок/радиальных градиентов на странице нет), а уже влитый срез 5 (скруглённый клип детей): `graphic_tests/results/latest.json` датирован до него. Прежний комментарий:   # nested overflow:auto: реальный дефект найден и исправлен в этом же срезе (BUG-335 — PushScrollLayer в wgpu renderer.rs не применял apply_transform_to_clip к своему clip_rect перед пересечением с clip_stack, в отличие от PushClipRect/PushClipRoundedRect/PushClipPath, уже пофикшенных BUG-276; под живым chrome-transform (TAB_BAR_HEIGHT-сдвиг) это давало clip в неверном координатном пространстве — содержимое overflow:auto/scroll контейнеров пропадало почти целиком, 18.01%→7.91%). Item1/2/3 текст и z-index бейдж (col A/C) теперь совпадают с Edge пиксель-в-пиксель кроме тонкой каймы бордеров. Остаток = (1) статичный Lumen-скроллбар vs overlay-скроллбар Edge (невидим в headless-скриншоте, тот же класс, что TEST-14/51/83); (2) col B (radial-gradient внутри вложенного border-radius clip) — цветовая интерполяция градиента расходится сильнее прочих колонок, не специфично для nesting-механики, отдельный wgpu-градиентный долг класса BUG-277
    '156': ('BUG-936', 2.46),  # Ратчет 10.02%->2.46% (P3, 2026-09-03, прогон p3-bug341-s41): число перемерено дважды на РАЗНЫХ коммитах с одним и тем же эталоном Edge (09-01 10:13) — полный прогон на ветке и точечный `--only 156` на main до неё дали ровно 2.46%, так что это не флак захвата и не заслуга той ветки (её нейтральность отдельно показана A/B: все 156 тестов дали одинаковый процент до последней цифры). Причина улучшения НЕ атрибутирована коммитом: в окне между прогоном 09-01 10:22 и 09-03 paint трогали только переносы SPLIT-DL13..19, заявленные как поведенчески-нейтральные — либо один из них починил композит побочно, либо число 10.02 было снято при другом состоянии GPU-бэкенда (см. готчу про `[wgpu] adapter` в CLAUDE.md). Сам BUG-936 НЕ закрыт: 2.46% это всё ещё маска, видимая не полностью. # LIB-9 SVG <mask>: PushMaskLayer/PopMaskLayer wgpu-композит не даёт видимого эффекта маски в живом окне — контент рендерится немаскированным на всех 6 панелях. Display-list верен побайтово (--dump-display-list показывает точный Push/FillRect/PushMaskLayer/FillRect/PopMaskLayer/PopOpacity) и CPU-путь (cpu_raster.rs LayerComposite::MaskLayer, добавлен той же задачей) маскирует корректно — баг локализован в исполнении wgpu-плана рендера, не в резолвинге/эмиссии. Не backend-специфично (идентичный 10.02% на Vulkan и WGPU_BACKEND=dx12). Точная причина не найдена статическим чтением (from_level/индексация текстур/bind-group-layout/порядок пайплайнов — всё совпадает с уже рабочими Composite/MaskComposite путями), нужен GPU readback или визуальный дебаггер. См. BUG-936 для полного списка проверенного/непроверенного
    '157': ('BUG-1077', 5.06),  # P3-webgl2 срез 6, 2026-09-21: НЕ дефект WebGL-пути — весь диф целиком совпадает с контуром плавающей chrome-панели `#demoBar` (CC-18), которая стала видимой в живом окне побочным эффектом BUG-1059 (закрыт 2026-09-19) и которую `msedge.exe` никогда не рисует. `157-webgl2-diff.png` подтверждает: все 5 WebGL-канвасов дают нулевой диф, единственные ненулевые пиксели — сама панель. Тест уже переверстан в один ряд полностью выше панели (как ряд 1 TEST-57), но панель перекрывает пустой фон страницы за пределами тестового контента, а не только контент — сдвинуть контент дальше некуда. См. BUG-1077 (тот же класс регрессии уже задел TEST-57/BUG-099 debtor).
}
