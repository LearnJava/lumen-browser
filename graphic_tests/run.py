...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '78': ('BUG-127', 4.59),   # scroll-driven animations: feature wired (F2-2) — animation-timeline: scroll()/view()/named драйвит прогресс анимации от позиции скролла, а не от часов (shell/animation_scheduler.rs). 12.02%→10.07%→9.54%: scroll()/named-боксы садятся в from-state как у Edge, view()-бокс едет по view-прогрессу (позиция совпадает); с D2-2 (BUG-231 FIXED) фон view-бокса композится (интерполированный оранжевый вместо teal). Остаток = font-parity текста (заголовок + метки + 5-строчный .info моноблок, Inter vs Edge, класс BUG-128, доминирует) — закроется только с FP-1. Baseline 4.59 ратчечен (прогон 2026-06-25 full-run)
    ...
    '58': ('BUG-100', 2.49),    # ::first-letter / ::first-line РЕАЛИЗОВАНЫ (дрейф трекера, прогон 2026-06-25): drop-cap «O» флоатится (extract_first_letter_float + 7 тестов), first-line зелёная+жирная. Diff-картинка подтверждает: фича работает, остаток = font-parity тела абзаца (Inter vs Edge sans → разные метрики → перенос строк сдвигается, ghosting глифов) + edge-AA большого 48px drop-cap-глифа (правило 3)
