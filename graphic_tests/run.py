...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '101': ('BUG-247', 0.71),   # INTERACTION border-radius × overflow:hidden: round-rect clip детей ПОЧИНЕН (BUG-249, 3.90% → 0.71%): оба бэкенда маскируют subtree по скруглённому контуру (offscreen-слой, как clip-path), а не square scissor. Остаток 0.71% = sub-pixel AA скруглённых краёв под gdigrab (класс BUG-247, OPEN)
    ...
    '53': ('BUG-128', 1.71),    # background-origin: `font` shorthand больше не теряет font-size/line-height (BUG-114 FIXED, 9.62%→1.71%) — h3 13px/700, .label 11px monospace применяются (раньше всё 16px default → крупный текст распирал строки). Геометрия боксов совпадает с Edge. Остаток = font-parity (sans-serif/monospace рисуются Inter vs Edge, rule 3) + накопленный line-height-дрейф. Класс BUG-128
    # '54' убран (прогон 2026-07-04, REMOVE): BUG-173 FIXED — SVG <path> stroke переведён на нативный DrawSvgStroke (срез d87dae63); свежий full-build 0.26% (≤0.5%). Fill был нативным с 2026-06-30 (DrawSvgFill), stroke стал нативным 2026-07-04 → triangle-soup AA-швы устранены с обеих сторон
    ...
    # '60' убран (прогон 2026-07-04, REMOVE): BUG-173 FIXED — нативный DrawSvgStroke (d87dae63) убрал triangle-soup AA-швы дэша/кривых; свежий full-build 0.40% (≤0.5%). Был 1.41%
    ...
    '65': ('BUG-127', 2.08),   # CSS features: --ipc 2026-06-26 = 2.08% (стабильно). 98% в изоляции после удаления был state-ordering/tab-focus capture artefact; full-run до удаления = 1.40%. Восстановлен в KNOWN_DEBTORS с детерминированным baseline
    ...
    '36': ('BUG-176', 0.96),    # border-radius: BUG-176 FIXED (DEBTOR) 2026-07-04 P3-ревизия — свежий full-build gdigrab = 0.96% стабильно (был 1.11%). elliptical-corner kappa гипотеза ОПРОВЕРГНУТА (CPU-путь та же геометрия = 0.208%; diff = 1px-кайма по всему периметру, прямые кромки включ.). Остаток = инхерентный femtovg-vs-Edge edge-AA (класс BUG-124/247), квадратные рамки закрыты BUG-175. Ратчет 1.11→0.96
    '31': ('BUG-184', 0.60),    # clip-path: BUG-184 FIXED (DEBTOR) 2026-07-04 P3-ревизия. Геометрия всех форм верна. Свежий full-build gdigrab = 0.59% (bad=4318): inset()/rect-клипы = 0px (PushClipRect → ножницы, пиксель-в-пиксель); circle 1244px + ellipse 856px + polygon/path 2108px = femtovg AA-fringe shape-fill надувает НЕпрямоугольную clip-кромку ~на 1px наружу относительно AA-ядра Edge (Lumen/Edge сходятся по центру формы, Lumen на 1px шире радиусом по всем 4 сторонам). ДОКАЗАТЕЛЬСТВО инхерентности: CPU-путь (tiny_skia `--screenshot`) той же геометрии = 0.06% (bad=459; polygon/path 2108→89, circle 1244→188, ellipse 856→182) — растеризатор femtovg, не движок. Класс BUG-176 (border-radius edge-AA, CPU 0.208% vs femtovg 0.96%) / BUG-247 / BUG-173. Совпасть = inward-AA для clip-маски (femtovg не поддерживает) или CPU-coverage-маска (крупный переписыв, вне scope). polygon/path (диагонали, доминируют) не инсетятся чисто
    '30': ('BUG-144', 4.27),    # CSS filter/backdrop-filter: row-flip (BUG-144) + gradient hard-stop row 2 (BUG-085) + backdrop colour-matrix/combo больше не тёмные (CPU backdrop-пайплайн 7.56%→4.36%) + blur-качество (edge-bleed clamp + 3-pass Gaussian 4.36%→4.27%). Ревизия P3 2026-07-04: свежая сборка main = 4.27% (стабильно), diff-декомпозиция = filter/edge AA (thin 1px vertical AA-stripes по кромкам всех filter-карт rows 1-3, класс BUG-176/247) + blur-kernel AA (row 4) + card-label text (rule 3, класс BUG-128). Точечного P3-дефекта нет; ратчет 4.36→4.27
    ...
    # '39' и '40' убраны: BUG-085 FIXED (conic-gradient). 0.48% normal, 0.05% --ipc
    ...
    '45': ('BUG-240', 1.02),    # multiple-backgrounds: «4.73%» был артефактом устаревшего gdigrab-снимка (run.py без --build). Свежая сборка 2026-06-25 = 1.02%; все слои (.no-repeat/.repeated/тайлы background-size) рисуются корректно — свежий рендер совпадает с Edge пиксель-в-пиксель. Остаток 1.02% = font-parity 9 monospace-меток (rule 3, класс BUG-128) + ~0.14% верхний ряд y=0 (canvas-фон не закрашивает строку 0: html-бокс с propagated body-фоном #ff00ff сдвинут на y=1 схлопыванием margin:1px у .__f сквозь body без overflow:hidden/BFC)
    ...
    '19': ('BUG-219', 9.05),    # object-fit: геометрия всех 5 режимов + object-position верна (BUG-181: средние RGB совпадают, лучший сдвиг 0,0, letterbox корректен) — остаток = image-resampling AA на высокочастотном контенте (perceptron-диаграмма + agi rusty-текстура, area-avg ≠ Edge downscale kernel)
    ...
    '62': ('BUG-128', 8.85),    # scroll-snap: column flex-grow не распределял free space — фикснут BUG-104 (63.70% → 2.32%); все цветные заливки контейнеров совпадают с Edge пиксель-в-пиксель (diff), остаток = font-parity (метки секций A/1/NW/Stop, Inter vs Edge) + border-radius edge-AA (BUG-176). РЕГРЕССИЯ 2026-06-26: baseline 2.32% → 8.85% в --ipc (full-run gdigrab 2.32% был артефактом захвата)
    ...
    '76': ('BUG-176', 0.64),    # motion path: боксы фикснуты BUG-125 (anchor-on-path, 3.18% → 0.54%). BUG-230 FIXED 2026-06-24: `calc()`-стопы градиента больше не схлопываются → диагональный трек-индикатор `.track-diag` (`linear-gradient(... calc(50% ± 2px) ...)`) теперь рисуется (0.54% → 0.64%, в пределах ±2% gdigrab-шума). Остаток = edge-AA тонкой диагональной линии трека (sub-pixel, класс BUG-176) + font-parity меток (rule 3, BUG-128)
    ...
    '59': ('BUG-101', 17.15),   # image-set() / cross-fade() РЕАЛИЗОВАНЫ. Грамматика cross-fade() приведена к CSS Images L4 §4 (BUG-101, 24.18% → 17.15%): unprefixed `cross-fade(url, url, 30%)` — устаревшая 3-арг webkit-форма с висячим bare `<percentage>` — теперь невалидна (как в Edge/Chromium) → ячейки cf-30/cf-70 пустые, совпадают с Edge (центр обеих ячеек = фон #1a202c в обоих движках). `-webkit-cross-fade()` и валидная L4-форма `cross-fade(<image> <pct>?, <image>)` по-прежнему рисуются. Остаток = ресэмплинг фото-картинок (image-set/url ячейки row1 + webkit cross-fade row2, класс BUG-219) + font-parity monospace-меток (rule 3). parse_cross_fade в layout/src/style.rs
