...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '53': ('BUG-277', 1.16),    # Ратчет 8.17→1.16 (BUG-277 срез 15, 2026-08-05): квады `DrawBackgroundImage` в wgpu не проходили через накопленный `PushTransform`, поэтому фон ложился на CHROME_H=69px выше своего бокса. Текущее число НИЖЕ headless-CPU замера той же страницы (1.59%) — расхождения бэкендов больше нет, остаток = font-parity меток + ресэмплинг картинки (класс BUG-128), запись пора перецелить. A/B на одном бинарнике: `LUMEN_NO_IMG_XFORM=1` 7.95 / фикс 1.16. Прежний комментарий:   # background-origin: femtovg-baseline была 1.71% (font-parity, класс BUG-128 — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 5.45% (совпадает с BUG-277 рядом «53»). DS-9 (2026-07-23, P1): ратчет 5.45%→8.17% — тулбар удвоил высоту хрома (CHROME_H=72), TEST-00/03 калибруются на 0.00%, сдвиг только меняет sub-pixel snapping фоновых слоёв (класс BUG-124/PS-1), не новый дефект
    ...
    '83': ('BUG-277', 6.14),    # Ратчет 9.82→6.14 (BUG-277 срез 4, 2026-08-05: фикс SDF скруглённого прямоугольника — скролл-карточки страницы скруглены; часть выигрыша от влитого BUG-128 ч.3). Ратчет 11.91→9.82 (P3 2026-08-04, BUG-128: дефолт документа serif). scroll-behavior: femtovg-baseline была 7.88% (font-parity + overlay scrollbar, класс BUG-128 — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 11.91% (совпадает с BUG-277 рядом «83»)
