...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '39': ('BUG-085', 1.62),    # gradients: 12.05%→1.62%→1.18% (D2-1 2026-06-23). repeating + все radial рисуются per-pixel в device-разрешении (обход 256-тексельного LUT femtovg) → smooth-radial пиксель-в-пиксель; ellipse-as-circle вынесен в BUG-238 FIXED (был 69% диффа). baseline держим 1.62 (улучшение 0.44 в пределах ±2% gdigrab-полосы, вердикт OK). Остаток 1.18% = AA hard-stop repeating (.rep-linear/.rep-radial) + gdigrab crop-edge шум верх/низ кадра
