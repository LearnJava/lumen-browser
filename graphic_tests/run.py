...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '31': ('BUG-184', 0.60),    # clip-path: геометрия всех форм верна. Декомпозиция diff (0.60%) по рядам: inset()/rect-клипы = 0px (PushClipRect → ножницы, пиксель-в-пиксель); circle 1308px + ellipse 858px + polygon/path 2245px = femtovg AA-fringe shape-fill надувает НЕпрямоугольную clip-кромку ~на 1px наружу относительно AA-ядра Edge (Lumen/Edge сходятся по центру формы, Lumen на 1px шире радиусом по всем 4 сторонам). Невидимо иначе как попиксельно (rule 2); тот же класс, что BUG-176 (border-radius edge-AA) и BUG-173 (SVG stroke AA-швы). Совпасть = inward-AA для clip-маски (femtovg не поддерживает) или подгон под точное downscale-ядро Edge. polygon/path (диагонали, доминируют) не инсетятся чисто
