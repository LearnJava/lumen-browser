...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '78': ('BUG-127', 10.07),   # scroll-driven animations: feature wired (F2-2) — animation-timeline: scroll()/view()/named драйвит прогресс анимации от позиции скролла, а не от часов (shell/animation_scheduler.rs). 12.02%→10.07%: scroll()/named-боксы садятся в from-state как у Edge, view()-бокс едет по view-прогрессу (позиция совпадает). Остаток = (1) font-parity текста (заголовок + метки + 5-строчный .info моноблок, Inter vs Edge, класс BUG-128, доминирует) + (2) цвет view-бокса не композится — анимированный background-color не применяется в живом окне без relayout (BUG-231, только opacity/transform compositor-offloadable)
