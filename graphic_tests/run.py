...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '59': ('BUG-277', 10.03),   # Ратчет 23.65→10.03 (BUG-277 срез 15, 2026-08-05): два дефекта wgpu — квад `DrawCrossFade` не проходил через накопленный `PushTransform` (плюс `CrossFadeVertex` не реализовывал `VertexPos`), и `CROSS_FADE_SHADER_SRC` смешивал straight-alpha RGB вместо премультиплированных цветов (CSS Images L4 §4.2) → 50%-ная смесь поверх прозрачной области выходила вдвое темнее Edge. Остаток 10.03% против 5.19% на headless-CPU: боксы `cross-fade() 30%/70%` пусты и у Edge (старый синтаксис без префикса невалиден — совпадение, не дефект), разрыв сидит в ресэмплинге фотографического контента. A/B: `LUMEN_NO_IMG_XFORM=1` 21.17 / фикс 10.03. Прежний комментарий:   # image-set() / cross-fade() РЕАЛИЗОВАНЫ, femtovg-baseline была 17.15% (грамматика приведена к CSS Images L4 §4, класс BUG-101 — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 23.65% (совпадает с BUG-277 рядом «59»)
