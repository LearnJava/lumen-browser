...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '47': ('BUG-226', 2.27),    # SVG basic shapes: <line> рисовался как FillRect по всему bbox (диагональ = сплошной оранжевый прямоугольник) — фикснут BUG-189 (3.71% → 2.27%): line теперь штрихуется как толстый сегмент (DrawSvgPath) между концами по диагонали bbox. Остаток = SVG-штрих не центрирован на кромке (Lumen рисует stroke целиком ВНУТРИ бокса по border-box-модели, Edge центрирует: 5px наружу + 5px внутрь — orange-core 79×59 vs Edge 89×69, ±5px = stroke-width/2) → BUG-226, затрагивает rect/circle/ellipse во всех SVG-тестах + stroke-edge/rounded-corner AA (класс BUG-176)
