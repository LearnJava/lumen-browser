...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '21': ('BUG-248', 3.13),    # border-style dashed/dotted/double: AA-фаза штрих/точка-сегментов + субпиксельный сдвиг периодического узора vs Edge (геометрия верна). Класс BUG-247 (dash AA), но для CSS-бордеров
    '56': ('BUG-250', 1.83),    # mix-blend-mode: рассеянный diff по подписям сетки блендов = font-parity после смены FontMeasurer (BUG-250). Сами блендинг-режимы корректны
