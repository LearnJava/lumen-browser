...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '76': ('BUG-230', 0.54),    # motion path: боксы фикснуты BUG-125 (anchor-on-path вместо top-left-on-path, 3.18% → 0.54%) — горизонтальные/диагональные/cubic боксы пиксель-в-пиксель с Edge (маскирование диагональной полосы row 2 → 0.01%). Остаток = диагональный трек-индикатор `.track-diag` рисуется `linear-gradient(... calc(50% ± 2px) ...)`, а `calc()` в позиции color-stop схлопывает градиент → линия не рисуется (BUG-230, домен градиентов)
