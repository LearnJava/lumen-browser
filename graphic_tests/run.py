...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '65': ('BUG-277', 1.69),   # flex align-content. Ратчет 5.45→1.69 (BUG-277 срез 4, 2026-08-05, фикс SDF скруглённого прямоугольника: карточки страницы скруглены). femtovg-baseline была 2.08% (класс BUG-127 — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 5.45% (совпадает с BUG-277 рядом «65»)
    '63': ('BUG-277', 1.81),    # masonry. Ратчет 5.46→1.81 (BUG-277 срез 4, 2026-08-05, фикс SDF скруглённого прямоугольника: плитки скруглены; часть выигрыша — от влитого BUG-128 ч.2/3, страница текстовая). femtovg-baseline была 2.02% (border-radius edge-AA + font-parity, класс BUG-176 — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным, ратчет к живому wgpu-числу 5.46% (совпадает с BUG-277 рядом «63»)
    ...
    '45': ('BUG-277', 0.87),    # Ратчет 5.81→0.87 (BUG-277 срез 9, 2026-08-05): страница почти целиком собрана из градиентных слоёв, затухающих в `transparent`, поверх диагональных `135deg`-подложек — то есть ровно из двух дефектов среза (straight-alpha интерполяция стопов и UV-, а не пиксельное направление линии). Живое окно теперь ближе к Edge, чем детерминированный CPU-снимок (0.87% против 1.03%): растровые бэкенды приближают премультиплированную интерполяцию дроблением сегмента на 16 стопов (BUG-190), а шейдер считает её точно. Историческая справка: femtovg-baseline была 1.02% (font-parity + 1px y=0 row, класс BUG-128 — история сохранена). BUG-287/BUG-277 (2026-07-16): P1-wgpu-flip сделал wgpu дефолтным — wgpu рисует layered backgrounds иначе, ратчет к живому wgpu-числу 5.81% (совпадает с BUG-277 рядом «45»)
