...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '101': ('BUG-277', 3.66),   # INTERACTION border-radius × overflow:hidden. Ратчет 20.00→3.66 (BUG-277 срез 4, 2026-08-05): фикс SDF скруглённого прямоугольника — сами контейнеры теперь совпадают с Edge. Остаток целиком = ВТОРОЙ дефект того же бага: `PushClipRoundedRect` в wgpu кладёт в clip_stack только прямоугольник (scissor не умеет форм), поэтому дети не обрезаются по скруглению — круг c1 остаётся квадратом, у пилюли c5 квадратные торцы. Отдельный механизм (нужен offscreen+маска, как MaskComposite), см. срез 5. femtovg-baseline была 0.71% (sub-pixel AA скруглённых краёв, класс BUG-247 — история сохранена)
