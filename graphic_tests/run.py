...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '39': ('BUG-085', 1.62),    # gradients: 12.05%→1.62%→1.18% (D2-1 2026-06-23). repeating + все radial рисуются per-pixel в device-разрешении (обход 256-тексельного LUT femtovg) → smooth-radial пиксель-в-пиксель; ellipse-as-circle вынесен в BUG-239 FIXED (был 69% диффа). baseline держим 1.62 (улучшение 0.44 в пределах ±2% gdigrab-полосы, вердикт OK). Остаток 1.18% = AA hard-stop repeating (.rep-linear/.rep-radial) + gdigrab crop-edge шум верх/низ кадра
    ...
    '78': ('BUG-127', 10.07),   # scroll-driven animations: feature wired (F2-2) — animation-timeline: scroll()/view()/named драйвит прогресс анимации от позиции скролла, а не от часов (shell/animation_scheduler.rs). 12.02%→10.07%→9.54%: scroll()/named-боксы садятся в from-state как у Edge, view()-бокс едет по view-прогрессу (позиция совпадает); с D2-2 (BUG-231 FIXED) фон view-бокса композится (интерполированный оранжевый вместо teal). Остаток = font-parity текста (заголовок + метки + 5-строчный .info моноблок, Inter vs Edge, класс BUG-128, доминирует) — закроется только с FP-1. Baseline 10.07 удержан (9.54 в пределах ±2 gdigrab-шума, ратчет вниз требует <8.07)
    ...
    '58': ('BUG-100', 4.92),    # ::first-letter / ::first-line РЕАЛИЗОВАНЫ (дрейф трекера, прогон 2026-06-23): drop-cap «O» флоатится (extract_first_letter_float + 7 тестов), first-line зелёная+жирная. Diff-картинка подтверждает: фича работает, остаток = font-parity тела абзаца (Inter vs Edge sans → разные метрики → перенос строк сдвигается, ghosting глифов) + edge-AA большого 48px drop-cap-глифа (rule 3)
