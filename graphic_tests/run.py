...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '57': ('BUG-099', 0.91),    # <canvas> getContext("2d") — Phase 2. Ратчет вниз 4.14→2.96 (прогон 2026-06-23) →0.91 (2026-07-29). IPC даёт 27.96% (режим-зависимый артефакт захвата); нормальный gdigrab = 0.91%. Ратчет 2.96→0.91: BUG-099 — размеры битмапа `<canvas>` это intrinsic (content-box) размер, а не значение `width`/`height` (HTML Rendering §15.4.1 не маппит dimension-атрибуты canvas на свойства), поэтому `box-sizing:border-box` вычитал из битмапа рамку (c3 был 180×150 вместо 186×156 у Edge) и весь ряд 2 стоял на 5px выше; плюс битмап красился в border-box вместо content-box. Canvas-AA закрыт 2026-07-29 (дробное покрытие в rasterize.rs), но диф от этого не сдвинулся — остаток определяется 1px сдвигом бокса ряда 2 (класс PS-1/BUG-124) + font-parity меток (BUG-128) + неприменяемым lineJoin (зарубка в углу strokeRect c6)
