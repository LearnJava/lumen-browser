...
KNOWN_DEBTORS: dict[str, tuple[str, float]] = {
    ...
    '101': ('BUG-249', 3.90),   # INTERACTION border-radius × overflow:hidden: round-rect clip детей расходится с Edge; юнит-деп TEST-36/14 зелёные → дефект в клипе интеракции. Диагностика `--bisect 101`
    ...
    '60': ('BUG-173', 1.41),   # SVG stroke advanced остаток (triangle-soup AA-швы, dash-on-curve). --ipc 2026-06-26 = 0.20%, нормальный режим = 1.41% (стабильно, был 1.40% в full-run до удаления)
    ...
    '119': ('BUG-173', 0.81),   # РЕГРЕССИЯ 2026-06-29 → BUG-262: свежий full-build 16.52% (>0.81% baseline), перепроверено x2 стабильно. Скачок 0.81→16.52 = новый дефект paint-order, не AA-остаток BUG-173. Baseline НЕ поднят (rule 4b) — гейт честно держит REGRESS до фикса P3
    ...
    '78': ('BUG-127', 5.76),   # scroll-driven animations: feature wired (F2-2) — animation-timeline: scroll()/view()/named драйвит прогресс анимации от позиции скролла, а не от часов (shell/animation_scheduler.rs). 12.02%→10.07%→9.54%: scroll()/named-боксы садятся в from-state как у Edge, view()-бокс едет по view-прогрессу (позиция совпадает); с D2-2 (BUG-231 FIXED) фон view-бокса композится (интерполированный оранжевый вместо teal). Остаток = font-parity текста (заголовок + метки + 5-строчный .info моноблок, Inter vs Edge, класс BUG-128, доминирует) — закроется только с FP-1. --ipc 2026-06-26: 10.07→5.76
