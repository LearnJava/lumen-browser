...
На момент написания: 2543 тестов, 14 крейтов (`shell`, `core`, `network`, `storage`, `knowledge`, `bench`, `dom`, `html-parser`, `css-parser`, `layout`, `paint`, `font`, `encoding`, `image`). При прохождении следующих фаз появится `lumen-ai` и др.
...
- **Font fallback частично работает — face switching ✅, codepoint cascade ⬜.** Renderer держит `Vec<LoadedFace>` + `font_provider: Option<Arc<dyn FontProvider>>` (по умолчанию `SystemFontIndex`); при `font-family: Roboto` ищет «Roboto» в системных шрифтах и грузит. Если ни одно имя из CSS-списка не нашлось — fallback на bundled Inter. **Что ещё не работает:** codepoint cascade — если в выбранном face нет глифа для символа (эмодзи / CJK у Roboto-Latin), показывается `.notdef` (`?` или пустой box) вместо поиска другого face. Это последний шаг 1A. Generic CSS-family (`serif`/`sans-serif`/`monospace`) пока пропускаются — Phase 0 не имеет per-generic-table-ы.
