...
- **Rust is pinned to 1.97.0** and **sccache is off** — see §Code conventions. A stale `target/` after a toolchain change surfaces as `E0786 "only metadata stub found for rlib dependency core"`, which reads like a broken dependency and is cured by `cargo clean`.
