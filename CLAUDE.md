...
- No `panic!` / `unwrap()` in production code; allowed in tests. The `panic!` half is **machine-checked** since 2026-08-18 (`clippy::panic = "deny"`); `unwrap()`/`expect()` are still prose only. Note `clippy.toml`'s `allow-panic-in-tests` frees only the body of a `#[test]` fn — a plain helper in a test crate still trips the lint, which is why test roots carry `#![allow(clippy::panic)]`.
