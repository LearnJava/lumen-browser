...
- **A top-level synchronous `throw` is still invisible — the engine prints `script error: …` on stderr but never fires the window `error` event — even though the promise-rejection half of the same class of defect ([BUG-716](bugs/BUG-716-FIXED.md)) was fixed 2026-08-22.** `unhandledrejection`/`rejectionhandled` now dispatch (`v8::Isolate::set_promise_reject_callback` in `v8_runtime.rs`, deferred onto the isolate's own microtask queue); `window.onerror`/`addEventListener('error')` for a synchronous exception still do not — that half is the still-open umbrella [BUG-591](bugs/BUG-591-OPEN.md) (no bridge from the host `TryCatch` back to page script). Originally verified 2026-08-21 with two three-line probes under `--dump-layout`, before the fix: at that time `Promise.reject(new Error(...))` fired neither `addEventListener('unhandledrejection')` nor `window.onunhandledrejection` and printed nothing on stderr — re-probe before relying on that half again. `testharness.js` reports both classes of failure *through those two events* (`error_handler`), so a failed assertion inside a **promise chain** should now surface as an ordinary FAIL instead of the harness going quiet until wptrunner's timeout (~9 s of wall clock against 0.05 s for a resolved test); a failed assertion inside a **synchronous** top-level script still degrades to TIMEOUT via BUG-591. **Consequence for any triage of a WPT TIMEOUT:** an empty browser log is still not proof the page hung for the synchronous-throw case; the promise-chain case needs re-measurement (P2/WPT-RUN — the numbers below predate the fix and are likely stale for that slice). Instrument the run with `python tests/wpt/rejection_trace.py --on` (adds a JS-level rejection tracker to Lumen's `testharnessreport.js`, prints `LUMEN_UNHANDLED_REJECTION: …`, classified by `tests/wpt/timeout_audit.py`), then `--off` — never leave it on for a run whose numbers are meant to be trusted, it wraps `Promise.prototype.then`. **The complement matters just as much:** a TIMEOUT whose log *does* carry a `script error:`/`module error:` line has already named its own cause — 313 of the 1 488 unexplained TIMEOUTs in the WPT-RUN-5 snapshot printed an exception naming a missing API (`el.style = "..."` in a strict-mode helper, `getClientRects`, `document.fonts.load`, `CSS.px`), and they are TIMEOUTs rather than FAILs only because the same missing `error` event keeps `testharness.js` from seeing them. `timeout_audit.py` maps those texts onto the owning bugs and files the rest under `swallowed_errors` (`--json`), so grep the browser output for `error:` before assuming a hang (WPT-RUN-6 slice 14).
