...
- **`<picture>` / `<img srcset>` integrated в layout, но DPR=1.0 — Phase 0 заглушка.** `lumen-layout::box_tree::resolve_image_source` для каждого `<img>` теперь вызывает `pick_picture_source` (если parent — `<picture>`) или `pick_img_source` (одиночный) с `PictureParams { dpr: 1.0, supported_types: None }`. Это значит density-кандидаты `srcset="… 2x"` Phase 0 **никогда не выбираются** — picker всегда берёт `1x`. Реальный DPR живёт в `Renderer.scale_factor` (получается из winit `ScaleFactorChanged`) и пока не пробрасывается в layout-вызов (layout сейчас вообще не знает DPR). Когда P3 сделает relayout-on-resize и пробросит `inner_size`/`scale_factor` в `lumen-shell::resumed()` → `layout`, нужно расширить `box_tree::resolve_image_source` параметром `dpr: f32` и передавать его в `PictureParams`. Picker-API менять не придётся — параметр уже есть. Intrinsic dimensions у выбранного `<source>` применяются как presentational hint после CSS-каскада (только в пустые `style.width`/`style.height`) — иначе они перекрыли бы author CSS, что не соответствует HTML5 §10. **`<source>` и `<track>` теперь `display: none`** в default UA stylesheet — раньше они порождали пустые BoxKind::Block.
