...
- **Готово (CSS math-функции — CSS Values L4 §10 и §10.6):** `Length::Calc(Box<CalcNode>)` хранит AST выражения; `CalcNode::resolve(em, pb, vp) -> Option<f32>` рекурсивно вычисляет в `f32`-пиксели. Поддерживаются: `calc(expr)`, `min(a, b, ...)`, `max(a, b, ...)`, `clamp(min, val, max)`. `clamp(a, b, c) ≡ max(a, min(b, c))` — при `min > max` побеждает `min`. Все 4 функции — top-level value или вложенные внутри `calc()`, `min()`, `max()`, `clamp()` (в любых комбинациях). Парсер — recursive-descent поверх `tokenize_calc`: top-level value, начинающееся с ASCII-буквы и содержащее `(`, обрабатывается через `parse_math_function_value` (`looks_like_function_call` → tokenize → parse_calc_expr); лексер выдаёт токены `Num(f32, unit)` / `Ident(name)` (case-insensitive) / `Plus` / `Minus` / `Star` / `Slash` / `LParen` / `RParen` / `Comma`. Грамматика: `expr := term (('+'|'-') term)*`, `term := factor (('*'|'/') factor)*`, `factor := ('-'|'+') factor | Ident '(' arg-list ')' | Num | '(' expr ')'`, `arg-list := expr (',' expr)*`. `parse_function_call` обрабатывает 4 имени: `calc` (ровно 1 expr), `min`/`max` (1+ expr через `,`), `clamp` (ровно 3 expr). Неизвестное имя функции (`sin`, `abs`, `pow`) → declaration invalid (None). Унарный `-` — через `0 - factor` рекурсивно (корректно для `calc(-10px + 5px)` и `calc(20px + (-5px))`); унарный `+` — no-op. Поддерживаемые единицы внутри math-функций: те же что у `parse_length` (px/em/rem/vh/vw/vmin/vmax/%) + unitless `Number`. Неизвестная единица (`pt`/`mm`) → declaration invalid. Деление на 0 → `None`. Пустой `min()` / `max()` → None. Интеграция с var(): `var()` разворачивается раньше — `width: min(var(--w), 50px)` работает. Phase 0 ограничения: `line-height: calc(...)` использует общий путь «резолв в px → делим на font-size», что для unitless-чистого calc даст неверный результат (используйте bare `line-height: 1.5`); нет `sin()` / `cos()` / `tan()` / `abs()` / `sign()` / `mod()` / `rem()` / `pow()` / `sqrt()` / `exp()` / `log()` / `hypot()` (CSS Values L4 §10.7-10.9).
...
- **Всего тестов в workspace:** 969 (на момент последнего обновления).
...
6. **`[P1]` CSS — типизированные значения деклараций** — length / color / calc. Селекторы Level 3 готовы полностью. **CSS Variables (`--name` + `var()`), `calc()`, `min()`/`max()`/`clamp()` — реализованы** в `lumen-layout` (см. «Состояние подсистем»). Осталось: типизированный `Length` для всех свойств (сейчас Length типизирован для большинства, но в каскаде значения хранятся строкой и парсятся в момент apply), типизированные `Color`-значения у всех свойств, `sin()`/`cos()`/`abs()`/`mod()`/`hypot()` и прочие math-функции из CSS Values L4 §10.7-10.9, registered custom properties (`@property`).
