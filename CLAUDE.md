...
На момент написания: 3805 тестов, 14 крейтов (`shell`, `core`, `network`, `storage`, `knowledge`, `bench`, `dom`, `html-parser`, `css-parser`, `layout`, `paint`, `font`, `encoding`, `image`). При прохождении следующих фаз появится `lumen-ai` и др.
...
1. **Скрины не сохраняем в репо.** `graphic_tests/screenshots/*.png` — рабочие артефакты для текущего анализа, не коммитим (ни Edge-эталон, ни Lumen-захват, ни diff). В коммит идёт только обновлённый `graphic_tests/BUGS.md`.
...
4. **`graphic_tests/BUGS.md` обновляем при каждом прогоне, историю оставляем.** Найден новый — добавляем следующим номером. Уже зафиксированный воспроизвёлся — обновляем дату/скриншот-описание, статус не трогаем. Фикснули — меняем статус на `FIXED` с датой, **запись не удаляем**. WONTFIX до Phase N+ — тоже остаётся в файле.
