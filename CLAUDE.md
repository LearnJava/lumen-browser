...
- No `panic!` / `unwrap()` in production code; allowed in tests. `panic!` and `expect()` are **machine-checked** since 2026-08-18 (`clippy::panic` / `clippy::expect_used = "deny"`); `unwrap()` is still prose only. Note `clippy.toml`'s `allow-*-in-tests` frees only the body of a `#[test]` fn — a plain helper in a test crate still trips the lint, which is why test roots carry `#![allow(clippy::panic, clippy::expect_used)]`. Grandfathered sites carry a **function-scoped** `#[allow]`, never a crate-scoped one, so the rule keeps applying to new code; every allow is listed in `docs/lint-policy.md` §10.
