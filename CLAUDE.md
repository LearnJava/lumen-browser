...
- **Всего тестов в workspace:** 883 (на момент последнего обновления).
...
6. **`[P1]` CSS — типизированные значения деклараций** — length / color / calc. Селекторы Level 3 готовы полностью. **CSS Variables (`--name` + `var()`) — реализованы** в `lumen-layout` (см. «Состояние подсистем»): custom_props inherited через cascade, three-pass cascade с custom-pass перед main-pass, рекурсивный expand_vars с depth limit 32. Осталось: типизированный `Length` для всех свойств (сейчас Length типизирован, но в каскаде хранится строкой), типизированные `Color`-значения у всех свойств, `calc()` арифметика.
