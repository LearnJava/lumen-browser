...
- **Готово (CSS Properties and Values L1 §1.1 — `@property`):** `compute_style` строит локальный `registry: HashMap<&str, &PropertyRule>` из `sheet.properties` (повтор имени — last-wins через `insert`). На init `style.custom_props` после `inherited.clone()` делается `retain`, который убирает у потомка значения тех зарегистрированных имён, что объявлены с `inherits: false` — для них потомок должен видеть либо локальную декларацию, либо `initial-value`, а не родительское значение. После custom-pass (когда уже применены все cascade-declared `--name` записи) вызывается `apply_property_initial_values`, который для каждого зарегистрированного имени без значения в `custom_props` подставляет `initial-value` (если он есть). Делается ДО main-pass, чтобы `var(--registered)` в обычных свойствах увидел initial-value-fallback при отсутствии декларации. Phase 0 ограничения: syntax-валидация не делается (любая строка проходит как `*`-универсал); registered custom properties с `syntax != "*"` парсятся, но трактуются как `*` при применении.
...
- **Всего тестов в workspace:** 1121 (после merge css-wide-keywords).
...
6. **`[P1]` CSS — типизированные значения деклараций** — length / color / calc. Селекторы Level 3 готовы полностью. **CSS Variables (`--name` + `var()`), `calc()`, `min()`/`max()`/`clamp()`, scientific math (`sin/cos/tan/asin/acos/atan/atan2/pow/sqrt/exp/log/hypot/abs/sign/mod/rem/round`), CSS-wide keywords (inherit / initial / unset / revert) — реализованы** в `lumen-layout` (см. «Состояние подсистем»). Осталось: типизированный `Length` для всех свойств (сейчас Length типизирован для большинства, но в каскаде значения хранятся строкой и парсятся в момент apply), типизированные `Color`-значения у всех свойств, **syntax-валидация в `@property`** (сейчас только `*`-универсал, parser принимает любой syntax-string, layout применяет initial-value/inherits без проверки типа значения).
