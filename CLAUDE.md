...
- **Отложено:** flex, grid, float, абсолютное позиционирование, units кроме px (em / rem / %), color spaces CSS4 (`lab`, `lch`, `oklab`, `oklch`, `color()`), hue в `turn`/`rad`/`grad`, `box-sizing`, borders, text-decoration, font-weight/style как separate inline-property, селектор-matching с back-tracking.
- 79 unit + 4 snapshot-unit + 12 snapshot-integration = 95 тестов.
...
- **Всего тестов в workspace:** 379 (на момент последнего обновления).
...
```
*            css-dimensions         — CSS width/height (px): явные размеры блоков; 7 новых тестов
