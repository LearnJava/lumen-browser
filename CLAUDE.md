...
- **Live-window BiDi/MCP `script.evaluate` can hang indefinitely in some working sessions.** Spawning `lumen --bidi-port <port>` and driving it (no other flags — plain window mode) sometimes never finishes installing the live window's JS runtime: every `script.evaluate` reports `"JS context not available"` forever, well past the normal install race that `LumenTestharnessExecutor`/`tests/wpt/verify_devx6_bidi_scenarios.py` already tolerate with a bounded retry. Reproduced independent of page content/URL scheme (`http://127.0.0.1:<port>/…`) and independent of which BiDi command triggered the eval — looks like a class of gap similar to the known `ffmpeg -f gdigrab` access-denied issue in some Windows sessions (DEVX-1/DEVX-4 notes), not something to root-cause inside an unrelated task. `browsingContext.navigate` itself (`DocumentReady`) and all non-eval BiDi commands are unaffected — only `script.evaluate`-dependent automation (`click`/`type` synthetic-event dispatch, `eval`, anything polling a JS global) is at risk. If you hit this, don't burn time chasing it inside your task — bound your `script.evaluate` retries and report/skip, same pattern as `verify_devx6_bidi_scenarios.py`'s `JsContextUnavailable`. A related-looking mechanism, [BUG-296](bugs/BUG-296-FIXED.md), turned out to be a **separate, now-fixed** bug: a stale on-disk `last_session.db` (session restore, CWD-relative store path — not a "default homepage" feature) could reopen a leftover tab and race an automation driver's `browsingContext.navigate`. `--bidi-port`/`--mcp-live-port` launches now skip session restore entirely, so that specific race is gone — but the `script.evaluate`-install hang described above is a distinct, still-open gap.
