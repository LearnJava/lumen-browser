...
tiny-skia) + `driver/tests/snapshot_cpu.rs` compares 20 geometry pages against
...
`SAVE_CPU_SNAPSHOTS=1`. `PAGES` holds only pages with ≥2% non-background geometry; coverage of
text/image/gradient pages grows as `cpu_raster` gains those primitives.
...
**Defined:** `NetworkTransport`, `StorageBackend`, `SearchProvider`, `FilterListSource`, `RequestFilter`, `EncodingDetector`, `EventSink`, `DnsResolver`, `HstsEnforcement`, `HttpCredentialProvider`, `FontProvider`, `JsRuntime` (`NullJsRuntime` stub), `JsFetchProvider`, `JsWebSocketProvider` / `JsWebSocketSession` / `JsWsEvent`, `BrowserSession` (ADR-006, `core/src/ext.rs:1514`), `IdbBackend` (`lumen-storage::indexed_db`), `MemoryPressureSource` + `MemoryPressureLevel` (ADR-008 §10H, `core/src/ext.rs` + `core/src/memory_pressure.rs`; Win32/Linux platform impls; `NullMemoryPressureSource` for tests), `EvictableCache` + `CacheRegistry` (ADR-008 §10D.3, `core/src/ext.rs`; implemented by `GlyphAtlas`, `ImageDecodeCache`, `LayerCache`; P3 shell wires `CacheRegistry::broadcast_pressure()` to `MemoryPressureSource` poll loop).
