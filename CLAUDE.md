...
| `docs/ci-offload.md` | **Plan for moving part of the task gate to GitHub Actions** (the repo is public, so standard runners are free and unlimited): what moves and what stays local, cache/checkout/concurrency strategy under the 10 GB cache cap, how the assistant consumes run results via `gh`, and the rollout order. **Mostly implemented as of 2026-08-19** (five jobs in `ci.yml`, cache/checkout/concurrency, `bench-gate.yml` deleted); the tail — artifact upload, the §8 desync with the merge rule, and `snapshot-cpu` as a blocking gate — is owned by P2 (`ROADMAP.md` rows `CI-5`/`CI-6`, plus `PERF-7`). Read before touching `.github/workflows/*` or proposing "let CI do it". |
