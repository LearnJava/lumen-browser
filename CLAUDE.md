...
4b. **Known-debtors ratchet (user-approved 2026-06-11).** The only sanctioned way to handle a page that cannot reach 0.5% because the required feature is deferred to a later phase (e.g. 57-canvas → BUG-099, `getContext("2d")` is Phase 2): list it as `(page, "BUG-NNN", baseline%)` in the `KNOWN_DEBTORS` table of the (planned) `cpu_vs_edge` gate. The page is still rendered and diffed every run; its red percentage is always printed. Semantics: actual > baseline → FAIL (something already-working broke on that page); == baseline → OK; < baseline → FAIL with "lower the baseline to X" (ratchet — only moves down); ≤ 0.5% → FAIL with "remove the entry" (the list cannot go stale). Exit from the list happens **only by measurement**, never because a feature is declared done. Each entry requires an OPEN BUG-NNN whose text confirms (via diff image) the deviation is localized to the unimplemented feature's region. Thresholds themselves still never change (rule 4a). Full plan: `docs/plans/cpu-vs-edge-gate.md`.
