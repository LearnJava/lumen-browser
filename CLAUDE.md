...
### `lumen-storage` ✅ (in-memory + SQLite-backed)
...
- **Отложено:** TTL для cookies (отдельная таблица или поле `expires_at INTEGER`), namespace helpers (`cookies::`, `history::`, `profile::`), `clear_origin(origin)` для быстрой чистки всех данных источника, шифрование (`SQLCipher`-style — Phase 3 профили), миграции схемы (через `user_version` PRAGMA), FTS5-таблица для knowledge layer §12.1 (отдельный модуль).
- 30 тестов: 17 InMemoryStorage (CRUD, origin-изоляция, top_level_site, list_keys, snapshot round-trip с binary и кириллицей, ошибки десериализации) + 13 SqliteStorage (CRUD, overwrite, missing-delete noop, origin/top_level_site партиционирование, None ≡ Some(""), list_keys, binary BLOB-ы, кириллица в keys/values, persistence across open).
...
- **Всего тестов в workspace:** 1160 (после merge css-property-time-resolution).
