...
### `lumen-storage` ✅ (in-memory + SQLite-backed + Cookie jar)
...
- **Отложено:** Set-Cookie HTTP-парсер (атрибуты `Domain` / `Path` / `Max-Age` / `Expires` / `Secure` / `HttpOnly` / `SameSite`), namespace helpers (`history::`, `profile::`), `clear_origin(origin)` для быстрой чистки всех данных источника, шифрование (`SQLCipher`-style — Phase 3 профили), миграции схемы (через `user_version` PRAGMA), FTS5-таблица для knowledge layer §12.1 (отдельный модуль), интеграция CookieJar с HttpClient (Cookie header генерация при request, Set-Cookie parsing при response).
- 50 тестов: 17 InMemoryStorage (CRUD, origin-изоляция, top_level_site, list_keys, snapshot round-trip с binary и кириллицей, ошибки десериализации) + 13 SqliteStorage (CRUD, overwrite, missing-delete noop, origin/top_level_site партиционирование, None ≡ Some(""), list_keys, binary BLOB-ы, кириллица в keys/values, persistence across open) + 20 CookieJar (domain_matches exact/sub/not-prefix/not-reverse, path_matches exact/trailing-slash/continuation-slash/root, set+get roundtrip, set overwrites, delete, expired-filter, clear_expired, clear_session, secure only over HTTPS, subdomain matching, top_level_site partitioning, lowercase normalization, path filtering, SameSite roundtrip).
...
- **Всего тестов в workspace:** 1180 (после merge cookie-jar).
