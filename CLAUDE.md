...
Создание worktree для новой задачи:
```bash
# из основного клона:
git worktree add ../lumen-<имя-задачи> -b <имя-ветки>
cd ../lumen-<имя-задачи>
```

После этого сессия живёт в `../lumen-<имя-задачи>/` и не трогает чужой рабочий tree. По завершении (после merge в main):
```bash
git worktree remove ../lumen-<имя-задачи>
```
...
- **`:has(rs1, rs2, …)` (CSS Selectors L4 §17.2):** `PseudoClass::Has(Vec<RelativeSelector>)`. Каждый relative selector — `(combinator?, ComplexSelector)`. Combinator опционален; если опущен — implicit descendant. Specificity contributes max-of-list (как `:is`); сам combinator никакой specificity не даёт. Парсер `parse_relative_selector_list` распознаёт `>`/`+`/`~` в начале каждого элемента списка через запятую.
- **Отложено:** `:not(complex)` со списком селекторов или combinator-ами, namespace prefix в селекторах, типизированные значения деклараций (length / color / calc / `--var`).
- 95 тестов.
...
- **Отложено:** flex, grid, float, абсолютное позиционирование, `%` в margin/padding/width/height (нужен containing block), единицы `ch`/`ex` (требуют font metrics), color spaces CSS4 (`lab`, `lch`, `oklab`, `oklch`, `color()`), реальная отрисовка bold/italic вариантов в paint.
...
- **Всего тестов в workspace:** 708 (на момент последнего обновления).
...
- (CSS4 functional pseudo-classes полностью покрыты: `:is`, `:where`, `:has`, `:nth-*`, `:not(compound)`.)
