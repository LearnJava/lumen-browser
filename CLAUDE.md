...
- **Отложено:** tRNS для grayscale/RGB (single-color transparency, не палитровый случай — Phase 0 рендерит как opaque), Adam7 interlacing (7 sub-images с фиксированной геометрией), JPEG (свой DCT/Huffman/marker parser — отдельный крейт-объём работы), WebP / AVIF (Phase 2+).
- 76 unit-теста (CRC32, signature, chunk reader, IHDR, BitReader, Huffman build/decode, fixed/dynamic/stored inflate, LZ77, adler32, фильтры 0–4, PLTE/tRNS парсеры + palette expansion, bit-unpacking MSB-first для 1/2/4-bit + grayscale scaling, **downsample_16bit_to_8bit**) + 24 интеграционных на реальных PNG-фикстурах (RGB 3×2, RGBA 4×2, Gray 4×4, GrayAlpha 2×2, mixed filters 2×4, Paeth 2×2, palette 4×2 без tRNS, palette 3×3 с full tRNS, palette 2×2 с partial tRNS, grayscale-with-PLTE error, missing-PLTE error, gray 1-bit 8×2, gray 2-bit 4×2, gray 4-bit 3×2 с padding, palette 1-bit 8×1, palette 4-bit с tRNS 4×2, **gray 16-bit 2×2, GrayA 16-bit 2×2, RGB 16-bit 2×2, RGBA 16-bit 2×2, gray 16-bit с фильтрами None/Sub/Up 2×3**, rejects бракованных). Фикстуры сгенерированы Python-zlib скриптом (см. `tests/fixtures/`).
...
- **Всего тестов в workspace:** 1086 (после merge png-16bit).
