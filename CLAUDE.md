...
- **Готово (orchestrator `decode_png`):** проверка 8-байтовой сигнатуры `89 50 4E 47 0D 0A 1A 0A`; первый чанк обязательно IHDR; auxiliary-чанки (sRGB / gAMA / pHYs / tEXt / iCCP / cHRM) игнорируются (PNG §11.3 — ancillary safe-to-ignore); `PLTE` / `tRNS` парсятся и валидируются по правилам ordering; IDAT-ы конкатенируются в один zlib-поток; IEND маркирует конец. Поддержаны `PixelFormat::{Gray8, GrayAlpha8, Rgb8, Rgba8}` при `bit_depth = 8` без interlacing, **palette при `bit_depth = 1/2/4/8`** (формат выхода — `Rgb8` без `tRNS`, `Rgba8` с `tRNS`), **grayscale при `bit_depth = 1/2/4/8`** (выход — `Gray8` после scaling). 16-bit и interlaced проверяются раньше любой path-specific логики и возвращают `Unsupported(...)`. `Ihdr::pixel_format()` остался для не-палитровых и не-sub-byte-grayscale случаев — палитра и sub-byte grayscale обрабатываются в orchestrator-е отдельно.
- **Отложено:** 16-битная глубина (умножение байт на пиксель × 2), tRNS для grayscale/RGB (single-color transparency, не палитровый случай — Phase 0 рендерит как opaque), Adam7 interlacing (7 sub-images с фиксированной геометрией), JPEG (свой DCT/Huffman/marker parser — отдельный крейт-объём работы), WebP / AVIF (Phase 2+).
- 72 unit-теста (CRC32, signature, chunk reader, IHDR, BitReader, Huffman build/decode, fixed/dynamic/stored inflate, LZ77, adler32, фильтры 0–4, PLTE/tRNS парсеры + palette expansion, **bit-unpacking MSB-first для 1/2/4-bit + grayscale scaling**) + 19 интеграционных на реальных PNG-фикстурах (RGB 3×2, RGBA 4×2, Gray 4×4, GrayAlpha 2×2, mixed filters 2×4, Paeth 2×2, palette 4×2 без tRNS, palette 3×3 с full tRNS, palette 2×2 с partial tRNS, grayscale-with-PLTE error, missing-PLTE error, **gray 1-bit 8×2, gray 2-bit 4×2, gray 4-bit 3×2 с padding, palette 1-bit 8×1, palette 4-bit с tRNS 4×2**, rejects бракованных). Фикстуры сгенерированы Python-zlib скриптом (см. `tests/fixtures/`).
...
- **Всего тестов в workspace:** 1038 (после merge shell-dump-modes + css-round-strategies + css-property-at-rule).
