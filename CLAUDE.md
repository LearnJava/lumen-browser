...
and the `<img>` grey placeholder quad (`DrawImage` → `rgba8(217,217,217,255)`; the headless CPU
path registers no decoded pixels, so it mirrors the GPU renderer's placeholder fallback — page
