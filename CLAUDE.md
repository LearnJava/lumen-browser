...
- **A page-script failure that happens inside a promise (or in any top-level `<script>`) is invisible: the engine dispatches neither `unhandledrejection` nor the window `error` event, so an automated harness sees a silent hang rather than a failure.** [BUG-716](bugs/BUG-716-OPEN.md) (no `v8::Isolate::set_promise_reject_callback` anywhere in the workspace — `grep` it) and its umbrella [BUG-591](bugs/BUG-591-OPEN.md) (no bridge from the host `TryCatch` back to page script). Verified 2026-08-21 with two three-line probes under `--dump-layout`: `Promise.reject(new Error(...))` fires neither `addEventListener('unhandledrejection')` nor `window.onunhandledrejection` and prints nothing on stderr; a top-level `throw` prints `script error: …` on stderr but still does not fire `addEventListener('error')`. `testharness.js` reports both classes of failure *through those two events* (`error_handler`), so under WPT a failed assertion inside a promise chain does not become a FAIL — the harness goes quiet and wptrunner kills it on the timeout (~9 s of wall clock against 0.05 s for a resolved test). **Consequence for any triage of a WPT TIMEOUT:** an empty browser log is not evidence that the page hung — it is the expected appearance of an ordinary assertion failure. Instrument the run with `python tests/wpt/rejection_trace.py --on` (adds a JS-level rejection tracker to Lumen's `testharnessreport.js`, prints `LUMEN_UNHANDLED_REJECTION: …`, classified by `tests/wpt/timeout_audit.py`), then `--off` — never leave it on for a run whose numbers are meant to be trusted, it wraps `Promise.prototype.then`. **The complement matters just as much:** a TIMEOUT whose log *does* carry a `script error:`/`module error:` line has already named its own cause — 313 of the 1 488 unexplained TIMEOUTs in the WPT-RUN-5 snapshot printed an exception naming a missing API (`el.style = "..."` in a strict-mode helper, `getClientRects`, `document.fonts.load`, `CSS.px`), and they are TIMEOUTs rather than FAILs only because the same missing `error` event keeps `testharness.js` from seeing them. `timeout_audit.py` maps those texts onto the owning bugs and files the rest under `swallowed_errors` (`--json`), so grep the browser output for `error:` before assuming a hang (WPT-RUN-6 slice 14).
