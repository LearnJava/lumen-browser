...
Current phase: **Phase 2 — v0.5 «Interactive» (complete)**, app version **v0.5.0**. Phase 0 (prototype) closed 2026-05-26; Phase 1 «Reader» largely complete. Phase 2 delivered: an embedded JS engine (QuickJS at the time, replaced by V8 in S12b — see the paragraph below), Canvas 2D, CSS Grid, Shadow DOM, accessibility tree, forms, find-in-page, DevTools/CDP, knowledge layer.
...
| `docs/invariants.md` | **One page of things a local change must not break** — layering, engine identity, privacy surface, storage, rendering, threading, automation levels, plus the machine-checked code rules with their gate. One line per rule + a link to the ADR/policy that argues it; `[gate: …]` marks the ones a build/CI failure already catches. Read before a change that crosses a crate boundary or touches a shared path. It is an index: on conflict the linked source wins, and a rule no ADR states does not belong there. |
...
- **`AutomationCommand::Screenshot` (`resource://screenshot` over `--mcp-live-port`) cannot see ANY per-frame overlay** — `render_current_page_to_png` rasterizes `self.display_list`, but the caret bar, spellcheck squiggles, validation tooltips and the DS-15 anonymous-profile border are all built fresh every frame as local `page_buf`/`anim_dl`/`overlay_buf` values inside `on_redraw_requested` and hand straight to `r.render(&combined, &overlay_buf, …)` — never written back into `self.display_list`. Measured on the page's OWN already-shipped `<input>` caret (FRAME-7 slice 1): click + `type` through MCP updates the value, but the automation screenshot shows no caret at all. A probe needing to *see* one of these overlays has no automation surface today — this is not evidence that the overlay itself is missing.
...
- **A `debug_assert!` guarding DOM-tree correctness (or any other JS-reachable invariant) is not a check — it compiles to nothing under `[profile.dev-release]`/`[profile.release]`** (both inherit `release`, no `debug-assertions` override), which is the profile every session builds and every test corpus runs against. `Document::append_child`/`insert_before`/`insert_after` (`crates/engine/dom/src/lib.rs`) guard the DOM §4.2.3 inclusive-ancestor cycle check this way, so `appendChild`/`insertBefore` never throw `HierarchyRequestError` on a cycle in the build anyone actually runs — the insertion silently creates a real cycle in the tree and hangs the engine with no crash, no panic, no log line ([BUG-954](bugs/BUG-954-OPEN.md)). Before trusting a `debug_assert!` as the enforcement for a JS-visible spec requirement, check whether it is the *only* guard.
...
- **`<video src="">`/`<audio src="">` never fire `loadstart`** — `<video>` still fires `error`; `<audio>` fires neither event at all ([BUG-955](bugs/BUG-955-OPEN.md)). A probe arming `loadstart` before its assertions hangs on an empty-string `src`, not just a missing one.
