...
На момент написания: 883 тестов, 13 крейтов (`shell`, `core`, `network`, `storage`, `bench`, `dom`, `html-parser`, `css-parser`, `layout`, `paint`, `font`, `encoding`, `image`). При прохождении следующих фаз появятся `lumen-knowledge`, `lumen-ai` и др.
...
### `lumen-image` 🟡 (PNG decode end-to-end, 8-битные форматы + palette)
...
- **Готово (orchestrator `decode_png`):** проверка 8-байтовой сигнатуры `89 50 4E 47 0D 0A 1A 0A`; первый чанк обязательно IHDR; auxiliary-чанки (sRGB / gAMA / pHYs / tEXt / iCCP / cHRM) игнорируются (PNG §11.3 — ancillary safe-to-ignore); `PLTE` / `tRNS` парсятся и валидируются по правилам ordering; IDAT-ы конкатенируются в один zlib-поток; IEND маркирует конец. Поддержаны `PixelFormat::{Gray8, GrayAlpha8, Rgb8, Rgba8}` при `bit_depth = 8` без interlacing, **и palette при `bit_depth = 8`** (формат выхода — `Rgb8` без `tRNS`, `Rgba8` с `tRNS`). Прочие комбинации (16-bit, interlaced, sub-byte, sub-byte palette) явно возвращают `Unsupported(...)` или `BadPalette(...)`. `Ihdr::pixel_format()` остался для не-палитровых случаев и явно паникует через `unreachable!()` при попытке вызова на palette — контракт «палитра обрабатывается орхестратором отдельно».
- **Отложено:** 16-битная глубина (умножение байт на пиксель × 2), 1/2/4-битная palette (требует bit-unpacking из upacked PNG-потока), tRNS для grayscale/RGB (single-color transparency, не палитровый случай — Phase 0 рендерит как opaque), Adam7 interlacing (7 sub-images с фиксированной геометрией), JPEG (свой DCT/Huffman/marker parser — отдельный крейт-объём работы), WebP / AVIF (Phase 2+).
- 61 unit-теста (CRC32, signature, chunk reader, IHDR, BitReader, Huffman build/decode, fixed/dynamic/stored inflate, LZ77, adler32, фильтры 0–4, **PLTE/tRNS парсеры + palette expansion**) + 14 интеграционных на реальных PNG-фикстурах (RGB 3×2, RGBA 4×2, Gray 4×4, GrayAlpha 2×2, mixed filters 2×4, Paeth 2×2, **palette 4×2 без tRNS, palette 3×3 с full tRNS, palette 2×2 с partial tRNS, grayscale-with-PLTE error, missing-PLTE error**, rejects бракованных). Фикстуры сгенерированы Python-zlib скриптом (см. `tests/fixtures/`).
...
8. **`[P2]` Картинки на страницах — продолжение** — крейт `lumen-image` декодирует PNG (8-битные Gray/GrayA/RGB/RGBA + **palette с опц. tRNS**, фильтры 0–4, свой DEFLATE/inflate). Дальнейшие подзадачи: интеграция `<img>`-элемента в HTML/DOM/layout (с учётом intrinsic dimensions и `width`/`height` атрибутов), GPU-загрузка декодированных пикселей в paint (новый pipeline для RGBA-quad с per-image текстурой, не glyph atlas), сетевая подгрузка `<img src="http(s)://...">`, свой JPEG-декодер (отдельный объём — DCT, marker parsing, Huffman, color conversion), 16-bit / 1-2-4-bit palette / Adam7 в PNG.
