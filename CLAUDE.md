...
- **Отложено:** `:has(...)`, `:not(complex)` со списком селекторов или combinator-ами, namespace prefix в селекторах, типизированные значения деклараций (length / color / calc / `--var`), `!important` как отдельное поле.
- 79 тестов.
...
- **Отложено:** flex, grid, float, абсолютное позиционирование, `%` в margin/padding/width/height (нужен containing block), единицы `vh`/`vw`/`vmin`/`vmax`/`ch`/`ex`, color spaces CSS4 (`lab`, `lch`, `oklab`, `oklch`, `color()`), hue в `turn`/`rad`/`grad`, font-weight/style как separate inline-property, селектор-matching с back-tracking.
...
- **Всего тестов в workspace:** 480 (на момент последнего обновления).
