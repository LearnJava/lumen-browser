...
### `lumen-knowledge` 🟡 (FTS5 над историей + заметки)
...
- **Отложено:** §12.2 Range API (для восстановления highlight-наложений при повторном открытии страницы через DOM range coordinates), §12.3 read-later (snapshot HTML + индексация текста), §12.4 поиск по открытым вкладкам (live-индекс без disk-persistence), §12.5 локальный AI (HNSW vector index, опционально), §12.7 export/import session, **русский Porter-stemmer** через external FTS5 tokenizer (C-callback), ё↔е equivalence (custom tokenizer pre-pass), `KnowledgeStore` trait в `lumen-core::ext` для plug-able backends, экспорт заметок в Markdown / JSON.
- 24 unit-теста: 11 HistoryFts (index+search, no-hits, implicit AND, OR query, BM25 ранжирование, cyrillic text, index overwrites, unindex, search limit, clear, case-insensitive) + 13 Notes (add+get, update preserves created_at, delete, list_for_url ASC, recent DESC, search by selection, search by comment, no-match, FTS synced after update, FTS synced after delete, cyrillic, count+clear, get missing).
...
- **Всего тестов в workspace:** 1278 (после merge css-counters).
