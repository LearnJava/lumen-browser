...
| P3-h3 | P3 | | active | | | В работе (P1): срез 1 ✅ 2026-07-04 — QUIC varint (RFC 9000 §16) + HTTP/3 frame codec (RFC 9114 §7.2), pure parse/serialize без IO (crates/network/src/h3/, 17 тестов). Остаётся: QPACK (RFC 9204), QUIC transport (UDP+TLS 1.3+packet protection+loss recovery+congestion control), stream framing, h3_do_request | HTTP/3 |
