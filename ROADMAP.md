...
| P3-spell | P3 | | active | | | PARTIAL 2026-07-03 (смешанное программирование, nemotron-3-ultra, срезы 1-2): lumen_core::spell — pure-Rust движок Hunspell + срез 2: shell::spellcheck (load_dictionaries из data/spell, MultiDictionary, extract_words/misspelled_ranges, build_spell_overlay) + обвязка main.rs (фоновая загрузка словарей при старте, squiggly в paint для фокусного input/textarea, живая приёмка на словарях LibreOffice en_US+ru_RU PASS), 24 теста; дошить — suggestions в контекст-меню, contenteditable | Spell check (Hunspell, русский словарь) |
...
| P3-webvtt | P3 | | active | | | PARTIAL 2026-07-03 (смешанное программирование, step-3.7-flash, срезы 1-3): lumen_dom::vtt — спек-парсер cue + collect_video_tracks + active_cues/strip_cue_markup/resolve_cue_box + срез 3: shell::tracks (load_video_tracks c fetch-замыканием, build_cue_overlay, collect_video_rects) + обвязка main.rs (фетч .vtt при загрузке, cue-оверлей в paint, живая MCP-приёмка PASS), 40 тестов; дошить — TextTrack JS API, playback-клок вместо t=от-навигации | WebVTT субтитры для <video> (<track>) |
