...
| RP-1 | P3 | RP | ready | M | | %-длины width/height/margin/padding резолвятся против containing-block, а не cb_width=0 (тип Length уже умеет, block-поток передаёт 0) | RP-1: Проценты в block-потоке |
| RP-2 | P3 | RP | ready | M | | layout-viewport отслеживает живой inner_size окна; убрать хардкод 1024×720 (обработчик Resized + relayout уже есть) | RP-2: Relayout под живой размер окна |
| RP-3 | P3 | RP | ready | S | | gzip/deflate ContentDecoder + объявить в Accept-Encoding (есть только brotli; flate2 уже в депах) | RP-3: HTTP gzip/deflate декодер |
