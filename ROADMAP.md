...
| P3-webvtt | P3 | | active | | | PARTIAL 2026-07-02 (смешанное программирование, step-3.7-flash): lumen_dom::vtt — спек-парсер cue (заголовок/BOM/timing/settings/NOTE-STYLE-REGION, lenient) + collect_video_tracks(<track> детей <video>), 16 тестов; дошить — фетч src в шелле, TextTrack JS API, рендер активного cue поверх video-бокса | WebVTT субтитры для <video> (<track>) |
