...
| P3-h3 | P3 | | active | | | В работе (P1): срез 1 ✅ 2026-07-04 — QUIC varint (RFC 9000 §16) + HTTP/3 frame codec (RFC 9114 §7.2); срез 2 ✅ 2026-07-04 — QPACK field-section codec (RFC 9204, static table only = поведение при SETTINGS_QPACK_MAX_TABLE_CAPACITY=0; переиспользует HPACK-примитивы); срез 3 ✅ 2026-07-04 — Alt-Svc parser + AltSvcCache (RFC 7838, h3-only, TTL-expiry, детерминируемый по Instant), pure parse без IO (crates/network/src/h3/, 44 теста). Остаётся: QPACK dynamic table + encoder/decoder streams, QUIC transport (UDP+TLS 1.3+packet protection+loss recovery+congestion control), stream framing, h3_do_request | HTTP/3 |
