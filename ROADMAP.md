...
| B-1 | P2 | P2-usability | blocker | XL | BUG-222 | | B-1: QuickJS вне UI-потока (разблокировка) |
| U-1.2 | P2 | P2-usability | wait | L | BUG-171,BUG-172 | | U-1 этап 2: весь пайплайн вне UI-потока |
| U-5 | P2 | P2-usability | ready | S | BUG-167 | | U-5: Fullscreen пересчитывает вьюпорт |
| U-6 | P2 | P2-usability | ready | | BUG-104,BUG-126,BUG-144,BUG-085 | | U-6: рендер-паритет high-deviation |
| OBS | P2 | P2-usability | ready | M | BUG-221 | | Наблюдаемость: CPU-снимок = окно (полная замена gdigrab) |
| U-4opt | P2 | P2-usability | opt | | | | U-4 опции: WASM SIMD/threads, live-aliasing, WebGPU backend |
...
| P2-knowledge | P2 | | active | | | | Knowledge layer ядро (FTS, аннотации, read-later, focus mode) |
| P2-viewport | P2 | | done | | | | <meta viewport> parsing + page zoom |
| P2-ui | P2 | | planned | | | | Кастомизация UI (drag&drop панелей, темы) |
...
| P2-viewtrans | P2 | | planned | | BUG-103 | | View Transitions API |
| P2-masonry | P2 | | planned | | BUG-105,BUG-143 | | CSS Masonry layout |
| P2-scrolldriven | P2 | | planned | | BUG-127 | | CSS Scroll-Driven Animations L1 |
| P2-motionpath | P2 | | planned | | BUG-125 | | CSS Motion Path L1 (offset-path) |
...
| P3-ws | P3 | | planned | | | | WebSockets + SSE + Fetch API + AbortController |
