...
| SDC | P1 | | active | | | Shell-as-driver-client: управление ЖИВЫМ видимым окном Lumen напрямую командами (Selenium/WebDriver-подобно, без headless-прокладки) — закрытие 8A.7 Фазы 4 по `docs/plans/8A.7-shell-as-driver-client-plan.md` + ADR-006. Разрез по владению: SDC-1a (P1 driver) → SDC-1b (P3 shell) → {SDC-2 (P1 фронты), SDC-3 (P3 графтест)}. Цепочка зависимостей строгая | SDC: Управление живым окном (8A.7 Ф4) |
| SDC-1a | P1 | SDC | ready | M | | 8A.7 Ф4 (driver-часть): дореализовать заглушки `click/type_text/scroll/eval/wait` в `crates/driver/src/winit_session.rs:838-915` (сейчас `Err("8A.7"/"8A.8")`) + опубликовать командный интерфейс `AutomationCommand{Navigate,Click,Type,Scroll,Eval,Screenshot,Wait}` (enum/трейт) для шелла. Нативный ввод + auto-wait по ADR-006. Interface-first: shell-проводку делает P3 (SDC-1b) | SDC-1a: WinitSession-команды (driver) |
| SDC-1b | P3 | SDC | wait | M | | 8A.7 Ф4 (shell-часть, ТРЕБУЕТ SDC-1a; `lumen-shell` = территория P3): подключить `AutomationCommand` в живое окно — варианты `LoadEvent` (main.rs:105), обработка в `ApplicationHandler` (main.rs:7410) против активного таба, слушающий поток (`lumen_ipc`/loopback) + CLI-флаг. Ответ наружу через `EventLoopProxy`, ack `Painted` после `RedrawRequested` | SDC-1b: канал команд в живое окно (shell) |
| SDC-2 | P1 | SDC | wait | M | | Протокол-фронты на живое окно через канал SDC-1b (ТРЕБУЕТ SDC-1b). `lumen-bidi-server`: снять in-memory `BidiState` (CAPABILITIES.md:200) → **MVP-команды**: `browsingContext.navigate`/`captureScreenshot`, `script.evaluate`, `input.performActions` против живой сессии; `lumen-mcp` на тот же путь (`screenshot`/`eval` больше не `Err`). По ADR-006 BiDi+MCP, CDP опц./последним | SDC-2: BiDi/MCP на живое окно (MVP) |
| SDC-3 | P3 | SDC | wait | M | | `graphic_tests/run.py` — потребитель канала SDC-1b (ТРЕБУЕТ SDC-1b): один живой экземпляр на весь прогон (запуск + калибровка TEST-00 один раз), цикл Navigate→ждать ack→скриншот→diff. Уровень 2-3 `testing.md` против реального окна вместо process-per-test gdigrab | SDC-3: графтест на одном живом окне |
