...
| SDC-2 | P1 | SDC | ready | M | | Протокол-фронты на живое окно через канал SDC-1b (ТРЕБУЕТ SDC-1b — done). `lumen-bidi-server`: снять in-memory `BidiState` (CAPABILITIES.md:200) → **MVP-команды**: `browsingContext.navigate`/`captureScreenshot`, `script.evaluate`, `input.performActions` против живой сессии; `lumen-mcp` на тот же путь (`screenshot`/`eval` больше не `Err`). По ADR-006 BiDi+MCP, CDP опц./последним | SDC-2: BiDi/MCP на живое окно (MVP) |
| SDC-3 | P3 | SDC | ready | M | | `graphic_tests/run.py` — потребитель канала SDC-1b (ТРЕБУЕТ SDC-1b — done): один живой экземпляр на весь прогон (запуск + калибровка TEST-00 один раз), цикл Navigate→ждать ack→скриншот→diff. Уровень 2-3 `testing.md` против реального окна вместо process-per-test gdigrab | SDC-3: графтест на одном живом окне |
