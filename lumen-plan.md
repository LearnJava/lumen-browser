...
- 🔄 runtime task source priorities [P4] — runtime-task-priorities — 2026-05-14
...
- 🟡 `lumen-shell` — точка входа: три режима (пустое окно / файл / URL). Внешний CSS через `<link rel=stylesheet>`: загружается с диска (относительно HTML-файла) или по сети (относительно базового URL). Bundled Inter-Regular.ttf через `include_bytes!`. **HTML event loop framework + integration в winit-loop** в `lumen-shell::runtime`/`Lumen` — TaskQueue + MicrotaskQueue (drain-all) + EventLoop::step, rAF с cancel, observer registries (Resize/Intersection/Mutation), reentrancy через `Rc<RefCell>` + `EventLoopHandle::clone`; Lumen дёргает `about_to_wait → step()×N` (cap 256), `Resized → deliver_observer_records(Resize)`, `RedrawRequested → run_rendering_step(timestamp_ms)` перед render. 19 unit-тестов runtime. Phase 0 ограничения: reload через queue_task отложен (требует Rc<RefCell<Lumen>>-рефакторинга под JS engine).
