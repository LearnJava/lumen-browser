...
- 🟡 `lumen-knowledge` (§12) — базовая FTS5-таблица `history_fts(url, title, text)` поверх SQLite с tokenizer `unicode61` и bm25-ранжированием готова. API: index/unindex/search(query, limit)/clear. 11 тестов. Отложено: §12.2 заметки, §12.3 read-later, §12.4 поиск по открытым вкладкам, Porter-stemmer для русского.
