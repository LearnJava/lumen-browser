...
- 🔄 css-relative-lengths — css-relative-lengths — 2026-05-12
...
- 🟡 `lumen-layout` — block-flow + **inline-flow** с specificity-based style cascade и line wrapping: compound и complex selectors (combinators, attribute, structural и функциональные pseudo, `:not`), наследование (color, font-size, line-height), color (named + hex 3/4/6/8 digit + rgb/rgba/hsl/hsla с modern syntax), display (block/inline/none), margin/padding (включая shorthand). `TextMeasurer` trait + `layout_measured()` для word-wrap по реальным шрифтовым метрикам. `InlineRun` объединяет текстовые узлы и inline-элементы (`<a>`, `<span>`, `<em>`, `<strong>`, и т.д.) в один поток строк с per-сегментными стилями. `serialize_layout_tree` + golden snapshot-тесты (`UPDATE_SNAPSHOTS=1` для регенерации). 66 тестов. Отложено: flex/grid, float, абсолютное позиционирование, text-decoration, font-weight/style на inline-уровне
