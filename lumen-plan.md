...
- **`[P1]`** `StackingContextId`, `PaintOrder`, `PropertyTrees { transform, scroll, effect, clip }`, типизированные `Length` / `Color` (как уже есть в `lumen-layout::values`, дополнить полями ComputedStyle), trait `AnimationInterpolator` со stub-реализацией.
