...
|---|---|---|---|
| 1A | **`[P1]` Quirks-mode application на hot-paths cascade/layout.** Detection готов (`DocumentMode` enum + `quirks_mode::detect_document_mode`). Осталось: реально читать `Document.mode` и переключать legacy CSS-поведения (table sizing, inline-block alignment, body propagation для background, font-size inheritance в `<table>`). | Половина legacy-сайтов перестаёт рендериться неправильно. | Никого. |
