...
- 🔄 DnsResolver trait + CachedDnsResolver [P3] — dns-resolver-trait — 2026-05-14
...
- 🟡 `lumen-shell` — точка входа: три режима (пустое окно / файл / URL). Внешний CSS через `<link rel=stylesheet>`: загружается с диска (относительно HTML-файла) или по сети (относительно базового URL). Bundled Inter-Regular.ttf через `include_bytes!`. **HTML event loop framework** в `lumen-shell::runtime` — TaskQueue + MicrotaskQueue (drain-all) + EventLoop::step (одна task → microtask checkpoint), `request_animation_frame` / `cancel_animation_frame` / `run_rendering_step` (HTML §8.1.5.1 snapshot-pattern для recursive rAF в next frame), observer registries (Resize/Intersection/Mutation) с snapshot для disconnect-внутри-delivery, reentrancy через `Rc<RefCell>` + `EventLoopHandle::clone`. 19 unit-тестов. Phase 0 framework — integration в winit-loop отдельной задачей.
- 🟡 `lumen-html-parser` — минимальный токенизатор (Data/Tag/Attribute/Comment, **расширенный набор ~250 named entities** через сортированную const-таблицу + numeric, **RAWTEXT для `<script>`/`<style>`**, **RCDATA для `<title>`/`<textarea>`**, **DOCTYPE с PUBLIC/SYSTEM** — public_id/system_id как `Option<String>` чтобы различать missing/empty) + lenient tree builder. **Модуль `quirks_mode::detect_document_mode` (HTML5 §13.2.5.1)** — exact-match public/system IDs + ~55 prefix-правил + HTML 4.01 Frameset/Transitional + XHTML 1.0 правила; tree_builder применяет detection при первом DOCTYPE-токене, при отсутствии DOCTYPE — Quirks (§13.2.6.4.1). **Модуль `srcset` (HTML5 §4.8.4.3.5 + §4.8.4.3.7)** — `parse_srcset` и `pick_best_for_density(dpr)` для `<img srcset>` / `<source srcset>`; density (`Nx`) и width (`Nw`) descriptors, picker по density (w-descriptors требуют `sizes` атрибута — отдельная задача). 150 тестов. Отложено: CDATA, полный набор named entities (~2125 имён в spec), legacy без `;`, insertion modes, application Quirks-mode переключений в layout/cascade, `sizes` атрибут и viewport-based w-descriptor picker
