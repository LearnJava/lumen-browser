...
- 🟡 `lumen-image` — собственный декодер растровой графики. PNG-декодер для 8-битных Gray / GrayAlpha / RGB / RGBA + **palette (color_type 3) с опц. tRNS + 1/2/4-bit grayscale и palette** (sub-byte unpack + scaling по PNG §13.12) без interlacing: свой CRC32 (IEEE 802.3 reflected), chunk reader, IHDR + PLTE + tRNS parsers, DEFLATE/inflate (RFC 1951: stored/fixed/dynamic Huffman, LZ77 окно 32 КБ), zlib-обёртка (RFC 1950 + adler-32), развёртка фильтров скан-линий (None/Sub/Up/Average/Paeth), bit-unpacking MSB-first, grayscale-масштабирование (1-bit×255, 2-bit×85, 4-bit×17), расширение палитровых индексов в Rgb8 / Rgba8. Никаких сторонних crate-ов (см. §5). 72 unit + 19 integration тестов на реальных PNG-фикстурах. Отложено: 16-bit глубина, Adam7 interlacing, JPEG, WebP, AVIF — отдельными задачами.
