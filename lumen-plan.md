...
8. **`[P2]` Картинки на страницах — продолжение** — крейт `lumen-image` декодирует **PNG** (Gray/GrayA/RGB/RGBA при bit_depth ∈ {8, 16}, **16-bit downsample-ится до 8 бит на канал**, palette при bit_depth ∈ {1,2,4,8} с опц. tRNS, sub-byte 1/2/4-bit для grayscale, tRNS для non-palette grayscale/RGB → GrayAlpha8/Rgba8 с бинарным match-ом, **Adam7-interlacing для всех color types**, фильтры 0–4, свой DEFLATE/inflate) и **JPEG baseline + progressive** (SOF0/SOF2, 8-bit, YCbCr 1/3 компонент, chroma subsampling 4:4:4 / 4:2:2 / 4:2:0, restart intervals, multi-scan progressive с DC/AC initial+refinement по §G; свой marker parser + canonical Huffman + IDCT + YCbCr→BT.601 RGB). **`lumen_image::decode(bytes)` диспатч по сигнатуре** между PNG и JPEG; `ImageError` объединяет ошибки декодеров и UnknownFormat. **`<img>` интегрирован в layout + paint + shell pipeline:** `BoxKind::Image { src, alt }`, HTML presentational hints (width/height атрибуты → CSS свойства, перекрываемые author-CSS), `DisplayCommand::DrawImage` в display list, renderer **загружает декодированные пиксели в GPU** через `Renderer::register_image` и рисует через image-pipeline (Rgba8Unorm texture, per-image bind group, alpha-blending). Shell обходит DOM перед layout, fetch + decode `<img src>` с диска/сети, заполняет **intrinsic dimensions** из декодированного изображения если author не задал width/height; ключ register_image = raw src совпадает с `DisplayCommand::DrawImage.src`. Дальнейшие подзадачи: `object-fit` / `object-position` для fitting картинки в коробку, inline-replaced (`<img>` внутри `InlineRun`-а с текстом — требует переделки фрагментов), WebP / AVIF, реальный sRGB→linear color management в pipeline.
...
- **`[P1+P4]` Preload scanner.** 🟡 **P1-часть готова** — `lumen_html_parser::preload_scanner::scan_preload_hints(&str) -> Vec<PreloadHint>` распознаёт `<link rel=stylesheet|preload|preconnect|dns-prefetch>`, `<script src>`, `<img src|srcset|sizes>`, `<source srcset|sizes>`; 27 unit-тестов. Осталось: **P4** — интеграция в shell-pipeline (когда запускать over chunks, как пробрасывать в HttpClient до того, как main parser догнал hint). Особенно полезно над streaming pipeline (см. roadmap «Ближайшее» п.3).
...
- **`[P3]` Safe Browsing — готово.** `lumen-storage::safe_browsing::SafeBrowsingList` (SQLite) + `SafeBrowsingFilter` (реализует `RequestFilter`, подключается в `HttpClient::with_filter`). Полные SHA-256 хэши, не 4-byte prefixes; canonicalize URL по упрощённому Safe Browsing v4 §4.4 + до 20 host-suffix×path-trim вариантов на запрос; ThreatType { Malware / SocialEngineering / UnwantedSoftware / PotentiallyHarmful / Other(_) } для forward-compat импорта чужих списков. Без облачного API — соответствует принципу №1 «приватность». Отложено: 4-byte prefixes с full-hash callback, public-suffix list для безопасной обрезки host-suffixes ниже eTLD+1, percent-encoding нормализация против `%2E%2E` обхода, фоновое обновление списков (URL feed + dedup).
