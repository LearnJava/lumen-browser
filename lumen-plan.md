...
- 🔄 preload-scanner: tokenizer-уровневый сборщик subresource-URL [P1] — preload-scanner — 2026-05-14
...
15. **`[P3]` DoT resolver** в network-слое. **DoH готов:** `lumen_network::DohResolver` — RFC 8484 GET с `?dns=<base64url>`, собственный wire-format DNS message encoder/decoder (RFC 1035 §4) с поддержкой compression pointers, base64url без padding, AAAA+A с dual-stack preference (RFC 6724 §6), IP-литералы bypass. Подключается через `HttpClient::with_dns_resolver`. Осталось: `DotResolver` (DNS-over-TLS, RFC 7858) — собственный TLS-сокет без HTTP, reuse wire-format encoder/decoder из `lumen_network::doh` (сейчас `pub(crate)`, при работе над DoT перевести в `pub`).
