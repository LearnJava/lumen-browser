...
- **`[P1+P2]` `<picture>` / `srcset` / `sizes` + `loading="lazy"`.** Viewport+DPR-aware resource selection до загрузки (часть preload scanner-а); lazy loading через IntersectionObserver. Отдельно от полного `<video>` / `<audio>` pipeline-а — это базовый image-respond-to-viewport, нужный на любом адаптивном сайте. **P1**: 🟡 готовы `srcset` parser + density-picker, `sizes` attribute + width-picker, `<picture>`/`<source>` selection через `pick_picture_source` (`lumen_html_parser::picture`), ведущий `not` + `prefers-color-scheme` в media-condition — см. [SUBSYSTEMS.md](SUBSYSTEMS.md). Осталось: IntersectionObserver event source для lazy, L4 nested `(not (...))`-формы в media-condition, intrinsic dimensions из `<source>` для CLS. **P2** — image-side GPU upload + intrinsic dimensions + integration в shell.
