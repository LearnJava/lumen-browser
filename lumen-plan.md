...
Над проектом параллельно работают **4 программиста** (P1–P4). Раскрой задач по программистам и доменные зоны — в `CLAUDE.md`, раздел «Распределение задач между программистами». Если в сессии тебе сказали «ты программист N» — твои задачи помечены `[PN]` в разделе «Roadmap — приоритизация задач» этого файла.
...
- ✅ `HttpCredentialProvider` — trait `credentials(&HttpAuthChallenge) -> Option<HttpCredentials>` для HTTP Basic + Digest по `(origin, realm, scheme)`. Реализация `lumen-network::StaticCredentialProvider` (in-memory, fallback-chain) для тестов / curl-style; потребитель `HttpClient::with_credentials(...)` — RFC 7617 Basic + RFC 7616 Digest (MD5 / SHA-256 / qop=auth + legacy). UI-popup и keyring — следующие задачи P4 / платформенного слоя
...
6. **`[P1]` CSS — типизированные значения деклараций** — length / color / calc. Селекторы Level 3 готовы полностью. **CSS Variables (`--name` + `var()`), `calc()`, `min()`/`max()`/`clamp()`, scientific math (`sin/cos/tan/asin/acos/atan/atan2/pow/sqrt/exp/log/hypot/abs/sign/mod/rem/round`), CSS-wide keywords (inherit / initial / unset / revert), @property syntax-валидация (`<length>`, `<percentage>`, `<color>`, `<integer>`, `<number>`, `<angle>`, `<length-percentage>`, `<custom-ident>`, union через `|`) — реализованы** в `lumen-layout` (см. [SUBSYSTEMS.md](SUBSYSTEMS.md)). Осталось: типизированный `Length` для всех свойств (сейчас Length типизирован для большинства, но в каскаде значения хранятся строкой и парсятся в момент apply), типизированные `Color`-значения у всех свойств.
7. **`[P3]` Tab session export / import** (§12.7) — сериализация в snapshot-формат lumen-storage. Простое, экономит много боли.
8. **`[P2]` Картинки на страницах — продолжение** — крейт `lumen-image` декодирует **PNG** (Gray/GrayA/RGB/RGBA при bit_depth ∈ {8, 16}, **16-bit downsample-ится до 8 бит на канал**, palette при bit_depth ∈ {1,2,4,8} с опц. tRNS, sub-byte 1/2/4-bit для grayscale, tRNS для non-palette grayscale/RGB → GrayAlpha8/Rgba8 с бинарным match-ом, **Adam7-interlacing для всех color types**, фильтры 0–4, свой DEFLATE/inflate) и **JPEG baseline** (SOF0, 8-bit, YCbCr 1/3 компонент, chroma subsampling 4:4:4 / 4:2:2 / 4:2:0, restart intervals; свой marker parser + canonical Huffman + IDCT + YCbCr→BT.601 RGB). **`lumen_image::decode(bytes)` диспатч по сигнатуре** между PNG и JPEG; `ImageError` объединяет ошибки декодеров и UnknownFormat. **`<img>` интегрирован в layout + paint + shell pipeline:** `BoxKind::Image { src, alt }`, HTML presentational hints (width/height атрибуты → CSS свойства, перекрываемые author-CSS), `DisplayCommand::DrawImage` в display list, renderer **загружает декодированные пиксели в GPU** через `Renderer::register_image` и рисует через image-pipeline (Rgba8Unorm texture, per-image bind group, alpha-blending). Shell обходит DOM перед layout, fetch + decode `<img src>` с диска/сети, заполняет **intrinsic dimensions** из декодированного изображения если author не задал width/height; ключ register_image = raw src совпадает с `DisplayCommand::DrawImage.src`. Дальнейшие подзадачи: `object-fit` / `object-position` для fitting картинки в коробку, inline-replaced (`<img>` внутри `InlineRun`-а с текстом — требует переделки фрагментов), progressive JPEG (SOF2), WebP / AVIF, реальный sRGB→linear color management в pipeline.
...
15. **DoH + DoT — готовы.** `lumen_network::DohResolver` (RFC 8484): GET с `?dns=<base64url>`, собственный wire-format DNS message encoder/decoder (RFC 1035 §4) с поддержкой compression pointers, base64url без padding, AAAA+A с dual-stack preference (RFC 6724 §6), IP-литералы bypass. `lumen_network::DotResolver` (RFC 7858): собственный TCP+TLS-сокет (rustls), без HTTP-обёртки, тот же wire-format переиспользован (`encode_query`/`decode_answer_ips` теперь `pub`); TCP framing (RFC 1035 §4.2.2 `[u16 BE length][message]`), bootstrap через `new(server_name, server_addr)` или фабрики `cloudflare()`/`google()`/`quad9()` с hardcoded IP. Оба подключаются через `HttpClient::with_dns_resolver`.
...
### Browser fundamentals (аудит на основе Chromium / Firefox / Servo / Ladybird)

Список упущений в roadmap-е, обнаруженных при сравнении с реальными движками 2026-05-14. Каждый пункт **критичен для функциональности** при достижении соответствующей фазы — без них браузер не работает как браузер, а не Phase-0-демо. Маркер `[PN]` — основной программист; в большинстве случаев пункт затрагивает несколько доменов, и в скобках пометка о cross-team нужна.

#### Phase 1 (Reader) — добавить к существующему scope

- **`[P4]` HTML event loop + microtasks + rendering steps + observers.** 🟡 **Framework + integration в winit-loop + task source priorities + requestIdleCallback готовы** (`lumen-shell::runtime` + Lumen-handlers — см. [SUBSYSTEMS.md](SUBSYSTEMS.md) → `lumen-shell`). about_to_wait → step()×N (cap 256), Resized → deliver_observer_records(Resize), RedrawRequested → run_rendering_step(timestamp_ms) перед render(); `TaskQueue` обходит `TaskSource::PRIORITY_ORDER` на pop; `run_idle_callbacks(remaining_ms, now_ms)` доступен для шедулинга idle-работы. **Осталось:** reload через queue_task (требует Rc<RefCell<Lumen>>), интеграция `run_idle_callbacks` в Lumen-loop (в about_to_wait после step → Idle), правильный ordering rendering steps stage (style → layout → paint вместе с layout invalidation), `scheduler.postTask`, PerformanceObserver, реальные triggers observers (mutation/intersection events). Framework полезен JS-engine-у в будущем: JS-функция обернётся в Rust closure и попадёт в ту же очередь, без переписывания.
- **`[P1+P2]` Stacking contexts + правильный CSS Painting Order** (CSS 2.1 Appendix E). 7-уровневый порядок paint (background → border → block descendants → floats → non-positioned inline → positioned по z-index, рекурсивно). Stacking context создаётся: `opacity<1`, `transform != none`, `filter`, `clip-path`, `mask`, `isolation: isolate`, `mix-blend-mode != normal`, `will-change` для любого из этих, `position+z-index`, flex/grid item с z-index. Сейчас paint в порядке DOM-обхода — z-index работает случайно. **P1** — модель stacking-ов и order computation в layout; **P2** — paint-side traversal в правильном порядке.
- **`[P2+P1]` Compositor thread + property trees.** Отдельные TransformTree / ScrollTree / EffectTree / ClipTree, копируются на compositor thread. Двухбуферная commit-модель (pending tree / active tree, атомарный swap). Off-main-thread scroll: если JS висит — страница всё ещё скроллится. В плане `compositor` упомянут как крейт, но без архитектурного плана. **P2** — compositor pipeline / GPU primitives / layer tree (paint runtime); **P1** — построение property trees из style/layout.
- **`[P2]` Stacking-aware hit testing.** Отдельная структура (O(log n) lookup), обновляется на composite commit; учитывает z-index, `pointer-events: none`, transform inversion. Текущий обход layout-дерева работает на demo, не на странице с 10k нод и hover-эффектами. Привязано к compositor layer tree — поэтому owner тот же.
- **`[P1]` Quirks mode vs standards mode — application в layout/cascade.** Detection реализован (`DocumentMode` enum + `quirks_mode::detect_document_mode` по HTML5 §13.2.5.1, см. `lumen-dom` / `lumen-html-parser`). Осталось: реально читать `Document.mode` на hot-paths layout/cascade и переключать десятки legacy CSS-поведений (table sizing, inline-block alignment, body propagation для background, font-size inheritance в `<table>`, и т.д.). Без application половина legacy-страниц всё ещё рендерится как standards.
- **`[P3]` Same-Origin Policy enforcement + CORS preflight.** SOP checks при fetch / postMessage / storage / cookies; CORS OPTIONS preflight для non-simple requests; credentials mode (omit / same-origin / include). У нас есть CSP / HSTS / SRI как **storage**, но без enforcement-а — браузер дырявый, нельзя пускать в сеть untrusted content.
- **`[P3]` Mixed-content blocking + `<iframe sandbox>`.** HTTPS-страница не грузит HTTP-script / css / iframe (blockable); images / video — upgradable. `<iframe sandbox>` flags (allow-scripts / forms / same-origin / popups / top-navigation). Базовая security-модель для встраивания контента.
- **`[P1+P4]` Preload scanner.** Отдельный быстрый pre-parser, бежит впереди main parser-а по uncovered HTML chunks и стартует fetch для `<link rel=preload>` / `<script src>` / `<img src>` до DOM construction. Critical для perceived performance, особенно над streaming pipeline (см. п.3 выше). **P1** — отдельный быстрый mode tokenizer-а, **P4** — shell-уровень оркестрация (когда запускать, куда отдавать chunks).

#### Phase 2 (Interactive) — без этого современный веб не функционален

- **`[P1+P4]` Shadow DOM + custom elements + `<template>` + `<slot>`.** Отдельные shadow trees, изоляция стилей (правила document не пересекают shadow boundary, `::part` / `::slotted` / inheritance исключения), composed tree для cascade и event propagation. Custom elements registry, lifecycle callbacks (`connectedCallback` / `attributeChangedCallback` / `adoptedCallback`). Declarative Shadow DOM через `<template shadowrootmode>` — без JS. Web Components в плане отсутствуют — без них половина современных сайтов сломается. **P1** — Shadow DOM cascade + composed tree + tree-builder для template/slot; **P4** — JS bindings (`Element.attachShadow`, `customElements.define`) + lifecycle dispatch.
- **`[P1+P4]` Accessibility tree + platform bridges.** Параллельное layout-дереву, ARIA roles + computed name/description, обновляется на DOM/style mutations. Platform-specific: на Linux — D-Bus сервер для AT-SPI; на Windows — UIA COM; на macOS — NSAccessibility. Принцип «русский first-class» обязывает поддержку NVDA / Orca / VoiceOver. Focus management (`:focus-visible`, focus trap, autofocus, tabindex), reduced motion / high contrast / forced-colors preferences. **P1** — построение accessibility tree из DOM/layout + ARIA semantics + focus model; **P4** — platform bridges (UIA / AT-SPI / NSAccessibility) + focus dispatch в shell.
- **`[P1+P4]` Forms runtime.** Constraint Validation API (`:valid` / `:invalid` / `:user-invalid` / `:placeholder-shown` / `ValidityState`), submission algorithm (HTML5 §4.10.21 — entry list construction, encoding selection через уже готовые form encoders), native file picker через OS dialog с `accept` / `multiple` / `webkitdirectory`, `<datalist>` combobox, native `<select>` dropdown, color / date / range picker UI, autofill popup поверх существующего autofill storage по `autocomplete` атрибуту. **P1** — `ValidityState`, validation pseudo-classes, submission algorithm; **P4** — native pickers, autofill popup, validation tooltip UI.
- **`[P1+P2]` `<picture>` / `srcset` / `sizes` + `loading="lazy"`.** Viewport+DPR-aware resource selection до загрузки (часть preload scanner-а); lazy loading через IntersectionObserver. Отдельно от полного `<video>` / `<audio>` pipeline-а — это базовый image-respond-to-viewport, нужный на любом адаптивном сайте. **P1**: 🟡 готовы `srcset` parser + density-picker, `sizes` attribute + width-picker, `<picture>`/`<source>` selection через `pick_picture_source` (`lumen_html_parser::picture`), ведущий `not` + `prefers-color-scheme` в media-condition, intrinsic dimensions у `<source>`/`<img>` для CLS — см. [SUBSYSTEMS.md](SUBSYSTEMS.md). Осталось: IntersectionObserver event source для lazy, L4 nested `(not (...))`-формы в media-condition. **P2** — image-side GPU upload + integration в shell (PickedSource intrinsic-поля доступны на picker-стороне).
- **`[P4]` IME composition events** (`compositionstart` / `update` / `end`, `KeyboardEvent.isComposing`). Без этого ввод японского / китайского / корейского сломан полностью. Интегрируется через winit IME API + DOM events; затрагивает `<input>`, `<textarea>`, `contenteditable`.
- **`[P3]` Brotli + Range requests.** Brotli decoder (60-70% веба сжатие) — свой код по политике (compression engine). Range — для `<video>` seek, PDF page-load, resume downloads. **HTTP/1.1 keep-alive + connection pool** реализован отдельно (см. `lumen-network` в [SUBSYSTEMS.md](SUBSYSTEMS.md) / `git log --oneline | grep http-keepalive`).
- **`[P4]` Find in page (Ctrl+F).** Поиск по rendered text (с учётом `display:none`, generated content), highlight совпадений, scroll-to-match. Обход layout-дерева, не DOM-а. Минимум для browser-as-tool.
- **`[P4]` DevTools / Inspector минимум.** DOM tree view + computed styles panel + network log с фильтрами. Без этого debug собственного движка невозможен. Стандарт — Chrome DevTools Protocol (CDP), реализуется как WebSocket-сервер; даёт автоматически совместимость с внешними CDP-клиентами (puppeteer, playwright, остальные tools).
- **`[P1+P2]` `mix-blend-mode` / `background-blend-mode` / `isolation` / `backdrop-filter`.** Сейчас parsing-а нет. Нужны isolation groups в compositor pipeline; 16 blend modes (multiply/screen/overlay/darken/.../luminosity); `backdrop-filter` — отдельный pass blur на снапшоте под элементом.

#### Phase 3+ — без этого браузер не полнофункциональный

- **`[P3]` WebSockets (RFC 6455) + Server-Sent Events + Fetch API runtime.** WS: HTTP upgrade, frame-based binary protocol, ping/pong, permessage-deflate. SSE: `text/event-stream` + auto-reconnect. Fetch: Request / Response / Headers objects, ReadableStream body, AbortController.
- **`[P3]` HTTP auth — Basic + Digest готовы.** `lumen-network::HttpClient::with_credentials(Arc<dyn HttpCredentialProvider>)` поверх trait-точки в `lumen-core::ext`. На 401 + `WWW-Authenticate` клиент парсит challenge-list (RFC 7235 §2.1), выбирает strongest (Digest+SHA-256 > Digest+MD5 > Basic), опрашивает провайдер по `(origin, realm, scheme)`, собирает `Authorization` header (RFC 7617 для Basic, RFC 7616 для Digest с MD5/SHA-256 + qop=auth + legacy RFC 2069). MD5 и SHA-256 — собственные реализации (RFC 1321 / FIPS 180-4); rustls/ring оставлены TLS-у. `StaticCredentialProvider` — in-memory backend для тестов и `curl -u`. Один retry на hop, Authorization не переносится через 3xx (RFC 7235 §3.1). **Осталось:** Negotiate/NTLM для корп. AD (потребует platform GSSAPI/SSPI или Kerberos-стек — отдельный exception?), client certificates mTLS (rustls support есть, нужен UI / config), UI-popup для credentials.
- **`[P3]` OCSP stapling + CT log enforcement + invalid cert UI.** Современный TLS стандарт; rustls умеет, нужно включить + правильный UI для invalid certs (NET::ERR_CERT_*).
- **`[P3]` Safe Browsing equivalent.** Hash-prefix lookup malware / phishing URL до навигации. Можно через локальный фильтр-список (как uBlock infrastructure), без облачного API — соответствует принципу приватности.
- **`[P4]` Back/forward cache (bfcache).** Снапшот DOM+JS heap для мгновенного back. Eligibility rules (no `unload` handlers, no open IndexedDB tx, и десятки других).
- **`[P4]` Navigation API + History API runtime.** `history.pushState` / `popstate`, `navigate` event. Storage есть, runtime событий — нет.
- **`[P1+P2+P4]` Web Animations API runtime** поверх parsed `@keyframes` / transitions. Compositor-driven для transform/opacity (главный win — не блокирует main thread), main-thread для color / layout-properties. **P1** — интерполяция computed value в момент времени `t` (по cubic-bezier / steps timing function), invalidation animated-свойств; **P2** — compositor offload для transform / opacity (без main thread на frame); **P4** — animation timeline scheduling в rendering steps stage.
- **`[P1+P4]` `<contenteditable>` + Input Events Level 2 + Selection / Range API.** Самая сложная часть DOM editing; даже Chrome / Firefox страдают. Минимум для notes UI и любых in-browser editor-ов. **P1** — DOM mutations модель + Selection / Range типы + `beforeinput` / `input` event firing; **P4** — input dispatch (key + IME + drag-drop + paste), undo/redo stack в shell.
- **`[P3+P4]` Service Worker runtime.** Fetch interception, push delivery, background sync, cache strategies (network-first / cache-first / stale-while-revalidate). Registry есть в storage, runtime — нет; SW требует отдельного worker-context в JS engine. **P3** — fetch interception API (`HttpClient` hook поверх существующего RequestFilter pattern), push delivery, background sync queue в storage; **P4** — отдельный JS worker context + lifecycle + `clients` API.
- **`[P3+P4]` Spell check** через Hunspell-совместимые словари + OS API (Spellcheck на Windows, NSSpellChecker на macOS). Squiggly underline в render, context menu suggestions. Русский словарь — часть «русский first-class». **P3** — словарь storage / loader / Hunspell-формат парсер; **P4** — squiggly render в shell, context menu suggestions UI, OS API integration (платформенный).
- **`[P2]` Variable fonts axes runtime.** `font-variation-settings`, interpolation по wght / wdth / slnt axes. font-stretch уже parsed, но рендерер всегда static. Требует поддержки fvar / gvar / avar tables в `lumen-font` (уже отмечено как отложено).
- **`[P2]` Color management + Display P3 / Rec2020 / ICC profiles.** Для `<canvas>` / `<img>` / CSS `color()` функций (CSS Color L4). Без этого wide-gamut контент рендерится неверно — фотографии с P3-профилем выглядят как sRGB.
- **`[P1+P2+P4]` Print pipeline runtime.** `@page` rules parsed, pagination algorithm (`break-before` / `-after` / `-inside` parsed, не применяется), page numbering, headers / footers, PDF generation (display list → PDF writer; PDF writer — отдельный задел или 6-й exception).
- **`[P1+P4]` GC integration JS ↔ DOM (cycle collector).** DOM nodes держат JS wrappers, JS wrappers держат DOM-узлы. Cycle collector (как у Gecko / Blink) нужен — иначе утечки на каждом cross-reference. Существенно повлияет на API между Rust DOM и JS engine; решение «как именно» — отдельная архитектурная задача при интеграции QuickJS / V8. **P1** — DOM-side wrapper hooks + lifecycle для трекинга cross-references; **P4** — JS engine integration + cycle collector algorithm + рабочий API на стыке.
- **`[P4]` Permission prompt UI + Download UI.** Storage есть; нужны модальные / shelf-popup-ы для `navigator.geolocation`, notification grant, file download progress / cancel / open folder. MIME-based handler routing (PDF в встроенный viewer; .exe — confirm-before-save).
- **`[P4]` GPU process / sandbox.** Реальный browser-grade sandbox: seccomp (Linux), AppContainer (Windows), App Sandbox (macOS), GPU процесс отдельно от renderer-а. Часть «site isolation» из существующего roadmap-а, но более широкая — sandbox активируется на каждом процессе.
