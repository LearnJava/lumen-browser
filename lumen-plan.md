...
| 4A | **`[P1+P2]` `<picture>` / `srcset` / `sizes` finishing.** 🟡 Готовы parser + pickers + `pick_picture_source` + **L4 nested `(not (...))` / `((...))` в media-condition** (ветка `media-query-nested-not`, `MediaClause::Nested` + paren-aware split) + **layout-side integration** (ветка `picture-srcset-integration`): `lumen-layout::box_tree::resolve_image_source(doc, img_id, viewport)` вызывает `pick_picture_source` (если parent — `<picture>`) или `pick_img_source` (одиночный `<img srcset>`) с DPR=1.0 (Phase 0; renderer.scale_factor пока не пробрасывается в layout — отдельный шаг при relayout-on-resize) и intrinsic-dims fallback на голый `src` для битой разметки. Intrinsic dimensions у выбранного `<source>` (width/height атрибуты) применяются как presentational hint: заполняют пустые `style.width`/`style.height` после CSS-каскада, author CSS поверх — выигрывает. `<source>` и `<track>` переведены в `Display::None` (default UA stylesheet), чтобы не порождать пустых boxes. 9 новых unit-тестов в `lumen-layout`. Осталось: IntersectionObserver event source для `loading="lazy"`. | P3 lazy-loading в shell. | Никого — P2 GPU upload не зависит от lazy. |
...
| 2A | 🟡 **`[P3]` SOP / CORS preflight + mixed-content blocking + `<iframe sandbox>` flags enforcement.** CSP/HSTS/SRI уже parsed — подключаем к fetch path. **Security base реализован в ветке `network-security-base`:** `Origin` tuple (HTML LS §7.5), `classify_subresource_request` (W3C Mixed Content + Fetch §3.2.7 destinations), `SandboxFlags` u32-bitset + `parse_sandbox_value` (HTML LS §7.6.5, все 14 keyword-ов) — все три как pure-classifiers без enforcement в HttpClient. **Mixed-content enforcement реализован в ветке `mixed-content-enforcement`:** `MixedContentPolicy { top_level, mode: Disabled|SpecDefault|Strict }` + builder `HttpClient::with_mixed_content_policy` + `fetch_subresource(url, destination)`; classify в `fetch_with_redirect` ПОСЛЕ HSTS upgrade и ДО RequestFilter/RequestStarted, эмит `RequestBlocked { reason: "mixed-content: blockable|optionally-blockable" }`, per redirect-hop. `fetch` (top-level navigation) намеренно policy не enforces. **CORS preflight classifier+cache реализован в ветке `cors-preflight`:** `lumen-network::cors` — pure-логика. **CORS preflight enforcement в HttpClient реализован в ветке `cors-preflight-enforcement`:** `HttpClient::with_cors_cache(Arc<PreflightCache>)` + `fetch_cors(CorsRequest, Option<RequestDestination>)`. `CorsContext` per-hop в `fetch_with_redirect`: cache.allows shortcut → OPTIONS preflight через тот же `fetch_single` (с pre-formatted Origin/ACRM/ACRH extra-headers) если miss и `needs_preflight` → `evaluate_preflight_response` → cache.insert на success / `RequestBlocked { reason: "cors-preflight: <CorsError>" }` на fail. Actual cross-origin запрос с method=cors.method и extra-headers `Origin: <serialize>` + author-headers (с фильтрацией дубликатов наших Host/Connection/UA/Accept/Accept-Encoding/Authorization/Range/If-Range/Content-Length/Origin). Actual-response validation: `check_cors_response_headers` per cross-origin hop ДО status-branching; fail → `RequestBlocked { reason: "cors-response: <CorsError>" }`. Redirect re-classify под `Origin::from_url(next)` с тем же `cors_ctx`. **Phase 0 ограничения**: HttpClient GET-only (POST/PUT/PATCH без body — body pipeline отдельная задача); cookie-jar не интегрирован (credentials_mode влияет только на ACAO=`*` rejection и ACAC=`true` requirement); forbidden request-headers caller отфильтровывает заранее. Остаётся: **sandbox-application в DOM-загрузчике shell-я**. | Без enforcement браузер нельзя выпускать в публичную сеть. | Только `lumen-network` + shell. |
...
| 3A | 🟡 **`[P3]` DPR + scroll в shell**. **DPR/scale_factor реализован в ветке `shell-dpr-support`**: `Renderer.scale_factor: f64` + `set_scale_factor` для `ScaleFactorChanged`, viewport uniform делится на DPR, 1 CSS px = `scale_factor` device px. **Scroll-state реализован в ветке `shell-scroll-state`**: `Lumen { scroll_y, content_height }` + `Renderer::render(content, overlay, scroll_y)` — page-полоса смещается на `-scroll_y`, overlay (find-bar) остаётся viewport-locked. MouseWheel (LineDelta 40 px / PixelDelta /DPR), стрелки ↑/↓ (40 px, auto-repeat), PageDown/PageUp/Space (90% viewport), Home/End. Кламп `[0, max(0, content_height − vh)]`, reload сбрасывает scroll. **Scroll-to-match для find реализован в ветке `find-scroll-to-match`**: `find::scroll_to_match(match_rect, viewport_height, current_scroll) -> Option<f32>` (pure-fn, верхняя четверть viewport-а, `None` если уже виден) + `Lumen::scroll_to_active_match` дёргается после next/prev/backspace/text-input; clamp в `[0, max_scroll]` оставлен caller-у. **Vertical scrollbar overlay реализован в ветке `scrollbar-overlay`**: `lumen-shell::scrollbar` — `build_scrollbar_overlay(scroll_y, content_height, vw, vh)` + `thumb_geometry(...)` pure-fn в Y-axis, 8 px ширина у правого края, MIN_THUMB_HEIGHT=24, скрыт при `content_height <= viewport_height`. Wiring в RedrawRequested перед find-bar в overlay-буфере. 12 unit-тестов. **Drag-to-scroll + track-click page-jump реализованы в ветках `scrollbar-drag` и `scrollbar-track-click`**: `scrollbar::classify_track_click(x, y, scroll_y, ch, vw, vh) -> TrackClick { None|Thumb|Above|Below }` — единая точка решения для MouseDown в Lumen. Thumb → старт `ScrollDrag` (`start_scroll_y`, `start_mouse_y` снапшот) + `scroll_for(current_y, ch, vh) = start + Δy × max_scroll / (vh − thumb_h)` (без clamp, caller клампит через `scroll_to`-обёртку — закреплённый-под-пальцем thumb). Above → `scroll_by(-page_step(vh))` (та же 90%-viewport-формула, что у клавиши PageUp); Below → `+page_step(vh)` (PageDown). None → клик мимо. Lumen хранит `cursor_position` (winit physical px → CSS px через DPR) + `scroll_drag: Option<ScrollDrag>`; CursorMoved во время drag обновляет scroll, Released / reload сбрасывают. 17 unit-тестов на интерактивность: 9 на classify_track_click + 8 на ScrollDrag::scroll_for. **Smooth-scroll реализован в ветке `smooth-scroll`**: модуль `lumen-shell::scroll_anim` — `ScrollAnim { start_y, target_y, start_time_ms }` + `ease_out_cubic(t)` + `sample(now_ms) -> (f32, done: bool)`. `Lumen.scroll_anim: Option<ScrollAnim>`; `start_smooth_scroll(target)` cancel-ит активную и стартует новую (clamped, no-op если target ≈ scroll_y), `scroll_by_smooth(delta)` аддитивен поверх target-а активной анимации (вместо текущего scroll_y) — repeat-input «дописывает» delta к точке назначения. Wheel / arrow / page / Home/End / track-click-page-jump / find-scroll-to-match подключены к smooth-вариантам; drag thumb — инстант (через `scroll_to`). `RedrawRequested` тикает `advance_scroll_anim()` → `request_redraw()` пока не завершилась. Reload и page-load сбрасывают anim. 12 unit-тестов (easing + ScrollAnim::sample, включая backwards). Осталось: relayout-on-resize (передавать реальный `inner_size` в layout вместо hardcoded 1024×720 — требует переноса layout-вызова в `Lumen::resumed()` после создания окна) + горизонтальный scroll + momentum (free-flick на trackpad) + cursor-icon feedback на hover. | Реальные страницы на 4K выглядят корректно (DPR ✅); длинные страницы пролистываются (scroll ✅); Ctrl+F прыгает к match ✅; scrollbar viewport-locked + drag ✅. Layout-viewport — отдельным шагом. | Только `lumen-shell` + узкая правка `lumen-paint::renderer`. |
| 3B | **`[P3]` HTML event loop integration в Lumen-loop** (правильный ordering rendering steps stage `style → layout → paint`, `scheduler.postTask`, реальные triggers observers, reload через queue_task с Rc<RefCell<Lumen>>). 🟡 Framework + winit-integration + task source priorities + requestIdleCallback + `run_idle_callbacks` в about_to_wait готовы. | P1 / P2 видят правильный rAF tick — им ничего делать не надо. | Только `lumen-shell::runtime`. |
...
| 5B | ✅ **`[P3]` HTTP Range requests + `<video>` seek**. RFC 7233 single-range (closed / open-ended / suffix `bytes=-N`) + If-Range реализованы (`fetch_range(url, RangeSpec, Option<RangeValidator>)`). **Multi-range / multipart/byteranges** реализован в ветке `http-multi-range`: `fetch_multi_range(url, &[RangeSpec], Option<RangeValidator>)` + `RangeRequest::{Single,Multi}` (internal type) + `parse_multipart_byteranges` + `parse_boundary_from_content_type` + типы `MultiRangeResponse`/`RangePart`. Нормализует все три формы ответа (200 fallback / 206 single-Content-Range / 206 multipart) в единый `Vec<RangePart>`. Невалидные spec-ы внутри Multi молча отбрасываются. Network-layer готов, осталась shell-интеграция (resume downloads + `<video>` seek по таблице сегментов — отдельные задачи). | `<video>` seek готов к подключению, PDF page-load, resume downloads. | Только `lumen-network` — уже сделано. |
