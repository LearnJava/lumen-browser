...
15. **`[P3]` DoH / DoT resolver** в network-слое. **Инфраструктура готова:** `DnsResolver` trait в `lumen-core::ext` + `SystemDnsResolver` (default) + `CachedDnsResolver` (lumen-storage, TTL-кеш поверх `DnsCache`) — `git log --oneline | grep dns-resolver-trait`. Осталось: реализации `DohResolver` (HTTPS POST к RFC 8484 endpoint с `application/dns-message` payload, реиспользует `HttpClient`) и `DotResolver` (TLS-wrapped DNS protocol). Подключаются через тот же `HttpClient::with_dns_resolver`.
