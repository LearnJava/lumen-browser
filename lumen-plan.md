...
- 🟡 Provisional (1 подключено: `brotli-decompressor` в `lumen-network` через `BrotliContentDecoder` за `ContentDecoder`; HttpClient теперь шлёт `Accept-Encoding: br` и декодирует `Content-Encoding: br` ответы). Ожидают подключения: image decoders (JPEG/WebP/GIF), `icu4x`, `ruzstd`, `idna`, `publicsuffix`, `hyphenation`, `woff2`, `hunspell-rs`/`spellbook`, `quinn`. Каждый — за trait в `lumen-core::ext`, подключается по мере того, как фаза реально упирается в задачу. Полная таблица + graduation criteria — в §5.
...
| 2A | **`[P1+P2]` Stacking contexts impl** в layout (наполнить `StackingContextId` от Sprint 0 реальной логикой). Stacking context создают: `opacity<1`, `transform != none`, `filter`, `clip-path`, `mask`, `isolation: isolate`, `mix-blend-mode != normal`, `will-change`, `position+z-index`, flex/grid item с z-index. | P2 п.2A (painting order traversal) — drop-in переход с пустого stub-а на реальные данные. | P2 уже пишет painting order против stub. |
| 2B | **`[P2+P1]` Property trees построение** (TransformTree / ScrollTree / EffectTree / ClipTree из style+layout). | P2 п.1B (compositor commit). | P2 уже пишет compositor против пустых trees. |
| 3A | **`[P1+P2+P3]` Web Animations interpolation** (cubic-bezier / steps timing → computed value в момент `t`, invalidation animated-свойств). Зависит от п.1B (типизированные Length/Color). | P2 п.3B (compositor offload для transform/opacity), P3 (animation scheduling в rendering steps stage). | Stub interpolator из Sprint 0 уже даёт компилируемый код. |
| 3B | **`[P1+P3]` Push-tokenizer + incremental tree builder** в `lumen-html-parser`. Pull-tokenizer уже работает (~270 тестов); push-режим — отдельный API поверх той же state machine. | P3 п.4B (streaming pipeline — window-first init + async fetch chunks). | До п.3B P3 продолжает streaming-shell в blocking-режиме. |
| 4A | **`[P1+P2]` `<picture>` / `srcset` / `sizes` finishing.** 🟡 Готовы parser + pickers + `pick_picture_source`. Осталось: IntersectionObserver event source для `loading="lazy"`, L4 nested `(not (...))`-формы в media-condition. | P3 lazy-loading в shell. | Никого — P2 GPU upload не зависит от lazy. |
...
| 1B | 🟡 **`[P2+P1]` Compositor thread + layer tree scaffolding** против `PropertyTrees` от Sprint 0. Scaffolding ✅ — trait-ы `Layer` / `LayerTree` / `Compositor` + `BasicLayerTree::single_layer` + `InProcessCompositor` в `lumen-paint::compositor`. Осталось: (a) подключить consumer-ов (2A painting order через `compositor.active_tree()`, не `&DisplayList`); (b) двухбуферная commit-модель `pending → active` с atomic swap (когда compositor пойдёт в отдельный поток); (c) DisplayCommand-расширения под blend / clip / layer push-pop. | Foundation для: п.2B (hit testing), off-main-thread scroll, п.3B (Web Animations compositor path), п.4 (mix-blend-mode/backdrop-filter pipeline), GPU process у P3. **Самый большой enabler в P2.** | Структура pipeline готова до того, как P1 закончит property trees — drop-in переход. |
...
| 2B | **`[P3]` `publicsuffix` через `PublicSuffixList`** (provisional) + cookie domain matching + eTLD+1 для Safe Browsing host-suffix обрезки. | Safe cookie matching, точная host-suffix обрезка для Safe Browsing. | Только `lumen-network` + `lumen-storage::cookies`. |
...
| 5B | **`[P3]` HTTP Range requests + `<video>` seek**. RFC 7233 single-range уже реализованы (`fetch_range`); осталось multi-range / suffix `bytes=-N` / If-Range, интеграция в shell для resume downloads. | `<video>` seek, PDF page-load, resume downloads. | Только `lumen-network`. |
...
| `publicsuffix` | P3 п.2B | После SOP/CORS | `PublicSuffixList` |
...
- **`[P1+P2]` `<picture>` / `srcset` / `sizes` + `loading="lazy"`.** 🟡 **P1**: готовы parser + pickers + `pick_picture_source`. Осталось: IntersectionObserver event source для lazy, L4 nested `(not (...))`. **P2** — image-side GPU upload + integration в shell.
