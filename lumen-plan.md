...
| # | Задача | Что разблокирует | Что НЕ блокирует |
|---|---|---|---|
| 1A | **`[P3]` Brotli через `brotli-decompressor`** (provisional, расширяет `ContentDecoder`). 60-70% реального веба отдают `Content-Encoding: br`. | «Реальный сайт» у P2 показывает читаемый HTML, а не мусор. | Только `lumen-network`. |
...
|---|---|---|---|
| `brotli-decompressor` | P3 п.1A | **Сразу после Sprint 0** | расширение `ContentDecoder` |
...
- **`[P3]` Brotli + Range requests.** Brotli — **provisional `brotli-decompressor`** (см. таблицу выше); Range — single-range уже реализован, multi-range / suffix / If-Range остались.
