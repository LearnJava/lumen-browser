...
- 🔄 HTTP Range: suffix bytes=-N + If-Range (P3 5B шаг a) [P3] — range-suffix-if-range — 2026-05-15
...
| 1B | 🟡 **`[P2+P1]` Compositor thread + layer tree scaffolding** против `PropertyTrees` от Sprint 0. Scaffolding ✅ (trait-ы `Layer` / `LayerTree` / `Compositor` + `BasicLayerTree::single_layer` + `InProcessCompositor` в `lumen-paint::compositor`). Two-buffer commit-модель ✅ (`commit` → pending; `flush_pending()` атомарно промотирует pending → active; `has_pending()` для invalidate-логики). DisplayCommand layer-ops stubs ✅ (`PushClipRect/PopClip`, `PushOpacity/PopOpacity`, `PushBlendMode/PopBlendMode` + `BlendMode` enum со всеми 16 режимами; эмиттер ещё не выпускает, renderer игнорирует). Осталось: (a) подключить consumer-ов (2A painting order через `compositor.active_tree()`, эмиттер DisplayCommand layer-ops при walk-е LayoutBox-а); (b) compositor thread (отдельный поток с Mutex/channel вокруг pending — drop-in замена `InProcessCompositor`); (c) реальный layer-pipeline для PushClipRect/PushOpacity/PushBlendMode (off-screen-layer composite + scissor). | Foundation для: п.2B (hit testing), off-main-thread scroll, п.3B (Web Animations compositor path), п.4 (mix-blend-mode/backdrop-filter pipeline), GPU process у P3. **Самый большой enabler в P2.** | Структура pipeline готова до того, как P1 закончит property trees — drop-in переход. |
