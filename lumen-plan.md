...
- 🔄 claudemd-trim-merge-history — claude/ecstatic-hellman-a7e03e — 2026-05-14
...
- **`[P1+P2]` `<picture>` / `srcset` / `sizes` + `loading="lazy"`.** Viewport+DPR-aware resource selection до загрузки (часть preload scanner-а); lazy loading через IntersectionObserver. Отдельно от полного `<video>` / `<audio>` pipeline-а — это базовый image-respond-to-viewport, нужный на любом адаптивном сайте. **P1**: 🟡 готовы `srcset` parser + density-picker, `sizes` attribute + width-picker, `<picture>`/`<source>` selection через `pick_picture_source` (`lumen_html_parser::picture`) — см. CLAUDE.md «Состояние подсистем». Осталось: IntersectionObserver event source для lazy, `not`/`only`/`prefers-color-scheme` в media-condition, intrinsic dimensions из `<source>` для CLS. **P2** — image-side GPU upload + intrinsic dimensions + integration в shell.
...
- **`[P3]` Brotli + Range requests.** Brotli decoder (60-70% веба сжатие) — свой код по политике (compression engine). Range — для `<video>` seek, PDF page-load, resume downloads. **HTTP/1.1 keep-alive + connection pool** реализован отдельно (см. `lumen-network` в «Состояние подсистем» / `git log --oneline | grep http-keepalive`).
