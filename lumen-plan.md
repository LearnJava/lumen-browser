...
> **Гибридная стратегия хранения (LMDB для горячих key-value).** SQLite остаётся единственной permanent exception для storage. Однако для подсистем с профилем «частое чтение/запись простых key-value без SQL» — **localStorage, sessionStorage, IndexedDB, HTTP cache** — при появлении bottleneck рассмотреть добавление LMDB (`heed` — safe Rust binding) как provisional accelerator за тот же `StorageBackend` trait. Обоснование: zero-copy mmap-чтение, потокобезопасное MVCC без блокировок на read path, минимальный код (~30 КБ). Firefox использует аналогичный подход: LMDB для IndexedDB, SQLite для реляционного (история, закладки). Graduation criterion: «замерить latency localStorage / IndexedDB под нагрузкой; если SQLite WAL < 1 мс на p99 — LMDB не нужен». Решение — не раньше Phase 2, когда появятся реальные потребители (JS `localStorage` API, `IndexedDB` API, HTTP cache с большим трафиком).
...
- **Гибридный backend:** SQLite для реляционных данных (cookies, history, bookmarks); при bottleneck — LMDB (`heed`) для горячих key-value (localStorage, sessionStorage, IndexedDB, HTTP cache) за общий `StorageBackend` trait (см. §5).
...
| `StorageBackend` | БД для cookies / IndexedDB | heed (LMDB), sqlite, in-memory |
