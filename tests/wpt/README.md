...
- `tests/wpt/find_missing_resources.py` — static scan for out-of-category
  ...
  checkout and `--ids` (both WPT-RUN-5 slice 30) counts the **manifest ids**
  each missing file blocks rather than the files referencing it — the two are
  not proportional (one `encoding` file with 17 `<meta name="variant">` lines
  is one file and seventeen ids), so ranking the backlog by files
  under-weights exactly the categories where it is worst. Files served by a
...
- `tests/wpt/long_timeout_audit.py` — **ours** (WPT-RUN-5 slice 30) — prices a
  ...
  compares the arms under `run_corpus.py`'s scoring rule, so the sample's mean
  delta projects onto the population as pass-rate points. `--census` prints the
  population and why an id is not probeable (needs `testdriver`, generated by
  wptserve, needs `.sub.`) without starting a browser; `--stratum unblocked`
  samples only ids whose source shows no structural reason to hang;
  `--selftest` covers the pure functions. It deliberately does not run
  `wptrunner`: the corpus run owns the pinned ports, and a squatting
