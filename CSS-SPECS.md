...
| `mask-clip` / `mask-composite` | 🟡 | `mask-clip` full `<coord-box> \| no-clip` grammar ✅: `padding-box`/`content-box`/`fill-box` wrap the mask group in `PushClipRect`/`PopClip` (scissor path; `fill-box` = content box for CSS boxes, CSS Box 4 §1); `border-box`/`stroke-box`/`view-box`/`no-clip` = no-op (border box or unclipped) 2026-07-18; `mask-composite` parsed per-layer ✅ 2026-07-29 (multi-layer infra: `ComputedStyle::mask_layers: Vec<MaskLayer>`, все 8 longhand-ов — списки с cycling по §4.9), **render** ⬜ — применяется только верхний слой; Porter-Duff add/subtract/intersect/exclude по каналу маски требует сборки маски в отдельный офскрин во всех трёх бэкендах (`// CSS: mask-composite` в `emit_push_mask`) |
