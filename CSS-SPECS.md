...
| CSS Backgrounds & Borders L3 | [css3-background](https://www.w3.org/TR/css3-background/) | 🟡 | borders/radius/box-shadow/bg-color/image/size/pos/repeat; `border-image` (source/slice/width/outset/repeat + shorthand) entirely missing — no parsing, no 9-slice paint algorithm (BUG-492, ревизия P3 2026-09-03). Sibling draft **CSS Borders and Box Decorations L4** (`css-borders-4`, still CSSWG draft) entirely missing too: `corner-shape`/`border-shape` + longhands/shorthands, `box-shadow-*` component longhands, paired `border-*-radius` shorthands, `border-clip`, `hairline` `<line-width>` keyword — no parsing, no corner-clip paint algorithm (BUG-491, ревизия P3 2026-09-03) |
...
| CSS Inline L3 | [css-inline-3](https://www.w3.org/TR/css-inline-3/) | 🟡 | line-height leading; baseline grid; `baseline-shift` ✅ 2026-06-21 (p4-baseline-shift: SVG 1.1 §10.9.2 / CSS Inline L3 §5.2 — non-inherited `SvgBaselineShift` enum Baseline/Sub/Super/Length/Percentage; presentational attribute + CSS property; CSS overrides attr; wired through `emit_svg_text` as vertical y-shift; `sub` lowers by 0.2×font-size, `super` raises by 0.4×font-size, positive length raises); `text-box-trim`/`text-box-edge`/shorthand `text-box` ❌ not implemented at all — no `ComputedStyle` field, needs a new font-metrics-based line-box trim algorithm ([BUG-538](bugs/BUG-538-OPEN.md), reclassified ДОРАБОТКА 2026-09-13) | **#31** |
...
| Media Queries L4/L5 | [mediaqueries-4](https://www.w3.org/TR/mediaqueries-4/) | 🟡 | prefers-reduced-motion ✅; hover/any-hover/pointer/any-pointer ✅ 2026-06-14 (p4-media-hover-pointer: desktop defaults hover/fine); prefers-contrast/prefers-reduced-data ✅ 2026-06-16 (p4-media-contrast-data); prefers-reduced-transparency ✅ 2026-06-19 (p4-prefers-reduced-transparency); scripting ✅ 2026-06-19 (p4-media-scripting: `MediaScripting` none/initial-only/enabled, desktop default `enabled` — Lumen ships QuickJS, matches Edge); inverted-colors ✅ 2026-06-20 (p4-media-inverted-colors: `MediaInvertedColors` none/inverted, desktop default `none`, matches Edge); resolution/min-/max-resolution ✅ 2026-09-07 (BUG-1019: `x`/`dppx`/`dpi`/`dpcm` + minimal `calc()`) | **#35** |
...
| CSS Functions & Mixins | [css-mixins-1](https://www.w3.org/TR/css-mixins-1/) | 🟡 | `@function` rule parsed+stored; call-site evaluation (positional args/defaults, local decls, `result:`) wired end-to-end in style.rs; `returns` typing + conditional group rules deferred. `@mixin`/`@apply`/`@contents` (BUG-518, срез 1): flat `@result` declarations, positional args/defaults, locals, nested `@apply`, `@contents` fallback/override — end-to-end via `expand_mixin_apply` (`layout/style/substitute.rs`). Срез 2: a nested style rule inside `@result` now materializes into its own standalone top-level `Rule` (`lumen_css_parser::parse`'s post-parse pass, `mixins::collect_mixin_nested_rules`), combining with the `@apply` call site's selector the same way CSS Nesting combines `&`; deferred: only `sheet.rules` is scanned (not `@media`/`@supports`/`@layer`/`@scope`/shadow-tree sheets), `Decl`/`@contents` values inside a nested rule are copied literally rather than resolved against the mixin's own scope, a nested `@apply` inside a nested rule's body is dropped, `type()` parameter validation, `attr()` inside `@result`, cross-stylesheet/`@import`/shadow-DOM interaction, CSSOM reflection. Срез 3: `@layer`-aware `@apply` name resolution (`mixin-layers.html`) — see [T3] At-Rules below. Срез 4: cross-stylesheet/`@import` mixin visibility already worked (multiple `<style>`/`@import`/adopted sheets all fold into one flat `Stylesheet` before parsing/cascade — no code change, 2 new tests); shadow-DOM interaction was blocked on unrelated [BUG-1009](bugs/BUG-1009-FIXED.md) (shadow-tree-scoped stylesheets didn't match their own descendants at all, mixins or not), fixed 2026-09-06 — `mixin-shadow-dom.html` unblocked, not re-verified by this entry | **#54** |
...
| CSS Anchor Positioning | [css-anchor-position-1](https://www.w3.org/TR/css-anchor-position-1/) | 🟡 | core wired (anchor-name/position-anchor/anchor()/anchor-size()/inset-area, see T4 table below); `position-try*` fallback sub-feature (`position-try-fallbacks`/`position-try-order`/`position-try` shorthand, `@position-try`/`CSSPositionTryRule`) entirely unimplemented — [BUG-562](bugs/BUG-562-OPEN.md); inline `element.style` CSSOM (parse/validate/serialize) for `anchor()`/`anchor-size()`/`position-area` also missing — [GAP-ANCHORCSSOM](ROADMAP.md) | **#56** |
...
| CSS View Transitions L2 | [css-view-transitions-2](https://drafts.csswg.org/css-view-transitions-2/) | 🟡 | `@view-transition { navigation: auto/none; }` at-rule ✅ P1 2026-09-22 (`crates/engine/css-parser/src/parser/at_rules.rs::ViewTransitionRule`); cross-document (same-origin, both-sides-opt-in) navigation reuses the L1 cross-fade driver end-to-end (`crates/shell/src/lumen/navigation.rs::maybe_capture_mpa_view_transition_snapshot`/`maybe_reveal_mpa_view_transition`) — no separate MPA animation engine, pending snapshot reset on every navigation-error path (P3-viewtransnav, ROADMAP.md); `types` opt-in list / per-navigation `NavigationActivation` skip reasons not modeled | **#58** |
| CSS Fill & Stroke L3 | [fill-stroke-3](https://www.w3.org/TR/fill-stroke-3/) | 🟡 | fill/stroke/fill-opacity/stroke-opacity/stroke-width ✅ 2026-05-27; fill-rule/stroke-linecap/linejoin/miterlimit/dasharray/dashoffset ✅; paint-order ✅ 2026-06-14 (p4-paint-order: `SvgPaintOrder` inherited field + `emit_svg_shape` fill/stroke reorder); `text-anchor`/`dominant-baseline` as CSS properties ✅ 2026-06-21 (p4-svg-text-anchor: inherited `Option` fields folded through `apply_svg_presentational_hints` so author CSS overrides the presentation attribute and inherits from `<g>`); text-painting pair from L4 (`text-decoration-fill`/`text-decoration-stroke`) + legacy `-webkit-text-stroke` entirely missing — no parsing, and neither independent text render path (`cpu_raster.rs`/`femtovg_backend.rs`) has any glyph-outline stroke capability (BUG-521, ревизия P3 2026-09-06) | **#59** |
| CSS Scroll Snap L2 | [css-scroll-snap-2](https://www.w3.org/TR/css-scroll-snap-2/) | 🟡 | snapchanging/snapchanged events: SnapChangeEvent (snapTargetBlock/Inline) + лэйаут-резолв снапнутых узлов (find_snapped_nodes/SnapTargets) + QuickJsRuntime::fire_snap_changing/changed; shell-диспатч при scroll-snap завершении — Phase 1 2026-06-10 | **#60** |
| CSS Ruby L1 | [css-ruby-1](https://www.w3.org/TR/css-ruby-1/) | 🟡 | `ruby-position`/`ruby-align`/`ruby-merge` ✅ 2026-07-04 (p4-ruby-css-props: parse → inherited ComputedStyle fields → `RubyBox::from_style` drives `lay_out_ruby`: align distribution + separate/merge pairing); `<ruby>` box-tree inline integration ✅ 2026-09-25 (GAP-RUBYBOX, P1: `BoxKind::Ruby` atomic inline-level box, `build_ruby_box`/`build_ruby_group_box` partition base/annotation groups, `layout_dispatch` calls `lay_out_ruby` and offsets the composed subtree); `<rtc>` multi-annotation grouping semantics and floated/positioned content inside base/annotation groups remain a documented remainder | **#61** |
| MathML Core (CSS props) | [mathml-core](https://www.w3.org/TR/mathml-core/) | 🟡 | `math-style`/`math-depth` ✅ 2026-07-04 (p4-mathml-css-props: parse → inherited ComputedStyle fields, `auto-add`/`add(n)`/`<integer>` resolved to computed integer vs inherited → `lay_out_mathml`: compact mfrac scaling + script scale from depth delta, `MATH_SCRIPT_SCALE` 0.71/level); `<math>` box-tree integration ⬜ (module has no pipeline callers — P1); `font-size: math` ⬜ | **#62** |
| CSS Color HDR L1 | [css-color-hdr](https://drafts.csswg.org/css-color-hdr/) | 🟡 | `dynamic-range-limit` ✅ 2026-09-04 (BUG-508: keyword + `dynamic-range-limit-mix()` incl. arbitrary nesting, parse → `ComputedStyle::dynamic_range_limit` → inherited → `getComputedStyle()` canonical serialization); no HDR display pipeline exists in Lumen, so the value has no rendering effect. `computed.html`/`inheritance.html` residual live FAILs are the pre-existing BUG-493 same-tick `getComputedStyle` cache gap, not this property. `DynamicRangeLimit::interpolate` exists (componentwise lerp, mirrored in the Web Animations JS shim) but is NOT wired into the native CSS Animations/Transitions engine (Phase-0 5-property animatable cap, `crate::animation`) or exercised by `element.animate()` in the live shell — `interpolation.html`'s live "64/64 pass" is a BUG-493 vacuous same-tick pass, not real verification (ДОРАБОТКА, see `bugs/BUG-508-OPEN.md`) | **#63** |
| CSS Text Size Adjustment L1 | [css-size-adjust-1](https://drafts.csswg.org/css-size-adjust-1/) | 🟡 | `text-size-adjust` / legacy `-webkit-text-size-adjust` ✅ 2026-09-05 (BUG-513: `auto \| none \| <percentage [0,∞]>`, parse → `ComputedStyle::text_size_adjust` → inherited, `none` computes to `100%` per spec → `getComputedStyle()` serializes both spellings); no mobile auto-inflation rendering pipeline in Lumen, so the value has CSSOM/animation observability only, no rendering effect. `TextSizeAdjust::interpolate` exists (regular percentage lerp) but, same as `dynamic-range-limit`/BUG-508, is NOT wired into the native CSS Animations/Transitions engine or `element.animate()` — ДОРАБОТКА, see `bugs/BUG-513-FIXED.md` | **#64** |
...
| `overflow-block` / `overflow-inline` | ✅ | flow-relative axis, resolved to `overflow-x`/`overflow-y` by `writing-mode` (BUG-505) |
| `overflow-anchor` | 🟡 | CSS Scroll Anchoring 1 §propdef-overflow-anchor: `auto \| none` parsed + computed-value round-trip (BUG-524 срез 1); no effect — no anchor-node-selection/suppression-heuristic/scroll-offset-adjustment algorithm exists in layout yet. Whole-module gap tracked here, not as a P3 bug (BUG-524 → ДОРАБОТКА, 2026-09-25): per-scroll-container anchor selection, suppression triggers, offset compensation on relayout in the shell scroll path |
| `visibility` | ✅ | visible/hidden (space reserved) |
| `zoom` | 🟡 | P3 2026-08-10: `<number>` / `<percentage>` / `normal` / `reset`, compounds down the tree (`ComputedStyle::effective_zoom`); scales `font-size` and the absolute (`px`) lengths of the non-inherited box-model properties — width/height/min/max, margin, padding, inset, border-width, row/column-gap. Relative units need no handling (they resolve against already-zoomed bases); `font-size` takes the compounded factor only when it resolved from a zoom-independent basis (`px`/`rem`/viewport units, `FontSizeBasis`) — an `em`/`%`/inherited one takes just the element's own, since the ancestors' is already in the basis. Not scaled: `line-height` and inherited length properties (`text-indent`, `letter-spacing`, `word-spacing`) when specified on the zoomed element itself, `rem` (resolves against the unzoomed root font-size), a `calc()` font-size's absolute terms, and a `border-width` given in `em` (the cascade resolves it to px against the already-zoomed font-size, so the multiply here double-applies). `reset` is treated as `1.0` rather than as "ignore ancestors". P3 2026-09-13 (BUG-532): the inline `style.zoom=` CSSOM setter now validates per the L1 §5 grammar (`normal | <number [0,∞]> | <percentage [0,∞]>`, rejecting `auto`/`reset`/`document`/negative — stricter than `parse_zoom`'s own rendering-time tolerance for `reset`). Still open: `getComputedStyle()` doesn't un-zoom reported values per §zoom-om (`computed-initial.html` FAILs) |
...
| `scroll-marker-group` | 🟡 | CSS Overflow L5 §scroll-marker-group-property: `none \| [before\|after] [tabs\|links]?` parsed + computed-value round-trip (BUG-505 срез 6); no layout effect — nothing generates the `::scroll-marker-group` box yet, so the property has nowhere to place |
| `scroll-target-group` | 🟡 | CSS Overflow L5 §scroll-target-group: `none \| auto` parsed + computed-value round-trip (BUG-505 срез 6); no effect — descendant scroll-snap targets aren't grouped into an implicit `::scroll-marker` set yet |
| `::scroll-marker` / `::scroll-marker-group` / `::scroll-button()` | 🟡 | CSS Overflow L5 selector-grammar recognition only (BUG-505 срез 6): valid/invalid forms resolve correctly in `querySelector`/`CSS.supports(selector())`/`insertRule` (incl. `::scroll-button(<direction>)`'s closed direction set + trailing `:focus`/`:disabled`/`:enabled`), but none of the three ever matches an element — no box generation, snap-target iteration, or activation/interaction model (materially larger follow-up, per the bug's own original scoping) |
...
| `resolution`/`min-/max-resolution` | ✅ | Media Queries L4 §6.3; `x`/`dppx`/`dpi`/`dpcm` units + a minimal `calc()` (`+`/`-`/`*`/`/`, single-level operands — matches vendored coverage); canonicalizes to `dppx`, `calc(...)` wrapper preserved on serialization even when it collapses to one number; `MediaContext::resolution_dppx` desktop default `1.0` (matches `window.devicePixelRatio`'s own hardcoded default — no dynamic per-window scale-factor plumbing exists yet); fixed the feature-value paren scanner along the way (took the first `)`, not the balanced one — cut `calc(...)`'s outer value short); 7 tests (BUG-1019, P3 2026-09-07) |
...
| `@function` | 🟡 | CSS Functions and Mixins L1; `<name>(<params>) [returns <type>]?` parsed+stored (`FunctionRule`, css-parser); `<name>(<args>)` call sites in property values resolved end-to-end (positional args + defaults, local `--x:` decls, `result:` via `calc()`/`var()`) in layout (`expand_custom_functions`, style.rs); names tree-scoped across shadow trees (CSS Scoping L1 §3.5, `expand_custom_functions_scoped`, BUG-519); deferred: `returns` type-checking, conditional group rules in body, named args (p4-css-function 2026-07-15, test 143) |
| `@mixin` / `@apply` / `@contents` | 🟡 | CSS Functions and Mixins L1 (BUG-518, срез 1, P3 2026-09-06); `@mixin --name(<params>) { <locals>; @result { ... } }` parsed+stored (`MixinRule`, css-parser); `@apply --name(<args>) [{ <block> }]` — a marker `Declaration` (`MIXIN_APPLY_MARKER`) at its exact source position, expanded in layout (`expand_mixin_apply`, style/substitute.rs) into `@result`'s declarations, with positional args/defaults resolved at the CALL SITE's scope, `--x:` locals resolved against the mixin's own scope, nested `@apply` inside `@result` recursing, and `@contents` substituted by the caller's own block or the mixin's inline fallback. **Срез 2 (BUG-518, P3 2026-09-06):** a nested style rule inside `@result` (`.foo { ... }`, `&.foo { ... }`, bare `& { ... }`, further nested one level down) is parsed into `MixinResultItem::NestedRule` and, once the whole stylesheet is parsed, `mixins::collect_mixin_nested_rules` combines it with each `@apply` call site's own selector the same way CSS Nesting combines `&` (`lumen_css_parser::expand_nesting`), materializing it into its own standalone top-level `Rule` that the ordinary cascade then matches independently — `lumen_css_parser::parse`'s post-parse pass, gated on `mixin_rules` being non-empty (a no-op for every page that doesn't use `@mixin`). Deferred: only the sheet's flat top-level `rules` are scanned (a call site inside `@media`/`@supports`/`@layer`/`@scope`/a shadow-tree sheet is out of scope), a nested rule's own `Decl`/`@contents` declarations are copied literally rather than resolved against the mixin's/call site's scope (correct only when they don't depend on `var()`/mixin parameters — true of every vendored test), a nested `@apply` inside a nested rule's own body is dropped, `type(<syntax>)` parameter validation/coercion, `attr()` inside a `@result` declaration, `@apply` argument-count strictness (an omitted required argument is left unbound rather than invalidating the call), cross-stylesheet/`@import` mixin visibility, shadow-DOM encapsulation, `@layer` interaction, CSSOM reflection. **Срез 3 (BUG-518, P3 2026-09-06):** `@layer` interaction for name resolution — a `@mixin` declared directly inside `@layer name { ... }` carries that layer's name (`MixinRule::layer`); both `@apply` lookup sites (`expand_apply_rule` and `collect_mixin_nested_rules`) now rank same-named mixins by CSS Cascade L5 §6.4.5 layer priority (unlayered beats every layer, a later-declared layer beats an earlier one) before falling back to source order, closing `mixin-layers.html`. Still deferred: cross-stylesheet/`@import` visibility, shadow-DOM encapsulation, CSSOM reflection. **Срез 4 (BUG-518, P3 2026-09-06):** cross-stylesheet/`@import` visibility needed no code — every inline `<style>`/`@import`/adopted stylesheet already folds into one flat `Stylesheet` before `@mixin`/`@apply` ever run (`build_page_cascade`'s text-level concatenation, `Stylesheet::merge_from` for adopted sheets), so срез 1's cascade-time forward-reference resolution already covers it; pinned with 2 new tests (`mixin-cross-stylesheet.html`/`mixin-from-import.html` transcriptions). Shadow-DOM encapsulation investigated and found blocked one layer below mixins: [BUG-1009](../bugs/BUG-1009-FIXED.md) — a shadow tree's own regular (non-`:host`/`::slotted`) selectors never matched its own descendants at all, so `@apply` inside such a rule was unreachable regardless of mixin logic; fixed 2026-09-06, `mixin-shadow-dom.html` not re-verified by this entry. **Срез 6 (BUG-518, P3 2026-09-07):** CSSOM reflection, read-only slice — `Rule::css_text`/`MixinRule::css_text`/`ApplyRule::css_text`/`MixinResultItem::css_text` (css-parser) serialize a top-level `@mixin`/`@result`/`@contents`/an `@apply`-bearing style rule to `cssText`, exposed as a new `CSSMixinRule` (`document.styleSheets[i].cssRules`) alongside the existing `CSSStyleRule`/`CSSMediaRule`; closes 5 of `mixin-cssom.tentative.html`'s 6 subtests (confirmed end-to-end through the real V8 shim, `crates/js/tests/cases/bug518_mixin_cssom.rs`, not just unit-level). Still deferred: `.cssRules` navigation into `@result`'s own children (nothing in scope needed it), `insertRule`/`deleteRule` on an *owned* sheet (`@apply is not legal at top level` subtest — only a constructed sheet has these today, [BUG-897](bugs/BUG-897-FIXED.md)/CSSOM-5), and `mixin-invalidation.tentative.html` in full — its `style.color = ...` mutation pattern needs a live per-declaration `.style` setter no CSSOM rule kind has yet, a gap broader than this bug's own scope. **Срез 7 (BUG-518, P3 2026-09-07, реклассификация):** the remainder (that 6th subtest + all of `mixin-invalidation.tentative.html`) moved out of BUG-518's point-bug scope into a new ROADMAP task, [CSSOM-8](ROADMAP.md) — CSSOM-5 only ever added `insertRule`/`deleteRule` for a *constructed* sheet, the owned-sheet registry (`document.styleSheets`) still has zero mutation surface, and no CSSOM rule object has a live `.style` setter; both satisfy docs/probe-method.md §8's reclassification bar (absent + family-sized, not a point fix). `@mixin`/`@apply`/`@contents`/`@result` themselves are unaffected — fully implemented per срезы 1-6 above. **Срез 8 (BUG-518, P3 2026-09-07):** landed concurrently with срез 7's reclassification — `CSSStyleSheet.insertRule`/`.deleteRule` on an *owned* sheet implemented anyway (`Stylesheet::insert_rule`/`delete_rule`, already shared with the constructed-sheet path, wired to the owned-sheet registry via `_lumen_stylesheet_insert_rule`/`_lumen_stylesheet_delete_rule`), closing `mixin-cssom.tentative.html`'s 6th subtest (`@apply` at top level already fails to parse as a single top-level rule, no special-casing needed) — the whole file now passes. Narrows CSSOM-8's remaining scope to just the `.style`-setter half; `mixin-invalidation.tentative.html` remains BUG-518's entire open remainder, still blocked on that setter. **Срез 9 (BUG-518/CSSOM-8, P1 2026-09-18):** the `.style`-setter half implemented for an owned sheet's top-level and `@media`-nested `CSSStyleRule` (`Stylesheet::set_rule_style_text`/`set_media_child_style_text`, shared `CSSStyleDeclaration.prototype` with element `.style`) — but `mixin-invalidation.tentative.html` stays fully red: it needs `.style` on a rule nested inside `@mixin`'s own `@result` (no addressing scheme exists for that), and no CSSOM mutation of any kind yet reaches `getComputedStyle()` in the same tick (`stylesheet_nodes` is rebuilt wholesale from DOM text on relayout, same gap срез 8's `insertRule`/`deleteRule` already left open) |
...
### [T5] Text Size Adjustment L1

| Feature | Status | Notes |
|---------|--------|-------|
| `text-size-adjust` / `-webkit-text-size-adjust` | 🟡 | `auto \| none \| <percentage [0,∞]>` ✅ 2026-09-05 (BUG-513); inherited, initial `auto`, `none` computes to `100%`. Both spellings read the same `ComputedStyle::text_size_adjust` field. `TextSizeAdjust::interpolate` exists (regular percentage lerp) but isn't wired into the native animation engine yet — ДОРАБОТКА, same class as `dynamic-range-limit`/BUG-508. No mobile auto-inflation rendering pipeline — no rendering effect. `calc(10% * sibling-index())` (one `parsing/text-size-adjust-valid.html` case) is out of scope — `sibling-index()` (CSS Values L5) isn't implemented anywhere in the engine |
