...
mod linux_hid {
    ...
    impl HidDevice for LinuxHidDevice {
        ...
        fn read_timeout(&self, timeout_ms: i32) -> Result<[u8; 65], Ctap2Error> {
            ...
            // BUG-168: `libc_poll` is itself a safe wrapper (its body holds the
    ...
    // BUG-168: pub(crate) so the in-crate `#[cfg(linux)]` tests can call it
    ...
    pub(crate) fn descriptor_is_fido(desc: &[u8]) -> bool {
        ...
        while i < desc.len() {
            ...
            match tag {
                ...
                // BUG-168: inner `if` folded into the match guard (clippy::collapsible_if).
