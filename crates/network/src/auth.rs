...
pub(crate) fn build_digest_authorization(
    ...
    let hash_str: fn(&str) -> String = if use_sha256 { sha256_hex } else { md5_hex };
    ...
    // HA1
    let mut ha1 = hash_str(&format!("{}:{}:{}", creds.username, realm, creds.password));
    if session_mode {
        ha1 = hash_str(&format!("{ha1}:{nonce}:{cnonce}"));
    }

    // HA2 (qop=auth и legacy — оба считают MD5(method:uri))
    let ha2 = hash_str(&format!("{method}:{uri}"));

    // response-digest
    let response = if has_qop_auth {
        hash_str(&format!("{ha1}:{nonce}:{nc_str}:{cnonce}:auth:{ha2}"))
    } else {
        // RFC 2069 legacy.
        hash_str(&format!("{ha1}:{nonce}:{ha2}"))
    };
...
fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    // Padding: append 0x80, 0x00 до длины ≡ 56 (mod 64), 64-bit length big-endian.
    let bit_len = (input.len() as u64).wrapping_mul(8);
    let mut padded = input.to_vec();
    padded.push(0x80);
    while padded.len() % 64 != 56 {
...
mod tests {
    ...
    // SHA-256 test vectors from FIPS 180-4 (NIST examples).
    #[test]
    ...
    #[test]
    fn sha256_two_block() {
        // 56-байтная строка — два блока (56 + padding > 56 ⇒ перепол).
        assert_eq!(
    ...
    fn digest_md5_sess_uses_chained_ha1() {
        ...
        let nonce = "N";
        let cnonce = "C";
        let inner = md5_hex(&format!("{user}:{realm}:{password}"));
        let ha1_sess = md5_hex(&format!("{inner}:{nonce}:{cnonce}"));
