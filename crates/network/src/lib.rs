...
// Реэкспорт HTTP/1.1-ядра (SPLIT-NW0), чтобы `do_request` в этом файле
// продолжал ссылаться на голые имена без правки call-site-ов — механический
// перенос, публичная поверхность не растёт (все элементы ниже `pub(crate)`,
// видны только внутри крейта).
pub(crate) use http1::chunked::read_response_streamed;
pub(crate) use http1::response::{parse_status, read_response};

// Тот же реэкспорт для `mod tests` (ниже в этом файле) — под `#[cfg(test)]`,
// иначе имена «неиспользуемые» в non-test сборке библиотеки: тесты этих
// unit-ов остались в `mod tests`, а не переехали вместе с кодом (см.
// коммит), поэтому им нужен доступ к внутренним типам HTTP/1.1-ядра.
#[cfg(test)]
pub(crate) use http1::chunked::{BodyFraming, BodyReader, StreamDecode, detect_stream_decode, read_body_to_eof, read_chunked, read_framed_buffered};
