...
/// Метаданные аудиодорожки контейнера — частота дискретизации и число
/// каналов. Только детект, без декодирования PCM (GAP-MEDIADECODE срез 12);
/// вывод звука для `<video>` — предмет отдельного, ещё не начатого среза.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// видеодорожку в RGBA8, зная общую длительность и текущую позицию. Вывод
/// звука для видео вне объёма этого trait-anchor'а: у `<audio>` сегодня свой
/// путь (`audio_element.rs`), а декодирование PCM аудиодорожки видео —
/// часть более широкого будущего среза, не первого подключения FFmpeg.
pub trait VideoDecodeSession: Send {
    ...
    /// Метаданные аудиодорожки контейнера, если она есть и её кодек
    /// декодируется — `None` для немых контейнеров и для реализаций, не
    /// добывающих эту информацию (реализации, предшествующие срезу 12).
    fn audio_track(&self) -> Option<AudioTrackInfo> {
        None
    }

    /// Декодировать следующую порцию PCM аудиодорожки как интерливленные
    /// (per-channel) 16-битные сэмплы, продолжая от текущей позиции чтения
    /// демуксера — `max_samples` считается на канал, не байт; результат
    /// может быть короче на EOF. Вывод в аудио-устройство (маршрутизация в
    /// платформенный `rodio`/`cpal`-backend) вне ответственности этого
    /// trait'а — он отдаёт только сырой PCM (GAP-MEDIADECODE срез 13).
    ///
    /// Отдельный от [`Self::audio_track`] метод: `audio_track() == None`
    /// значит «дорожки нет», а `Err` здесь — «дорожка есть, но её PCM не
    /// декодировать» (неподдерживаемый формат сэмплов, битый поток) либо
    /// «эта реализация не умеет декодировать PCM вовсе» (дефолт ниже —
    /// для реализаций, предшествующих срезу 13).
    ///
    /// # Errors
    /// Строка с диагностикой: нет открытой аудиодорожки, неподдерживаемый
    /// формат сэмплов, ошибка декодера.
    fn decode_audio_pcm(&mut self, max_samples: usize) -> std::result::Result<Vec<i16>, String> {
        let _ = max_samples;
        Err("эта реализация VideoDecodeSession не декодирует PCM аудио".to_string())
    }
}
...
pub trait WebSocketSession: Send {
    ...
    /// Как [`Self::recv`], но не блокирует дольше `timeout`, если новых данных
    /// не приходит — возвращает `Ok(None)` вместо бесконечного ожидания.
    ///
    /// Нужен реализациям, которые держат сессию под `Mutex`, разделяемым с
    /// вызовами `send_text`/`send_binary`/`close` с другого потока (JS):
    /// без ограничения по времени фоновый поток, читающий следующее
    /// сообщение, удерживает мьютекс вечно, пока сокет простаивает — любой
    /// JS-вызов, которому нужен тот же мьютекс, блокируется навсегда вместе
    /// с ним (BUG-307: однопоточный JS-движок виснет целиком). Реализация
    /// по умолчанию не поддерживает частичный неблокирующий опрос и просто
    /// делегирует в [`Self::recv`], игнорируя `timeout`.
...
pub trait JsFetchProvider: Send + Sync {
    ...
    /// Opens a WebTransport unidirectional stream on the session `handle`
    /// names — GAP-WEBTRANSPORT/`P3-webtransport` срез 3b, first consumer of
    /// the handle [`webtransport_connect`](Self::webtransport_connect)
    /// hands back.
    ///
    /// Returns the opened stream's QUIC identifier — an opaque number to
    /// script, only used so a later native call can address the right
    /// stream. Default implementation always reports "unsupported", matching
    /// [`webtransport_connect`](Self::webtransport_connect)'s default; only
    /// `lumen-network::HttpClient` overrides it.
