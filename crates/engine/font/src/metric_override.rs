...
/// Parses an `ascent-override`/`descent-override`/`line-gap-override` value.
///
/// Returns `Some(fraction)` for a valid non-negative `<percentage>` (`"90%"`
/// → `Some(0.9)`), `None` for `"normal"` or any unparseable value — both mean
/// "no override, use the face's real metric" to the caller.
