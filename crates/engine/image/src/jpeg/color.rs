...
mod tests {
    ...
    fn clamping_works_for_out_of_range_chroma() {
        // Экстремальные значения не должны панику или wrap-around.
        // Y=0, Cb=+127, Cr=−128 → B стремится за 255, R за -ve. Clamp ловит.
        let (r, g, b) = ycbcr_to_rgb(0, 255, 0);
        // u8 уже гарантирует 0..=255; убеждаемся, что значения не panic-ят
        // и принимают «крайние» позиции, как ожидаем по уравнениям.
        assert_eq!(r, 0, "R при отрицательном underflow должен быть 0");
        assert_eq!(b, 255, "B при положительном overflow должен быть 255");
        // G тоже clamped (на середине формулы).
        let _ = g;
