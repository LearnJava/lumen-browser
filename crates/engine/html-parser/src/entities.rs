...
//! tokenizer эмитит `&` буквально и продолжает.
//!
...
/// Таблица (name-with-semicolon, decoded-string). **Должна** быть
/// отсортирована по first column для бинарного поиска. Проверяется
/// тестом ниже.
...
mod tests {
    ...
    #[test]
    fn lookup_greek_letters() {
        assert_eq!(lookup_named_entity("alpha;"), Some("\u{03B1}"));
        assert_eq!(lookup_named_entity("Omega;"), Some("\u{03A9}"));
    }

    #[test]
    fn lookup_arrows() {
        assert_eq!(lookup_named_entity("rarr;"), Some("\u{2192}"));
        assert_eq!(lookup_named_entity("hArr;"), Some("\u{21D4}"));
    }
