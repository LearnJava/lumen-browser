...
/// назван измерением (см. bugs/BUG-405-FIXED.md, срез 8).
///
...
/// (см. bugs/BUG-405-FIXED.md, срез 7).
///
...
// ─────────────────────────────────────────────────────────────────────────────
// BUG-405 срез 13 — укладка текстового run-а считается один раз
// ─────────────────────────────────────────────────────────────────────────────

/// BUG-405 срез 13: повторно встреченный run не укладывается второй раз.
///
/// Гейт стоит на счётчике попаданий, а не на времени фазы `collect`: «один и
/// тот же run укладывается один раз» — утверждение о механизме, оно не зависит
/// ни от железа, ни от содержимого страницы.
///
/// Два контроля на ложно-зелёный. Первый: ДРУГАЯ строка обязана дать промах —
/// кэш, отвечающий готовым на любой запрос, был бы тут зелен. Второй: плечо
/// отката обязано перестать попадать, иначе счётчик не различает два пути.
///
/// Сдвиг по вертикали, наоборот, обязан ПОПАДАТЬ: позиции в ключе нет, её
/// воспроизводит перо при проигрывании плана.
#[test]
#[ignore = "requires GPU adapter"]
fn text_run_cache_serves_repeated_runs() {
    let mut r = Renderer::new_headless(INTER.to_vec(), 64, 128, ColorSpace::Srgb)
        .expect("headless renderer");
    r.set_font_provider(None);

    let dl = text_dl("Lumen", 20.0);
    let (h0, m0) = r.text_run_cache_stats();
    r.render_to_image(&dl, 0.0, 0.0).expect("первый кадр");
    let (h1, m1) = r.text_run_cache_stats();
    assert_eq!(h1 - h0, 0, "первый кадр не мог попасть в пустой кэш");
    assert_eq!(m1 - m0, 1, "run не дошёл до укладки");

    r.render_to_image(&dl, 0.0, 0.0).expect("второй кадр");
    let (h2, m2) = r.text_run_cache_stats();
    assert_eq!(h2 - h1, 1, "повторный кадр всё ещё укладывает run заново");
    assert_eq!(m2 - m1, 0, "повторный кадр промахнулся");

    // Позиции в ключе нет — тот же текст ниже обязан быть попаданием.
    r.render_to_image(&text_dl("Lumen", 48.0), 0.0, 0.0).expect("кадр со сдвигом");
    let (h3, m3) = r.text_run_cache_stats();
    assert_eq!(h3 - h2, 1, "сдвиг по вертикали засчитан промахом — позиция попала в ключ");
    assert_eq!(m3 - m2, 0, "сдвинутый run уложен заново");

    // Контроль на ложно-зелёный №1: другая строка — другой ключ.
    r.render_to_image(&text_dl("Browser", 20.0), 0.0, 0.0).expect("кадр с другой строкой");
    let (h4, m4) = r.text_run_cache_stats();
    assert_eq!(m4 - m3, 1, "другая строка взята из кэша — ключ не различает текст");
    assert_eq!(h4 - h3, 0, "другая строка засчитана попаданием");

    // Контроль на ложно-зелёный №2: плечо отката обязано обойти кэш целиком.
    r.set_text_run_cache_enabled(false);
    r.render_to_image(&dl, 0.0, 0.0).expect("кадр без кэша");
    let (h5, m5) = r.text_run_cache_stats();
    assert_eq!(h5 - h4, 0, "плечо отката всё ещё попадает (контроль негоден)");
    assert_eq!(m5 - m4, 0, "плечо отката обязано обходить кэш целиком");
    r.set_text_run_cache_enabled(true);
}

/// BUG-405 срез 13: кадр с попаданиями побайтово равен кадру без кэша.
///
/// План хранит шаги укладки, а не готовые вершины, ровно ради этого: перо
/// стартует с `rect.x` и накапливает те же слагаемые в том же порядке, значит
/// расхождение невозможно даже в ULP — гейт побайтовый, а не численный.
///
/// Сравниваются три кадра: плечо отката, кадр промахов и кадр попаданий.
/// Последний и есть предмет проверки — первые два вместе доказывают, что
/// расхождение искали бы там, где оно возможно. Проверка «текст вообще
/// нарисован» обязательна: три пустых кадра сравнивались бы зелено.
#[test]
#[ignore = "requires GPU adapter"]
fn text_run_cache_matches_relayout() {
    let mut r = Renderer::new_headless(INTER.to_vec(), 64, 128, ColorSpace::Srgb)
        .expect("headless renderer");
    r.set_font_provider(None);
    let dl = text_dl("Lumen", 20.0);
    ...
    r.set_text_run_cache_enabled(false);
    // Первый кадр плеча отката заодно наполняет атлас: иначе кадр промахов
    // сравнивался бы с кадром, у которого глифов в атласе ещё не было.
    r.render_to_image(&dl, 0.0, 0.0).expect("прогрев атласа");
    let plain = r.render_to_image(&dl, 0.0, 0.0).expect("плечо отката");
    ...
    let (h1, m1) = r.text_run_cache_stats();
    let hit = r.render_to_image(&dl, 0.0, 0.0).expect("кадр попаданий");
    let (h2, _) = r.text_run_cache_stats();

    assert_eq!((h1 - h0, m1 - m0), (0, 1), "кадр промахов оказался не тем, чем назван");
    assert_eq!(h2 - h1, 1, "кадр попаданий не попал в кэш — сравнивать нечего");

    assert_eq!(plain.data, miss.data, "кадр промахов разошёлся с плечом отката");
    assert_eq!(plain.data, hit.data, "кадр попаданий разошёлся с плечом отката");

    for (name, img) in [("откат", &plain), ("промахи", &miss), ("попадания", &hit)] {
        let dark = img.data.chunks_exact(4).filter(|px| px[0] < 64 && px[2] < 64).count();
