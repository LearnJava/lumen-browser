...
mod tests {
    ...
    // нельзя нарушить: структурный хэш ДОЛЬШЕ различает, чем Debug. Если два
    // Debug-представления различны, структурные хэши обязаны быть различны —
    // иначе кадр с изменившимся содержимым получит старый хэш и НЕ будет
    // перерисован (ложный skip → устаревшие пиксели на экране).
    ...
    fn hash_corpus() -> Vec<DisplayCommand> {
        ...
        vec![
            ...
            // Холодные варианты (Debug-fallback) + unit-вариант.
            backdrop_cmd(),
            DisplayCommand::PopBackdropFilter,
            DisplayCommand::PopTransform,
        ]
