...
pub struct FemtovgBackend {
    ...
    blend_layer_stack: Vec<blend::BlendLayerEntry>,
...
impl FemtovgBackend {
    ...
    /// Execute the display list into the current render target under the scroll
    /// transform stack. `skip` (ADR-016 M3.2.1c-3) lists overlay command ranges to
    /// omit — on the band path the backend rasters the band *without* overlay
    /// content and replays it afterwards ([`replay_overlays`](Self::replay_overlays)).
    ...
    ///
    /// Drawn straight to the screen target ([`present_content_band`](Self::present_content_band)
        ...
        match cmd {
            ...
            DisplayCommand::PushBlendMode { mode, bounds } => {
                if *mode == BlendMode::Normal {
                    ...
                    self.canvas.save();
                    self.canvas.global_composite_operation(blend::blend_to_composite(*mode));
                    self.layer_stack_depth += 1;
                } else {
                    ...
                    match acquired {
                        Some((src_id, bbox)) => {
                            ...
                            self.blend_layer_stack.push(blend::BlendLayerEntry {
                                mode: *mode,
                                src_image_id: src_id,
                                backdrop_rgba,
                                backdrop_w,
...
impl RenderBackend for FemtovgBackend {
            ...
            BandDraw::Expose { retained, prev_origin, expose } => {
                    ...
                    (band, self.retained_band)
                {
                    ...
                    for strip in expose.iter().flatten() {
                        self.run_content_pass(content, band_src, Some(*strip), frame_log_lvl2, skip);
                    }
                }
            }
            // Full band re-raster (direct-to-screen path when `band == None`).
            BandDraw::Repaint => {
                self.run_content_pass(content, band_src, None, frame_log_lvl2, skip);
            }
        }
    ...
    fn register_image(&mut self, src: String, image: Arc<Image>) -> Result<(), String> {
        ...
        let rgba: Vec<RGBA8> = image::image_to_rgba8_vec(&image);
    ...
    fn register_snapshot(&mut self, id: u64, image: &Image) -> Result<(), String> {
        ...
        let rgba = image::image_to_rgba8_vec(image);
        let img = ImgRef::new(&rgba, image.width as usize, image.height as usize);
