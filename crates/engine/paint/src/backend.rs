...
pub trait RenderBackend: Send {
    ...
    /// Сообщает версию списка `content`, который придёт в ближайший
    /// [`render`](Self::render) (BUG-405 срез 39).
    ///
    /// Кадр прокрутки решает две вещи хэшами по всему списку: «кадр тождественен
    /// прошлому» (пропуск) и «содержимое полосы то же» (блит вместо
    /// перерисовки). Оба хэша — O(n) по display list-у, и на кадре ПОПАДАНИЯ
    /// они считаются по списку, который shell вообще не трогал: страница
    /// свёрстана, меняется только скролл. Перепись среза 39: 0.76 мс на кадр,
    /// 37 % честного кадра попадания — крупнейшая статья.
    ///
    /// Версия закрывает ровно эту дыру: пока она та же, свёртка content-части
    /// переиспользуется, и кадр сворачивает только overlay. Контракт shell-а:
    ///
    /// * значение обязано МЕНЯТЬСЯ при любом изменении списка — и замене, и
    ///   правке на месте (иначе кадр покажет устаревшие пиксели);
    /// * `0` — «версия неизвестна», мемоизация выключена. Его обязан передавать
    ///   всякий, кто рисует производный список (анимационная патч-копия,
    ///   подсветка поиска, split-view), а не сам retained-список.
    ///
    /// Дефолт — no-op; бэкенды без мемоизации считают хэши как раньше.
    /// Проверка контракта на рантайме — `LUMEN_VERIFY_DL_EPOCH=1`.
    fn set_content_epoch(&mut self, _epoch: u64) {}

    /// Объявляет готовые дайджесты хвоста `overlay`, который придёт в
    /// ближайший [`render`](Self::render)/[`render_with_anim`](Self::render_with_anim)
    /// (BUG-405 срез 57, п.85).
    ///
    /// `Some((start, digests))` — `digests[i]` обязан быть
    /// [`crate::display_list::hash_one_command`] от `overlay[start + i]`, то
    /// есть от команды, которая ДЕЙСТВИТЕЛЬНО придёт по этому индексу в
    /// ближайшем `render`/`render_with_anim`. Единственный законный
    /// источник — сегмент, чьи байты бэкенд гарантированно не менял со
    /// времени, когда дайджест был посчитан (например, кэш-хит хром-сегмента
    /// оверлея, чей `DisplayList` при попадании клонируется БЕЗ изменений).
    /// Ошибочный диапазон подменит дайджест чужой команды — то же
    /// последствие, что у ложного попадания любого кадрового кэша (плохой
    /// пиксель НАВСЕГДА, а не на один кадр), поэтому это поле — на разовое
    /// объявление КАЖДЫЙ кадр, а не «включить один раз»: shell обязан
    /// передавать `None`, если применимость под вопросом.
    ///
    /// Дефолт — no-op; бэкенды без такого кэша продолжают считать дайджест
    /// оверлея целиком, поведение не меняется.
    fn set_overlay_digest_reuse(&mut self, _reuse: Option<(usize, Vec<u64>)>) {}
