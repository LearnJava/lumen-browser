...
impl Renderer {
                ...
                s.spawn(|| {
                    loop {
                        ...
                        if crate::frame_log_level() >= 2 {
                            eprintln!(
                                "[frame:wgpu]     imgjob {}x{} -> {}x{}: resize {:.1}ms rgba+icc {:.1}ms ({})",
                                raw.width, raw.height, tw, th,
                                t_resize.as_secs_f64() * 1000.0,
                                (t0.elapsed() - t_resize).as_secs_f64() * 1000.0,
                                src,
                            );
                        }
                        if let Ok(mut slot) = results[i].lock() {
    ...
    fn render_impl(
        ...
        let t_sub_prefetch = t_frame0.elapsed();
        ...
        let t_sub_resolve = t_frame0.elapsed();
        ...
        if phase_log {
            // Разбивка фазы faces: хэш DL / префетч face-ов / резолв face_id
            // / image pre-pass (CPU-ресайз). Диагностика холодного кадра.
            eprintln!(
                "[frame:wgpu]   faces-sub: hash {:5.2}ms prefetch {:6.2}ms resolve {:5.2}ms imgpre {:6.2}ms",
                t_sub0.as_secs_f64() * 1000.0,
                (t_sub_prefetch - t_sub0).as_secs_f64() * 1000.0,
                (t_sub_resolve - t_sub_prefetch).as_secs_f64() * 1000.0,
                (t_after_faces - t_sub_resolve).as_secs_f64() * 1000.0,
            );
        }
