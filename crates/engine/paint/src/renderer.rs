...
impl PipelineDeps {
    ...
    ///
    /// Порядок — по измеренной цене на DX12/Intel: `filter`+`blur` стоили
    /// 0.79 с из 0.9 с самого дорогого кадра прокрутки `lenta.ru`, поэтому они
    /// первыми — прогрев обязан успеть закрыть именно их до первой прокрутки.
