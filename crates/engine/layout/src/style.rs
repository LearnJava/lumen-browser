...
/// Собирает текстовое содержимое элемента (все Text-потомки в порядке DOM).
/// Используется для получения «raw value» `<textarea>` в Phase 0.
fn element_text_content(doc: &Document, node: NodeId) -> String {
...
fn collect_text(doc: &Document, node: NodeId, out: &mut String) {
    for &child in &doc.get(node).children {
        match &doc.get(child).data {
            ...
            NodeData::Element { .. } => collect_text(doc, child, out),
            _ => {}
        }
    }
}
...
///
/// Возвращает `Some(true)` если элемент является кандидатом для constraint
/// validation и удовлетворяет всем ограничениям, `Some(false)` если нарушает
/// хотя бы одно, `None` если элемент вообще не является form-control-кандидатом.
///
/// Phase 0 — pure attribute-based, без JS `setCustomValidity()` и без
/// runtime form state (пользовательские изменения через UI).
///
/// Проверяемые ограничения:
///   - `valueMissing`: `required` + пустое/отсутствующее `value`
///   - `typeMismatch`: `type=email` и значение не соответствует базовому
///     email-синтаксису (непустая localpart + `@` + непустой domain)
///   - `typeMismatch`: `type=url` и значение не начинается с `<scheme>://`
///   - `rangeUnderflow`/`rangeOverflow`: повторно использует `matches_in_range`
fn form_validity(doc: &Document, node: NodeId) -> Option<bool> {
    ...
    // Только input/select/textarea могут быть кандидатами для constraint
    // validation. Disabled — «barred from constraint validation» (HTML5 §4.10.21.2).
    let t_lower;
    let (is_input, t_str) = match tag {
        "input" => {
            t_lower = input_type_lower(doc, node);
            let t = t_lower.as_str();
            // Barred input types: hidden, button, submit, reset, image.
    ...
    // --- valueMissing ---
    // required + (value absent OR empty after trimming)
    if node_ref.get_attr("required").is_some() {
        let missing = if is_input {
            ...
            // textarea «value» — текстовое содержимое (между тегами).
            element_text_content(doc, node).trim().is_empty()
        } else {
            // select: нет selected option с непустым value (Phase 0: упрощение)
    ...
    if is_input {
        ...
        if !value.is_empty() {
            // --- typeMismatch: email ---
            if t_str == "email" && !is_valid_email_syntax(value) {
                return Some(false);
            }
            // --- typeMismatch: url ---
            if t_str == "url" && !is_valid_url_syntax(value) {
                return Some(false);
            }
        }
        // --- rangeUnderflow / rangeOverflow ---
        if let Some(false) = matches_in_range(doc, node) {
            return Some(false);
        }
    }
...
/// Базовая проверка email-синтаксиса (HTML5 §4.10.5.1.5, «valid e-mail address»).
/// Phase 0: localpart не пуст + `@` + domain с хотя бы одной `.` между
/// непустыми частями. Не реализует полный RFC 5322 — для UA достаточно.
fn is_valid_email_syntax(value: &str) -> bool {
    // Multiple addresses (multiple attr) не поддерживаем в Phase 0.
    let value = value.trim();
    ...
    // Domain must contain at least one dot separating two non-empty parts.
...
/// Базовая проверка URL-синтаксиса (HTML5 §4.10.5.1.15, «valid URL»).
/// Phase 0: наличие признанной схемы и `://`. Полный URL-парсер — отдельная задача.
fn is_valid_url_syntax(value: &str) -> bool {
    let value = value.trim();
    // Accept common schemes: http, https, ftp, ftps, ws, wss, file, data, blob.
    ...
    // data: and javascript: URIs don't use ://
    if let Some(pos) = value.find(':') {
        let scheme = &value[..pos];
        if matches!(scheme, "data" | "javascript" | "mailto" | "tel") {
...
fn parse_radius_length(val: &str, em_basis: f32, viewport: Size, is_quirks: bool) -> Option<Length> {
    let len = parse_length_q(val, is_quirks)?;
    match len {
