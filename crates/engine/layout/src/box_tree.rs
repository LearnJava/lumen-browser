...
mod tests {
    ...
    #[test]
    fn nested_svg_viewbox_scaling() {
        // Outer <svg> has viewBox, inner <svg> has different viewBox.
        // Inner viewBox should apply scaling to its children separately.
        let html = r#"
            <svg viewBox="0 0 100 100" width="100" height="100">
                <rect x="0" y="0" width="50" height="50" />
                <svg viewBox="0 0 50 50" width="50" height="50" x="50" y="50">
        ...
        // Find the root SVG shape.
        fn find_svg_shape_count(b: &super::LayoutBox) -> usize {
            let mut count = 0;
            if matches!(b.kind, super::BoxKind::SvgShape { .. }) {
                count += 1;
            }
            for child in &b.children {
                count += find_svg_shape_count(child);
            }
            count
        }

        // Expect: outer rect + inner rect = 2 SvgShape boxes
        let shape_count = find_svg_shape_count(&root);
        assert_eq!(shape_count, 2, "expected 2 SvgShape boxes (outer rect + inner rect)");
    }

    #[test]
    fn nested_svg_transform_composition() {
        // Outer <svg> has transform attr, inner <svg> has another transform.
        // Transforms should compose: inner = outer.compose(inner).
        let html = r#"
            <svg viewBox="0 0 100 100" width="100" height="100" transform="scale(2)">
                <svg viewBox="0 0 50 50" width="50" height="50" x="0" y="0" transform="translate(10, 10)">
                    <rect x="0" y="0" width="25" height="25" />
        ...
        // Verify structure builds without panic (transform composition works).
        assert!(!root.children.is_empty(), "layout should produce non-empty tree");
    }

    #[test]
    fn nested_svg_preserveAspectRatio() {
        // Inner <svg> with preserveAspectRatio should preserve aspect ratio
        // independently from outer <svg>.
        let html = r#"
            <svg viewBox="0 0 100 100" width="100" height="100">
        ...
        // Verify inner SVG layout completes without panic.
        assert!(!root.children.is_empty(), "nested SVG with preserveAspectRatio should layout");
    }

    #[test]
    fn deeply_nested_svg_viewbox_cascade() {
        // Three levels of nesting: each <svg> has its own viewBox.
        // viewBox should cascade properly at each level.
        let html = r#"
            <svg viewBox="0 0 200 200" width="200" height="200">
                <svg viewBox="0 0 100 100" width="100" height="100" x="0" y="0">
                    <svg viewBox="0 0 50 50" width="50" height="50" x="0" y="0">
        ...
        // Verify triple-nested SVG structure.
        fn count_svg_roots(b: &super::LayoutBox) -> usize {
            let mut count = if matches!(b.kind, super::BoxKind::SvgRoot { .. }) { 1 } else { 0 };
            for child in &b.children {
                count += count_svg_roots(child);
            }
            count
        }

        let svg_count = count_svg_roots(&root);
        assert_eq!(svg_count, 3, "expected 3 SvgRoot elements (outer + 2 nested)");
    }

    #[test]
    fn nested_svg_group_with_transform() {
        // <g> group with transform inside nested <svg>.
        // Transform should apply to group's children.
        let html = r#"
            <svg viewBox="0 0 100 100" width="100" height="100">
                <svg viewBox="0 0 50 50" width="50" height="50" x="0" y="0">
                    <g transform="scale(2)">
        ...
        // Verify group + rect structure.
        fn count_shapes(b: &super::LayoutBox) -> usize {
            let mut count = if matches!(b.kind, super::BoxKind::SvgShape { .. }) { 1 } else { 0 };
            for child in &b.children {
                count += count_shapes(child);
            }
            count
        }

        let shape_count = count_shapes(&root);
        assert_eq!(shape_count, 1, "expected 1 rect inside group");
    }
}
