...
impl PageSource {
    ...
    pub(crate) fn load(
        ...
        // This fallback path (headless/tests — no GpuSession window) has no
        // access to the App's session-scoped stores, same as `cache_backend`
        // below (also always `None` here); the live-tab path is
