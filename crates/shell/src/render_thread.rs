...
//!
//! # Результат GL-threading spike (M1.1, Windows, 2026-07-10)
//!
//! ADR-016 требовал сначала «спайкнуть» создание GL-контекста вне главного
//! потока. Замер на этой машине (winit 0.30 + glutin + femtovg): **создание
//! бэкенда прямо на рендер-потоке падает** с `the underlying handle is not
//! available` — winit отдаёт Win32 window handle только на потоке, где окно
//! создано (главном). Прокси корректно **откатывается на in-process** путь
//! (сообщение в stderr, регрессии нет — окно рисует как обычно). Вывод:
//! контекст обязан **создаваться на main** (где handle валиден), а затем
//! *передаваться* рендер-потоку (`make_not_current` на main → `make_current`
//! на потоке — ровно для этого у `FemtovgBackend` есть ручной `unsafe impl
//! Send`). Эта передача контекста — правка в `lumen-paint` (femtovg-бэкенд) и
//! составляет следующий срез **M1.2**. Текущий срез поставляет проверенную
//! инфраструктуру прокси/коммитов/коалесцинга и безопасный фолбэк.
...
impl ThreadedRenderBackend {
    ...
    ///
    /// `ctor` вызывается **на рендер-потоке** и создаёт настоящий бэкенд там же
    /// (femtovg `Canvas` + GL-контекст `!Send` обязаны создаваться на потоке, где
    /// будут использоваться). Возвращает `Err(msg)`, если бэкенд не создался —
    /// вызывающая сторона откатывается на in-process путь.
    ///
    /// # Errors
    /// Возвращает строку с описанием, если конструктор бэкенда вернул ошибку
    /// (например, GL-контекст не создался на не-главном потоке — тогда shell
...
{
    // GL-контекст + femtovg Canvas создаются здесь, на рендер-потоке (ADR-016 M1
    // spike: доказать, что glutin/femtovg работает вне главного потока).
