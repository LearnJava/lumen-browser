...
pub(crate) fn run_dump(
    ...
    match kind {
        ...
        DumpKind::Layout => {
            let vp = dump_vp;
            let parsed = parse_and_layout(&raw.bytes, raw.content_type.as_deref(), &raw.base, &event_sink, vp, &mut std::collections::HashSet::new(), None, None, None, None, &NullHyphenationProvider, false, deterministic::DetConfig::default(), false, None, false, None, None, lumen_core::ColorSpace::Srgb, false, raw.csp_header.as_deref(), raw.sync_xhr_document_policy, raw.sync_xhr_permissions_policy)?;
