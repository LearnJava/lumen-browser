...
//!
//! Twelve lists exercise disc/circle/square/decimal/lower-alpha/lower-roman markers
//! plus inside-position, `none`, `::marker { color }`, `::marker { content }`, and
//! two `@counter-style` lists (bracket/hashnum — CSS Counter Styles L3).
...
//! `list-style-type:none` list → 30 markers for 32 items.
...
fn test_32_list_markers() {
    ...
    // 12 lists × items: 6 lists of 3 + 4 lists of 2 + 2 custom-counter-style lists of 3 = 32 <li>.
    // (inside, none, custom-marker, content-marker + @counter-style bracket/hashnum)
    let lis = session.all_layout_boxes_by_selector("li").expect("query li");
    assert_eq!(lis.len(), 32, "expected 32 li boxes");
    ...
    // flat snapshot. 6 regular×3 + 2 inside + 2 custom-color + 2 content + 6 counter-style = 30
    ...
    let markers = snap
        ...
        .count();
    assert_eq!(markers, 30, "expected 30 marker boxes (32 items − 2 in the none list)");
}
