#!/usr/bin/env python3
"""
Оркестратор задач Lumen (Windows-only вариант).

Снимок orchestrator.py до добавления поддержки Linux/BSD (коммит
c164753a, 2026-07-13). Восстановлен по просьбе пользователя после того,
как переделка под Linux сопровождалась ошибками при запуске на Windows.
Актуальная кроссплатформенная версия — scripts/orchestrator.py.

Автоматический запуск сессий Claude Code для разработчиков P1-P5.
Каждая задача — отдельная сессия с чистым контекстом.

Использование:
    python scripts/orchestrator_windows.py                              # БЕЗ аргументов — пошаговый мастер
    python scripts/orchestrator_windows.py P1                                   # один разработчик
    python scripts/orchestrator_windows.py P1 P2                                # два в параллель
    python scripts/orchestrator_windows.py P1 P2 P3 P4 P5                       # все пятеро
    python scripts/orchestrator_windows.py P1 --max-tasks 3                     # лимит задач
    python scripts/orchestrator_windows.py P1 --new                             # стартовать с нуля
    python scripts/orchestrator_windows.py P1 --model haiku                     # сразу на Haiku (alias)
    python scripts/orchestrator_windows.py P1 --fallback-model haiku            # резерв при лимите
    python scripts/orchestrator_windows.py P1 --model sonnet --fallback-model haiku   # стартуем на Sonnet, резерв Haiku
    python scripts/orchestrator_windows.py P1 --coders team --max-tasks 1       # цикл: Claude-брифы → кодеры → Claude-ревью
    python scripts/orchestrator_windows.py P5 --max-tasks 1                     # P5: один прогон ревизии
    python scripts/orchestrator_windows.py --stop P1                            # мягкая остановка
    python scripts/orchestrator_windows.py --stop-all                           # остановить всех
    python scripts/orchestrator_windows.py --status                             # статус всех
...
def dispatch_run(
    ...
    print()
    print("Для остановки: python scripts/orchestrator_windows.py --stop P1")
    print("Остановить всех: python scripts/orchestrator_windows.py --stop-all")
...
def _wiz_equiv_cmd(developers, max_tasks, fallback_preset, force_new, initial_model,
    ...
    parts = [f"python scripts/orchestrator_windows.py {' '.join(developers)}"]
    if max_tasks > 0:
