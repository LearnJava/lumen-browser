# A/B pixel diff for the static/animated split (p1-exp-wgpu-only, §2).
#
#   python scripts/exp/anim_split_ab_diff.py <page.html> [scroll_css_px]
#
# Runs the same binary twice — with the split and with LUMEN_NO_ANIM_SPLIT=1
# (the scroll compositor is ON in both runs) — drives the window to the same
# scroll position over MCP-live, captures both via PrintWindow
# (capture_pid.ps1) and reports the pixel difference.
#
# Deterministic ONLY for scroll-driven animations (animation-timeline:
# scroll()/view()); time-driven content (GIF, CSS time animations) will
# show phase differences between the two runs — filter those by eye.
#
...
def run_one(page, scroll_px, disable_split, out_png):
    env = dict(os.environ)
    ...
    if disable_split:
        env["LUMEN_NO_ANIM_SPLIT"] = "1"
    else:
        env.pop("LUMEN_NO_ANIM_SPLIT", None)
...
def main():
    page = sys.argv[1] if len(sys.argv) > 1 else "samples/bench-anim-scroll.html"
    scroll_px = int(sys.argv[2]) if len(sys.argv) > 2 else 3600
    Path(".tmp").mkdir(exist_ok=True)
    a, b = ".tmp/ab_split_on.png", ".tmp/ab_split_off.png"
    ...
    if ndiff:
        diff.save(".tmp/ab_split_diff.png")
        print("[abdiff] diff mask: .tmp/ab_split_diff.png")
