...
def dispatch_run(
    ...
    if len(developers) == 1:
        run_task_loop(
            developers[0], max_tasks, fallback_preset, force_new, initial_model, coders_mode,
        )
        return
    ...
    for dev in developers:
        cmd = f'python "{script}" {dev}{max_arg}{fb_arg}{new_arg}{model_arg}{coders_arg}'
        title = f"Lumen {dev}"
        if os.name == "nt":
...
def _wiz_equiv_cmd(developers, max_tasks, fallback_preset, force_new, initial_model, coders_mode) -> str:
...
def run_wizard() -> None:
    ...
    try:
        ...
        cmd = _wiz_equiv_cmd(developers, max_tasks, fallback_preset, force_new, initial_model, coders_mode)
        print()
        print("-" * 60)
        ...
        dispatch_run(developers, max_tasks, fallback_preset, force_new, initial_model, coders_mode)
    except (EOFError, KeyboardInterrupt):
        print()
