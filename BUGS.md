...
### BUG-008 · `<del>`, `<ins>`, `<u>`, `<s>` — text-decoration не применялся

**Статус:** FIXED 2026-05-15 (ветка `ua-stylesheet-extend`).  
Добавлен UA-stylesheet `ua_text_decoration_line` в `crates/engine/layout/src/style.rs`: `<u>` / `<ins>` → underline, `<s>` / `<del>` / `<strike>` → line-through. Применяется в `compute_style` после `ua_font_style` / `ua_font_weight`, перед author CSS — так что любая author-декларация (включая inline) перекрывает. 5 юнит-тестов (`ua_u/ins/s/del/strike_*`).

### BUG-009 · `<a>` без UA-стилей (цвет + underline)

**Статус:** FIXED 2026-05-15 (ветка `ua-stylesheet-extend`).  
`<a>` с атрибутом `href` получает синий `#0000EE` (HTML5 Rendering §15.3.2) и underline через `ua_color` + `ua_text_decoration_line`. Named anchor (`<a name="x">` без href) UA-стилей не получает — цвет наследуется, текст без подчёркивания. `:visited` (`#551A8B`) не реализован — отдельная задача, нужно отслеживание истории посещений. 4 юнит-теста (`ua_a_with_href_*`, `ua_a_without_href_*`, `author_css_overrides_ua_link_color`, `inline_style_overrides_ua_link_decoration`).
