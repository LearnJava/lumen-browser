...
BUG-067 | OPEN  | js     | document_pip_* тесты падают: «Document Picture-in-Picture API init failed: Exception generated by QuickJS» — 7 unit-тестов в dom::tests всегда возвращают false | crates/js/src/dom.rs (document_pip module)
BUG-068 | OPEN  | shell  | clippy lumen-shell: reader_view.rs:292,309 «collapsible_if» — два nested if-let схлопываемы; pre-existing с D-3, блокирует clippy -D warnings | crates/shell/src/reader_view.rs:292
BUG-069 | OPEN  | layout | collect_picture_unsupported_type_falls_back: HEIC source не скипается при fallback к img src — `collect_picture_sources` возвращает heic-source вместо jpg | crates/engine/layout/src/lib.rs:12835
BUG-070 | OPEN  | js     | document_pip тесты (7 шт.) падают: «Document Picture-in-Picture API init failed: Exception generated by QuickJS» — pre-existing с p1-document-pip | crates/js/src/dom.rs:5000
BUG-071 | OPEN  | mcp    | `MockSession` в lumen-mcp не реализует методы `set_clock`, `set_rng_seed`, `freeze_fingerprint` из трейта `BrowserSession` (добавлены P1 в N-2 deterministic mode) — компиляция `--workspace` падает с E0046 | crates/mcp/src/server.rs:508
```
