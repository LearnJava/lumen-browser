...
| [BUG-224](bugs/BUG-224-OPEN.md) | OPEN | layout | Multi-column `column-span:all` auto-height регрессия: `test_33_multi_column` падает на чистом main (`.mc[4] should be 660x88, got 660x64`) после влития p3-bug198 (box_tree.rs ~204 строки). Пиксельно TEST-33 ≈0.10% (малозаметно), но расчётная высота расходится с ground-truth. |
