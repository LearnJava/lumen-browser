...
BUG-125 | OPEN  | paint | INTERACTION TEST-100 (transform×overflow) 9.57%: клиппинг трансформированных детей контейнером overflow:hidden расходится с Edge во всех 6 ячейках, включая поворот самого клип-контейнера; диагностика: run.py --bisect 100 | crates/engine/paint/src/display_list.rs
BUG-126 | OPEN  | paint | INTERACTION TEST-101 (border-radius×overflow) 4.04%: overflow:hidden у скруглённого бокса не режет детей по скруглённому контуру (углы бара, круг из квадрата, pill, вложенные радиусы); --bisect 101 | crates/engine/paint/src/display_list.rs
BUG-127 | OPEN  | paint | INTERACTION TEST-102 (opacity×z-index) 17.04%: контрольные ячейки c2/c5 проходят, падают group-opacity сценарии — двойное затемнение перекрытия внутри opacity-группы, z-index не заперт в opacity stacking context, вложенная opacity 0.6×0.5 ≠ 0.3; --bisect 102 | crates/engine/paint/src/compositor.rs
BUG-128 | OPEN  | paint | INTERACTION TEST-103 (filter×transform) 29.11%: фильтр на трансформированном элементе и filter как containing block расходятся во всех ячейках, включая контроль c5 (rotate+gradient без фильтра); --bisect 103 | crates/engine/paint/src/display_list.rs
BUG-129 | OPEN  | paint | INTERACTION TEST-104 (mask×gradient×radius) 51.97%: градиентная маска поверх градиентного фона/скруглений/бордера расходится во всех ячейках, включая контроль без маски (gradient+radius) — вероятно два независимых дефекта; --bisect 104 | crates/engine/paint/src/display_list.rs
BUG-130 | OPEN  | layout | INTERACTION TEST-105 (float/clear×margin) 4.84%: отступы флоатов, clearance+margin-top, перенос флоатов на новую строку и фон in-flow соседа под флоатом расходятся (включая контроль c5 — обычные блоки с margin внутри ячейки); --bisect 105 | crates/engine/layout/src/box_tree.rs
BUG-131 | OPEN  | paint | INTERACTION TEST-106 (transform×z-index) 4.02%: transformed-элемент как stacking context — negative z внутри, порядок z-детей в rotate/scale-родителе; контроль c5 без transform тоже в diff-области (см. BUG-133 как вероятную общую причину); --bisect 106 | crates/engine/paint/src/display_list.rs
BUG-132 | OPEN  | paint | INTERACTION TEST-107 (shadow×radius×overflow) 2.18%: силуэт box-shadow у скруглённого бокса и клип тени родителем с overflow:hidden расходятся; --bisect 107 | crates/engine/paint/src/display_list.rs
BUG-133 | OPEN  | paint | INTERACTION TEST-108 (вложенные transform) 4.62%: ВИЗУАЛЬНО ПОДТВЕРЖДЕНО — дочерний transform не композируется с родительским: c0 rotate(15)∘rotate(−15) даёт повёрнутый бокс вместо осевого, c4 три вложенных rotate(10deg) дают ~10° вместо 30°, c3 ребёнок игнорирует translate родителя, c2 scale родителя не масштабирует translate ребёнка. Вероятная общая причина для части BUG-125/128/131/134 — чинить первым; --bisect 108 | crates/engine/paint/src/property_trees.rs
BUG-134 | OPEN  | paint | INTERACTION TEST-109 (clip-path×transform×radius) 14.10%: clip-path не переносится сквозь transform элемента (circle на rotate, inset на scale, polygon на translate), клип родителя не режет transformed-ребёнка, clip-path∩border-radius; --bisect 109 | crates/engine/paint/src/display_list.rs
