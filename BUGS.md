...
BUG-119 | OPEN   | layout/cascade | selector rule index (P1 bb1f8e99) causes 6 run.py regressions: TEST-27(rtl +1.9%), TEST-28(containment +0.8%), TEST-29(container-q +1.24%), TEST-40(conic +1.3%), TEST-41(table +1.68%), TEST-68(font-var +1.05%). All were PASS before rule index. RuleIndex skips some CSS rules that match; investigate candidates() coverage for these pages. | crates/engine/layout/src/rule_index.rs
