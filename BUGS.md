...
BUG-116 | OPEN   | layout         | auto table layout: columns split available width equally instead of CSS 2.1 §17.5.2 content-based distribution (min/max-content per column, distribute extra). compute_table_col_widths has no text measurer, so it cannot size columns to content. TEST-64 residual ~15% — Age column too wide pushes City column ~95px right of Edge. Needs measurer threaded through lay_out_table → compute_table_col_widths + per-column intrinsic width pass. | crates/engine/layout/src/box_tree.rs:4670
...
TEST-64: FAIL 14.90%   table                   ← BUG-116 (was BUG-106 24.85%; heading UA fixed, residual = auto column widths)
