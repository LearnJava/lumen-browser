...
BUG-023 | OPEN        | paint           | opacity property completely ignored (renders at 100%)
...
**Скриншот:** `graphic_tests/screenshots/13-*.png` *(не в репо)*

**Описание:**
Свойство `opacity` не применяется. Все элементы рендерятся с альфой 1.0 независимо от значения.

- 6 div-ов с `opacity` от 0.1 до 1.0 — в Lumen все одного цвета (полностью непрозрачные).
- 3 вложенных div-а с `opacity` 0.2/0.5/1.0 на группе — в Lumen все три рисуются полностью.

**Воспроизведение:**
```html
<div style="background: #3182ce; opacity: 0.2;"></div>  <!-- должно быть бледно-голубым -->
<div style="background: #3182ce; opacity: 1.0;"></div>  <!-- должно быть ярко-синим -->
```
**Ожидается:** первый div бледный, второй насыщенный.
**Факт:** оба одинаково насыщенного цвета.

**Где смотреть:** `lumen-paint::display_list` — `emit_box` / `emit_opacity_layer`; проверить, создаётся ли `PushLayer`/`PopLayer` с alpha при `opacity < 1.0`. `lumen-layout::style.rs` — проверить, что поле `opacity: f32` корректно парсится из каскада.
