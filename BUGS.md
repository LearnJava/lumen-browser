...
BUG-088 | OPEN   | css-parser/layout | individual CSS transform properties (translate/rotate/scale) rendering diverges — TEST-46: 4.63% (improved from 9.57%); code fully implemented in apply_declaration + property_trees, remaining gap likely antialiasing difference between Edge and Lumen (pixel-snapping scope) | crates/engine/layout/src/style.rs:10832–10866, property_trees.rs:679–687
...
BUG-132 | OPEN  | paint | INTERACTION TEST-101 (border-radius×overflow) 4.04%: overflow:hidden у скруглённого бокса режет детей прямоугольником вместо скруглённого клипа (PushClipRect использует scissor-test, а не маску); нужна новая команда PushClipRoundedRect в DisplayCommand + реализация в femtovg_backend | crates/engine/paint/src/display_list.rs:2197, backends/femtovg_backend.rs:1417–1421
