...
BUG-118 | OPEN   | test/snapshot  | snapshot_cpu reference PNGs outdated for 12 pages (39-gradients, 40-conic-gradients, 41-table, 30-css-filter, 46-individual-transforms, 28-css-containment, 29-container-queries, 32-list-markers, 27-direction-rtl, 34-forms, 24-vertical-align, 54-svg-path-stroke): references were last saved before BUG-117/107/106/096 fixes, causing stale diffs. Verify current render is correct, then regenerate: SAVE_CPU_SNAPSHOTS=1 cargo test -p lumen-driver --features cpu-render | graphic_tests/snapshots/cpu/
