...
BUG-072 | OPEN  | js     | Form Constraint Validation API init failed: FORM_VALIDATION_SHIM ссылается на bare `HTMLInputElement`/`HTMLTextAreaElement`/`HTMLSelectElement`/`HTMLButtonElement` (строки 169-172) — в install_dom эти конструкторы не определены глобально → ReferenceError «HTMLInputElement is not defined», шим не устанавливается. Нужны typeof-гварды | crates/js/src/form_validation.rs:169
BUG-073 | OPEN  | js     | chrome_runtime_absent (no_automation_markers.rs) падает: D-6 extension-stub в WEB_API_SHIM безусловно ставит window.chrome.runtime, ломая anti-CDP-detection маркер. Конфликт extension-поддержки и anti-fingerprint — нужно решение P1/P2 (exposing chrome.runtime только при загруженном расширении) | crates/js/src/dom.rs:10131
```
