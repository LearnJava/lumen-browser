...
BUG-120 | OPEN   | layout/text    | C0 control chars (e.g. U+0001) in body text render as a visible 1-line text box (19.2px line at 16px font) — Edge/Chromium renders them invisible/zero-advance, no line box. Divergence discovered via BUG-119 (corrupted test pages shifted content 20px in Lumen but not in Edge). Consider stripping Cc chars (except tab/LF/CR) at inline-item level. | crates/engine/layout/src/box_tree.rs
BUG-121 | OPEN   | test/driver    | snapshot_vs_edge gate (added 88cdb9e1, commit msg says "informational, always passes" but the code asserts) is red on main: ~25 pages over the flat 0.5% limit when Edge screenshots are present locally (18-images 57%, 33-multicol 14.9%, 30-filter 10%, 28-containment 2.24%, …) — CPU rasterizer text-AA/image divergence vs Edge with no text exemption and no per-page thresholds like run.py TESTS. `cargo test -p lumen-driver` (default features) fails on any machine with captured screenshots. Needs per-page thresholds or true informational mode. | crates/driver/tests/snapshot_vs_edge.rs
```
