...
BUG-149 | OPEN  | test/snapshot | snapshot_cpu красный на main (f22e2204, без локальных изменений): 21-border-style — 36780 differing bytes, 33-multi-column — 738; эталоны устарели после недавних мержей (рендер изменился без регенерации references, аналогично BUG-118). Регенерировать SAVE_CPU_SNAPSHOTS=1 после визуальной проверки, что новый рендер корректен | graphic_tests/snapshots/cpu/
