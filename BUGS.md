...
| [BUG-171](bugs/BUG-171-OPEN.md) | OPEN | shell | **Этап 1 ✅ 2026-06-19** (префетч-кэш `prefetch::PREFETCH_CACHE`): внешние скрипты + linked CSS теперь прогреваются в фоне preload-сканером во время streaming и читаются на UI-потоке мгновенно (та же `fetch_subresource`-проводка → те же байты, каскад/порядок скриптов не нарушены; in-flight дедуп + поколения навигации). Сетевое ожидание снято с UI-потока; adblock-`reload()` уже асинхронный (U-1 этап 1). **Остаётся этап 2**: JS-исполнение (~1.9 с) + layout всё ещё синхронно в `parse_and_layout` на UI → окно ещё морозится на CPU-фазе. Это ADR-006/BrowserSession (весь пайплайн, включая QuickJS, на per-tab поток) — разблокирован B-1, но крупная отдельная задача. Картинки/шрифты в кэш не заведены (этап 2 / BUG-172). | crates/shell/src/main.rs:6369 |
