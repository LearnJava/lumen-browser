...
- ✅ SVG layout pass (viewBox, rect/circle/ellipse/line/path, `<use>` with cycle detection); `<text>` with `text-anchor`/`dominant-baseline`/`baseline-shift` (presentation attribute **and** CSS property — CSS overrides the attribute; `text-anchor`/`dominant-baseline` inherit from `<g>`, `baseline-shift` is non-inherited); vertical writing modes (`vertical-rl/lr`).
