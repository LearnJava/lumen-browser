...
**Automation surfaces** — ✅ `--devtools-port` (CDP), `--bidi-port` (in-shell BiDi stub), headless `--dump-source`/`--dump-layout`/`--dump-display-list`, **`--screenshot <out.png> <url>`** (full-page deterministic CPU snapshot via `cpu-render`, no window/Edge/ffmpeg), **`--ipc-server`** (headless tab-control IPC: `CreateTab`/`NavigateTab`/`Screenshot`/`CloseTab` over TCP loopback, PNGs without gdigrab — TAB-4/5), `--print-to-pdf`; **live-window automation channel** (SDC-1b, 8A.7 Ф4): `AutomationCommand`/`AutomationReply` (`lumen-driver`) drained each frame in the shell's `ApplicationHandler` — `Navigate`/`Click`/`Type`/`Scroll` (incl. `Target::Selector` resolution via `lumen_layout::selector_query`) act on the live tab; `Eval` returns the real JS value as JSON; `Screenshot` renders the current content display-list to PNG (CPU path); `Wait` polls once per frame without blocking the event loop.
