...
`crates/js/src/canvas2d.rs:251` **fn** `flush_dirty` — Drain dirty canvases and return their current RGBA buffers
`crates/js/src/canvas2d.rs:278` **fn** `install_canvas2d_bindings` — Register the `_lumen_canvas2d_*` native functions on `globals`
...
`crates/js/src/file_input.rs:50` **fn** `register_file_token` — Register a file path and return an opaque token for JS access
`crates/js/src/file_input.rs:59` **fn** `clear_file_registry` — Revoke all tokens — should be called when a browsing context is torn down
`crates/js/src/file_input.rs:134` **fn** `install_file_input_bindings` — Install File / FileList classes, native read bindings, and `_lumen_deliver_file_list`
...
`crates/js/src/lib.rs:147` **fn** `deterministic_seed_from_url` — Compute a deterministic u64 seed from a URL for deterministic render mode (8F)
`crates/js/src/lib.rs:161` **struct** `QuickJsRuntime` — QuickJS-based JS runtime via `rquickjs`
`crates/js/src/lib.rs:329` **fn** `new`
`crates/js/src/lib.rs:406` **fn** `register_module_source` — Register an ES module by specifier so it can be `import`-ed by other modules
`crates/js/src/lib.rs:422` **fn** `set_import_map` — Set the import map (HTML LS §8.1.6.2) used by the module resolver
`crates/js/src/lib.rs:432` **fn** `eval_module` — Evaluate `source` as an ES module (HTML LS §8.1.3 `<script type=module>`)
`crates/js/src/lib.rs:502` **fn** `install_dom` — Install DOM Web API globals (`document`, `window`, `console`, etc.) into
`crates/js/src/lib.rs:1294` **fn** `set_cookie_banner_dismiss` — Enable or disable cookie-banner auto-dismiss for subsequent `install_dom` calls
`crates/js/src/lib.rs:1303` **fn** `set_deterministic_mode` — Enable deterministic render mode (8F)
`crates/js/src/lib.rs:1320` **fn** `freeze_fingerprint` — Freeze fingerprint APIs for canvas / audio / font enumeration (8F.3)
`crates/js/src/lib.rs:1363` **fn** `pump_workers` — Deliver messages posted by worker threads to their `Worker` JS instances
`crates/js/src/lib.rs:1388` **fn** `flush_canvas_updates` — Drain dirty Canvas 2D buffers for upload to the renderer
`crates/js/src/lib.rs:1402` **fn** `pump_broadcast_channels` — Deliver messages posted to this page's `BroadcastChannel` instances
`crates/js/src/lib.rs:1427` **fn** `pump_shared_workers` — Deliver messages posted by `SharedWorker` threads to this page's ports
`crates/js/src/lib.rs:1446` **fn** `take_navigate_request` — Consume any navigation request that JS placed via `location.href =` etc
`crates/js/src/lib.rs:1456` **fn** `take_history_url_updates` — Drain `history.pushState` / `history.replaceState` URL-update notifications
`crates/js/src/lib.rs:1467` **fn** `take_fullscreen_requests` — Drain all fullscreen requests queued by `element.requestFullscreen()` and
`crates/js/src/lib.rs:1475` **fn** `take_view_transition_events` — Drain all View Transition events queued by `document.startViewTransition`
`crates/js/src/lib.rs:1483` **fn** `take_dom_dirty` — Returns `true` if JS mutated the DOM since the last call, clearing the flag
`crates/js/src/lib.rs:1492` **fn** `take_raf_pending` — Returns `true` if `requestAnimationFrame` was called since the last call,
`crates/js/src/lib.rs:1500` **fn** `has_raf_pending` — Non-consuming peek: `true` if `requestAnimationFrame` callbacks are queued
`crates/js/src/lib.rs:1509` **fn** `take_timer_wakeup` — Take the next timer wakeup as Unix epoch ms, clearing the stored value
`crates/js/src/lib.rs:1518` **fn** `update_layout_rects` — Replace the layout bounding-rect table with a fresh snapshot
`crates/js/src/lib.rs:1526` **fn** `update_viewport_size` — Update the viewport dimensions
`crates/js/src/lib.rs:1535` **fn** `take_lazy_image_requests` — Drain lazy image load requests queued by `_lumen_request_lazy_image_load` in JS
`crates/js/src/lib.rs:1547` **fn** `update_scroll_states` — Replace the scroll-state table with a fresh snapshot from the layout tree
`crates/js/src/lib.rs:1556` **fn** `take_scroll_requests` — Drain JS-initiated scroll requests queued by `_lumen_request_scroll`
`crates/js/src/lib.rs:1563` **fn** `take_page_scroll_requests` — Drain JS page-level scroll requests from `window.scrollTo/scrollBy/scroll`
`crates/js/src/lib.rs:1569` **fn** `set_page_scroll_y` — Update the page scroll Y exposed to JS `window.scrollY / pageYOffset`
`crates/js/src/lib.rs:1578` **fn** `take_notification_requests` — Drain all OS notification requests queued by `new Notification(...)` in JS
`crates/js/src/lib.rs:1589` **fn** `take_window_open_requests` — Drain all popup window requests queued by JS `window.open(...)`
`crates/js/src/lib.rs:1598` **fn** `take_print_requests` — Drain all print requests queued by JS `window.print()` (W-2)
`crates/js/src/lib.rs:1607` **fn** `pointer_capture_nid` — Returns the DOM node nid that currently holds pointer capture (pointer_id=1)
`crates/js/src/lib.rs:1615` **fn** `take_pointer_capture` — Release the active pointer capture, returning the former capture target nid
`crates/js/src/lib.rs:1624` **fn** `take_console_messages` — Drain all `console.log/warn/error` messages queued since the last call
`crates/js/src/lib.rs:1633` **fn** `take_focus_requests` — Drain JS dialog focus requests queued by `_lumen_request_focus` / `_lumen_request_blur`
`crates/js/src/lib.rs:1642` **fn** `fire_dialog_close` — Close a `<dialog>` as the result of a `<form method="dialog">` submission
`crates/js/src/lib.rs:1659` **fn** `notify_focus_changed` — Notify the JS runtime that the shell moved keyboard focus to a new node
`crates/js/src/lib.rs:1675` **fn** `update_computed_styles` — Push a fresh snapshot of computed CSS styles into the JS runtime
`crates/js/src/lib.rs:1685` **fn** `set_document_visibility` — Update `document.hidden` / `document.visibilityState` and fire
`crates/js/src/lib.rs:1703` **fn** `notify_dom_content_loaded` — Transition `document.readyState` → `'interactive'` and fire
`crates/js/src/lib.rs:1715` **fn** `notify_window_loaded` — Transition `document.readyState` → `'complete'` and fire
`crates/js/src/lib.rs:1728` **fn** `deliver_scroll_progress` — Push viewport scroll progress into all active root-viewport `ScrollTimeline` instances
`crates/js/src/lib.rs:1746` **fn** `fire_element_scroll` — Fire a non-bubbling `scroll` Event on the DOM element identified by `nid`
`crates/js/src/lib.rs:1761` **fn** `fire_window_scroll` — Fire a non-bubbling `scroll` Event on the `window` object (page scroll)
`crates/js/src/lib.rs:1782` **fn** `fire_snap_changing` — Fire a CSS Scroll Snap L2 `snapchanging` event on a scroll container
`crates/js/src/lib.rs:1792` **fn** `fire_snap_changed` — Fire a CSS Scroll Snap L2 `snapchanged` event on a scroll container
`crates/js/src/lib.rs:1827` **fn** `deliver_long_animation_frame` — Deliver a Long Animation Frame (LoAF) entry to PerformanceObserver subscribers
`crates/js/src/lib.rs:1864` **fn** `run_gc_pass` — Tune the QuickJS GC based on the tab's lifecycle tier (10L)
...
`crates/engine/layout/src/box_tree.rs:1287` **struct** `ImageRequest` — Запрос на предзагрузку изображения: URL после picking-а по
`crates/engine/layout/src/box_tree.rs:1302` **fn** `collect_image_requests` — Обходит DOM и возвращает запросы на загрузку для всех `<img>`-элементов
`crates/engine/layout/src/box_tree.rs:1322` **fn** `collect_background_image_requests` — Обходит готовое layout-дерево и возвращает уникальные URL-ы из
`crates/engine/layout/src/box_tree.rs:1430` **struct** `LayoutBox`
`crates/engine/layout/src/box_tree.rs:1461` **struct** `InlineSegment` — Отрезок inline-контента с собственным стилем (до layout)
`crates/engine/layout/src/box_tree.rs:1501` **enum** `PseudoKind` — Marks an inline segment as the target of a CSS structural pseudo-element
`crates/engine/layout/src/box_tree.rs:1519` **struct** `InlineFrag` — Позиционированный текстовый фрагмент в строке (после layout)
`crates/engine/layout/src/box_tree.rs:1553` **enum** `BoxKind`
`crates/engine/layout/src/box_tree.rs:2171` **fn** `layout` — Lay out a document without a text measurer. For tests and headless dump modes
`crates/engine/layout/src/box_tree.rs:2195` **fn** `layout_measured` — Layout without a text measurer. For tests and headless modes; uses `layout_measured_hyp` with `dark_mode=false`
`crates/engine/layout/src/box_tree.rs:2208` **fn** `layout_measured_hyp` — Layout with a real hyphenation provider (for `hyphens: auto`)
`crates/engine/layout/src/box_tree.rs:2255` **fn** `lay_out_incremental` — Incremental re-layout pass: skips clean subtrees, re-lays out only dirty ones
`crates/engine/layout/src/box_tree.rs:2325` **fn** `build_iframe_document` — Parse inline HTML from an `<iframe srcdoc="...">` attribute (HTML spec §4.8.5)
`crates/engine/layout/src/box_tree.rs:7769` **fn** `resolve_auto_fill_fit_count` — CSS Grid Layout L3 §9 — Resolve `repeat(auto-fill|auto-fit, <track-list>)` count
`crates/engine/layout/src/box_tree.rs:7952` **fn** `measure_text_w` — Measures text width (letter_spacing applied between each character)
`crates/engine/layout/src/box_tree.rs:7971` **fn** `measure_text_w_families` — Как [`measure_text_w`], но учитывает CSS `font-family` каскад
`crates/engine/layout/src/box_tree.rs:8001` **fn** `measure_text_w_varied` — Как [`measure_text_w_families`], но учитывает CSS `font-variation-settings`
`crates/engine/layout/src/box_tree.rs:8911` **fn** `apply_container_styles` — CSS Container Queries L1: second-pass after layout
...
`crates/network/src/lib.rs:1649` **struct** `HttpProxy` — HTTP proxy configuration (RFC 7230 proxy behavior)
`crates/network/src/lib.rs:1661` **fn** `new` — Создать новый прокси без аутентификации
`crates/network/src/lib.rs:1670` **fn** `with_basic_auth` — Создать прокси с базовой аутентификацией (username:password)
`crates/network/src/lib.rs:1713` **struct** `HttpClient` — HTTP/1.1 + HTTPS клиент
`crates/network/src/lib.rs:1752` **fn** `new`
`crates/network/src/lib.rs:1778` **fn** `with_sink` — Подключить EventSink. По умолчанию sink-а нет (события не эмитятся)
`crates/network/src/lib.rs:1789` **fn** `with_filter` — Подключить RequestFilter. По умолчанию фильтра нет — `fetch` всегда
`crates/network/src/lib.rs:1801` **fn** `with_interceptor` — Подключить Service Worker перехватчик fetch-запросов. Проверяется
`crates/network/src/lib.rs:1810` **fn** `with_pool` — Подключить shared `ConnectionPool`. По умолчанию у каждого `HttpClient`
`crates/network/src/lib.rs:1820` **fn** `with_h2_pool` — Подключить shared `H2Pool` (RFC 9113 §9.1.1). По умолчанию HTTP/2
`crates/network/src/lib.rs:1829` **fn** `with_dns_resolver` — Подключить DNS-резолвер. По умолчанию — `SystemDnsResolver` (через
`crates/network/src/lib.rs:1846` **fn** `with_hsts` — Подключить HSTS-store (RFC 6797). По умолчанию — нет:
`crates/network/src/lib.rs:1862` **fn** `with_credentials` — Подключить credential-провайдер для HTTP authentication (RFC 7235 /
`crates/network/src/lib.rs:1873` **fn** `with_tab` — Указать `TabId`, который попадёт в каждое emit-ое событие. В Phase 0
`crates/network/src/lib.rs:1893` **fn** `with_mixed_content_policy` — Подключить mixed-content policy (W3C Mixed Content §5). По умолчанию
`crates/network/src/lib.rs:1917` **fn** `with_content_decoder` — Зарегистрировать `ContentDecoder` для одного encoding. Декодер попадает
`crates/network/src/lib.rs:1963` **fn** `with_cors_cache` — Запросить только диапазон байт ресурса (RFC 7233). Если сервер
`crates/network/src/lib.rs:1975` **fn** `with_cookie_jar` — Attach a cookie store. The provider receives `Cookie:` injection
`crates/network/src/lib.rs:1999` **fn** `with_http_cache` — Подключить HTTP response cache (RFC 7234)
`crates/network/src/lib.rs:2010` **fn** `with_proxy` — Подключить HTTP прокси (RFC 7230). По умолчанию прокси не подключён — запросы
`crates/network/src/lib.rs:2023` **fn** `with_socks5_proxy` — Подключить SOCKS5 прокси (RFC 1928) для туннелирования всех TCP-соединений
`crates/network/src/lib.rs:2034` **fn** `with_fingerprint_profile` — Установить HTTP fingerprinting profile (Standard/Strict/Tor) для Chrome-matching
`crates/network/src/lib.rs:2042` **fn** `fingerprint_profile` — Получить текущий HTTP fingerprinting profile
`crates/network/src/lib.rs:2053` **fn** `with_tls_profile` — Override the TLS fingerprint profile independently of the HTTP profile
`crates/network/src/lib.rs:2059` **fn** `tls_profile` — Получить текущий TLS fingerprinting profile
`crates/network/src/lib.rs:2093` **fn** `fetch_cors` — CORS-enabled fetch для cross-origin subresource (Fetch §3-§4)
`crates/network/src/lib.rs:2141` **fn** `fetch_range`
`crates/network/src/lib.rs:2208` **fn** `fetch_multi_range` — Multi-range запрос (RFC 7233 §4.1). Один request на несколько
`crates/network/src/lib.rs:2294` **fn** `fetch_subresource` — Загрузить подресурс с проверкой mixed-content по подключённой
`crates/network/src/lib.rs:2392` **fn** `fetch_conditional` — Perform a **conditional GET** (RFC 7232) and report whether the resource
`crates/network/src/lib.rs:2447` **enum** `ConditionalFetch` — Outcome of [`HttpClient::fetch_conditional`]
`crates/network/src/lib.rs:2467` **fn** `fetch_page` — Fetch a top-level page and return the response body together with all
`crates/network/src/lib.rs:2982` **struct** `InMemoryFetchInterceptor` — In-memory реализация `FetchInterceptor` для тестов без SQLite
`crates/network/src/lib.rs:2988` **fn** `new`
`crates/network/src/lib.rs:2995` **fn** `insert` — Добавить запись: ответ для (origin, url) берётся из кэша без сети
...
`crates/engine/paint/src/backends/femtovg_backend.rs:184` **struct** `FemtovgBackend` — femtovg/OpenGL рендер-бэкенд (Phase 2, ADR-010)
`crates/engine/paint/src/backends/femtovg_backend.rs:656` **fn** `new` — Создаёт оконный femtovg-бэкенд из winit-окна
...
`crates/engine/paint/src/display_list.rs:782` **type** `DisplayList`
`crates/engine/paint/src/display_list.rs:811` **fn** `fit_image_rect` — CSS Images L3 §5.5 — `object-fit` placement: где располагается
`crates/engine/paint/src/display_list.rs:866` **fn** `fit_image_quad` — Финальный GPU-quad для `<img>`: пересечение «полного» placement-rect
`crates/engine/paint/src/display_list.rs:939` **fn** `cull_display_list` — Returns `true` if the display list contains any `backdrop-filter` element
`crates/engine/paint/src/display_list.rs:970` **fn** `contains_backdrop_filter` — Cheap pre-check the renderer uses to decide whether computing a frame
`crates/engine/paint/src/display_list.rs:1006` **fn** `hash_display_list` — Computes a content hash over a frame's display list plus the viewport state
`crates/engine/paint/src/display_list.rs:1034` **struct** `DiffResult` — Результат сравнения двух display-list-ов
`crates/engine/paint/src/display_list.rs:1046` **fn** `identical` — Создаёт DiffResult для идентичных display list-ов
`crates/engine/paint/src/display_list.rs:1060` **fn** `changed` — Создаёт DiffResult для изменённых display list-ов с заданным bounding rect
`crates/engine/paint/src/display_list.rs:1076` **fn** `diff_display_lists` — Сравнивает два display list-а по Debug hash каждой команды
`crates/engine/paint/src/display_list.rs:1194` **fn** `serialize_display_list`
`crates/engine/paint/src/display_list.rs:1590` **fn** `build_display_list`
`crates/engine/paint/src/display_list.rs:1606` **fn** `build_display_list_with_selection` — Like [`build_display_list`] but applies `::selection` CSS highlight styles
`crates/engine/paint/src/display_list.rs:1624` **fn** `build_display_list_with_anim` — Like `build_display_list` but applies compositor animation overrides per node
`crates/engine/paint/src/display_list.rs:1660` **fn** `build_display_list_ordered` — Билдер display list-а, **уважающий painting order** (CSS 2.1 Appendix E)
`crates/engine/paint/src/display_list.rs:1671` **fn** `build_display_list_ordered_dpr` — Like [`build_display_list_ordered`] but resolves `image-set()` background
`crates/engine/paint/src/display_list.rs:1720` **fn** `build_display_list_ordered_with_anim` — Like [`build_display_list_ordered`] but applies compositor animation overrides per node
`crates/engine/paint/src/display_list.rs:1731` **fn** `build_display_list_ordered_with_anim_dpr` — Like [`build_display_list_ordered_with_anim`] but resolves `image-set()`
`crates/engine/paint/src/display_list.rs:1785` **fn** `build_print_display_list` — Builds a print display list from paginated layout
`crates/engine/paint/src/display_list.rs:1847` **fn** `split_at_page_breaks` — Splits a print display list at `PageBreak` markers
`crates/engine/paint/src/display_list.rs:1874` **fn** `strip_background_graphics` — Removes background-graphics paint commands from each print page when the
`crates/engine/paint/src/display_list.rs:2910` **fn** `is_image_set` — CSS Images L4 §5 — is `value` an `image-set()` / `-webkit-image-set()` expression?
`crates/engine/paint/src/display_list.rs:3039` **fn** `select_image_set_url` — CSS Images L4 §5 — selects the best `image-set()` candidate URL for `dpr`
`crates/engine/paint/src/display_list.rs:3763` **fn** `point_on_resize_grip` — Возвращает `true`, если точка (`px`, `py`) попадает в resize-grip элемента
`crates/engine/paint/src/display_list.rs:12932` **fn** `emit_text_with_highlights` — CSS Custom Highlight API L1 — helper to emit DrawText with highlight name
...
---
*Total: 3933 symbols in 22 crates*
