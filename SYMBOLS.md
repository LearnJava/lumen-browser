...
`crates/js/src/shared_worker.rs:61` **type** `SharedWorkerOutbox` — Outbound queue owned by a single page runtime
`crates/js/src/shared_worker.rs:92` **fn** `drain_messages` — Drain all messages a runtime's shared-worker ports have received
...
`crates/js/src/v8_runtime.rs:783` **fn** `set_global_user_agent_override` — Set (or clear with `None`) the process-global `navigator.userAgent` override
`crates/js/src/v8_runtime.rs:800` **fn** `user_agent_override_script` — Build the JS snippet that redefines `navigator.userAgent` to `ua`
`crates/js/src/v8_runtime.rs:816` **fn** `set_global_timezone_override` — Set (or clear with `None`) the process-global IANA timezone override
`crates/js/src/v8_runtime.rs:850` **fn** `timezone_override_script` — Build the JS snippet that sets the global timezone-override marker
`crates/js/src/v8_runtime.rs:878` **fn** `new` — Create a new V8 runtime on a dedicated thread
`crates/js/src/v8_runtime.rs:937` **fn** `set_cookie_banner_dismiss` — Set whether the cookie-banner auto-dismiss shim (7C.3) is injected on
`crates/js/src/v8_runtime.rs:949` **fn** `pump_broadcast_channels` — Deliver messages posted to this page's `BroadcastChannel` instances
`crates/js/src/v8_runtime.rs:965` **fn** `pump_workers` — Deliver messages posted by worker threads to their `Worker` JS
`crates/js/src/v8_runtime.rs:981` **fn** `pump_shared_workers` — Deliver messages posted by `SharedWorker` threads to this page's
`crates/js/src/v8_runtime.rs:1002` **fn** `take_notification_requests` — Drain all OS notification requests queued by `new Notification(...)` in JS
`crates/js/src/v8_runtime.rs:1010` **fn** `take_navigate_request` — Consume any navigation request that JS placed via `location.href =` etc
`crates/js/src/v8_runtime.rs:1019` **fn** `take_timer_wakeup` — Take the next timer wakeup as Unix epoch ms, clearing the stored value
`crates/js/src/v8_runtime.rs:1027` **fn** `take_print_requests` — Drain all print requests queued by `window.print()`
`crates/js/src/v8_runtime.rs:1043` **fn** `set_deterministic_mode` — Enable or disable deterministic render mode (8F) before calling `install_dom`
`crates/js/src/v8_runtime.rs:1059` **fn** `with_sw_worker_store` — Attach a `SwWorkerStore` so that `_lumen_sw_activate_script` can spawn and
`crates/js/src/v8_runtime.rs:1066` **fn** `take_dom_dirty` — Returns `true` if JS mutated the DOM since the last call, clearing the flag
`crates/js/src/v8_runtime.rs:1073` **fn** `take_dom_touched` — BUG-341 S7: drain the set of nodes touched by tracked DOM-mutation
`crates/js/src/v8_runtime.rs:1079` **fn** `take_raf_pending` — Returns `true` if `requestAnimationFrame` was called since the last call,
`crates/js/src/v8_runtime.rs:1085` **fn** `has_raf_pending` — Non-consuming peek: `true` if rAF callbacks are queued
`crates/js/src/v8_runtime.rs:1091` **fn** `raf_pending_flag` — ADR-016 M2.3: shared, lock-free handle to the rAF-pending flag
`crates/js/src/v8_runtime.rs:1097` **fn** `dom_dirty_flag` — ADR-016 M2.3: shared, lock-free handle to the DOM-dirty flag
`crates/js/src/v8_runtime.rs:1103` **fn** `update_layout_rects` — Replace the layout bounding-rect table with a fresh snapshot
`crates/js/src/v8_runtime.rs:1109` **fn** `update_viewport_size` — Update the current viewport dimensions
`crates/js/src/v8_runtime.rs:1115` **fn** `take_lazy_image_requests` — Drain lazy image load requests queued by JS
`crates/js/src/v8_runtime.rs:1121` **fn** `update_scroll_states` — Replace the scroll-state table with a fresh snapshot from the layout tree
`crates/js/src/v8_runtime.rs:1127` **fn** `take_scroll_requests` — Drain JS-initiated scroll requests queued by `_lumen_request_scroll`
`crates/js/src/v8_runtime.rs:1133` **fn** `take_page_scroll_requests` — Drain JS page-level scroll requests from `window.scrollTo/scrollBy`
`crates/js/src/v8_runtime.rs:1139` **fn** `set_page_scroll_y` — Update the page scroll Y exposed to JS `window.scrollY`/`pageYOffset`
`crates/js/src/v8_runtime.rs:1145` **fn** `fire_element_scroll` — Fire a non-bubbling `scroll` Event on the DOM element identified by `nid`
`crates/js/src/v8_runtime.rs:1155` **fn** `fire_window_scroll` — Fire a non-bubbling `scroll` Event on the `window` object (page scroll)
`crates/js/src/v8_runtime.rs:1164` **fn** `fire_snap_changing` — Fire a CSS Scroll Snap L2 `snapchanging` event on a scroll container
`crates/js/src/v8_runtime.rs:1170` **fn** `fire_snap_changed` — Fire a CSS Scroll Snap L2 `snapchanged` event on a scroll container
`crates/js/src/v8_runtime.rs:1194` **fn** `update_computed_styles` — Push a fresh snapshot of computed CSS styles into the JS runtime
`crates/js/src/v8_runtime.rs:1203` **fn** `update_custom_properties` — Push a fresh snapshot of resolved CSS custom properties into the JS
`crates/js/src/v8_runtime.rs:1213` **fn** `set_document_visibility` — Update `document.hidden` / `document.visibilityState` and fire
`crates/js/src/v8_runtime.rs:1224` **fn** `take_window_open_requests` — Drain all popup window requests queued by JS `window.open(...)`
`crates/js/src/v8_runtime.rs:1230` **fn** `take_console_messages` — Drain all `console.log/warn/error` messages queued since the last call
`crates/js/src/v8_runtime.rs:1236` **fn** `take_history_url_updates` — Drain `history.pushState`/`history.replaceState` URL-update notifications
`crates/js/src/v8_runtime.rs:1242` **fn** `take_history_traversals` — Drain all `history.go(n)`/back/forward traversal deltas queued by JS
`crates/js/src/v8_runtime.rs:1248` **fn** `take_nav_updates` — Drain all Navigation API update requests queued by `_lumen_navigation_request`
`crates/js/src/v8_runtime.rs:1254` **fn** `take_nav_intercept_result` — Drain `NavigateEvent` intercept results queued during event dispatch
`crates/js/src/v8_runtime.rs:1260` **fn** `take_fullscreen_requests` — Drain all fullscreen requests queued by `element.requestFullscreen()`/`exitFullscreen()`
`crates/js/src/v8_runtime.rs:1266` **fn** `take_view_transition_events` — Drain CSS View Transition events queued by `document.startViewTransition()` natives
`crates/js/src/v8_runtime.rs:1274` **fn** `take_focus_requests` — Drain JS dialog focus requests queued by `_lumen_request_focus`/`_lumen_request_blur`
`crates/js/src/v8_runtime.rs:1283` **fn** `pointer_capture_nid` — Returns the DOM node nid that currently holds pointer capture (pointer_id=1)
`crates/js/src/v8_runtime.rs:1291` **fn** `take_pointer_capture` — Release the active pointer capture, returning the former capture target nid
`crates/js/src/v8_runtime.rs:1308` **fn** `flush_canvas_updates` — Drain dirty `<canvas>` 2D buffers for GPU re-upload. Mirrors
`crates/js/src/v8_runtime.rs:1320` **fn** `register_img_bitmaps` — Register decoded `<img>` bitmaps for canvas `drawImage`, keyed by node id
`crates/js/src/v8_runtime.rs:1336` **fn** `debug_heap_stats` — V8 isolate heap statistics: `(total_heap_size, used_heap_size)` in bytes
`crates/js/src/v8_runtime.rs:1349` **fn** `set_import_map` — Install the import map (HTML LS §8.1.6.2) used to resolve bare module
`crates/js/src/v8_runtime.rs:1455` **fn** `install_console_natives` — Register the three console natives (`_lumen_console_log`,
`crates/js/src/v8_runtime.rs:1526` **fn** `install_dom` — Install DOM-core native bindings (`_lumen_*`, 184 functions) and the
...
`crates/js/src/worker.rs:80` **fn** `post_to_worker` — Send a JSON-serialized message to a live worker thread
`crates/js/src/worker.rs:91` **fn** `terminate_worker` — Terminate a worker and remove it from the registry
`crates/js/src/worker.rs:101` **fn** `drain_messages` — Drain all pending messages sent from worker threads to the main thread
...
`crates/engine/layout/src/box_tree.rs:333` **fn** `set_layout_result_cache` — Enables/disables the BUG-341 S36 layout-result cache and clears its
`crates/engine/layout/src/box_tree.rs:341` **fn** `layout_result_cache_enabled` — Whether the BUG-341 S36 layout-result cache is currently enabled
`crates/engine/layout/src/box_tree.rs:350` **struct** `LayoutResultCacheStats` — BUG-341 S36 — per-pass tally of what the cache-checked wrapper did,
`crates/engine/layout/src/box_tree.rs:366` **fn** `take_layout_result_cache_stats` — Returns the accumulated [`LayoutResultCacheStats`] and resets the tally
`crates/engine/layout/src/box_tree.rs:386` **fn** `set_incremental_box_build` — Enables/disables incremental box-build reuse for subsequent
`crates/engine/layout/src/box_tree.rs:391` **fn** `incremental_box_build_enabled` — Whether incremental box-build reuse is currently enabled on this thread
`crates/engine/layout/src/box_tree.rs:403` **struct** `BoxBuildStats` — BUG-341 S4/S15 — per-pass tally of what the box-build stage rebuilt versus
`crates/engine/layout/src/box_tree.rs:482` **fn** `take_box_build_stats` — Returns the accumulated [`BoxBuildStats`] and resets the tally
`crates/engine/layout/src/box_tree.rs:505` **fn** `set_box_build_diagnostics` — Enables/disables the BUG-341 S18 per-node build census, clearing the log
`crates/engine/layout/src/box_tree.rs:514` **fn** `take_box_build_log` — Drains the BUG-341 S18 build census — the `NodeId` of every box really built
`crates/engine/layout/src/box_tree.rs:547` **fn** `set_box_time_diagnostics` — Enables/disables the BUG-341 S20 per-box timing census, clearing the log
`crates/engine/layout/src/box_tree.rs:555` **fn** `take_box_build_time_log` — Drains the BUG-341 S20 per-box timing census — see [`BOX_BUILD_TIME_LOG`]
`crates/engine/layout/src/box_tree.rs:566` **struct** `BoxCopyStats` — BUG-341 S18/S19 census: what one incremental box-build pass spent on
`crates/engine/layout/src/box_tree.rs:589` **fn** `take_box_copy_stats` — Drains the BUG-341 S18/S19 copy census — see [`BoxCopyStats`]
`crates/engine/layout/src/box_tree.rs:629` **fn** `take_box_probe_ns` — Drains the BUG-341 S25 probe timers — see [`PROBE_NS`] / [`STYLE_MISS_NS`]
`crates/engine/layout/src/box_tree.rs:821` **struct** `ViewBox` — SVG `viewBox="min-x min-y width height"` attribute. Maps SVG user-unit space
`crates/engine/layout/src/box_tree.rs:836` **struct** `PreserveAspectRatio` — SVG `preserveAspectRatio` attribute for aspect-ratio preservation
`crates/engine/layout/src/box_tree.rs:847` **enum** `SvgAlignX` — SVG preserveAspectRatio horizontal alignment
`crates/engine/layout/src/box_tree.rs:858` **enum** `SvgAlignY` — SVG preserveAspectRatio vertical alignment
`crates/engine/layout/src/box_tree.rs:869` **enum** `SvgMeetOrSlice` — SVG preserveAspectRatio meet-or-slice mode
`crates/engine/layout/src/box_tree.rs:879` **enum** `SvgTextAnchor` — SVG `text-anchor` attribute for text horizontal alignment
`crates/engine/layout/src/box_tree.rs:892` **enum** `SvgDominantBaseline` — SVG `dominant-baseline` attribute for text vertical alignment
`crates/engine/layout/src/box_tree.rs:916` **enum** `SvgBaselineShift` — SVG 1.1 §10.9.2 / CSS Inline Layout L3 §5.2 — `baseline-shift`. Vertical shift
`crates/engine/layout/src/box_tree.rs:933` **struct** `SvgTransform` — SVG transformation data from the `transform` presentation attribute
`crates/engine/layout/src/box_tree.rs:941` **fn** `identity` — Creates an identity transform (no transformation)
`crates/engine/layout/src/box_tree.rs:946` **fn** `translate` — Creates a translation transform
`crates/engine/layout/src/box_tree.rs:951` **fn** `compose` — Multiplies this transform by another, composing them
`crates/engine/layout/src/box_tree.rs:966` **fn** `transform_point` — Applies this transform to a point (x, y)
`crates/engine/layout/src/box_tree.rs:975` **enum** `SvgShapeKind` — Geometric primitive for an SVG shape element in SVG user units (before viewBox scaling)
`crates/engine/layout/src/box_tree.rs:992` **enum** `FormControlKind` — Вид form control — используется в `BoxKind::FormControl` для paint-специализаций
`crates/engine/layout/src/box_tree.rs:1088` **fn** `collect_selectlist_label` — Collect the selected `<option>` label from a `<selectlist>` element
`crates/engine/layout/src/box_tree.rs:1126` **fn** `is_selectlist` — Returns `true` when `node` is a `<selectlist>` element (Customizable Select)
`crates/engine/layout/src/box_tree.rs:1218` **fn** `is_open_details` — Returns `true` when `id` is a `<details>` element with the `open` attribute set
`crates/engine/layout/src/box_tree.rs:2149` **struct** `ImageRequest` — Запрос на предзагрузку изображения: URL после picking-а по
`crates/engine/layout/src/box_tree.rs:2167` **fn** `collect_image_requests` — Обходит DOM и возвращает запросы на загрузку для всех `<img>`-элементов
`crates/engine/layout/src/box_tree.rs:2193` **fn** `collect_background_image_requests` — Обходит готовое layout-дерево и возвращает уникальные URL-ы из
`crates/engine/layout/src/box_tree.rs:2349` **struct** `LayoutBox`
`crates/engine/layout/src/box_tree.rs:2406` **struct** `BoxOrigin` — Where a layout box came from — the identity of a box for all
`crates/engine/layout/src/box_tree.rs:2432` **enum** `BoxRole` — Disambiguates the many boxes one DOM node — or no node at all — can
`crates/engine/layout/src/box_tree.rs:2460` **struct** `InlineSegment` — Отрезок inline-контента с собственным стилем (до layout)
`crates/engine/layout/src/box_tree.rs:2506` **enum** `PseudoKind` — Marks an inline segment as the target of a CSS structural pseudo-element
`crates/engine/layout/src/box_tree.rs:2538` **struct** `InlineFrag` — Позиционированный текстовый фрагмент в строке (после layout)
`crates/engine/layout/src/box_tree.rs:2577` **enum** `BoxKind`
`crates/engine/layout/src/box_tree.rs:3420` **fn** `layout` — Lay out a document without a text measurer. For tests and headless dump modes
`crates/engine/layout/src/box_tree.rs:3446` **fn** `layout_measured` — Layout without a text measurer. For tests and headless modes; uses `layout_measured_hyp` with `dark_mode=false`
`crates/engine/layout/src/box_tree.rs:3459` **fn** `layout_measured_hyp` — Layout with a real hyphenation provider (for `hyphens: auto`)
`crates/engine/layout/src/box_tree.rs:3482` **fn** `layout_measured_hyp_with_counters` — Like [`layout_measured_hyp`], but also returns the [`CounterMap`] the cascade
`crates/engine/layout/src/box_tree.rs:3549` **fn** `lay_out_incremental` — Incremental re-layout pass: skips clean subtrees, re-lays out only dirty ones
`crates/engine/layout/src/box_tree.rs:3588` **fn** `layout_streaming_incremental` — Streaming incremental layout (PH1-2b)
`crates/engine/layout/src/box_tree.rs:3635` **fn** `layout_mutation_incremental` — Incremental re-layout for JS DOM mutations (ADR-016 M4)
`crates/engine/layout/src/box_tree.rs:3691` **fn** `layout_mutation_incremental_restyle` — BUG-341 S5: incremental re-layout for a pure interactive-state transition
`crates/engine/layout/src/box_tree.rs:3874` **fn** `build_iframe_document` — Parse inline HTML from an `<iframe srcdoc="...">` attribute (HTML spec §4.8.5)
`crates/engine/layout/src/box_tree.rs:3949` **fn** `canvas_background_color` — CSS Backgrounds §3.11.1 — the canvas background color
`crates/engine/layout/src/box_tree.rs:5334` **fn** `incremental_build_box` — BUG-341 S4 — incremental box-build entry point
`crates/engine/layout/src/box_tree.rs:12294` **fn** `resolve_auto_fill_fit_count` — CSS Grid Layout L3 §9 — Resolve `repeat(auto-fill|auto-fit, <track-list>)` count
`crates/engine/layout/src/box_tree.rs:12477` **fn** `measure_text_w` — Measures text width (letter_spacing applied between each character)
`crates/engine/layout/src/box_tree.rs:12496` **fn** `measure_text_w_families` — Как [`measure_text_w`], но учитывает CSS `font-family` каскад
`crates/engine/layout/src/box_tree.rs:12526` **fn** `measure_text_w_varied` — Как [`measure_text_w_families`], но учитывает CSS `font-variation-settings`
`crates/engine/layout/src/box_tree.rs:13829` **fn** `apply_container_styles` — CSS Container Queries L1: second-pass after layout
...
`crates/engine/layout/src/lib.rs:1232` **fn** `collect_computed_styles` — Walks the layout tree and returns a map of `NodeId index → CSS property map`
`crates/engine/layout/src/lib.rs:1275` **fn** `collect_custom_properties` — Walks the layout tree and returns a map of `NodeId index → resolved custom
`crates/engine/layout/src/lib.rs:1338` **fn** `collect_layout_rects` — Walks the layout tree and returns a map of `NodeId index → [x, y, width, height]`
`crates/engine/layout/src/lib.rs:1372` **fn** `find_box_by_node` — Update the scroll position of a node in the layout tree
`crates/engine/layout/src/lib.rs:1379` **fn** `set_scroll_position`
`crates/engine/layout/src/lib.rs:1412` **fn** `collect_view_transition_names` — Find the innermost scroll container whose `clip_rect` contains `(x, y)`
`crates/engine/layout/src/lib.rs:1449` **fn** `collect_view_transition_groups`
`crates/engine/layout/src/lib.rs:1475` **fn** `find_scroll_container_at` — `x` and `y` are in CSS px, document-relative (same coordinate space as
`crates/engine/layout/src/lib.rs:1498` **fn** `find_scroll_container_for_node` — Find the nearest scrolling ancestor of `node` (inclusive of `node` itself),
...
`crates/engine/layout/src/selector_query.rs:626` **fn** `computed_style_to_map` — Serialises a [`ComputedStyle`] to a CSS property → resolved-value map
`crates/engine/layout/src/selector_query.rs:977` **fn** `computed_style_json` — Serialises a [`ComputedStyle`] into a deterministic JSON object string
`crates/engine/layout/src/selector_query.rs:999` **fn** `computed_style_json_by_selector` — Like [`computed_style_by_selector`] but returns the full computed-style JSON
`crates/engine/layout/src/selector_query.rs:1014` **struct** `MatchedRule` — One CSS rule that matched a specific DOM node
`crates/engine/layout/src/selector_query.rs:1034` **fn** `matched_rules_for_node` — Return all CSS rules from `sheet` whose selectors match `node` in `doc`
...
`crates/engine/paint/src/display_list.rs:1970` **enum** `FrameDelta` — How a frame differs from the previously presented one (ADR-016 M0.5)
`crates/engine/paint/src/display_list.rs:1992` **struct** `FrameFingerprint` — Split fingerprint of a presented frame (ADR-016 M0.5)
`crates/engine/paint/src/display_list.rs:2007` **fn** `new` — Build a fingerprint for the current frame from its page content, surface
`crates/engine/paint/src/display_list.rs:2028` **fn** `delta_from` — Classify how this frame differs from the previously presented `prev`
`crates/engine/paint/src/display_list.rs:2192` **fn** `anim_split_compose_plan` — Static/animated split (EXPERIMENT.md §2): строит план отрисовки
`crates/engine/paint/src/display_list.rs:2496` **struct** `DiffResult` — Результат сравнения двух display-list-ов
`crates/engine/paint/src/display_list.rs:2508` **fn** `identical` — Создаёт DiffResult для идентичных display list-ов
`crates/engine/paint/src/display_list.rs:2522` **fn** `changed` — Создаёт DiffResult для изменённых display list-ов с заданным bounding rect
`crates/engine/paint/src/display_list.rs:2538` **fn** `diff_display_lists` — Сравнивает два display list-а по Debug hash каждой команды
`crates/engine/paint/src/display_list.rs:2645` **fn** `serialize_display_list`
`crates/engine/paint/src/display_list.rs:3098` **fn** `build_display_list`
`crates/engine/paint/src/display_list.rs:3114` **fn** `build_display_list_with_selection` — Like [`build_display_list`] but applies `::selection` CSS highlight styles
`crates/engine/paint/src/display_list.rs:3132` **fn** `build_display_list_with_anim` — Like `build_display_list` but applies compositor animation overrides per node
`crates/engine/paint/src/display_list.rs:3168` **fn** `build_display_list_ordered` — Билдер display list-а, **уважающий painting order** (CSS 2.1 Appendix E)
`crates/engine/paint/src/display_list.rs:3185` **fn** `build_display_list_ordered_dpr` — Like [`build_display_list_ordered`] but resolves `image-set()` background
`crates/engine/paint/src/display_list.rs:3290` **fn** `build_display_list_ordered_with_anim` — Like [`build_display_list_ordered`] but applies compositor animation overrides per node
`crates/engine/paint/src/display_list.rs:3301` **fn** `build_display_list_ordered_with_anim_dpr` — Like [`build_display_list_ordered_with_anim`] but resolves `image-set()`
`crates/engine/paint/src/display_list.rs:3324` **fn** `build_display_list_ordered_with_anim_split` — Static/animated split (EXPERIMENT.md §2): как
`crates/engine/paint/src/display_list.rs:3477` **fn** `build_print_display_list` — Builds a print display list from paginated layout
`crates/engine/paint/src/display_list.rs:3543` **fn** `split_at_page_breaks` — Splits a print display list at `PageBreak` markers
`crates/engine/paint/src/display_list.rs:3570` **fn** `strip_background_graphics` — Removes background-graphics paint commands from each print page when the
`crates/engine/paint/src/display_list.rs:4985` **fn** `is_image_set` — CSS Images L4 §5 — is `value` an `image-set()` / `-webkit-image-set()` expression?
`crates/engine/paint/src/display_list.rs:5113` **fn** `select_image_set_url` — CSS Images L4 §5 — selects the best `image-set()` candidate URL for `dpr`
`crates/engine/paint/src/display_list.rs:6142` **fn** `patch_scroll_layer` — In-place патч скролл-позиции overflow-контейнера в готовом display list —
`crates/engine/paint/src/display_list.rs:6272` **fn** `point_on_resize_grip` — Возвращает `true`, если точка (`px`, `py`) попадает в resize-grip элемента
`crates/engine/paint/src/display_list.rs:18889` **fn** `emit_text_with_highlights` — CSS Custom Highlight API L1 — helper to emit DrawText with highlight name
...
`crates/engine/paint/src/renderer.rs:3863` **fn** `load_counter` — Reads a diagnostics counter
`crates/engine/paint/src/renderer.rs:4681` **fn** `new`
`crates/engine/paint/src/renderer.rs:4886` **fn** `new_headless` — Creates a headless `Renderer` for off-screen rendering without a winit window
`crates/engine/paint/src/renderer.rs:5798` **fn** `warm_lazy_pipelines_blocking` — Прогревает ленивые пайплайны синхронно, на вызывающем потоке
`crates/engine/paint/src/renderer.rs:5813` **fn** `pipelines_compiled` — Сколько ленивых пайплайнов **этот** рендер скомпилировал за свою жизнь
`crates/engine/paint/src/renderer.rs:5826` **fn** `hot_pipeline_threads` — Сколько РАЗНЫХ потоков скомпилировало пять горячих пайплайнов этого
`crates/engine/paint/src/renderer.rs:5839` **fn** `hot_pipelines_built_on_ui_thread` — Сколько горячих пайплайнов пришлось скомпилировать самому UI-потоку —
`crates/engine/paint/src/renderer.rs:5949` **fn** `submissions` — Сколько командных списков **кадра** отправил в очередь этот рендер
`crates/engine/paint/src/renderer.rs:5960` **fn** `rrect_clip_levels` — Сколько скруглённых клипов этот рендер обслужил offscreen-уровнем
`crates/engine/paint/src/renderer.rs:5970` **fn** `cull_merges` — Сколько разрезов пасса родителя склеено обратно после выброса
`crates/engine/paint/src/renderer.rs:5979` **fn** `plan_passes` — Сколько пассов (элементов плана кадра) закодировал этот рендер
`crates/engine/paint/src/renderer.rs:5990` **fn** `state_elisions` — Сколько команд состояния пасса не отправлено, потому что нужное
`crates/engine/paint/src/renderer.rs:6000` **fn** `draw_merges` — Сколько вызовов `draw` слито с предыдущим (BUG-405 срез 10)
`crates/engine/paint/src/renderer.rs:6009` **fn** `set_state_elision_enabled` — Включает/выключает отсев повторных команд состояния пасса
`crates/engine/paint/src/renderer.rs:6020` **fn** `atlas_bytes_uploaded` — Сколько байт пикселей атласа глифов отправлено в GPU этим рендером
`crates/engine/paint/src/renderer.rs:6031` **fn** `atlas_uploads` — Сколько раз атлас глифов заливался в GPU этим рендером
`crates/engine/paint/src/renderer.rs:6040` **fn** `set_atlas_partial_upload_enabled` — Включает/выключает построчную заливку атласа (BUG-405 срез 11) на этом
`crates/engine/paint/src/renderer.rs:6050` **fn** `filter_passes` — Сколько render-пассов закодировали filter-элементы планов этого
`crates/engine/paint/src/renderer.rs:6062` **fn** `shadow_draws` — Сколько внешних теней (`box-shadow`) этот рендер нарисовал
`crates/engine/paint/src/renderer.rs:6073` **fn** `nested_shader_clips` — Сколько вложенных скруглённых клипов этот рендер обслужил ВТОРЫМ
`crates/engine/paint/src/renderer.rs:6084` **fn** `coverage_cache_stats` — Сколько раз покрытие SVG-супа взято готовым из кэша, а не пересчитано
`crates/engine/paint/src/renderer.rs:6095` **fn** `svg_shape_cache_stats` — Сколько команд SVG получили готовую фигуру из кэша (BUG-405 срез 12),
`crates/engine/paint/src/renderer.rs:6104` **fn** `set_svg_shape_cache_enabled` — Включает/выключает мемоизацию фигур SVG (BUG-405 срез 12)
`crates/engine/paint/src/renderer.rs:6110` **fn** `text_run_cache_stats` — Попаданий и промахов кэша укладки текста (BUG-405 срез 13) за жизнь
`crates/engine/paint/src/renderer.rs:6119` **fn** `set_text_run_cache_enabled` — Включает/выключает мемоизацию укладки текстового run-а (BUG-405 срез 13)
`crates/engine/paint/src/renderer.rs:6128` **fn** `set_coverage_cache_enabled` — Включает/выключает кэш покрытия SVG-супов (BUG-405 срез 9)
`crates/engine/paint/src/renderer.rs:6137` **fn** `set_nested_shader_clip_enabled` — Включает/выключает второй шейдерный контур (BUG-405 срез 8)
`crates/engine/paint/src/renderer.rs:6146` **fn** `set_shadow_analytic_enabled` — Включает/выключает аналитическую размытую тень (BUG-405 срез 7)
`crates/engine/paint/src/renderer.rs:6156` **fn** `set_blur_merge_enabled` — Включает/выключает склейку вертикального прохода блюра с композитом
`crates/engine/paint/src/renderer.rs:6167` **fn** `set_cull_merge_enabled` — Включает/выключает склейку пасса родителя вокруг выброшенного
`crates/engine/paint/src/renderer.rs:6175` **fn** `warmed_pipeline_count` — Сколько ленивых ячеек пайплайнов уже заполнено (BUG-405/406)
`crates/engine/paint/src/renderer.rs:7254` **fn** `with_font_provider` — Заменяет источник лукапа face-ов. Полезно для тестов (mock-provider) и
`crates/engine/paint/src/renderer.rs:7262` **fn** `set_font_provider` — Заменяет `FontProvider` на работающем рендере. Используется shell-ом,
`crates/engine/paint/src/renderer.rs:7279` **fn** `preload_fallback_chain` — Эагерно загружает указанные family-имена через текущий `FontProvider`,
`crates/engine/paint/src/renderer.rs:7294` **fn** `gpu_fingerprint` — Returns the normalized GPU fingerprint (vendor/renderer strings)
`crates/engine/paint/src/renderer.rs:7307` **fn** `preload_curated_fallbacks` — Shortcut: эагерно загружает `CURATED_FALLBACK_FAMILIES` (Noto Color
`crates/engine/paint/src/renderer.rs:7651` **fn** `register_image` — Регистрирует декодированное изображение в GPU-cache под ключом `src`
`crates/engine/paint/src/renderer.rs:8027` **fn** `unregister_image` — Снимает регистрацию изображения. После этого `DrawImage` для `src`
`crates/engine/paint/src/renderer.rs:8036` **fn** `clear_images` — Снимает регистрацию всех картинок (например, при переходе на новую
`crates/engine/paint/src/renderer.rs:8044` **fn** `has_image` — Зарегистрирована ли картинка с таким `src` (для shell-логирования)
`crates/engine/paint/src/renderer.rs:8062` **fn** `upload_layer_snapshot` — Загружает CPU-пиксели (`Rgba8`, 4 байта/пиксель) как именованный
`crates/engine/paint/src/renderer.rs:8131` **fn** `evict_layer_snapshot` — Удаляет снимок с `id`. GPU-память освобождается при drop-е
`crates/engine/paint/src/renderer.rs:8137` **fn** `clear_layer_snapshots` — Удаляет все снимки (например, при переходе на новую страницу)
`crates/engine/paint/src/renderer.rs:8144` **fn** `has_layer_snapshot` — Зарегистрирован ли снимок с таким `id`
`crates/engine/paint/src/renderer.rs:8149` **fn** `layer_cache` — Получить ссылку на layer cache для статистики / монитора GPU памяти
`crates/engine/paint/src/renderer.rs:8157` **fn** `set_backdrop_cache_enabled` — Enables or disables the `backdrop-filter` result cache (CSS Filter
`crates/engine/paint/src/renderer.rs:8166` **fn** `clear_backdrop_cache` — Drops every cached `backdrop-filter` texture and its metadata. The next
`crates/engine/paint/src/renderer.rs:8173` **fn** `backdrop_cache_len` — Number of live cached `backdrop-filter` textures (for stats / tests)
`crates/engine/paint/src/renderer.rs:8180` **fn** `backdrop_cache_on_memory_pressure` — Forwards a memory-pressure signal to the `backdrop-filter` cache and
`crates/engine/paint/src/renderer.rs:8193` **fn** `atlas_on_memory_pressure` — Forwards a memory-pressure signal to the glyph atlas so it can evict
`crates/engine/paint/src/renderer.rs:8206` **fn** `atlas_resets` — Сколько раз атлас глифов сбрасывался из-за исчерпания места (BUG-435)
`crates/engine/paint/src/renderer.rs:8244` **fn** `layer_cache_mut` — Получить мutable ссылку для прямого управления кэшем (advanced usage)
`crates/engine/paint/src/renderer.rs:8250` **fn** `access_layer` — Отметить layer как используемый текущим render pass
`crates/engine/paint/src/renderer.rs:8257` **fn** `cache_layer` — Кэшировать layer слой. Returns `true` if this is a new layer, `false` if updated
`crates/engine/paint/src/renderer.rs:8263` **fn** `return_layer_to_pool` — Return an off-screen layer texture to the pool for recycling (Phase 2 ADR-008)
`crates/engine/paint/src/renderer.rs:8279` **fn** `promote_layer` — Promote a node to its own GPU layer for `will-change: transform/opacity/filter`
`crates/engine/paint/src/renderer.rs:8290` **fn** `is_layer_promoted` — Returns `true` if the given node has a promoted GPU layer
`crates/engine/paint/src/renderer.rs:8295` **fn** `demote_layer` — Remove the promoted GPU layer for a node, freeing its cache entry
`crates/engine/paint/src/renderer.rs:8301` **fn** `clear_layer_cache` — Очистить весь layer cache (полная эвикция) и очистить texture pool
`crates/engine/paint/src/renderer.rs:8307` **fn** `texture_pool_len` — Get the number of free textures in the pool (for diagnostics)
`crates/engine/paint/src/renderer.rs:8312` **fn** `texture_pool_len_for_size` — Get the number of free textures of a specific size (for diagnostics)
`crates/engine/paint/src/renderer.rs:8320` **fn** `texture_pool_report` — Однострочная сводка по пулу offscreen-слоёв для `LUMEN_MEM_REPORT`
`crates/engine/paint/src/renderer.rs:8338` **fn** `clear_texture_pool` — Clear all pooled textures (e.g., when resizing or memory pressure is high)
`crates/engine/paint/src/renderer.rs:8344` **fn** `snapshot_dimensions` — Возвращает `(width, height)` снимка, или `None` если `id` не зарегистрирован
`crates/engine/paint/src/renderer.rs:8350` **fn** `resize` — Resizes the render target. For windowed mode, reconfigures the wgpu surface
`crates/engine/paint/src/renderer.rs:8380` **fn** `set_scale_factor` — Обновить device-pixel-ratio. Вызывается shell-ом по `WindowEvent::ScaleFactorChanged`
`crates/engine/paint/src/renderer.rs:8390` **fn** `scale_factor` — Текущий device-pixel-ratio. Для отладки / тестов (UI обычно его не читает —
`crates/engine/paint/src/renderer.rs:8400` **fn** `target_color_space` — Target color space for this renderer's output surface
`crates/engine/paint/src/renderer.rs:8409` **fn** `set_canvas_background` — Updates the root-element canvas background used as the framebuffer clear colour
`crates/engine/paint/src/renderer.rs:8436` **fn** `set_page_offset` — Фиксированное смещение страницы в CSS px (ADR-016 M0.4, BUG-405 срез 38)
`crates/engine/paint/src/renderer.rs:8446` **fn** `page_offset` — Текущее смещение страницы (см. [`set_page_offset`](Self::set_page_offset))
`crates/engine/paint/src/renderer.rs:8489` **fn** `viewport_size` — Текущий viewport в **logical** (CSS) пикселях: `physical / scale_factor`
`crates/engine/paint/src/renderer.rs:9451` **fn** `render` — `scroll_y ≥ 0`, `scroll_x ≥ 0`. Negatives caller обязан клампить до 0
`crates/engine/paint/src/renderer.rs:9464` **fn** `render_with_anim` — Как [`render`](Self::render), но с диапазонами анимируемых сегментов
`crates/engine/paint/src/renderer.rs:14919` **fn** `render_to_image_cpu` — CPU-based rasterization using tiny-skia (feature="cpu-render" only)
`crates/engine/paint/src/renderer.rs:14945` **fn** `render_tile`
`crates/engine/paint/src/renderer.rs:14984` **fn** `render_to_image` — Renders display commands and returns a CPU `Image` (RGBA8)
`crates/engine/paint/src/renderer.rs:15102` **fn** `render_print_pages` — Renders a print display list into one `Image` per page
...
---
*Total: 5552 symbols in 24 crates*
