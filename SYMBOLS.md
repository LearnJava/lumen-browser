...
`crates/engine/canvas/src/lib.rs:98` **fn** `as_str` — Canonical CSS string name for this operation
`crates/engine/canvas/src/lib.rs:122` **enum** `LineCap` — CSS `lineCap` — how line endpoints are rendered
`crates/engine/canvas/src/lib.rs:135` **fn** `from_str` — Parse from CSS string
`crates/engine/canvas/src/lib.rs:147` **enum** `LineJoin` — CSS `lineJoin` — how line segments connect at corners
`crates/engine/canvas/src/lib.rs:160` **fn** `from_str` — Parse from CSS string
`crates/engine/canvas/src/lib.rs:177` **struct** `DrawState` — All drawing state captured by `save()` and restored by `restore()`
`crates/engine/canvas/src/lib.rs:246` **struct** `ColorStop` — One colour stop in a [`CanvasGradient`]
`crates/engine/canvas/src/lib.rs:255` **enum** `GradientKind` — Gradient kind — stores the defining geometry in user (pre-CTM) space
`crates/engine/canvas/src/lib.rs:298` **struct** `CanvasGradient` — Canvas gradient object (`createLinearGradient` / `createRadialGradient` / `createConicGradient`)
`crates/engine/canvas/src/lib.rs:307` **fn** `linear` — Create a linear gradient from `(x0,y0)` to `(x1,y1)`
`crates/engine/canvas/src/lib.rs:311` **fn** `radial` — Create a radial gradient between two circles
`crates/engine/canvas/src/lib.rs:315` **fn** `conic` — Create a conic gradient starting at `angle` (radians) around `(cx,cy)`
`crates/engine/canvas/src/lib.rs:320` **fn** `add_color_stop` — Add a colour stop at `offset ∈ [0,1]`
`crates/engine/canvas/src/lib.rs:326` **fn** `sample` — Sample the gradient colour at device pixel `(x, y)`
`crates/engine/canvas/src/lib.rs:390` **enum** `RepeatMode` — Pattern repetition mode (`createPattern` second argument)
`crates/engine/canvas/src/lib.rs:406` **struct** `CanvasPattern` — Canvas pattern object (`createPattern`)
`crates/engine/canvas/src/lib.rs:419` **fn** `new` — Create a new pattern from RGBA8 pixel data
`crates/engine/canvas/src/lib.rs:424` **fn** `sample` — Sample the pattern colour at device pixel `(x, y)`
`crates/engine/canvas/src/lib.rs:456` **enum** `PaintSource` — Paint source: a solid colour, a gradient, or a pattern
`crates/engine/canvas/src/lib.rs:471` **fn** `sample` — Sample the paint at device pixel centre `(x + 0.5, y + 0.5)`
`crates/engine/canvas/src/lib.rs:482` **fn** `as_color_or_black` — Return the solid colour, or transparent black if this is a gradient/pattern
`crates/engine/canvas/src/lib.rs:534` **struct** `Context2D` — HTML Canvas 2D rendering context
`crates/engine/canvas/src/lib.rs:604` **fn** `new` — Create a new context with a transparent black buffer and identity CTM
`crates/engine/canvas/src/lib.rs:640` **fn** `set_noise_generator` — Set the optional noise generator for fingerprint randomization
`crates/engine/canvas/src/lib.rs:647` **fn** `get_image_data` — Get a copy of pixel data with optional noise applied (for `getImageData()`)
`crates/engine/canvas/src/lib.rs:660` **fn** `from_pixels` — Create a context pre-filled with the given RGBA8 pixel buffer
`crates/engine/canvas/src/lib.rs:670` **fn** `width` — Canvas width in device pixels
`crates/engine/canvas/src/lib.rs:672` **fn** `height` — Canvas height in device pixels
`crates/engine/canvas/src/lib.rs:675` **fn** `color_space` — Canvas color space (sRGB, Display P3, or Rec2020)
`crates/engine/canvas/src/lib.rs:678` **fn** `set_color_space` — Set the canvas color space for wide-gamut image handling
`crates/engine/canvas/src/lib.rs:681` **fn** `pixels` — Raw RGBA8 pixel data (no noise applied)
`crates/engine/canvas/src/lib.rs:684` **fn** `resize` — Resize the canvas (clears the buffer and resets the CTM to identity)
`crates/engine/canvas/src/lib.rs:695` **fn** `scale_resize` — Resize the canvas by scaling existing pixels to the new dimensions (nearest-neighbour)
`crates/engine/canvas/src/lib.rs:727` **fn** `save` — `save()` — push the current drawing state onto the stack
`crates/engine/canvas/src/lib.rs:752` **fn** `restore` — `restore()` — pop and restore the most recently saved drawing state
`crates/engine/canvas/src/lib.rs:777` **fn** `translate` — `translate(tx, ty)` — apply a translation to the current CTM
`crates/engine/canvas/src/lib.rs:784` **fn** `rotate` — `rotate(angle)` — rotate by `angle` radians clockwise around the origin
`crates/engine/canvas/src/lib.rs:791` **fn** `scale` — `scale(sx, sy)` — apply a uniform or non-uniform scale
`crates/engine/canvas/src/lib.rs:799` **fn** `transform` — `transform(a, b, c, d, e, f)` — post-multiply the CTM by the given matrix
`crates/engine/canvas/src/lib.rs:812` **fn** `set_transform` — `setTransform(a, b, c, d, e, f)` — replace the CTM with the given matrix
`crates/engine/canvas/src/lib.rs:817` **fn** `reset_transform` — `resetTransform()` — reset the CTM to the identity matrix
`crates/engine/canvas/src/lib.rs:835` **fn** `clear_rect` — `clearRect(x, y, w, h)` — erase region to transparent black
`crates/engine/canvas/src/lib.rs:853` **fn** `fill_rect` — `fillRect(x, y, w, h)` — fill region with current `fillStyle`
`crates/engine/canvas/src/lib.rs:863` **fn** `stroke_rect` — `strokeRect(x, y, w, h)` — stroke the outline of a rectangle
`crates/engine/canvas/src/lib.rs:874` **fn** `begin_path` — `beginPath()` — discard current path
`crates/engine/canvas/src/lib.rs:880` **fn** `move_to` — `moveTo(x, y)` — start a new sub-path at user-space `(x, y)`
`crates/engine/canvas/src/lib.rs:888` **fn** `line_to` — `lineTo(x, y)` — add a line segment from pen to `(x, y)`
`crates/engine/canvas/src/lib.rs:900` **fn** `close_path` — `closePath()` — add a line back to the current sub-path start
`crates/engine/canvas/src/lib.rs:911` **fn** `bezier_curve_to` — `bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y)` — cubic Bézier from pen
`crates/engine/canvas/src/lib.rs:932` **fn** `quadratic_curve_to` — `quadraticCurveTo(cpx, cpy, x, y)` — quadratic Bézier from pen
`crates/engine/canvas/src/lib.rs:945` **fn** `arc` — `arc(cx, cy, r, startAngle, endAngle[, anticlockwise])` — add circular arc
`crates/engine/canvas/src/lib.rs:968` **fn** `ellipse` — `ellipse(cx, cy, rx, ry, rotation, startAngle, endAngle[, anticlockwise])`
`crates/engine/canvas/src/lib.rs:1015` **fn** `arc_to` — `arcTo(x1, y1, x2, y2, radius)` — tangent arc between two lines
`crates/engine/canvas/src/lib.rs:1065` **fn** `rect` — `rect(x, y, w, h)` — add a closed rectangle sub-path
`crates/engine/canvas/src/lib.rs:1074` **fn** `fill` — `fill()` — fill the current path with `fillStyle`
`crates/engine/canvas/src/lib.rs:1087` **fn** `stroke` — `stroke()` — stroke the current path with `strokeStyle`
`crates/engine/canvas/src/lib.rs:1114` **fn** `clip` — `clip()` — intersect the current clipping region with the current path (even-odd rule)
`crates/engine/canvas/src/lib.rs:1128` **fn** `fill_with_path2d` — `fill(path2d)` — fill a `Path2D` object using the current `fillStyle`
`crates/engine/canvas/src/lib.rs:1143` **fn** `stroke_with_path2d` — `stroke(path2d)` — stroke a `Path2D` object using the current `strokeStyle`
`crates/engine/canvas/src/lib.rs:1159` **fn** `clip_with_path2d` — `clip(path2d)` — intersect the clipping region with a `Path2D` object (even-odd rule)
`crates/engine/canvas/src/lib.rs:1172` **fn** `is_point_in_path2d` — `isPointInPath(path2d, x, y)` — test whether `(x, y)` lies inside a `Path2D`
`crates/engine/canvas/src/lib.rs:1191` **fn** `draw_image` — `drawImage(src_pixels, src_w, src_h, dx, dy, dw, dh)` — blit source image onto canvas
`crates/engine/canvas/src/lib.rs:1216` **fn** `draw_image_cropped` — `drawImage(src, sx, sy, sw, sh, dx, dy, dw, dh)` — the 9-argument form with
`crates/engine/canvas/src/lib.rs:1276` **fn** `put_image_data` — `putImageData(data, sw, sh, dx, dy)` — write RGBA8 pixel data directly to canvas
`crates/engine/canvas/src/lib.rs:1299` **fn** `create_image_data` — `createImageData(sw, sh)` — return a zero-filled RGBA8 buffer of `sw × sh` pixels
`crates/engine/canvas/src/lib.rs:1309` **fn** `fill_text_glyphs` — Draw pre-rasterized glyph bitmaps at text position
...
`crates/engine/dom/src/lib.rs:1074` **fn** `new`
`crates/engine/dom/src/lib.rs:1104` **fn** `design_mode` — Current value of `document.designMode` (HTML LS §6.6.3)
`crates/engine/dom/src/lib.rs:1109` **fn** `set_design_mode` — Set `document.designMode`. Driven by the JS shim's setter
`crates/engine/dom/src/lib.rs:1114` **fn** `character_set` — `document.characterSet`/`charset`/`inputEncoding` (DOM §7.3, BUG-358)
`crates/engine/dom/src/lib.rs:1120` **fn** `set_character_set` — Set the document's encoding name. Called once by the shell right after
`crates/engine/dom/src/lib.rs:1125` **fn** `content_type` — `document.contentType` (DOM §4.5, BUG-358)
`crates/engine/dom/src/lib.rs:1131` **fn** `set_content_type` — Set the document's MIME type. Called once by the shell from the
`crates/engine/dom/src/lib.rs:1135` **fn** `root`
`crates/engine/dom/src/lib.rs:1143` **fn** `mode` — Текущий парсинг-режим. Tree builder выставляет его при
`crates/engine/dom/src/lib.rs:1149` **fn** `set_mode` — Установить режим. Использует tree builder при инициализации
`crates/engine/dom/src/lib.rs:1154` **fn** `viewport_meta` — Parsed `<meta name="viewport">` descriptor, if the page declared one
`crates/engine/dom/src/lib.rs:1160` **fn** `set_viewport_meta` — Set the viewport meta descriptor. Called by the HTML parser when it
`crates/engine/dom/src/lib.rs:1166` **fn** `get_selection` — Current selection. The shell updates this on mouse events; JS reads it
`crates/engine/dom/src/lib.rs:1171` **fn** `set_selection` — Replace the current selection
`crates/engine/dom/src/lib.rs:1176` **fn** `clear_selection` — Clear the selection
`crates/engine/dom/src/lib.rs:1188` **fn** `control_value` — The control's **current** value (HTML LS §4.10.5.5 «value»): the dirty
`crates/engine/dom/src/lib.rs:1207` **fn** `dirty_value` — The dirty value alone, `None` when the control still shows its default
`crates/engine/dom/src/lib.rs:1217` **fn** `set_control_value` — Set the control's value and raise its dirty value flag — the single
`crates/engine/dom/src/lib.rs:1223` **fn** `clear_control_value` — Drop the control's dirty value, so it falls back to its default —
`crates/engine/dom/src/lib.rs:1235` **fn** `control_checked` — The control's **current** checkedness (HTML LS §4.10.5.5): the dirty
`crates/engine/dom/src/lib.rs:1244` **fn** `dirty_checked` — The dirty checkedness alone, `None` when the control still tracks the
`crates/engine/dom/src/lib.rs:1253` **fn** `set_control_checked` — Set the control's checkedness and raise its dirty checkedness flag —
`crates/engine/dom/src/lib.rs:1260` **fn** `clear_control_checked` — Drop the control's dirty checkedness, so it falls back to the
`crates/engine/dom/src/lib.rs:1275` **fn** `target` — Текущий target — id из URL fragment (без ведущего `#`), к которому
`crates/engine/dom/src/lib.rs:1282` **fn** `set_target` — Установить current target (id без `#`). `None` — нет fragment-а в URL
`crates/engine/dom/src/lib.rs:1294` **fn** `attach_shadow` — Attach a shadow root to `host` and return its `NodeId`
`crates/engine/dom/src/lib.rs:1301` **fn** `shadow_root_of` — Return the shadow root attached to `host`, or `None` if not a shadow host
`crates/engine/dom/src/lib.rs:1306` **fn** `is_shadow_host` — Whether `id` is a shadow host (has an attached shadow root)
`crates/engine/dom/src/lib.rs:1310` **fn** `get`
`crates/engine/dom/src/lib.rs:1314` **fn** `get_mut`
`crates/engine/dom/src/lib.rs:1318` **fn** `len`
`crates/engine/dom/src/lib.rs:1322` **fn** `is_empty`
`crates/engine/dom/src/lib.rs:1334` **fn** `base_href` — HTML5 §4.2.3 — найти первый `<base href="...">` в документе и
`crates/engine/dom/src/lib.rs:1348` **fn** `document_element` — Returns the document's root element `NodeId` — the `<html>` element that is
`crates/engine/dom/src/lib.rs:1356` **fn** `body` — Returns the `<body>` element's `NodeId`, walking root → `<html>` → `<body>`
`crates/engine/dom/src/lib.rs:1366` **fn** `find_first_element` — Найти первый элемент, удовлетворяющий предикату. Pre-order обход
`crates/engine/dom/src/lib.rs:1387` **fn** `find_by_id` — Find a node by its `id` attribute (case-sensitive, per HTML spec)
`crates/engine/dom/src/lib.rs:1415` **fn** `node_count` — Number of nodes currently allocated in this document's arena (including the root)
`crates/engine/dom/src/lib.rs:1421` **fn** `create_element` — Create an element unconditionally. Used by the HTML parser — does **not** enforce
`crates/engine/dom/src/lib.rs:1433` **fn** `try_create_element` — Create an element, returning `Err(`[`NodeLimitExceeded`]`)` if the arena already
`crates/engine/dom/src/lib.rs:1446` **fn** `create_text` — Create a text node unconditionally. Used by the HTML parser — does **not**
`crates/engine/dom/src/lib.rs:1456` **fn** `try_create_text` — Create a text node, returning `Err(`[`NodeLimitExceeded`]`)` if the arena already
`crates/engine/dom/src/lib.rs:1466` **fn** `create_comment` — Create a comment node unconditionally. Used by the HTML parser — does **not**
`crates/engine/dom/src/lib.rs:1476` **fn** `try_create_comment` — Create a comment node, returning `Err(`[`NodeLimitExceeded`]`)` if the arena already
`crates/engine/dom/src/lib.rs:1489` **fn** `create_fragment` — Allocate a `DocumentFragment` node in the arena
`crates/engine/dom/src/lib.rs:1497` **fn** `set_template_content` — Register `fragment` as the content container for `template`
`crates/engine/dom/src/lib.rs:1503` **fn** `template_content` — Return the content `DocumentFragment` for a `<template>` element, or
`crates/engine/dom/src/lib.rs:1507` **fn** `create_doctype`
`crates/engine/dom/src/lib.rs:1536` **fn** `append_child` — Append `child` as the last child of `parent`. If `child` already has a parent, it is detached first
`crates/engine/dom/src/lib.rs:1553` **fn** `insert_after` — Insert `new_node` immediately after `reference` in their shared parent
`crates/engine/dom/src/lib.rs:1577` **fn** `detach` — Remove `node` from its current parent. The node itself stays in the arena and can be re-attached
`crates/engine/dom/src/lib.rs:1591` **fn** `insert_before` — Insert `new_node` immediately before `reference` in `reference`'s parent
`crates/engine/dom/src/lib.rs:1616` **fn** `deep_clone` — Deep-clone `node` and (if `deep`) all its descendants
`crates/engine/dom/src/lib.rs:1640` **fn** `acquire_js_ref` — Increment the JS wrapper reference count for `node_id`
`crates/engine/dom/src/lib.rs:1658` **fn** `release_js_ref` — Decrement the JS wrapper reference count for `node_id`
`crates/engine/dom/src/lib.rs:1674` **fn** `js_ref_count` — Returns the number of live JS wrapper objects currently referencing `node_id`
`crates/engine/dom/src/lib.rs:1687` **fn** `is_detached` — Returns `true` if `node_id` is not reachable from the document tree
`crates/engine/dom/src/lib.rs:1714` **fn** `dead_node_ids` — Returns the IDs of all nodes that are safe to collect from the arena
`crates/engine/dom/src/lib.rs:1757` **fn** `begin_composition` — Begin a new IME composition session in the given editable element
`crates/engine/dom/src/lib.rs:1774` **fn** `update_composition` — Update the active composition with new preedit text and selection range
`crates/engine/dom/src/lib.rs:1788` **fn** `end_composition` — End the active composition and return its final state
`crates/engine/dom/src/lib.rs:1798` **fn** `get_composition` — Get the current composition state without removing it
`crates/engine/dom/src/lib.rs:1806` **fn** `is_composing` — Check if an IME composition is currently active
`crates/engine/dom/src/lib.rs:1814` **fn** `get_composition_range` — Get the composition range (offset and length) if composition is active
`crates/engine/dom/src/lib.rs:1822` **fn** `get_composition_target` — Get the target node that is receiving composition input
`crates/engine/dom/src/lib.rs:1828` **fn** `fonts` — Get a reference to the document's FontFaceSet collection
`crates/engine/dom/src/lib.rs:1834` **fn** `fonts_mut` — Get a mutable reference to the document's FontFaceSet collection
`crates/engine/dom/src/lib.rs:1842` **fn** `set_timing_origin` — Set the timing origin (navigation start time in milliseconds since epoch)
`crates/engine/dom/src/lib.rs:1848` **fn** `current_time` — Get the current time relative to timing_origin (milliseconds)
`crates/engine/dom/src/lib.rs:1856` **fn** `mark` — Record a performance mark at the current time
`crates/engine/dom/src/lib.rs:1865` **fn** `measure` — Record a performance measure between two marks
`crates/engine/dom/src/lib.rs:1879` **fn** `performance_entries` — Get a reference to the performance entries collection
`crates/engine/dom/src/lib.rs:1885` **fn** `performance_entries_mut` — Get a mutable reference to the performance entries collection
`crates/engine/dom/src/lib.rs:1890` **fn** `performance_entries_by_type` — Get all performance entries of a specific type
`crates/engine/dom/src/lib.rs:1898` **fn** `performance_entries_by_name` — Get all performance entries with a specific name
`crates/engine/dom/src/lib.rs:1903` **fn** `clear_performance_entries` — Clear all performance entries
`crates/engine/dom/src/lib.rs:1916` **fn** `to_bytes` — Serialise the entire document to a compact binary blob (bincode)
`crates/engine/dom/src/lib.rs:1921` **fn** `from_bytes` — Deserialise a document from a binary blob produced by [`to_bytes`]
`crates/engine/dom/src/lib.rs:2018` **fn** `check_form_gate` — Гейт отправки форм по sandbox-флагу HTML §7.6.5
`crates/engine/dom/src/lib.rs:2039` **fn** `find_ancestor_form` — Найти ближайший предок `<form>` для узла `node`
`crates/engine/dom/src/lib.rs:2056` **fn** `find_ancestor_dialog` — Walk up the DOM from `node` and return the first ancestor `<dialog>` element
`crates/engine/dom/src/lib.rs:2075` **fn** `node_is_contenteditable` — True when `node` carries `contenteditable=""` or `contenteditable="true"`
`crates/engine/dom/src/lib.rs:2095` **fn** `find_editing_host` — Walk up the tree from `node` (inclusive) and return the nearest element
`crates/engine/dom/src/lib.rs:2117` **fn** `is_element_draggable` — Return `true` when `node` is draggable by default HTML5 rules (HTML LS §9.3.3)
`crates/engine/dom/src/lib.rs:2138` **fn** `set_pointer_capture` — Set pointer capture for `pointer_id` to `node` (W3C Pointer Events L3 §4.1)
`crates/engine/dom/src/lib.rs:2146` **fn** `release_pointer_capture` — Release pointer capture for `pointer_id` from `node`
`crates/engine/dom/src/lib.rs:2153` **fn** `has_pointer_capture` — Returns `true` if `node` currently holds pointer capture for `pointer_id`
`crates/engine/dom/src/lib.rs:2161` **fn** `pointer_capture_target` — Returns the element that holds pointer capture for `pointer_id`, if any
`crates/engine/dom/src/lib.rs:2175` **fn** `collect_dom_form_fields` — Собрать имена и значения submittable-контролов формы из DOM-атрибутов
`crates/engine/dom/src/lib.rs:2281` **struct** `ValidityState` — Validity state for a form control — HTML5 §4.10.21.1 `ValidityState` interface
`crates/engine/dom/src/lib.rs:2306` **fn** `valid` — Returns `true` when all flags are `false` (element satisfies all constraints)
`crates/engine/dom/src/lib.rs:2327` **fn** `element_validity` — Returns the validity state for `node`, or `None` if the node is not a
`crates/engine/dom/src/lib.rs:2436` **fn** `check_validity_form` — Returns `true` if all submittable controls in `form_id` satisfy their
`crates/engine/dom/src/lib.rs:2444` **fn** `invalid_controls_in_form` — Returns the `NodeId`s of all invalid (failing constraint validation) controls
`crates/engine/dom/src/lib.rs:2461` **fn** `submit_form` — Execute HTML5 form submission algorithm (§4.10.22 «Form submission»)
`crates/engine/dom/src/lib.rs:2600` **struct** `AnchorInfo` — Информация об якорной ссылке (`<a href>`), найденной в документе
`crates/engine/dom/src/lib.rs:2633` **struct** `FlatTree` — Pre-computed composed tree (flat tree) for Shadow DOM layout traversal
`crates/engine/dom/src/lib.rs:2643` **fn** `children_of` — Composed-tree children of `id`
`crates/engine/dom/src/lib.rs:2668` **fn** `is_plain` — Whether the composed tree *is* the DOM tree — no shadow host or slot
`crates/engine/dom/src/lib.rs:2681` **fn** `build_flat_tree` — Build the composed (flat) tree for the document
`crates/engine/dom/src/lib.rs:2777` **fn** `check_navigation_gate` — Гейт навигации по sandbox-флагу HTML §7.6.5
`crates/engine/dom/src/lib.rs:2807` **struct** `IframeInfo` — Данные элемента-хоста вложенного browsing context — URL содержимого и
`crates/engine/dom/src/lib.rs:2886` **fn** `collect_iframes` — Собрать все элементы-хосты вложенных browsing context (`<iframe>` и
`crates/engine/dom/src/lib.rs:2897` **fn** `check_popup_gate` — Гейт открытия popup-ов (`window.open()`, `target="_blank"`) по sandbox HTML §7.6.5
`crates/engine/dom/src/lib.rs:2916` **enum** `EditInputType` — Input event type per Input Events Level 2 §4.1.3
`crates/engine/dom/src/lib.rs:2947` **fn** `as_str` — The canonical `inputType` string for the `InputEvent` interface
`crates/engine/dom/src/lib.rs:2970` **struct** `InputEvent` — Data for a `beforeinput` or `input` DOM event (Input Events Level 2 §4.1)
`crates/engine/dom/src/lib.rs:2989` **fn** `trusted` — Construct a trusted input event (native input pipeline or automation
`crates/engine/dom/src/lib.rs:3000` **fn** `untrusted` — Construct an untrusted input event (synthesized by page script via
`crates/engine/dom/src/lib.rs:3021` **enum** `CompositionEventType` — Type of IME composition event (UI Events §5.2.5)
`crates/engine/dom/src/lib.rs:3032` **fn** `as_str` — The canonical DOM event name per UI Events §5.2.5
`crates/engine/dom/src/lib.rs:3046` **struct** `CompositionData` — Data for a `compositionstart` / `compositionupdate` / `compositionend` event
`crates/engine/dom/src/lib.rs:3073` **struct** `CompositionEvent` — An IME composition event (compositionstart / update / end)
`crates/engine/dom/src/lib.rs:3092` **fn** `new` — Create a new trusted composition event (native IME pipeline)
`crates/engine/dom/src/lib.rs:3104` **fn** `untrusted` — Create an untrusted composition event (synthesized by page script)
`crates/engine/dom/src/lib.rs:3115` **fn** `start` — Create a `compositionstart` event with initial IME text
`crates/engine/dom/src/lib.rs:3130` **fn** `update` — Create a `compositionupdate` event for interim preedit text
`crates/engine/dom/src/lib.rs:3145` **fn** `end` — Create a `compositionend` event for final committed text
`crates/engine/dom/src/lib.rs:3173` **fn** `split_text_node` — Split a text node at `byte_offset`, creating a second text node with the
`crates/engine/dom/src/lib.rs:3215` **fn** `insert_text_at` — Insert `text` into the text node at `pos`, returning the caret position
`crates/engine/dom/src/lib.rs:3273` **fn** `delete_range` — Delete the content of `range` from the document, returning a collapsed
`crates/engine/dom/src/lib.rs:3327` **fn** `insert_paragraph_break`
`crates/engine/dom/src/lib.rs:3356` **fn** `node_text_content` — Returns the full text content of `node` — concatenation of all descendant text nodes
`crates/engine/dom/src/lib.rs:3369` **fn** `locate_text_offset_range` — Locate the text node and local byte range covering `[start, end)` of
`crates/engine/dom/src/lib.rs:3413` **fn** `node_child_count` — Number of direct DOM children of `node`
`crates/engine/dom/src/lib.rs:3422` **fn** `node_length` — DOM-spec "length" of `node`: UTF-16 code-unit count for text nodes, child
`crates/engine/dom/src/lib.rs:3434` **fn** `range_text` — Extracts the text covered by `range` (WHATWG DOM §4.6 `stringification`)
...
`crates/driver/src/session.rs:2251` **fn** `computed_style_json` — Возвращает полный набор computed-style свойств первого элемента,
...
`crates/engine/layout/src/box_tree.rs:13013` **fn** `resolve_auto_fill_fit_count` — CSS Grid Layout L3 §9 — Resolve `repeat(auto-fill|auto-fit, <track-list>)` count
`crates/engine/layout/src/box_tree.rs:13196` **fn** `measure_text_w` — Measures text width (letter_spacing applied between each character)
`crates/engine/layout/src/box_tree.rs:13215` **fn** `measure_text_w_families` — Как [`measure_text_w`], но учитывает CSS `font-family` каскад
`crates/engine/layout/src/box_tree.rs:13245` **fn** `measure_text_w_varied` — Как [`measure_text_w_families`], но учитывает CSS `font-variation-settings`
`crates/engine/layout/src/box_tree.rs:14548` **fn** `apply_container_styles` — CSS Container Queries L1: second-pass after layout
...
`crates/engine/layout/src/style.rs:342` **fn** `compute_style_from_declarations` — Build a `ComputedStyle` from a flat list of declarations with neutral context
`crates/engine/layout/src/style.rs:632` **fn** `parse_grid_template_areas` — CSS Grid L1 §7.3 — parse `grid-template-areas` value
...
`crates/engine/layout/src/style/cascade_index.rs:378` **struct** `CascadeIndexStats` — BUG-341 S20 — tally of [`CascadeIndex`] rebuilds
`crates/engine/layout/src/style/cascade_index.rs:396` **fn** `add` — Folds `other` into `self` field by field
`crates/engine/layout/src/style/cascade_index.rs:433` **fn** `take_cascade_index_stats` — Returns the accumulated [`CascadeIndexStats`] and resets the tally
`crates/engine/layout/src/style/cascade_index.rs:439` **fn** `add_cascade_index_stats` — Folds a rayon worker's drained [`CascadeIndexStats`] into this thread's
`crates/engine/layout/src/style/cascade_index.rs:456` **struct** `PseudoCascadeStats` — BUG-341 S20 — per-pass tally of [`compute_pseudo_element_style`] calls
`crates/engine/layout/src/style/cascade_index.rs:468` **fn** `add` — Folds another tally into this one
`crates/engine/layout/src/style/cascade_index.rs:499` **fn** `set_pseudo_cascade_diagnostics` — Enables/disables the BUG-341 S20 pseudo-cascade census — see
`crates/engine/layout/src/style/cascade_index.rs:504` **fn** `take_pseudo_cascade_stats` — Returns the accumulated [`PseudoCascadeStats`] and resets the tally
`crates/engine/layout/src/style/cascade_index.rs:510` **fn** `add_pseudo_cascade_stats` — Folds a rayon worker's drained [`PseudoCascadeStats`] into this thread's
`crates/engine/layout/src/style/cascade_index.rs:531` **fn** `take_pseudo_cascade_sites` — Returns the per-pseudo split of [`PseudoCascadeStats`] and resets it
`crates/engine/layout/src/style/cascade_index.rs:536` **fn** `add_pseudo_cascade_sites` — Folds a rayon worker's drained per-pseudo split into this thread's map
`crates/engine/layout/src/style/cascade_index.rs:576` **fn** `clear_rule_idx_cache` — Drops every cached [`CascadeIndex`] on the current thread
`crates/engine/layout/src/style/cascade_index.rs:600` **fn** `sheet_has_quote_content` — CSS Generated Content L3 §3.2 — whether `sheet` can produce quote content
`crates/engine/layout/src/style/cascade_index.rs:612` **fn** `sheet_targets_pseudo` — BUG-341 S23 — whether `sheet` uses `pseudo` (name without the `::`) as the
`crates/engine/layout/src/style/cascade_index.rs:645` **fn** `set_shadow_sheets` — Install the per-shadow-host author stylesheets for the current layout pass
`crates/engine/layout/src/style/cascade_index.rs:651` **fn** `clear_shadow_sheets` — Drop all installed shadow-tree stylesheets (used by tests to avoid leaking
`crates/engine/layout/src/style/computed.rs:57` **struct** `ComputedStyle`
`crates/engine/layout/src/style/computed.rs:916` **fn** `outline_used_width` — CSS 2.1 §17.6.1 / Basic UI L4 §5.2 — **used** value `outline-width`
`crates/engine/layout/src/style/computed.rs:927` **fn** `text_rendering_eq` — Два стиля рендерят текст одинаково (цвет, размер, интерлиньяж, начертание,
`crates/engine/layout/src/style/computed.rs:945` **fn** `root` — Стартовые значения для корня документа
`crates/engine/layout/src/style/container.rs:27` **struct** `ContainFlags` — CSS Containment L3 §3 — `contain` property
`crates/engine/layout/src/style/container.rs:44` **enum** `ContentVisibility` — CSS Containment L3 §4 — `content-visibility`. NOT inherited. Initial: `Visible`
`crates/engine/layout/src/style/container.rs:65` **enum** `InterpolateSizeMode` — CSS Sizing L4 §4.5 — `interpolate-size` property value
`crates/engine/layout/src/style/container.rs:77` **enum** `ContainerType` — CSS Container Queries L1 §3.1 — `container-type`. NOT inherited. Initial: `Normal`
`crates/engine/layout/src/style/container.rs:87` **struct** `ContainerContext` — Resolved container dimensions, passed during style re-computation for container queries
`crates/engine/layout/src/style/container.rs:185` **fn** `evaluate_container_condition` — Evaluates a raw @container condition string against a `ContainerContext`
`crates/engine/layout/src/style/container.rs:465` **fn** `apply_container_rules` — Applies matching `@container` rules from `sheet` to `style`
...
`crates/engine/layout/src/style/values/background.rs:19` **enum** `ParsedGradient` — CSS Images L3/L4 §3.3/§3.7 — parsed linear / radial / conic gradient
`crates/engine/layout/src/style/values/background.rs:78` **enum** `GradientCorner` — CSS Images L3 §3.1 — `to <corner>` keyword of a `linear-gradient`'s
`crates/engine/layout/src/style/values/background.rs:105` **fn** `angle_deg` — Resolves the keyword to a true gradient-line angle (CSS degrees,
`crates/engine/layout/src/style/values/background.rs:118` **enum** `RadialShape` — CSS Images L3 §3.5 — ending-shape of a `radial-gradient`
`crates/engine/layout/src/style/values/background.rs:130` **enum** `RadialSize` — CSS Images L3 §3.5 — sizing keyword controlling the radii of a
`crates/engine/layout/src/style/values/background.rs:148` **fn** `radial_gradient_radii` — CSS Images L3 §3.5.1 — resolves a `radial-gradient` ending shape to concrete
`crates/engine/layout/src/style/values/background.rs:185` **enum** `BackgroundImage` — CSS Backgrounds L3 §3.1 / CSS Images L4 §4 — `background-image` value
`crates/engine/layout/src/style/values/background.rs:213` **enum** `BackgroundRepeat` — CSS Backgrounds L3 §3.4 — `background-repeat`
`crates/engine/layout/src/style/values/background.rs:224` **fn** `parse`
`crates/engine/layout/src/style/values/background.rs:243` **enum** `BgSizeAxis` — CSS Backgrounds L3 §3.5 — one axis of an explicit `background-size` value
`crates/engine/layout/src/style/values/background.rs:257` **fn** `resolve` — Resolve to a concrete px extent against `area` (the positioning-area
`crates/engine/layout/src/style/values/background.rs:268` **enum** `BackgroundSize` — CSS Backgrounds L3 §3.5 — `background-size`
`crates/engine/layout/src/style/values/background.rs:280` **enum** `BackgroundAttachment` — CSS Backgrounds L3 §3.6 — `background-attachment`
`crates/engine/layout/src/style/values/background.rs:288` **fn** `parse`
`crates/engine/layout/src/style/values/background.rs:309` **enum** `BackgroundOrigin` — CSS Backgrounds L3 §3.7 — `background-origin`. Non-inherited
`crates/engine/layout/src/style/values/background.rs:320` **fn** `parse`
`crates/engine/layout/src/style/values/background.rs:343` **enum** `BackgroundClip` — CSS Backgrounds L3 §3.8 — `background-clip`. Non-inherited
`crates/engine/layout/src/style/values/background.rs:357` **fn** `parse`
`crates/engine/layout/src/style/values/background.rs:379` **enum** `MaskClip` — CSS Masking L1 §4.6 — `mask-clip: <coord-box> | no-clip`
`crates/engine/layout/src/style/values/background.rs:399` **fn** `parse` — Parses a single `mask-clip` keyword (CSS Masking L1 §4.6)
`crates/engine/layout/src/style/values/background.rs:418` **struct** `BackgroundLayer` — CSS Backgrounds L3 §3 — один фоновый слой. Первый в Vec = верхний (рисуется последним)
`crates/engine/layout/src/style/values/background.rs:458` **enum** `ObjectFit` — CSS Images L3 §5.5 — `object-fit`. Применяется к replaced elements
`crates/engine/layout/src/style/values/background.rs:479` **fn** `parse`
`crates/engine/layout/src/style/values/background.rs:499` **enum** `ImageRendering` — CSS Images L3 §6.1 — `image-rendering`. Hint для движка о том, как
`crates/engine/layout/src/style/values/background.rs:519` **fn** `parse`
...
`crates/engine/layout/src/style/values/flexgrid.rs:27` **enum** `TextWrapMode` — CSS Text Module Level 4 §6.4.1 — `text-wrap-mode`. Inherited
`crates/engine/layout/src/style/values/flexgrid.rs:36` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:54` **enum** `TextWrapStyle` — CSS Text Module Level 4 §6.4.2 — `text-wrap-style`. Inherited
`crates/engine/layout/src/style/values/flexgrid.rs:67` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:83` **enum** `FlexDirection` — CSS Flexbox L1 §5.1 — `flex-direction`. Non-inherited
`crates/engine/layout/src/style/values/flexgrid.rs:96` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:112` **enum** `FlexWrap` — CSS Flexbox L1 §5.2 — `flex-wrap`. Non-inherited
`crates/engine/layout/src/style/values/flexgrid.rs:123` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:138` **enum** `FlexBasis` — CSS Flexbox L1 §7.3 — `flex-basis`. Non-inherited
`crates/engine/layout/src/style/values/flexgrid.rs:149` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:163` **struct** `GridRepeat` — CSS Grid Layout L3 §9 — `repeat(auto-fill | auto-fit | <count>, <track-list>)`
`crates/engine/layout/src/style/values/flexgrid.rs:172` **enum** `RepeatCount` — Count type for grid-template-columns/rows `repeat()`
`crates/engine/layout/src/style/values/flexgrid.rs:185` **enum** `GridTrackSize` — CSS Grid Layout L1 §7.2 — sizing function for a grid track
`crates/engine/layout/src/style/values/flexgrid.rs:219` **fn** `resolve_fixed` — Resolve to a concrete pixel size given container width, em, viewport
`crates/engine/layout/src/style/values/flexgrid.rs:228` **fn** `is_fr` — True for fractional tracks
`crates/engine/layout/src/style/values/flexgrid.rs:233` **fn** `fr` — Extract fr value
`crates/engine/layout/src/style/values/flexgrid.rs:238` **fn** `is_subgrid` — True when this track inherits its size from the parent grid (subgrid axis)
`crates/engine/layout/src/style/values/flexgrid.rs:243` **fn** `is_masonry` — True when this axis uses masonry placement (CSS Grid L3 §14)
`crates/engine/layout/src/style/values/flexgrid.rs:289` **fn** `parse_track_list` — Parse a track-list value string into a Vec of GridTrackSize
`crates/engine/layout/src/style/values/flexgrid.rs:420` **enum** `GridAutoFlow` — CSS Grid Layout L1 §8.5 — `grid-auto-flow`. Non-inherited
`crates/engine/layout/src/style/values/flexgrid.rs:433` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:447` **enum** `MasonryAutoFlow` — CSS Masonry Layout §9 — `masonry-auto-flow`. Controls the placement order
`crates/engine/layout/src/style/values/flexgrid.rs:460` **fn** `parse` — Parse a CSS `masonry-auto-flow` value string
`crates/engine/layout/src/style/values/flexgrid.rs:473` **enum** `GridLine` — CSS Grid Layout L1 §8.3 — a grid-line reference for grid-column-start,
`crates/engine/layout/src/style/values/flexgrid.rs:487` **fn** `parse`
`crates/engine/layout/src/style/values/flexgrid.rs:522` **enum** `PositionComponent` — Одна компонента `object-position`. Length-варианты резолвятся в px
`crates/engine/layout/src/style/values/flexgrid.rs:535` **fn** `resolve` — Резолв в финальный px-offset относительно левого/верхнего края
`crates/engine/layout/src/style/values/flexgrid.rs:546` **struct** `ObjectPosition` — CSS Images L3 §5.5 — `object-position` (две компоненты, x + y)
`crates/engine/layout/src/style/values/flexgrid.rs:583` **fn** `parse` — CSS Values L4 §9.4 — `<position>` для object-position. Phase 0
`crates/engine/layout/src/style/values/flexgrid.rs:685` **enum** `AlignValue` — CSS Box Alignment L3 §6.1 — значения для align-/justify- свойств
`crates/engine/layout/src/style/values/flexgrid.rs:712` **fn** `parse`
...
`crates/engine/layout/src/style/values/misc.rs:14` **enum** `Content` — CSS Content L3 — value свойства `content`
`crates/engine/layout/src/style/values/misc.rs:27` **enum** `ContentItem`
`crates/engine/layout/src/style/values/misc.rs:61` **enum** `Quotes` — CSS Generated Content L3 §3.2 — `quotes`. Inherited. Initial: `auto`
`crates/engine/layout/src/style/values/misc.rs:80` **fn** `pair_for_depth` — Returns the `(open, close)` glyph strings for the given nesting `depth`
`crates/engine/layout/src/style/values/misc.rs:102` **enum** `ScrollbarWidth` — CSS Scrollbars 1 — `scrollbar-width`. Inherited
`crates/engine/layout/src/style/values/misc.rs:113` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:125` **enum** `ScrollbarGutter` — CSS Overflow L3 — `scrollbar-gutter`
`crates/engine/layout/src/style/values/misc.rs:136` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:155` **enum** `ListStyleType` — CSS Lists L3 §2.1 — markers для list items
`crates/engine/layout/src/style/values/misc.rs:184` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:206` **enum** `ListStylePosition` — CSS Lists L3 §2.3 — `list-style-position`
`crates/engine/layout/src/style/values/misc.rs:215` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:226` **enum** `OverflowWrap` — CSS Text L3 §5.2 — `overflow-wrap`
`crates/engine/layout/src/style/values/misc.rs:237` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:251` **enum** `LineBreak` — CSS Text L3 §5.2 — `line-break`. Inherited. Initial: `Auto`
`crates/engine/layout/src/style/values/misc.rs:262` **enum** `WordBreak` — CSS Text L3 §5.1 — `word-break`
`crates/engine/layout/src/style/values/misc.rs:274` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:287` **enum** `Hyphens` — CSS Text L3 §6 — `hyphens`
`crates/engine/layout/src/style/values/misc.rs:300` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:314` **enum** `TouchAction` — CSS Pointer Events L3 / Touch Events — `touch-action`. NOT inherited. Initial: `Auto`
`crates/engine/layout/src/style/values/misc.rs:332` **enum** `Appearance` — CSS Basic UI L4 §5 — `appearance`. NOT inherited. Initial: `Auto`
`crates/engine/layout/src/style/values/misc.rs:350` **enum** `FieldSizing` — CSS Basic UI L4 §4.4 — `field-sizing`. NOT inherited. Initial: `Fixed`
`crates/engine/layout/src/style/values/misc.rs:360` **enum** `PointerEvents` — CSS Pointer Events L1. Default `auto`
`crates/engine/layout/src/style/values/misc.rs:374` **fn** `parse`
`crates/engine/layout/src/style/values/misc.rs:393` **enum** `Resize` — CSS Basic UI L4 §6 — `resize`. NOT inherited. Initial: `None`
`crates/engine/layout/src/style/values/misc.rs:417` **fn** `allowed_axes` — Разрешает логическую ось `resize` (`Block`/`Inline`) в физическую пару
`crates/engine/layout/src/style/values/scroll.rs:12` **enum** `ShapeOutside` — CSS Shapes L1 §3 — `shape-outside` value. NOT inherited. Initial: `None`
`crates/engine/layout/src/style/values/scroll.rs:21` **enum** `OffsetRotate` — CSS Motion Path L1 §3 — `offset-rotate`. NOT inherited. Initial: `Auto`
`crates/engine/layout/src/style/values/scroll.rs:32` **enum** `PrintColorAdjust` — CSS Color Adjustment L1 §5 — `print-color-adjust`. NOT inherited. Initial: `Economy`
`crates/engine/layout/src/style/values/scroll.rs:40` **enum** `FontSizeAdjust` — CSS Fonts L5 §4 — `font-size-adjust`. Inherited. Initial: `None`
`crates/engine/layout/src/style/values/scroll.rs:49` **enum** `WritingMode` — CSS Writing Modes L3 §2.1 — `writing-mode`. Inherited. Initial: `HorizontalTb`
`crates/engine/layout/src/style/values/scroll.rs:66` **enum** `TextOrientation` — CSS Writing Modes L3 §6.5 — `text-orientation`. Inherited. Initial: `Mixed`
`crates/engine/layout/src/style/values/scroll.rs:78` **enum** `UserSelect` — CSS UI L4 §6.2 — `user-select`. Inherited
`crates/engine/layout/src/style/values/scroll.rs:88` **fn** `parse`
`crates/engine/layout/src/style/values/scroll.rs:102` **enum** `ScrollBehavior` — CSS Overflow L3 — `scroll-behavior`. Inherited
`crates/engine/layout/src/style/values/scroll.rs:110` **struct** `ScrollSnapType` — CSS Scroll Snap L1 §3.1 — `scroll-snap-type: none | <axis> [mandatory | proximity]`
`crates/engine/layout/src/style/values/scroll.rs:116` **enum** `ScrollSnapAxis`
`crates/engine/layout/src/style/values/scroll.rs:127` **enum** `ScrollSnapStrictness`
`crates/engine/layout/src/style/values/scroll.rs:135` **struct** `ScrollSnapAlign` — CSS Scroll Snap L1 §6.1 — `scroll-snap-align: none | <axis-keyword>{1,2}`
`crates/engine/layout/src/style/values/scroll.rs:141` **enum** `ScrollSnapAlignKeyword`
`crates/engine/layout/src/style/values/scroll.rs:150` **enum** `ScrollSnapStop`
`crates/engine/layout/src/style/values/scroll.rs:158` **enum** `OverscrollBehavior` — CSS Overscroll Behavior L1 §2 — `overscroll-behavior: auto | contain | none`
`crates/engine/layout/src/style/values/scroll.rs:166` **fn** `parse`
...
`crates/engine/layout/src/style/values/transform.rs:23` **enum** `ShapeValue` — CSS Masking L1 §3.5 — `<length-percentage>` значение координаты/размера
`crates/engine/layout/src/style/values/transform.rs:33` **fn** `resolve` — Резолвит значение в px. `basis` — размер reference box по
`crates/engine/layout/src/style/values/transform.rs:48` **enum** `ClipPath` — CSS Masking L1 §3.5 — basic-shapes для `clip-path`. Phase 0
`crates/engine/layout/src/style/values/transform.rs:89` **enum** `TransformStyle` — CSS Transforms L1 §11 — функции `transform`. Phase 0 поддерживает
`crates/engine/layout/src/style/values/transform.rs:99` **enum** `BackfaceVisibility` — CSS Transforms L2 §5.1 — `backface-visibility: visible | hidden`
`crates/engine/layout/src/style/values/transform.rs:110` **enum** `TransformFn` — CSS transform functions — translate/scale/rotate/skew/skewX/skewY/matrix
`crates/engine/layout/src/style/values/transform.rs:148` **enum** `FilterFn` — CSS Filter Effects L1 §3 — функции `filter`. Phase 0 поддерживает
`crates/engine/layout/src/style/values/transform.rs:181` **struct** `GradientStop` — CSS Images L3 §3.4 — единичный `<color-stop>` градиента
`crates/engine/layout/src/style/values/transform.rs:194` **enum** `MaskMode` — CSS Masking L1 §6.4 — `mask-mode`. Selects which channel of the mask image
`crates/engine/layout/src/style/values/transform.rs:210` **enum** `MaskComposite` — CSS Masking L1 §4.7 — `mask-composite`. Determines how a mask layer is
`crates/engine/layout/src/style/values/transform.rs:226` **fn** `parse` — Parses a single `mask-composite` keyword (CSS Masking L1 §4.7)
`crates/engine/layout/src/style/values/transform.rs:249` **struct** `MaskLayer` — CSS Masking L1 §4.9 — один слой маски
...
`crates/shell/src/forms.rs:248` **fn** `toggle_checkbox` — Toggle a checkbox/radio input's current checkedness in the live DOM
`crates/shell/src/forms.rs:261` **fn** `set_value` — Set the current value of an `<input>` in the live DOM — typing, a picker
`crates/shell/src/forms.rs:271` **fn** `set_textarea_text` — Set a `<textarea>`'s current value (P3-spell slice 4)
`crates/shell/src/forms.rs:280` **fn** `apply_range_value` — Update a range input's `value` attribute from a click at `click_x` within
`crates/shell/src/forms.rs:305` **fn** `find_validation_error` — Depth-first walk: find the first form control that fails HTML5 constraint
`crates/shell/src/forms.rs:316` **fn** `find_control_rect_and_error` — Find rect and error message for a specific invalid control
`crates/shell/src/forms.rs:327` **fn** `find_all_validation_errors` — Collect all form controls that fail HTML5 constraint validation
`crates/shell/src/forms.rs:452` **fn** `find_box_rect` — Find the bounding rect of the LayoutBox for `node`. Returns `None` if the
`crates/shell/src/forms.rs:465` **fn** `find_layout_box` — Find the LayoutBox subtree for `node`. Returns `None` if the node has no box
`crates/shell/src/forms.rs:480` **fn** `collect_modal_dialogs` — Walk `doc` and collect all NodeIds with `data-lumen-modal` attribute
`crates/shell/src/forms.rs:504` **fn** `build_dialog_overlay` — Build a `::backdrop` + translated dialog overlay for a modal `<dialog>`
`crates/shell/src/forms.rs:545` **fn** `build_validation_tooltip` — Build a validation tooltip anchored below `anchor` (document coordinates)
`crates/shell/src/forms.rs:611` **fn** `collect_form_entries` — Собрать данные формы для submit
`crates/shell/src/forms.rs:657` **fn** `encode_form_fields` — Построить параметры отправки формы: `(action, method, body)`
`crates/shell/src/forms.rs:670` **fn** `encode_form_fields_multipart` — Encode form fields as `multipart/form-data` (RFC 7578)
`crates/shell/src/forms.rs:687` **fn** `get_form_enctype` — Return the `enctype` attribute of the `<form>` ancestor of `submit_node`,
`crates/shell/src/forms.rs:700` **fn** `enctype_of_form` — Normalised `enctype` of the `<form>` element `form_id` itself
`crates/shell/src/forms.rs:714` **fn** `build_form_submit`
`crates/shell/src/forms.rs:746` **fn** `make_get_url` — Построить итоговый URL для GET-формы: добавить `?body` к action URL
`crates/shell/src/forms.rs:786` **fn** `build_color_picker` — Build a color-swatch picker anchored below `anchor` (document coordinates)
`crates/shell/src/forms.rs:823` **fn** `hit_color_swatch` — If viewport-space point `(px, py)` lands on a swatch, return its `[r, g, b]`
`crates/shell/src/forms.rs:844` **fn** `swatch_to_css_color` — Format `[r, g, b]` as CSS `#rrggbb`
`crates/shell/src/forms.rs:854` **struct** `SelectOption` — One entry in a `<select>` dropdown list
`crates/shell/src/forms.rs:877` **fn** `collect_select_options` — Collect all direct `<option>` children of a `<select>` DOM node
`crates/shell/src/forms.rs:914` **fn** `build_select_dropdown` — Build a dropdown overlay anchored below (or above if near the bottom of the
`crates/shell/src/forms.rs:1051` **fn** `build_base_select_dropdown` — Build the option-list popover for a `<select appearance: base-select>`
`crates/shell/src/forms.rs:1157` **fn** `hit_select_option` — If viewport-space point `(px, py)` lands on an option row, return its index
`crates/shell/src/forms.rs:1194` **fn** `apply_select_choice` — Apply the selection of option at `opt_idx` to the `<select>` DOM node:
`crates/shell/src/forms.rs:1215` **enum** `DatePickerHit` — What a viewport-space click hit inside an open date picker
`crates/shell/src/forms.rs:1244` **fn** `is_leap_year` — True if `year` is a leap year
`crates/shell/src/forms.rs:1249` **fn** `days_in_month` — Number of days in the given month (1-based month, Gregorian calendar)
`crates/shell/src/forms.rs:1260` **fn** `first_weekday_of_month` — ISO weekday (0=Mon … 6=Sun) of the first day of the given month
`crates/shell/src/forms.rs:1275` **fn** `month_name` — English month name, 1-based
`crates/shell/src/forms.rs:1287` **fn** `parse_date_value` — Parse an ISO 8601 date string `YYYY-MM-DD` → `(year, month, day)`
`crates/shell/src/forms.rs:1298` **fn** `format_date_value` — Format `(year, month, day)` as `YYYY-MM-DD`
`crates/shell/src/forms.rs:1304` **fn** `today_year_month` — Return the current year and month derived from the system clock
`crates/shell/src/forms.rs:1328` **fn** `build_date_picker` — Build a calendar date-picker overlay anchored below `anchor` (document coords)
`crates/shell/src/forms.rs:1491` **fn** `hit_date_picker` — Hit-test a viewport-space click `(px, py)` against an open date picker
`crates/shell/src/forms.rs:1553` **fn** `advance_month` — Advance display month by `delta` months (positive = forward, negative = backward)
...
---
*Total: 5594 symbols in 24 crates*
