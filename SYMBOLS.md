...
`crates/core/src/ext.rs:1530` **trait** `SwBackend` — Per-origin Service Worker registration persistence
`crates/core/src/ext.rs:1554` **trait** `BrowserSession` — Browser automation session — unified interface for in-process tests, MCP agents,
`crates/core/src/ext.rs:1626` **struct** `NullBrowserSession` — Null implementation of `BrowserSession` — all methods return `NotImplemented`
`crates/core/src/ext.rs:1726` **enum** `MemoryPressureLevel` — OS memory pressure level (ADR-008, task 10H)
`crates/core/src/ext.rs:1746` **trait** `MemoryPressureSource` — Source of OS memory pressure signals (ADR-008, task 10H)
`crates/core/src/ext.rs:1753` **struct** `NullMemoryPressureSource` — Null implementation — always reports `Low`. For tests and platforms without
`crates/core/src/ext.rs:1775` **trait** `EvictableCache` — Common interface for all cross-tab shared memory caches (ADR-008, task 10D.3)
`crates/core/src/ext.rs:1809` **struct** `CacheRegistry` — Registry of all cross-tab shared memory caches (ADR-008, task 10D.3)
`crates/core/src/ext.rs:1815` **fn** `new` — Create an empty registry
`crates/core/src/ext.rs:1820` **fn** `register` — Register a cache. Caches are notified in registration order
`crates/core/src/ext.rs:1825` **fn** `broadcast_pressure` — Broadcast a memory pressure event to all registered caches
`crates/core/src/ext.rs:1832` **fn** `total_used_bytes` — Total memory currently used across all registered caches, in bytes
`crates/core/src/ext.rs:1840` **fn** `total_budget_bytes` — Total memory budget across all caches with a finite budget, in bytes
`crates/core/src/ext.rs:1849` **fn** `clear_all` — Evict all entries in every registered cache
`crates/core/src/ext.rs:1856` **fn** `len` — Number of registered caches
`crates/core/src/ext.rs:1861` **fn** `is_empty` — `true` if no caches are registered
...
`crates/js/src/lib.rs:26` **struct** `QuickJsRuntime` — QuickJS-based JS runtime via `rquickjs`
`crates/js/src/lib.rs:90` **fn** `new`
`crates/js/src/lib.rs:132` **fn** `install_dom` — Install DOM Web API globals (`document`, `window`, `console`, etc.) into
`crates/js/src/lib.rs:228` **fn** `pump_workers` — Deliver messages posted by worker threads to their `Worker` JS instances
`crates/js/src/lib.rs:247` **fn** `take_navigate_request` — Consume any navigation request that JS placed via `location.href =` etc
`crates/js/src/lib.rs:255` **fn** `take_dom_dirty` — Returns `true` if JS mutated the DOM since the last call, clearing the flag
`crates/js/src/lib.rs:264` **fn** `take_raf_pending` — Returns `true` if `requestAnimationFrame` was called since the last call,
`crates/js/src/lib.rs:273` **fn** `take_timer_wakeup` — Take the next timer wakeup as Unix epoch ms, clearing the stored value
`crates/js/src/lib.rs:282` **fn** `update_layout_rects` — Replace the layout bounding-rect table with a fresh snapshot
`crates/js/src/lib.rs:290` **fn** `update_viewport_size` — Update the viewport dimensions
`crates/js/src/lib.rs:299` **fn** `take_lazy_image_requests` — Drain lazy image load requests queued by `_lumen_request_lazy_image_load` in JS
`crates/js/src/lib.rs:311` **fn** `update_scroll_states` — Replace the scroll-state table with a fresh snapshot from the layout tree
`crates/js/src/lib.rs:320` **fn** `take_scroll_requests` — Drain JS-initiated scroll requests queued by `_lumen_request_scroll`
`crates/js/src/lib.rs:329` **fn** `update_computed_styles` — Push a fresh snapshot of computed CSS styles into the JS runtime
`crates/js/src/lib.rs:339` **fn** `set_document_visibility` — Update `document.hidden` / `document.visibilityState` and fire
`crates/js/src/lib.rs:357` **fn** `notify_dom_content_loaded` — Transition `document.readyState` → `'interactive'` and fire
`crates/js/src/lib.rs:369` **fn** `notify_window_loaded` — Transition `document.readyState` → `'complete'` and fire
...
`crates/engine/layout/src/box_tree.rs:64` **struct** `ViewBox` — SVG `viewBox="min-x min-y width height"` attribute. Maps SVG user-unit space
`crates/engine/layout/src/box_tree.rs:79` **struct** `PreserveAspectRatio` — SVG `preserveAspectRatio` attribute for aspect-ratio preservation
`crates/engine/layout/src/box_tree.rs:90` **enum** `SvgAlignX` — SVG preserveAspectRatio horizontal alignment
`crates/engine/layout/src/box_tree.rs:101` **enum** `SvgAlignY` — SVG preserveAspectRatio vertical alignment
`crates/engine/layout/src/box_tree.rs:112` **enum** `SvgMeetOrSlice` — SVG preserveAspectRatio meet-or-slice mode
`crates/engine/layout/src/box_tree.rs:122` **struct** `SvgTransform` — SVG transformation data from the `transform` presentation attribute
`crates/engine/layout/src/box_tree.rs:130` **fn** `identity` — Creates an identity transform (no transformation)
`crates/engine/layout/src/box_tree.rs:135` **fn** `compose` — Multiplies this transform by another, composing them
`crates/engine/layout/src/box_tree.rs:150` **fn** `transform_point` — Applies this transform to a point (x, y)
`crates/engine/layout/src/box_tree.rs:159` **enum** `SvgShapeKind` — Geometric primitive for an SVG shape element in SVG user units (before viewBox scaling)
`crates/engine/layout/src/box_tree.rs:176` **enum** `FormControlKind` — Вид form control — используется в `BoxKind::FormControl` для paint-специализаций
`crates/engine/layout/src/box_tree.rs:777` **struct** `ImageRequest` — Запрос на предзагрузку изображения: URL после picking-а по
`crates/engine/layout/src/box_tree.rs:792` **fn** `collect_image_requests` — Обходит DOM и возвращает запросы на загрузку для всех `<img>`-элементов
`crates/engine/layout/src/box_tree.rs:812` **fn** `collect_background_image_requests` — Обходит готовое layout-дерево и возвращает уникальные URL-ы из
`crates/engine/layout/src/box_tree.rs:912` **struct** `LayoutBox`
`crates/engine/layout/src/box_tree.rs:939` **struct** `InlineSegment` — Отрезок inline-контента с собственным стилем (до layout)
`crates/engine/layout/src/box_tree.rs:977` **enum** `PseudoKind` — Marks an inline segment as the target of a CSS structural pseudo-element
`crates/engine/layout/src/box_tree.rs:995` **struct** `InlineFrag` — Позиционированный текстовый фрагмент в строке (после layout)
`crates/engine/layout/src/box_tree.rs:1027` **enum** `BoxKind`
`crates/engine/layout/src/box_tree.rs:1134` **fn** `layout`
`crates/engine/layout/src/box_tree.rs:1148` **fn** `layout_measured`
`crates/engine/layout/src/box_tree.rs:1159` **fn** `layout_measured_hyp` — Layout with a real hyphenation provider (for `hyphens: auto`)
`crates/engine/layout/src/box_tree.rs:4720` **fn** `measure_text_w`
`crates/engine/layout/src/box_tree.rs:5585` **fn** `apply_container_styles` — CSS Container Queries L1: second-pass after layout
...
`crates/shell/src/tabs/strip.rs:52` **struct** `TabEntry` — Metadata for one browser tab
`crates/shell/src/tabs/strip.rs:67` **struct** `TabStrip` — State of the tab strip (tab list + active index)
`crates/shell/src/tabs/strip.rs:78` **fn** `new` — Create the initial tab strip with one blank tab
`crates/shell/src/tabs/strip.rs:91` **fn** `len` — Number of open tabs
`crates/shell/src/tabs/strip.rs:96` **fn** `push_blank` — Append a new blank tab and return its index
`crates/shell/src/tabs/strip.rs:109` **fn** `remove` — Remove the tab at `idx`. Returns the new active index (clamped to valid
`crates/shell/src/tabs/strip.rs:121` **fn** `set_active_title` — Update the title of the active tab
`crates/shell/src/tabs/strip.rs:131` **fn** `set_tab_state` — Update the lifecycle state of the tab at `idx`
...
---
*Total: 2274 symbols in 19 crates*
