...
| **U-2** | **Шейпинг текста (GSUB/GPOS) + CFF-контуры.** ~~Этап 1: GPOS-кернинг + GSUB-лигатуры для латиницы/кириллицы~~ ✅ 2026-06-18 (`lumen-font::{otlayout,gsub,gpos,shape}`, `Shaper::shape()`, проводка в CPU-растеризатор `cpu_raster::rasterize_text` → `--screenshot`/snapshot). См. «Recent merges». **Остался этап 2: CFF/CFF2 outlines** (`.otf` PostScript-шрифты пока не рисуют контуры в CPU/wgpu-путях). Опционально позже: проводка шейпинга в femtovg live-окно (femtovg шейпит сам через `fill_text`) и в per-char measurement. | P1 | XL (этап 1 ✅) | `lumen-font` (`cff.rs` — этап 2), `lumen-paint` (`cpu_raster.rs`) |
...
**Порядок для P1:** U-0 ✅ → U-1 этап 1 ✅ → U-3 (TAB-серия) ✅ → U-2 этап 1 (GSUB/GPOS шейпинг) ✅ 2026-06-18 → **U-2 этап 2 (CFF-контуры)** → U-4 (WASM, ранее у P2). U-1 этап 2 (пайплайн вне UI) — после разблокировки QuickJS `!Send`.
