...
CC-14/CC-15). Следующий срез — **S11** (см. очередь ниже), ветка ещё не заведена.
...
- [ ] **S11 — следующий срез, НЕ начат.** Верхняя статья на `CC12_HOVER`
      по-прежнему трансляция scrollbar-псевдоэлементов: 356 из 828 элементов
      не проходят fast-path (у них своя база наследования — chrome часто
      меняет `font-size`/`color`), это 5.8мс, плюс ~2.4мс на само сравнение
      баз. Два варианта в BUG-341 «S10» → «Where this leaves CC-12»:
      (а) дешёвый и точный — мемоизировать построенную базу родителя по
      указателю (соседи сейчас строят её заново, это и есть те 2.4мс);
      (б) крупный, но семантический — гонять трансляцию только для элементов,
      у которых вообще может быть скроллбар (`overflow != visible` + корень/
      `<body>`/текстовые контролы). Вариант (б) ближе к WebKit (там
      `::-webkit-scrollbar` не наследуется), но это смена поведения —
      **нужно решение пользователя, не перф-срез явочным порядком**.
      Ниже по списку: `cs_apply` (3.7мс) и `cs_match` (3.0мс), а `lay_out`
      (5.3мс) + `build_box` (3.6мс) вместе уже сопоставимы со стадией
      каскада. Гейт — по счётчику, не по wall-clock.
