...
**BUG-171 этап 2: весь пайплайн (fetch+JS+layout) вне UI-потока**  branch: `p1-bug171-pipeline-offthread`
Next step: перенести финальный `parse_and_layout` из синхронного `LoadDone` на фоновый поток, на UI слать готовый display list + layout-снимок. Опирается на ADR-014 (QuickJS-рантайм уже `Send`-хэндл). `crates/shell/src/main.rs:6369` (LoadDone) → `parse_and_layout:3129`.
