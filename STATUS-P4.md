...
- **graphic test 114 (`contain-intrinsic-size`)** — прогнать `python graphic_tests/run.py --only 114 --build`; при FAIL завести BUG-NNN и починить. Логика детерминирована (Edge поддерживает contain-intrinsic-size идентично), но визуальная сверка в этой сессии не выполнялась (GUI/gdigrab).
