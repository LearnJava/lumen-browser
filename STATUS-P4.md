...
### `object-fit` / `object-position` (P5 audit 2026-06-08)
- **Status:** Stub ready. `// CSS: object-fit, object-position` at `lumen-layout/src/box_tree.rs:888` marks where the replaced-element viewBox scaling and alignment should be controlled. Current code uses a fixed fit-to-box scale; the algorithm hook exists.
- **P4 task:**
  1. Add `object_fit: ObjectFit` (enum: `Fill | Contain | Cover | None | ScaleDown`) and `object_position: [LengthPercentage; 2]` to `ComputedStyle`.
  2. Wire `apply_declaration()` for `object-fit` and `object-position`.
  3. At `box_tree.rs:888`, apply the fit algorithm: compute scale/offset from `object_fit` and `object_position` and pass to the SVG/image viewBox renderer.
- **Entry points:** `lumen-layout/src/box_tree.rs:888` — `// CSS: object-fit, object-position`.
