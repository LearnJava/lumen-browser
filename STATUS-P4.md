...
|------|----------|-----------|
| 2026-07-29 | `content: url()` (CSS Generated Content L3 §2.1) — `content: url(...)` на `::before`/`::after` теперь даёт inline-replaced image-сегмент (был `_ => None`, молча игнорировался). `content_to_inline_segments` перестроена: url() разрывает текстовый run и эмитит image-сегмент (`is_element_box`, `img_width` = явный `width` или `2em`-плейсхолдер как у inline-`<img>`); повторно использует существующие `img_src`-поля InlineSegment/InlineFrag → paint без изменений (DrawImage). Фетч: у генерируемой картинки нет DOM-элемента (обычный `collect_image_requests` слеп), поэтому сегмент помечен `source_node = NodeId::from_index(0)` и подхватывается post-layout background-проходом `collect_background_image_requests`; sentinel отличает её от реального `<img>` (двойной фетч / поломка `loading=lazy` исключены). Отложено: intrinsic-размер по декодированной картинке (сейчас `2em`-плейсхолдер без явного `width`). 4 unit-теста | `crates/engine/layout/src/box_tree.rs` (content_to_inline_segments) |
