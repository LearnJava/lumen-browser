...
|--------|-----------|--------|
| CSS Masking остаток: `mask-composite` (нужна multi-layer `mask-image` инфраструктура — сейчас `mask_image` не список), `mask-clip: no-clip/fill-box/stroke-box/view-box`, femtovg URL image-mask backend (сейчас no-op). `mask-repeat: space` ✅ 2026-07-18 (p4-mask-repeat-space) | `CSS-SPECS.md:429-435` · `crates/engine/paint/src/display_list.rs:4887` | M |
