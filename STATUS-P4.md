...
_(none)_ — последняя задача p4-anchor-functions влита 2026-07-15.
...
|--------|-----------|--------|
| CSS Masking остаток: `mask-composite` (нужна multi-layer `mask-image` инфраструктура — сейчас `mask_image` не список), femtovg URL image-mask backend (сейчас no-op). `mask-repeat: space` ✅ 2026-07-18 (p4-mask-repeat-space); `mask-clip: fill-box/stroke-box/view-box/no-clip` ✅ 2026-07-18 (p4-mask-clip-boxes) | `CSS-SPECS.md:429-435` · `crates/engine/paint/src/display_list.rs:4887` | M |
...
|------|----------|-----------|
| 2026-07-18 | `justify-items` — контейнерный default для block-детей (CSS Box Alignment L3 §6.3) — блочный ребёнок с `justify-self: auto` (дефолт) теперь резолвит выравнивание к `justify-items` родителя-блок-контейнера. Значение протянуто новым параметром `parent_justify_items` в `lay_out_inner` (по образцу `outer_floats`); только рекурсия in-flow блочных детей передаёт `s.justify_items`, все independent-FC вызовы — `AlignValue::Auto` (inline-start, без регрессий). На месте §5.2-сдвига `justify-self` вычисляется `effective_justify` (auto → parent). Явный `justify-self` ребёнка перекрывает. 5 unit-тестов | `crates/engine/layout/src/box_tree.rs` (lay_out_inner) |
| 2026-07-18 | `inert` UA-правило (HTML Rendering §15.4.2) — `apply_ua_inert` в pre-cascade UA-фазе выставляет `pointer-events: none` для элемента с атрибутом `inert` и всех его потомков (инертность наследуется вниз по DOM, `inert::is_inert` идёт по цепочке предков). UA-происхождение → author `pointer-events` перекрывает. Дополняет уже существовавший layout-фильтр clickable-набора (`lib.rs`, `// CSS: inert`). Теперь `ComputedStyle.pointer_events` отражает инертность (getComputedStyle/cursor). 4 unit-теста | `crates/engine/layout/src/style.rs` (apply_ua_inert) |
| 2026-07-18 | `scrollbar-gutter` block-axis (CSS Overflow/Scrollbars L1 §6.2) — `overflow-x: scroll/auto` + `scrollbar-gutter: stable` теперь резервирует место под горизонтальный скроллбар, урезая `children_available_height` через уже существовавший `scrollbar_gutter_block()` (снят `#[allow(dead_code)]`); симметрично inline-осевому урезанию `content_width`. Собственная border-box высота бокса не меняется, ужимается только контент-область для `%`-высоты детей. `both-edges` для блок-оси не удваивается (не определён спекой). 5 unit-тестов | `crates/engine/layout/src/box_tree.rs` (children_available_height) |
| 2026-07-18 | `isolation: isolate` (CSS Compositing & Blending L1 §2.1) — изолированная blend-группа: `box_layer_ops` эмитит full-alpha offscreen-слой (`PushOpacity{1.0}`) когда `isolate` — единственный триггер (opacity<1/filter/backdrop-filter/mix-blend-mode уже изолируют сами) и бокс владеет stacking-context'ом; переиспользует существующую offscreen-машинерию opacity → авто-паритет femtovg/cpu/wgpu, без нового DisplayCommand. Stacking-context уже создавался. 3 unit-теста + граф-тест 148 | `crates/engine/paint/src/display_list.rs` (box_layer_ops) |
| 2026-07-18 | `background-repeat: space` / `mask-repeat: space` (CSS Backgrounds L3 §3.4) — общий хелпер `space_axis_geometry`: целые плитки прижаты к обоим краям, остаток распределён равными зазорами; `bg_tile_geometry` теперь возвращает `step_x/step_y`, все 4 копии тайлинг-цикла (femtovg+CPU+wgpu фон/маска) шагают по `step`. Раньше `space` молча сваливался в `repeat`. 3 unit-теста + граф-тест 147 | `crates/engine/paint/src/display_list.rs` (space_axis_geometry) |
| 2026-07-15 | `anchor()`/`anchor-size()` функции (CSS Anchor Positioning L1 §3.1/§4) — двухфазный резолв (локальный+глобальный registry, как у `inset-area`); по ходу найдены и исправлены 2 бага: отсутствующая конверсия абсолютных координат анкера в CB-relative offset, и стухший `anchor-size()` registry для sibling-анкеров с `position:absolute` | `crates/engine/layout/src/anchor.rs` |
| 2026-07-15 | `position: sticky` — резолв `%`/`em`/`rem` insets (`collect_sticky_boxes` теперь берёт `viewport` и резолвит через `LengthOrAuto::resolve()` вместо `to_px_opt()`); оставшийся P3-remainder (scroll-driven paint transform) не тронут | `crates/engine/layout/src/lib.rs:566-576` |
</content>
