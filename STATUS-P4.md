...
| Date | Property | Notes |
|------|----------|-------|
| 2026-06-21 | `text-anchor` / `dominant-baseline` as CSS properties | SVG 2 §11.6 / §11.10.2; раньше работали только как presentation-атрибуты (`parse_text_anchor`/`parse_dominant_baseline` в box_tree). Добавлены наследуемые `ComputedStyle.text_anchor: Option<SvgTextAnchor>` / `dominant_baseline: Option<SvgDominantBaseline>` (`None` = не задано CSS → атрибут/initial применяется при сборке box). Parse в `apply_declaration` (`start\|middle\|end` / `auto\|baseline\|hanging\|middle\|central\|text-before-edge\|text-after-edge`, unknown → keep) + inherit/unset + initial. Ключ: оба свойства добавлены в `ATTRS` списка `apply_svg_presentational_hints`, поэтому presentation-атрибут складывается в каскад как декларация наименьшего приоритета — author CSS его переопределяет, и значение наследуется от `<g>`/`<svg>` (чего не было при чтении атрибута только с самого `<text>`). box_tree читает `style.text_anchor.unwrap_or_default()` (старые `parse_*` функции удалены). Paint-алгоритм (`emit_svg_text` в display_list.rs) уже умел оба значения. 6 unit-тестов style.rs (parse/default/invalid/inherit-unset/initial) + 5 end-to-end box_tree.rs (attr middle, dominant-baseline hanging, CSS-only, CSS-overrides-attr, container-inheritance, default) + graphic test 127 + CPU-снэпшот 127 (snapshot_cpu.rs; pixel-diff с Edge исключён — глифы расходятся, rule #3; проверено `--dump-display-list`: start→x, middle→x−w/2, end→x−w) |
| 2026-06-20 | `inverted-colors` media feature | Media Queries L5 §5.8; `MediaInvertedColors` enum (None/Inverted) + `MediaFeature::InvertedColors` + `MediaContext.inverted_colors` поле (desktop-дефолт `None` в `Default` — ОС не инвертирует цвета); `parse_media_feature()` разбирает `inverted-colors: none\|inverted` (невалидное → Unsupported); `MediaFeature::matches()` сравнивает с ctx. Re-export `MediaInvertedColors` из css-parser lib.rs; `media_context_from_viewport` (layout) и `matchMedia` (js/dom.rs) берут дефолт через `..Default::default()` (изменений в call-site не потребовалось). OS/UA-интеграция (реальный режим инверсии цветов) деферирована shell-у — всегда matched none, как prefers-reduced-motion. 3 unit-теста parser.rs + graphic test 126 (none matched зелёный, inverted/maybe красные; Edge тоже по умолчанию inverted-colors:none) + демо в 1000000-final |
| 2026-06-19 | `scripting` media feature | Media Queries L5 §6.2; `MediaScripting` enum (None/InitialOnly/Enabled) + `MediaFeature::Scripting` + `MediaContext.scripting` поле (desktop-дефолт `Enabled` в `Default` — у Lumen встроенный QuickJS, как в Edge); `parse_media_feature()` разбирает `scripting: none\|initial-only\|enabled` (невалидное → Unsupported); `MediaFeature::matches()` сравнивает с ctx. Re-export `MediaScripting` из lib.rs; `media_context_from_viewport` (layout) и `matchMedia` (js/dom.rs) берут дефолт через `..Default::default()` (изменений в call-site не потребовалось). Real OS/UA-интеграция (отключение JS пользователем) деферирована shell-у — всегда matched enabled. 4 unit-теста parser.rs + graphic test 125 (enabled matched зелёный, none/initial-only/invalid красные; Edge тоже scripting enabled) + демо в 1000000-final |
| 2026-06-19 | `prefers-reduced-transparency` media feature | Media Queries L5 §5.7; `MediaReducedTransparency` enum (NoPreference/Reduce) + `MediaFeature::PrefersReducedTransparency` + `MediaContext.prefers_reduced_transparency` поле (desktop-дефолт `NoPreference` в `Default`); `parse_media_feature()` разбирает `prefers-reduced-transparency: no-preference\|reduce` (невалидное → Unsupported); `MediaFeature::matches()` сравнивает с ctx. Re-export из lib.rs; `media_context_from_viewport` (layout) и `matchMedia` (js/dom.rs) берут дефолт через `..Default::default()` (изменений в call-site не потребовалось). OS/UA-интеграция деферирована shell-у (всегда matched no-preference, как prefers-reduced-motion). 3 unit-теста parser.rs + graphic test 124 (no-preference matched зелёный, reduce/invalid красные; Edge тоже по умолчанию no-preference) + демо в 1000000-final |
| 2026-06-19 | `@supports font-tech()` / `font-format()` | CSS Conditional L4 §4 / CSS Fonts L4 §4.3; раньше функциональные тесты `font-tech(...)`/`font-format(...)` падали в `SupportsCondition::Unknown` → всегда `false`. Добавлены типизированные варианты `SupportsCondition::FontTech(String)` / `FontFormat(String)` (lowercase-аргумент, у `font-format` снимаются legacy-кавычки) + хелпер `match_func_arg()` для разбора `name( ... )` с балансом скобок. `evaluate()` сверяет аргумент со списками реальных возможностей `lumen-font`: `SUPPORTED_FONT_TECH = [features-opentype, variations]` (GSUB/GPOS + fvar/gvar/avar/HVAR/MVAR) и `SUPPORTED_FONT_FORMAT = [opentype, truetype, woff, woff2]` (`decode_woff1`/`decode_woff2` + glyf/CFF). Цветные глифы (COLR/CPAL, sbix, CBDT, SVG-in-OT), палитры, AAT/Graphite, incremental и контейнеры `collection`/`embedded-opentype`/`svg` → `false`. Сигнатура `evaluate()` не менялась — оба call-site (layout style.rs, js/dom.rs `CSS.supports`) получают поведение автоматически. 5 новых unit-тестов parser.rs (parse keyword/string, eval tech, eval format, комбинаторы) + graphic test 123 (verified `--dump-display-list`: a/b/c/d зелёные, e/f красные, g зелёный; Edge рендерит идентично — используются только согласованные с Edge кейсы) |
| 2026-06-19 | `line-height-step` | CSS Rhythmic Sizing L1 §2; inherited `ComputedStyle.line_height_step: f32` (initial `0.0` = off), парсинг `<length>` в абсолютные px (`em`/`rem` от собственного font-size; `none`/`normal`/`0`/отрицательное → off; проценты невалидны → игнор) + inherit/unset + `SUPPORTED_PROPERTIES`. `step_line_height(raw, step)` (box_tree.rs) округляет высоту line-box вверх до кратного шагу; применён к line-height inline-формат-контекста и высоте блока (вкл. `::first-line`). Paint `emit_inline_run` (display_list.rs) дублирует округление, чтобы укладка строк совпадала с layout; цветные inline-`<span>` фоны заливают округлённый line-box (нетекстовый сигнал в тесте). 6 unit-тестов style.rs + 2 box_tree.rs + graphic test 122 + демо в 1000000-final |
