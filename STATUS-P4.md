...
## In progress
_(none)_ — p4-computed-style-json влит 2026-06-10
...
### CSS Color 4 system colors — `Canvas`/`CanvasText`/`ButtonFace`/… (P1 feature p1-y4-color-scheme, 2026-06-10)
- **Status:** Resolution algorithm ready: `system_color(name_lc: &str, dark: bool) -> Option<Color>` in
  `lumen-layout/src/style.rs` (рядом с `named_color`). Резолвит ~20 системных ключевых слов
  (`Canvas`, `CanvasText`, `Field`, `FieldText`, `ButtonFace`, `ButtonText`, `LinkText`, `Highlight`,
  `GrayText`, `Mark`, `AccentColor`, …) в light/dark RGB. Used-scheme резолв — `ColorScheme::used_dark(prefer_dark)`.
  Form-control UA theming уже использует `used_dark` (см. `compute_style` `// CSS: system-color`).
- **P4 task:**
  1. Add a `CssColor::System(String)` (или enum системных цветов) variant; parse system-color keywords
     in the colour path (`parse_css_color_legacy` / `parse_color`), сохраняя имя (не RGB) для
     used-value-time резолва.
  2. At used-value time (paint/display-list или конец `compute_style`), резолвить `CssColor::System(name)`
     через `system_color(name, style.color_scheme.used_dark(dark_mode))`.
  3. Аналогично `currentcolor`: системный цвет контекстно-зависим, нельзя «запекать» в RGB на этапе
     парсинга — нужна тема элемента.
- **Entry points:** `lumen-layout/src/style.rs` — `system_color`, `ColorScheme::used_dark`;
  `compute_style` `// CSS: system-color` comment at the form-control UA block.
...
### `interpolate-size: allow-keywords` height transitions (P1 feature p1-bb10-interpolate-size, 2026-06-13)
- **Status:** Height transition algorithm ready in `lumen-layout/src/animation.rs`:
  - `InterpolateSizeMode` enum (`NumericOnly` | `AllowKeywords`, default `NumericOnly`) in `style.rs` (after `ContentVisibility`).
  - `TransitionScheduler` now animates `height` (5th entry in the `animatable` table in `sync()`): numeric `height` interpolates linearly; `height: auto` (None) resolves to its px value via `set_auto_height(node, px)` cache.
  - `AnimatedStyle.height: Option<Length>` carries the per-frame override (requires relayout, not compositor-offloadable).
  - `resolve_auto_height()` substitutes `Discrete("auto")` → `Length::Px` when the cache has a value.
  - 4 unit tests pass.
- **P4 task:**
  1. Add `pub interpolate_size: InterpolateSizeMode` field to `ComputedStyle` (initial `NumericOnly`). It is **inherited** per CSS Sizing L4 §4.5 — add to the inherited-copy path in `compute_style`.
  2. Parse `interpolate-size: numeric-only | allow-keywords` in `apply_declaration()`.
  3. Gate the auto-height resolution in `TransitionScheduler::sync()`: only resolve `Discrete("auto")` endpoints when `new.interpolate_size == InterpolateSizeMode::AllowKeywords`. Currently the algorithm always resolves if the cache has a value — wrap the `auto_resolved_px` computation in that condition.
  4. Shell wiring (describe for P3): after layout, call `scheduler.set_auto_height(node, resolved_content_height)` for elements with a `height` transition whose computed `height` is `auto`. Consumer reads `AnimatedStyle.height` and forces relayout with that height.
- **Entry points:** `lumen-layout/src/animation.rs` — `// CSS: interpolate-size` comments at the `animatable` table and the `auto_resolved_px` block in `sync()`; `InterpolateSizeMode` in `style.rs`.
