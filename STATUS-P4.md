...
|--------|-----------|--------|
| `:is()` / `:where()` / `:has()` selector matching | `CSS-SPECS.md:703` (сейчас `PseudoClass::Unsupported` в `crates/engine/css-parser/src/parser.rs`) | M |
| `@layer` / `@scope` / `@container` nested at-rule cascade wiring (2nd pass) | `CSS-SPECS.md:720` · `crates/engine/css-parser/src/parser.rs:900,3418` (nested at-rules внутри body не поддержаны) | L |
| `:state()` custom-state pseudo-class | `crates/js/src/element_internals.rs:183` (JS-сторона готова — `ElementInternals.states`; нет `PseudoClass::State`/парсинга `:state(name)`/матчера в `style.rs`) | M |
| `revert` — применение UA-stylesheet revert | `CSS-SPECS.md:162` (сейчас только парсится) | S |
...
| 2026-07-1x | `style()` container queries — честная containing-block height для % | commit `9e007f3b` |
| 2026-07-1x | `style()` container queries — honest per-property % basis | commit `5ac98b16` |
| 2026-07-04 | `mask-clip` painting-area clip (CSS Masking L1 §4.6) | commit `cb933907` |
</content>
