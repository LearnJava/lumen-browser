...
**Статус:** OPEN (срез 1 влит 2026-08-11: катастрофический кадр объяснён и снят — фоновый
прогрев ленивых пайплайнов; остаток ~0.24–0.33 с на expose-кадре не объяснён, см. «Срез 1» ниже)
...
1. **Expose-кадр всё ещё стоит 0.24–0.33 с** и в прогретом плече — это и есть оставшийся
   BUG-405. Ленивые пайплайны исключены измерением выше; исходные кандидаты (растеризация
   глифов в атлас, декод/upload впервые показанных картинок) не проверены ни разу.
2. Починить guard в `scroll_perf.py:179` (P2) и добавить `[frame] paint`-разбивку в wgpu-путь —
   тогда expose-кадр можно будет разложить по фазам, а не приписывать ему `[wgpu]`-строки
   вручную, как в срезе 1.
3. Кадр 0 (0.4–0.6 с) к этому багу не относится — там компилируются 5 стартовых пайплайнов,
   это BUG-406.
