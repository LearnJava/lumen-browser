...
**Статус:** OPEN — **последний срез S41 (2026-09-03)**; остаток по существу
один и назван в §S41 «Не сделано».

**Читать шапку с поправкой на S41.** Заголовок («~300× over budget») описывает
состояние на 2026-07-25 и устарел уже после частичной правки того же дня (6-7×).
Прескрипция «правильная общая правка — layout-result cache» из «Fix scope note»
ниже **отменена измерениями**: кэш строили и мерили четыре раза (S32 eager,
S33 снят, S36 повторно, S38 lazy) — все net-negative; S39 объяснил почему
(абсолютные координаты в `LayoutBox` → hit обязан перепозиционировать
поддерево), S40 закрыл оставшийся вариант «share, не clone» измерением
(3.1% hit-rate).

**S41 закрыл сам двойной проход, ради которого баг заведён** — и не
переиспользованием результата, а устранением второго вызова: проба Step-1 и
финальная раскладка были *разными* вызовами (разная поперечная ширина) без
причины. На документе хрома двойных раскладок **21 → 4**, реальных вызовов
`lay_out_inner` за полный проход **1038 → 949**. Остаток из 4 — обязательные
по корректности отказы (`content-visibility: auto` / консультация с
неопределённой высотой). Попутно найден и исправлен отдельный дефект геометрии
(горизонтальные поля колоночного flex-элемента списывались дважды).
...
## S41 — двойного прохода больше нет: проба и финальная раскладка были разными
вызовами не по существу, а из-за поперечной ширины. 21 → 4 двойных раскладки,
−8.6% всех вызовов `lay_out_inner`; попутно найден дефект геометрии — поля
колоночного flex-элемента списывались дважды

Ветка `p3-bug341-s41`, 2026-09-03. Возобновлено по явному запросу
пользователя, формулировка задачи — «реши то, что названо в §S40 “Not
attempted”».

### Перепись до правки

Новый счётчик `FlexColumnCensus` (`box_tree/diagnostics.rs`, включается
`set_flex_column_census`), печатается строкой `[s41-double]` из уже
существующего `bug341_s30_flex_key_census`. Он смотрит на остаток с другой
стороны, чем S30-S40: не «сколько вызовов мог бы обслужить кэш» (вопрос
закрыт на 3.1%), а «сколько вторых вызовов всё ещё делается и почему».

```
[s41-double] KEY cycle=0 needed=77 memo_served=56 probed=21 replayed=0
             double=21 (100.0% of probed) dirty=4 size=0 (grew=0) cross=17
```

Результат опроверг гипотезу, с которой срез начинался. Ожидалось, что остаток
несут элементы, которым flex-grow/shrink поменял главный размер (тогда лечение
— мерить intrinsic-высоту, не раскладывая). Таких **ноль**. Реальная причина у
17 из 21 — `cross`: поперечный размер элемента в финальном проходе не совпадал
с шириной, на которой бежала проба. То есть второй полной раскладки требовало
не отсутствие кэша и не природа алгоритма, а то, что **два вызова были сделаны
с разными аргументами без всякой на то необходимости**.

### Дефект геометрии, найденный по дороге

Проба «до правки» на минимальном документе (колонка 200px, элемент с
`margin-left:20px; margin-right:10px`) напечатала `a.x=20 a.width=140` при
ожидаемых 170. `lay_out_flex` отдавал `lay_out_inner` в качестве
`available_width` **border-box** поперечный размер, а `lay_out_inner` трактует
этот аргумент как место для *margin-box*'а и вычитает поля элемента ещё раз.
Итог: горизонтальные поля колоночного flex-элемента списывались дважды.

Дефект пережил и графтесты, и 3762 юнит-теста, потому что смещения не даёт:
`x` правильный, элемент просто уже на величину своих полей — щель по правому
краю. Это тот же повторяющийся шаблон, что и в BUG-451/452: заявка называла
один дефект (перф), проба перед правкой нашла второй (геометрия), и они
оказались одним и тем же местом кода.

### Правка

Обе части — в `lay_out_flex`, вынесены в общую функцию
`column_item_avail_cross`:

1. **Ширина, отдаваемая `lay_out_inner`, теперь margin-box** (`used_cross +
   m_l + m_r`), а не border-box. Это чинит двойное списание полей и делает
   растянутый элемент равным обычному блоку в потоке — что и проверяет один из
   новых тестов, сверкой flex-элемента с блочным ребёнком того же контейнера.
2. **Поперечная ширина резолвится ДО Step-1, и проба бежит на ней.**
   Зависимости от пробы у этого расчёта нет и не было: оба входа
   интринсиковые (`max_content_outer_width`/`min_content_outer_width` читают
   стиль и содержимое, не `rect` — свойство, на которое памятка BUG-802 уже
   опиралась), а выравнивание — чистый стиль. Ключ `FLEX_COLUMN_PROBE_HEIGHTS`
   переведён на ширину пробы, иначе выровненный и растянутый элементы одного
   узла столкнулись бы в одной ячейке.

Условие переигровки BUG-802 лишилось третьего члена («поперечный размер равен
полной ширине контента»): он больше **не может** не выполниться, потому что
проба и финальный проход теперь по построению идут на одной ширине. Счётчик
`double_cross` оставлен в переписи именно как регрессионная проверка этого
инварианта — он обязан читаться нулём.

Побочно правка ближе к спеке: CSS Flexbox §9.2 хочет content-based базовый
размер, померенный по **собственному** поперечному размеру элемента, а не по
ширине контейнера.

### Измерено

Тот же документ хрома, `--profile dev-release`:

```
[s41-double] KEY cycle=0 needed=61 memo_served=40 probed=21 replayed=17
             double=4 (19.0% of probed) dirty=4 size=0 (grew=0) cross=0
```

| | было | стало |
|---|---:|---:|
| двойных раскладок за проход | 21 | **4** |
| переигровок пробы | 0 | 17 |
| элементов, которым вообще нужен базовый размер | 77 | 61 |
| реальных вызовов `lay_out_inner` за полный проход | 1038 | **949** |

Падение `needed` с 77 до 61 — не изменение критерия, а тот же эффект второго
порядка: 17 элементов больше не раскладываются повторно, поэтому вложенные в
них flex-контейнеры не входятся заново. Счётчик вызовов (−8.6%)
детерминированный, от машины не зависит — в отличие от стены.

CC-12, 4 чередующихся раунда `main`/ветка на одной машине в одну сессию
(бюджет 2 мс):

```
KEY   p95  main 1.55 / 2.86 / 2.77 / 2.81   ветка 1.31 / 2.19 / 2.21 / 2.03
HOVER p95  main 1.35 / 0.96 / 1.04 / 1.97   ветка 1.22 / 1.82 / 0.99 / 3.60
```

KEY — лучше во всех четырёх раундах, и это как раз то плечо, которое на этой
машине сегодня вылезало за бюджет. HOVER — знак разницы скачет, по
`docs/perf-method.md` §4 это шум, а не регрессия; `p50` не двигается
(0.42/0.42/0.40/0.51 против 0.39/0.43/0.42/0.53).

### Гейты

`cargo test -p lumen-layout --lib`: 3765 прошло, 0 упало (3 теста добавлены
этим срезом). `lumen-paint` 1054, `lumen-shell` 1719 — зелёные. Clippy обоих
крейтов чистый. `python graphic_tests/dump_golden.py` — все 12 дампов
совпадают с эталоном байт в байт. Детерминированный CPU-набор
(`cargo test -p lumen-driver --features cpu-render cases::snapshot_cpu`) —
зелёный.

Плюс сверка всего корпуса в режиме `--ipc` (снимок по TCP через CPU-путь —
единственный, который можно гонять без захвата экрана): `main` и ветка
прогнаны подряд на одной машине, **все 156 тестов дали одинаковый процент
расхождения до последней цифры**, набор FAIL/DEBTOR совпал (13 FAIL, 48
должников — те же id). Это не замена официального гейта (`--ipc` рисует
CPU-путём, он не в паритете с femtovg живого окна — `docs/graphic-tests.md`),
но вместе с тремя эталонными наборами выше это независимое подтверждение, что
на корпусе картинка не поехала. Артефакты обоих `--ipc`-прогонов удалены, а
не закоммичены: `graphic_tests/results/*.json` — журнал официального прогона,
и подкладывать туда другое измерение нельзя.

### Не сделано

- ~~Полный пиксельный прогон `graphic_tests/run.py --continue-on-fail`~~ —
  **выполнен и влит тем же днём** (`9d26d22ec`, ветка `p3-bug341-s41`,
  смёрджена `6f93c4fd5`): против официального прогона 2026-09-01 ухудшений
  0, 3 улучшения (не от этой правки — отдельная дорожка LIB/SVG, влитая
  между прогонами). Этот пункт был закрыт до влития S41, но список остался
  недообновлён — правка P3, 2026-09-19.
- Оставшиеся 4 двойные раскладки — отказы по `content-visibility: auto` и по
  консультации с неопределённой высотой. Это не «недоделка»: обе величины не
  являются свойством одного бокса, поэтому переигрывать пробу там нельзя. Их
  можно убрать только сузив сами флаги (сегодня `INDEFINITE_HEIGHT_CONSULTED`
  ставится на всё поддерево, а нужен ответ «изменился бы результат?»). Не
  дефект в рамках этого бага.
- Убирание churn'а «один узел — несколько `LayoutBox`» (назван в S40) —
  по-прежнему не тронуто; именно оно подняло бы hit-rate механизма S40. Это
  дальнейшая перф-оптимизация, а не исправление конкретного дефекта —
  двойной проход, ради которого заведён этот баг, S41 уже устранил.

**Итог P3-ревизии 2026-09-19:** живого дефекта, который мог бы взять P3 как
баг, в этой заявке больше нет — заголовочный симптом (двойной layout)
устранён S41 и проверен полным гейтом; оба оставшихся пункта — либо
корректное поведение по спеке, либо будущая перф-работа архитектурного
масштаба (см. отменённые попытки кэша S32/S33/S36/S38 выше). Статус
оставлен `OPEN`, а не переведён в `FIXED`, потому что название бага
(«~300× over budget») и историческая шапка описывают ещё не закрытый
перф-гейт CC-14 (заблокирован независимой находкой `bind_model`/
`graft_geometry`, обе части которой уже исправлены отдельно — см. историю
выше); закрывать заявку целиком — решение владельца перф-трека (P1/CC-14),
не этой ревизии.
