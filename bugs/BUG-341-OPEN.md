...
- **Полный пиксельный прогон `graphic_tests/run.py --continue-on-fail`**
  (~20 мин, gdigrab, нельзя в фоне и нельзя трогать фокус) на момент записи
  не выполнен — правка двигает пиксели там, где у колоночного flex-элемента
  есть горизонтальные поля, так что этот гейт обязателен перед тем, как
  считать срез полностью сданным.
- Оставшиеся 4 двойные раскладки — отказы по `content-visibility: auto` и по
  ...
  ставится на всё поддерево, а нужен ответ «изменился бы результат?»).
- Убирание churn'а «один узел — несколько `LayoutBox`» (назван в S40) —
  по-прежнему не тронуто; именно оно подняло бы hit-rate механизма S40.
