...
## Подтверждение 2026-09-09 (P6, гейт BUG-557): набор стабилен, 6-й рецидив

На ветке `p6-e2e4-bug557` (правка только в JS-шиме, ни одна из семи страниц и ни
одного `.rs` не тронуты) `scoped-test.sh` даёт **тот же самый** список из семи
PNG с теми же числами (`55-text-rendering` 28713, `57-canvas-2d` 3600,
`32-list-markers` 79941, `34-forms` 4734, `45-multiple-backgrounds` 32024,
`51-scrollbar-rendering` 10092, `1000000-final` 2604), а `dump_golden.py` — те же
«4 из 12» с тем же диффом `14.40 → 13.41`. Дополнительный аргумент к разделу
выше: пять из семи страниц (`55`, `32`, `34`, `45`, `51`) вообще не содержат
`<script>`, а в `57-canvas-2d` скрипт не трогает ни одного изменённого члена —
то есть дрейф воспроизводится побайтово между ветками и не зависит от JS.
Влито по той же политике, что и BUG-805: гейт красный независимо от ветки.
