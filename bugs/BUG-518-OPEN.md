...
## Срез P3 2026-09-07 (срез 7)

Implemented `CSSStyleSheet.insertRule`/`.deleteRule` on an **owned**
(`document.styleSheets`) sheet — срез 6 left this as the last gap blocking
`mixin-cssom.tentative.html`'s 6th subtest. Until this slice only a
*constructed* sheet had the two methods (CSSOM-5 срез 3).
...
Status remains `OPEN` — the entire remaining scope is
`mixin-invalidation.tentative.html`, which needs a live per-declaration
`.style` setter for CSSOM rules (a gap broader than this bug, not
mixin-specific, noted since срез 6).
