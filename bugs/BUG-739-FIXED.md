...
* ширина самого `inline-grid`-контейнера — shrink-to-fit по-прежнему меряет его
  ...
  [BUG-740](BUG-740-FIXED.md) (закрыт round-robin аппроксимацией без полного
  track sizing) — после этого фикса дефект стал видимым, а не только
  измеримым;
