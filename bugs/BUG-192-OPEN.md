...
Живой прогон `--only 55` дважды подряд (второй раз `--no-cache`, для проверки
воспроизводимости) даёт стабильные **0.54%** — точное совпадение с baseline, без
дрейфа в любую сторону. Diff-картинка (`55-video-placeholder-diff.png`) структурных
отличий не показывает — только фоновый шум подписей `.label` и border-бокса
(тот же font-parity, что и в исходном расследовании). Недавние правки canvas-парсинга
(BUG-451/452, 2026-08-30/31) и правка порядка cascade/layout (BUG-443, 2026-08-30)
на этот тест не повлияли — новых дефектов не найдено, ратчет не требуется.
