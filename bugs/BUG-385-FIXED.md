...
4. Реальное перечисление шрифтов (Phase 1) обязано спрашивать разрешение
   `local-fonts` и требовать transient activation — на момент заведения
   разрешение отвечало `granted` по умолчанию, см. [BUG-386](BUG-386-FIXED.md);
   закрывать этот баг, не закрыв BUG-386, значило выдать список установленных
   шрифтов молча. BUG-386 закрыт 2026-08-10: `local-fonts` теперь `denied`.
...
6. **Перечисления шрифтов ОС по-прежнему нет.** Phase 0 отвечает `[]`. Пока
   был открыт [BUG-386](BUG-386-FIXED.md), это был единственный ответ, который
   ничего не выдаёт: `local-fonts` был разрешён по умолчанию. С его закрытия
   (2026-08-10) гейт §2 держит ветку закрытой сам — `local-fonts` = `denied`.
...
* [BUG-386](BUG-386-FIXED.md) — `permissions.query({name:'local-fonts'})`
  отвечал `granted` по умолчанию; вместе с этим багом определял, каким окажется
  поведение по умолчанию, когда перечисление шрифтов появится. Закрыт
  2026-08-10: ответ — `denied`, гейт §2 закрыт до появления способа спросить
  пользователя.
