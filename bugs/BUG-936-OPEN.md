...
## Перемер 2026-09-03 (P3, попутно с BUG-341 S41): 10.02 % → 2.46 %, причина не найдена

Полный gdigrab-прогон ветки `p3-bug341-s41` дал по TEST-156 **2.46 %** вместо
записанных 10.02 %, и `run.py` потребовал снизить baseline. Прежде чем двигать
храповик, число проверено на воспроизводимость и на атрибуцию:

- **Не флак захвата.** Два измерения на *разных* коммитах с одним и тем же
  эталоном Edge (снят 2026-09-01 10:13, лежит в слоте `p3-work`): полный
  прогон на ветке и точечный `--only 156` на `main` до неё — оба ровно 2.46 %.
- **Не заслуга ветки BUG-341 S41.** Её нейтральность показана отдельным A/B
  `main`-против-ветки: все 156 тестов дали одинаковый процент до последней
  цифры. Плюс точечный прогон выше сделан именно на коде *без* этой правки.
- **Причина не атрибутирована коммитом.** В окне между прогоном 2026-09-01
  10:22 (где было 10.04 %) и 2026-09-03 `crates/engine/paint` трогали только
  переносы `SPLIT-DL13…19`, заявленные как поведенчески-нейтральные. Либо один
  из них починил композит побочно, либо исходные 10.02 % сняты при другом
  состоянии GPU-бэкенда — записи `[wgpu] adapter` для того прогона нет, а
  CLAUDE.md прямо предупреждает, что paint-число без бэкенда не значит ничего.

**Баг не закрыт.** 2.46 % — это по-прежнему маска, видимая не полностью, а не
паритет с Edge; порог теста 0.5 %. Изменён только baseline храповика
(`KNOWN_DEBTORS['156']` 10.02 → 2.46), чтобы следующий прогон не выдавал FAIL
за улучшение. Кто возьмётся за корень — начинать надо с перепроверки самого
симптома: описание выше («контент рендерится немаскированным на всех 6
панелях») снято при 10 %, при 2.46 % оно почти наверняка уже не соответствует
картинке, и первым шагом нужен свежий визуальный diff, а не чтение кода.

## Перемер 2026-09-20 (P3): структурный дефект композита не воспроизводится — остаток AA-шум на кривых краях

Живой Edge-diff в этой сессии недоступен (`graphic_tests/run.py --only 156`
падает на TEST-00: gdigrab не находит калибровочный магента-маркер — в этой
песочнице нет реального фокусируемого окна для захвата экрана). Вместо этого
— два headless GPU-теста через `Renderer::new_headless`/`render_to_image`
(реальный wgpu-адаптер, без окна и без OS-захвата экрана):

1. **`crates/engine/paint/tests/cases/headless_tests.rs`** — четыре
   синтетических `PushOpacity`+`PushMaskLayer`/`PopMaskLayer` сценария,
   покрывающих ровно то, что раньше было проверено только статическим чтением
   кода: alpha-режим (`mask_layer_composite_applies_alpha_mask`), тот же
   сценарий под накопленным `PushTransform`
   (`..._under_transform` — прямая проверка «непроверенного» пункта из отчёта
   выше про квад композита), luminance-режим с серой заливкой
   (`..._applies_luminance_mask`) и составная маска из трёх прямоугольников
   в одном `Draw`-батче одного уровня, как panel 3 "H" на реальной странице
   (`..._applies_compound_mask`). **Все четыре проходят** — механизм
   `MaskLayerComposite` применяет маску корректно в каждом из этих случаев.
2. **`crates/driver/tests/cases/bug936_mask_layer_wgpu_vs_cpu.rs`** — прямое
   сравнение РЕАЛЬНОГО `graphic_tests/156-svg-mask.html`: один и тот же
   display list (`InProcessSession::display_list_for_compare`) рендерится
   через уже подтверждённый корректным CPU-путь (`screenshot_cpu_rgba`) и
   через headless wgpu (`Renderer::render_to_image`), затем пиксели
   сравниваются напрямую — без Edge, без окна, без gdigrab. Результат:
   **0.84 % расхождения** (6165/737280 px, строки 46–332, столбцы 21–639 —
   геометрически те же панели, что и старый Edge-diff bbox `x:20–660
   y:21–341`). Контрольный сдвиг wgpu-кадра на ±1px по любой оси **ухудшает**
   совпадение (0.84 % → 1.2–1.8 %), то есть это не проблема округления квада
   композита на целый пиксель. Выборка расходящихся пикселей — исключительно
   серые полутона близкой яркости (например `(131,131,131)` против
   `(196,196,196)`) и лёгкий цветной "проступ" на месте, где CPU даёт мягкий
   AA-край (`(249,225,225)`), а wgpu — чистый белый фон (255,255,255):
   характерная картина расхождения *качества сглаживания* на кривых/составных
   краях маски (circle/path-панели), а не отсутствия маскирования — ни одного
   образца с полностью немаскированным контентом (`(255,0,0)`-подобным) там,
   где CPU показывает фон, не найдено.

**Вывод: исходный симптом («GPU-композит не даёт видимого эффекта — контент
рендерится немаскированным») на текущем коде не воспроизводится ни в одном
проверенном сценарии.** Композит корректно обрабатывает alpha/luminance,
трансформы и составные маски. Остаток — это независимое расхождение
сглаживания между CPU (`tiny-skia`, аналитический AA) и wgpu-путём на
кривых краях, того же рода, что и общая известная особенность «CPU-растеризатор
и GPU — независимые пути» (см. CLAUDE.md «Known gotchas»); гоняться за
пиксельным паритетом двух независимых растеризаторов — отдельная, гораздо более
узкая задача, не «маска не работает». Статус (DEBTOR) и baseline ратчета не
менялись: 2.46 % — метрика другого измерения (Edge-diff живого окна), которое
в этой сессии переизмерить не удалось; закрывать баг на основании
headless-CPU-vs-wgpu числа было бы подменой метрики.

Тесты добавлены как постоянное регрессионное покрытие
(`#[ignore = "requires GPU adapter"]`, `cargo test -p lumen-paint --test all
-- --include-ignored mask_layer_composite` и `cargo test -p lumen-driver
--test all -- --include-ignored bug936`) — до этой сессии `MaskLayerComposite`
не имел ни одного автоматического теста, только статическое чтение кода и
Edge-скриншоты.
