...
Живой прогон `python graphic_tests/run.py --only 120 --no-cache` дважды подряд дал
стабильные 3.26% — без ратчета. Diff-картинка (`120-media-contrast-data-diff.png`) —
один сплошной прямоугольник ровно на месте swatch `.b`, никаких новых структурных
отличий или font-parity-шума в остальной части страницы. Диагноз не изменился:
единственный источник diff — `prefers-reduced-data`, которую Edge не поддерживает.
Точечного P3-дефекта нет, указатель STATUS-P3.md снят.
