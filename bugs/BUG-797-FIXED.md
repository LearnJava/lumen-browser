...
## Срез 8 (GAP-NAVCTX срез 16, 2026-09-13, `p1-gap-navctx-srez16`) — закрытие

Единственный пункт «не в этом срезе», оставшийся от среза 7, — BUG-883
(таймеры опенера в фоне) — закрыт отдельным срезом
([BUG-883](BUG-883-FIXED.md), 2026-09-13). С ним сняты все известные
блокеры собственной заявки этого бага: `opener`/`postMessage` реальны
(срезы 4/5), `windowFeatures`'s `noopener`/`noreferrer` парсится (срезы
6/7), фоновые таймеры опенера тикают (BUG-883). `rel="opener"`-карветаут
по-прежнему не реализован и не проверялся — не встречался ни в одном
пробнике, вне бюджета закрытия.

**Живая проверка** (`tests/wpt/run_report.py --root
html/semantics/embedded-content/bfcache --recursive --all`, dev-release) —
**не** подтверждает исходную заявку этого бага (0/6 harness OK и сейчас, как
и на 2026-08-21): все шесть файлов падают на `ReferenceError:
originSameOrigin is not defined` в общем ресурсе
`resources/common.js`/`back-forward-cache/resources/helper.sub.js` — тест
дожидается своего `loadScript()`-хелпера, объявляющего `const
originSameOrigin = …` на верхнем уровне динамически подгруженного
классического скрипта, и падает раньше, чем успевает дойти до `opener`/
`postMessage`. Это НЕ дефект `window.open()`, а отдельный пробел движка
(видимость `const`/`let` верхнего уровня динамически подгруженного
классического скрипта для модуля, читающего её как свободную переменную) —
заведён отдельно, [BUG-1049](BUG-1049-OPEN.md), в очередь P3, этим срезом
не чинится. Сама заявка BUG-797 (заглушка `opener`/`postMessage`) была
диагностирована по чтению кода, а не по этому прогону ([§ «Причина»
выше](#причина-локализована-чтением-кода-не-новым-живым-зондом)), и живой
замер `win-open` (собственный код-путь `window.open()`/`postMessage`,
`tests/wpt/verify_window_history_jsurl_gaps.py`, срезы 4/5) подтверждает её
закрытой напрямую — bfcache просто был неверным WPT-индикатором для этого
бага с самого начала (упирается в другой, независимый дефект первым).

Гейт: изменений в `crates/` в этом срезе нет — только сама проверка и
документы (`BUGS.md`→`BUGS-FIXED.md`, `ROADMAP.md`, этот файл).
