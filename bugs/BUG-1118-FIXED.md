...
## Проверка на стенде (2026-09-24, P6)

Стенд `.tmp/seqlab/` (из worktree `perf-base`, `server.py` + `run.py`, задержка 700 мс на ответ), сборка
`dev-release` ветки после срезов 1–2, видимое окно `--maximized`, `LUMEN_NO_ADBLOCK=1`, HTTP-кэш
`target/dev-release/data/cache` стирался перед каждым прогоном.

| прогон | `/img1.png` | `/api` | `/dyn.png` |
|---|---|---|---|
| Chrome 153 | 1453 | 2172 | 2188 |
| Lumen 1 | 2641 | 3016 | 3016 |
| Lumen 2 | 2734 | 3109 | 3109 |
| Lumen 3 | 1547 | 1734 | 1734 |
| Lumen 4 | 1547 | 1688 | 1688 |

`/dyn.png` уходит одновременно с `/api` во всех четырёх прогонах — критерий «не позже `/api`» выполнен
(до среза 1: `/api` 2500, `/dyn.png` 3203 — после ответа на `fetch()`). Общее отставание Lumen
от Chrome на всём водопаде — предмет задач PERF-14 (синхронный `fetch()`) и PERF-15 в `ROADMAP.md`, не этого бага.

Если HTTP-кэш не стирать, повторный прогон отдаёт все подресурсы из кэша и в журнале сервера остаются
