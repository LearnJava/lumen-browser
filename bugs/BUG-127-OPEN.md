...
- Свежая сборка main, живое окно, gdigrab: **3.60%** (стабильно на двух прогонах).
- Diff-декомпозиция: остаток по-прежнему чистый font-parity текста (заголовок,
  метки карточек, `.info` моноблок) — тот же класс BUG-128. From-state рамки
  всех 5 карточек и позиция view()-бокса совпадают с Edge пиксель-в-пиксель.
- Точечного P3-дефекта нет. Ратчет KNOWN_DEBTORS['78'] 4.64 → 3.60 (правдоподобно
  от накопившихся font-parity фиксов BUG-451/BUG-452 в других тестах).
  Закроется только с FP-1 (font-parity, домен P1).
