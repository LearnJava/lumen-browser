...
* **[BUG-433](BUG-433-OPEN.md)** — основной вклад: ряд 1 сжимается ниже
...
Гейт `cases::snapshot_cpu` этот фикс не видит и эталон `59-*.png` не менялся:
headless-путь `InProcessSession` подресурсы не качает вовсе (см. шапку
`snapshot_cpu.rs`, BUG-430), там ячейки пустые в обоих направлениях. Проверка
шла через `--screenshot`, который грузит подресурсы шеллом.
