...
**необработанные отклонения промисов Lumen нигде не сообщал** —
`v8::Isolate::set_promise_reject_callback` не был установлен (смежное —
[BUG-716](BUG-716-FIXED.md), исправлен 2026-08-22). Временный колбэк, печатающий `event`, `value` и
...
(нет картинок, блоки не в сетке) — отдельный [BUG-725](BUG-725-FIXED.md): DOM и
