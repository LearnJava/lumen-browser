...
`is_svg` (`crates/engine/image/src/lib.rs`) теперь идёт по прологу XML: после BOM
пропускает пробелы, комментарии `<!-- -->`, инструкции `<?…?>` и `<!DOCTYPE …>`
(с учётом `[ … ]` внутреннего подмножества) и требует, чтобы первым элементом был
`<svg`. Прежняя эвристика «`<?xml` + `<svg` где-то в первых 4096 байтах» убрана:
она принимала XHTML со встроенным `<svg>`.

Доверие `Content-Type: image/svg+xml` не сделано — байтового распознавания хватает
для симптома; MIME до декодера не доходит. Тесты:
`is_svg_skips_comments_and_doctype_internal_subset`, `decode_accepts_svg_after_leading_comment`.
