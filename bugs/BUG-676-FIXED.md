...
* `sr.contains`/`sr.getRootNode` отсутствуют тоже (тот же класс дефекта, что
  ...
  [BUG-599](BUG-599-FIXED.md) — но здесь подтверждён именно на `ShadowRoot`).
...
что уже отслеживается отдельно в [BUG-574](BUG-574-OPEN.md)/[BUG-599](BUG-599-FIXED.md)
(общий пробел на `Node.prototype`, не специфичный для `ShadowRoot`).

> Уточнение 2026-09-09 (P6): BUG-599 закрыт только по ветке «корень = документ»,
> и этот отложенный пункт перенесён в [BUG-1045](BUG-1045-FIXED.md) — там же
> подтверждено пробой, что дело не только в отсутствующем методе на `ShadowRoot`:
> узел ВНУТРИ теневого дерева тоже получает `Element`-обёртку вместо корня.
