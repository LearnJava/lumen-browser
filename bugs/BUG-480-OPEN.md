...
**Статус:** OPEN (ДОРАБОТКА → [FRAME](../ROADMAP.md))
**Тип:** нереализованная функциональность, не дефект реализованного кода. Вложенных контекстов просмотра в движке не было вовсе, поэтому по решению от 2026-08-28 запись принадлежит дорожке `FRAME` в `ROADMAP.md` (задачи `FRAME-1`…`FRAME-6`, владелец — P1), а не очереди P3. Переклассифицировано 2026-08-31, после закрытия среза 25. Файл сохраняет номер и путь: на него ссылаются `CLAUDE.md`, python-тулинг и двадцать пять описаний срезов ниже, и запись наблюдений остаётся полезной там, где лежит. **Срезы продолжаются здесь же** — это журнал работы, а не только отчёт о дефекте.
**Дата:** 2026-08-02
...
file fail on the unrelated [BUG-534](BUG-534-FIXED.md)). 1 subtest. `.ini`
...
динамически созданные фреймы ([BUG-885](BUG-885-FIXED.md)), мутации и события
...
изолятами, динамически созданные фреймы ([BUG-885](BUG-885-FIXED.md)),
...
([BUG-885](BUG-885-FIXED.md)), навигация/замена/удаление фрейма,
X-Frame-Options/CSP `frame-ancestors`, bfcache фреймов.
...
созданные фреймы ([BUG-885](BUG-885-FIXED.md)), навигация/замена/удаление
...
между изолятами, динамически созданные фреймы ([BUG-885](BUG-885-FIXED.md)),
навигация/замена/удаление фрейма, X-Frame-Options/CSP `frame-ancestors`,
...
динамически созданные фреймы ([BUG-885](BUG-885-FIXED.md)), навигация/замена/
...
## Срез 16 (P3, 2026-08-28) — мышь внутри фрейма: адресат события

Срез 14 привёл содержимое фрейма на экран, срез 15 дорисовал в нём картинки —
и с этого момента пользователь видит кнопку ребёнка, но нажать на неё не может.
Измерено ДО правки (`tests/wpt/verify_frame_hit_test.py`, нативный клик по
координате через MCP `click {point}`): контрольная кнопка РОДИТЕЛЯ отвечает
(`trusted=true`), кнопка внутри фрейма — молчит, ни слушателя, ни всплытия до
`document` ребёнка.

**Заявка называла одно, а нашлось два, и второй виден только со стороны
родителя.** Клик не только не доходил до ребёнка — он ДОХОДИЛ ДО РОДИТЕЛЯ:
проба ловила `parent-iframe-click`, то есть страница получала `click` прямо на
элементе `<iframe>`. Событие внутри вложенного browsing context родительскому
документу не принадлежит вовсе (DOM §2.9 строит путь в ОДНОМ дереве), так что
одной доставки в ребёнка было бы мало — путь родителя обязан на этой точке
кончаться. Отсюда ранний выход в [`handle_click_at`], а вместе с ним отпадают
форма и ссылка: единственный узел СТРАНИЦЫ под этой точкой — сам `<iframe>`.

* [`frames::pointer_target`] переводит точку страницы в под-документ. Спуск идёт
  **по попаданию в host-элемент**, а не по перебору прямоугольников: `hit_test`
  уже умеет z-index, `transform`, `pointer-events` и клипы, а «прямоугольник
  содержит точку» не умеет ничего из этого — фрейм, накрытый чужим
  позиционированным блоком, забирал бы клик себе. Кандидат ищется по ПАРЕ
  «host-узел + документ-хозяин»: `NodeId` уникален лишь внутри своего документа,
  и у вложенного фрейма хозяин — документ его собственного родителя.
* Попадание в сам host-бокс мимо контентной части (рамка, padding `<iframe>`)
  фреймом не считается: там точка адресует элемент родителя.
* Возвращается ОДИН результат на оба вопроса («узел страницы» и «узел ребёнка»),
  потому что спрашивают на каждом движении мыши, а раздельные вызовы означали бы
  два обхода layout страницы за одно движение.

**Ветка не дополняет страничную, а заменяет её — и это то же правило на всех
четырёх путях ввода.** Пока курсор над содержимым фрейма, `hovered_nid`
страницы пуст по построению, а его место занимает пара
[`Lumen::hovered_frame`] `(индекс фрейма, узел ребёнка)`; отсюда `mousedown`/
`mouseup` (они читают именно `hovered_nid`) молча перестают уходить родителю и
уходят ребёнку, а `:hover`/`:active` страницы не подсвечивают `<iframe>` как
свой элемент. Два поля, а не одно, по той же причине, что и в спуске: узел
ребёнка в `hovered_nid` означал бы подсветку и `mousedown` СВОЕГО бокса
страницы с совпавшим индексом. Индекс фрейма сбрасывается на замене списка
(обе точки, где пишется `Lumen::frames`) — пережить её он не может, указывал бы
на чужой хэндл.

Уход курсора из окна поверх фрейма шлёт ребёнку `mouseout`/`mouseleave`: без
этого под-документ навсегда остался бы «под курсором» и следующий вход послал
бы второй `mouseover` без парного ухода.

Отклонения среза: `mousedown`/`mouseup` и hover **не проверяемы живой пробой** —
они рождаются только из настоящего события ОС, а `InputCommand::Click`
(единственный путь автоматизации, включая MCP) зовёт `handle_click_at` напрямую,
без нажатия и отпускания; их покрывают юнит-тесты на адресацию плюс то, что
страница без фреймов не платит и не меняет ничего вовсе (ранний возврат по
пустому списку). Захват указателя (`pointer_capture_nid`) читается только у
страницы. Движение внутри фрейма едет обычным `pointermove` без
`getCoalescedEvents()`. Ссылки и формы ВНУТРИ фрейма по-прежнему ничего не
делают: первое — навигация фрейма, второе — relayout ребёнка, оба свои пункты
очереди. Курсор (`CSS cursor`) над фреймом берётся у `<iframe>`, а не у узла
ребёнка.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1621+2 ok (+5 новых на [`frames::pointer_target`]: перевод координат и узел
ребёнка вместе — узел без координат означал бы событие верному слушателю с
`clientX`/`clientY` от чужого вьюпорта; точка рядом с фреймом; хэндл без
посчитанного layout не глотает событие; рамка хоста принадлежит родителю;
спуск до самого глубокого фрейма со сложением смещений); `dump_golden.py` 12/12
(срез не трогает рисование вовсе); живая проба `verify_frame_hit_test.py` в
двух вариантах — плоском и вложенном (клик по кнопке ВНУКА, глубина 1): клик
доходит с `trusted=true`, координатами (60, 30) в системе ребёнка, всплывает до
его `document`, ответ уходит родителю через postMessage, и `parent-iframe-click`
не приходит. Регрессионно зелены все двенадцать проб срезов 4–15.

**Стороннее, найденное прогоном регрессий:** `verify_frame_subresources.py`
(срез 11) держал фикстурой строку `not-a-real-png-but-bytes-count` и после
среза 15 честно ловил `error` вместо `load` — исход `<img>` во фрейме означает
«пиксели есть», а не «байты прочитались». Фикстура стала настоящим PNG; сам
движок здесь ни при чём, устарела проба.

Не входит (очередь): скролл внутри фрейма (колесо над фреймом крутит страницу),
ссылки и формы ребёнка, захват указателя и `CSS cursor` фрейма, всё из очереди
среза 15.

## Срез 17 (P3, 2026-08-28) — скролл внутри фрейма

Срезы 14–16 привели содержимое фрейма на экран и научили его принимать клик;
всё, что длиннее своего окна, при этом оставалось недостижимым — колесо над
фреймом крутило страницу, а `window.scrollTo` из скрипта самого ребёнка не
делал вовсе ничего. Измерено ДО правки (`tests/wpt/verify_frame_scroll.py`):
`scrollY` остался 0, ни `scroll`, ни `scrollend` не пришли, пиксели не
двинулись (30 000 красных и 30 000 зелёных до и после), клик в ту же точку
попал в ту же полосу.

**Одно число, три читателя.** [`FrameHandle::scroll_y`] — единственное место,
где живёт прокрутка под-документа, и разойтись её читателям нельзя:
[`splice_one_frame`] складывает её в СДВИГ (не в клип: клип — это дырка в
странице, через которую видно ребёнка, и двигать её значит двигать сам фрейм),
[`pointer_target`] прибавляет её обратно перед hit-тестом (layout о прокрутке
не знает — её применяет вклейка), а шелл кладёт её в рантайм ребёнка как
`window.scrollY`.

* Предел — [`frame_max_scroll`]: высота ГОТОВОГО display list ребёнка минус его
  вьюпорт. То же правило, что у страницы («прокручивается ровно то, что
  нарисовано»), и потому непрокрашенная распорка не даёт прокрутки и во фрейме
  — расходиться двум ответам на один вопрос внутри одного движка нельзя.
* Оба входа кончаются в [`Lumen::apply_frame_scroll`]. Колесо
  ([`Lumen::try_scroll_frame`]) проверяется РАНЬШЕ overflow-контейнеров
  страницы: те всегда боксы СТРАНИЦЫ, то есть предки хоста, а ближайший к
  курсору скроллер — фрейм. Дойдя до своего края, фрейм возвращает `false`, и
  остаток жеста уходит выше по цепочке — CSS Overscroll Behavior L1 §3 со
  значением по умолчанию `auto`, ровно как у контейнеров.
* `window.scrollTo`/`scrollBy` из скрипта ребёнка копятся в ЕГО рантайме, а
  страничный дренаж ниже по циклу знает только `self.js_ctx` — поэтому запросы
  фреймов дренируются в том же пампе `about_to_wait`, где тикают их таймеры.
* Прокрутка фрейма пересобирает display list страницы целиком: точечного
  `patch_scroll_layer` здесь нет чему патчить — содержимое ребёнка приезжает
  отдельным списком, а не слоем внутри страничного.
* [`sync_frame_viewports`] зажимает прокрутку, оказавшуюся за новым пределом
  после релейаута (содержимое стало ниже, хост — выше).

**Заявка называла одно, а нашлось два, и второе видно только с прокруткой.**
Срез 16 намеренно отдавал ОДНУ точку на оба вопроса — «где искать узел» и «что
положить в `clientX`/`clientY`». Пока фрейм не прокручен, обе системы
совпадают; с прокруткой расходятся, и проба поймала это сразу: клик попал в
верный блок с `clientY` = 230 вместо 30, то есть слушатель ребёнка получил
координаты системы ДОКУМЕНТА там, где CSSOM-View §10 требует систему
ВЬЮПОРТА. Наружу теперь отдаётся вьюпортная точка ([`FramePointerHit::client`]),
а документная осталась внутренней переменной спуска.

Отклонения среза: `behavior: 'smooth'` применяется мгновенно — своей анимации
у прокрутки фрейма нет и тикать её негде (rAF под-документов не идёт, срез 1).
Колесо **не проверяемо живой пробой** по той же причине, что `mousedown` в
срезе 16: ни одна поверхность автоматизации не рождает событие колеса
(`InputCommand` его не знает), поэтому его закрывают юнит-тесты. Горизонтали и
клавиатурной прокрутки у фрейма нет, полосы прокрутки — тоже; overflow-
контейнер ВНУТРИ ребёнка по-прежнему не крутится (его боксов нет в списке
скролл-контейнеров страницы).

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1626+2 ok (+6 новых: предел по нарисованному, зажим и «сдвинулось/нет» как
ответ для цепочки, сдвиг вклейки при неподвижном клипе, расхождение двух
систем координат в прокрученном фрейме — оно проверяется УЗЛОМ, а не полем,
то есть чёрным ящиком, — и зажим устаревшей прокрутки на релейауте);
`dump_golden.py` 12/12 (страниц с фреймами в наборе нет, вклейка вне фреймов —
no-op); живая проба `verify_frame_scroll.py`: `scroll`/`scrollend` с `y=200`,
30 000 синих пикселей вместо 30 000 красных и зелёных, клик в ту же точку окна
попадает в приехавшую полосу с `clientY` = 30, и страница при этом не сдвинулась.
Регрессионно зелены все двенадцать проб срезов 4–16 (обе в двух вариантах).

Не входит (очередь): горизонтальная и клавиатурная прокрутка фрейма, его
полоса прокрутки, overflow-контейнеры внутри ребёнка, ссылки и формы ребёнка,
захват указателя и `CSS cursor` фрейма, плюс всё из очереди срезов 14–15.

## Срез 18 (P3, 2026-08-29) — элементы управления формы внутри фрейма

Срез 16 довёл клик до под-документа, но только как СОБЫТИЕ: собственное
поведение элемента шелл для ребёнка не исполнял вовсе — ранний возврат в
[`handle_click_at`] пропускает разбор формы вместе со ссылкой, потому что
единственный узел СТРАНИЦЫ под этой точкой сам `<iframe>`. Измерено ДО правки
(`tests/wpt/verify_frame_forms.py`): клики доходят до нужных элементов
(`id=cb`, `id=r2`, `id=sum`), а состояние остаётся `cb=false det=false
r2=false`, синих пикселей 0.

**Правило то же, что у страницы, а пересчитывается другое дерево.**
[`crate::forms`] принимает любой `&Document`, поэтому классификация клика
переиспользуется целиком: расходиться ответам на вопрос «что делает клик по
`<summary>`» внутри одного движка нельзя. Новое здесь — показать результат:
[`frames::relayout_frame_content`] считает layout ребёнка на ТОМ ЖЕ вьюпорте
и очищает его `content_dl`.

* Очистка списка, а не свой флаг: пустой `content_dl` — единственный признак
  «перерисовать», который понимает [`rebuild_frame_display_lists`], и через
  него правка сама доходит до списков всех предков фрейма.
* Доделывает работу [`sync_frame_viewports`] — не ради экономии кода, а
  потому что мутация могла подвинуть host-бокс ВЛОЖЕННОГО фрейма (раскрытый
  `<details>` над ним), и порядок обхода по глубине живёт только там. Оттуда
  же берётся зажим прокрутки, оказавшейся за новым краем: закрыть `<details>`
  можно на прокрученном до низа фрейме.
* Гейт «размер хоста не менялся — не пересчитывать» (срез 13) здесь
  срабатывает ВСЕГДА, поэтому layout считается напрямую; на этом и стоит
  первый юнит-тест среза.
* Событие `toggle` уходит прямым `eval_js` по хэндлу фрейма: `route_eval_js`
  знает только `self.js_ctx` — контекст страницы, — а событие принадлежит
  документу ребёнка (та же причина у [`Lumen::frame_mouse_event`]).

**Проба нашла три дефекта, из которых субъектом среза был один.** Первая
версия ставила элементам `position:absolute` и промахнулась мимо ВСЕХ:
инлайн-уровневый бокс с `position:absolute` из потока не выносится вовсе — в
дампе у него `position=absolute`, а лежит он в `InlineBlockRow` там, где стоял
бы и без позиционирования, тогда как соседний `<div>` уезжает куда сказано
([BUG-928](BUG-928-FIXED.md); проверено и на `<span display:inline-block>`,
так что это не про элементы формы). Вторая версия читала состояние по клику на
`<button>` — и промахнулась снова, потому что кнопка нулевой ширины
([BUG-926](BUG-926-FIXED.md), заведён P1 неделей раньше). Третий нашёл
КОНТРОЛЬ, а не субъект: нативный клик по радиокнопке не снимает отметку с
соседа по группе и на самой СТРАНИЦЕ (`parent-click … r1=true r2=true`), то
есть это общий пробел шелла ([BUG-927](BUG-927-OPEN.md)), а не фреймовый —
поэтому фрейм проведён тем же путём, что страница, и проба сверяется со
страницей, а не со спекой. Заводить расхождение внутри фрейма было бы хуже
самого пробела.

Отклонения среза: оверлеи (`<select>`, палитра, календарь, файловый диалог)
внутри фрейма по-прежнему не открываются — всплывающее окно рисуется по
прямоугольнику из layout СТРАНИЦЫ (`self.layout_box`) и адресуется одним
`NodeId` без фрейма, то есть это переделка трёх полей состояния и всех их
читателей, а не ветка; отправка формы ребёнка — это навигация фрейма,
отдельный пункт очереди. Оба случая пишут строку в stderr, а не молчат: клик
по `<select>` внутри фрейма иначе неотличим от потерянного события.
`form_state` страницы не трогается — он ключуется `NodeId` без фрейма и
читается только отправкой формы.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1629+2 ok (+3 новых: пересчёт при НЕИЗМЕННОМ вьюпорте — проверяется тем, что
проход по вьюпортам ДО него правку не видит; правка во вложенном фрейме
доходит до списка предка; зажим прокрутки после укорачивания содержимого);
`dump_golden.py` 12/12; живая проба `verify_frame_forms.py`: флажок,
радиокнопка и `<details>` внутри фрейма переключаются, 5460 синих пикселей
вместо нуля, контроль на странице ведёт себя так же. Регрессионно зелены
пробы срезов 16 и 17.

Не входит (очередь): ссылки ребёнка и отправка его формы (обе — навигация
фрейма), оверлеи форм внутри фрейма, ввод текста и фокус внутри фрейма, плюс
всё из очередей срезов 14–17.

## Срез 19 (P3, 2026-08-29) — ссылки ребёнка: навигация фрейма

Срез 16 довёл клик до под-документа как СОБЫТИЕ, срез 18 — собственное
поведение элементов управления формы. Ссылка не работала ни там, ни там по
одной причине: ранний возврат в [`handle_click_at`] пропускает её разбор
вместе с разбором формы — единственный узел СТРАНИЦЫ под этой точкой сам
`<iframe>`. Измерено ДО правки (`tests/wpt/verify_frame_links.py`): клик
доходит до самой ссылки (`child-click id=lnav`), а сервер пробы не видит ни
одного запроса, `hashchange` не приходит, пиксели не двигаются.

**Разбор адреса переиспользуется целиком, ново только «куда идёт результат».**
«Фрагмент или загрузка», «навигабельна ли схема», «тот же ли это документ» —
вопросы про URL, а не про то, кто кликнул, поэтому [`crate::links`] отвечает на
них одинаково для страницы и для фрейма. Своё в срезе — [`LinkTarget`]: `_self`
и пустой `target` меняют кликнувший фрейм, `_top` — страницу, `_parent` — фрейм
глубины ≥ 1 или страницу для глубины 0.

* **Адрес резолвит документ, в котором кликнули**, а не документ-хозяин: у
  хэндла появилось поле [`FrameHandle::base`]. Вывести его из `url` нельзя —
  `about:blank`/`about:srcdoc` наследуют базу хозяина. `target=_parent` меняет
  ЧУЖОЙ под-документ, но адрес всё равно написан кликнувшим, поэтому база
  навигации — параметр [`navigate_frame`], а не поле цели.
* **[`FrameLoadEnv`] — потому что провайдеров больше негде взять.** Два десятка
  клонов (сеть, хранилища, media-контекст, вьюпорт, детерминизм) существовали
  ТОЛЬКО внутри `parse_and_layout`; повторить загрузку под-документа позже, из
  живого окна, было нечем. Набор собран в одно `Clone`-значение, которое едет в
  `LoadedPage` и дальше в `Lumen`. Заодно это сняло 25-аргументную сигнатуру
  [`load_frame_sub_documents`] и позволило выделить из тела её цикла
  [`spawn_frame`] — навигация есть тот же путь, отличающийся ровно одним,
  откуда взят адрес.
* **Хэндл ЗАМЕЩАЕТСЯ, а не правится**: под-документ — другой `Document`, другой
  JS-контекст и другой каскад, от прежнего остаётся только место на странице.
  Вместе с ним уходит и всё его поддерево ([`drop_frame_subtree`], цикл до
  неподвижной точки: список плоский, и внук ссылается на документ среднего
  фрейма, который удаляется на этом же шаге). `host_src` нового хэндла остаётся
  прежним намеренно — это половина ключа, по которому вклейка узнаёт
  команду-заглушку, а заглушку рисует layout родителя по атрибуту `src`, до
  которого навигации дела нет.
* Прежний индекс после замены не адресует ничего, поэтому пересчитываются
  вьюпорты ВСЕХ фреймов: `refresh_frames(None)` против `Some(idx)` среза 18.
* Фрагмент внутри фрейма — зеркало [`Lumen::navigate_fragment`] страницы, теми
  же тремя шагами: сообщить JS ребёнка (`location` + `hashchange`), проставить
  `:target` в его дереве, пересчитать каскад и только потом искать бокс цели —
  до пересчёта правило `:target` ещё не применено.

**Проба нашла три дефекта при одном субъекте, и оба чужих — в местах, куда
симптом не показывал.**

1. `register_frame_document` всегда делал `push`, а поиск по `host_nid` берёт
   ПЕРВОЕ совпадение — значит после навигации `contentDocument`/`contentWindow`
   родителя вечно отдавали бы ВЫБРОШЕННЫЙ документ, `window.length` рос бы на
   каждую навигацию, а `window[i]` разъехался бы с порядком документа. Теперь
   [`upsert_binding`] замещает запись того же хоста с сохранением `bid`: при
   навигации вложенный browsing context остаётся тем же, меняется его документ.
2. `self.js_ctx` в живом окне почти всегда `None`: с ADR-023 движковый поток
   включён ПО УМОЛЧАНИЮ, и `set_js_ctx` кладёт хэндл в его состояние, оставляя
   UI-поле пустым. Первая версия среза брала родительский контекст оттуда — и
   молча не регистрировала под-документ и не слала `load` на хосте. Появился
   [`Lumen::clone_js_ctx`]: маршрутизатор `route_task_js` тут не годится, ему
   нужен ОДИН `FnOnce`, а загрузка зовёт у родителя четыре разных метода
   синхронно.
3. Третий нашла проба, меряя саму себя: под-документ, который ниже вьюпорта
   фрейма, НЕ закрашивает остаток окна своим фоном — фон `<body>` доходит
   только до высоты содержимого, и сквозь фрейм видно фон СТРАНИЦЫ. Пустая
   `d.html` давала ноль своих пикселей, и проверка «новый документ виден»
   краснела не из-за навигации. Это residual среза 14 (рисование фрейма), а не
   навигации, и он остаётся в очереди.

Отклонения среза: именованный `target` и `_blank` не поддержаны (спека для
имени, которого нет, предписывает СОЗДАТЬ окно — [BUG-883](BUG-883-OPEN.md)), в
лог уходит строка, а не тишина; неудачная навигация оставляет прежний документ
вместо документа-ошибки (запрос при этом уходит — проверено); запись в session
history не делается, поэтому back/forward по фреймам не ходит; загрузка идёт
синхронно на UI-потоке (как и первая загрузка фрейма, только там это поток
загрузки страницы); отправка формы ребёнка по-прежнему вне среза.

Проверка: clippy `-p lumen-shell --all-targets` и `-p lumen-js --all-targets
--features v8-backend` -D warnings; тесты shell 1630+2 ok (+2 новых: удаление
поддерева до внука с сохранением соседа, и `find_link`, отдающий сам `<a>` —
`target` читается именно с найденного предка), lumen-js +1 (замещение биндинга
на месте, проверенное тем, что видно из JS, а не длиной вектора);
`dump_golden.py` 12/12; живая проба `verify_frame_links.py` в двух вариантах —
обычная ссылка, фрагмент, `target=_top` и контрольная ссылка страницы;
относительный адрес в подкаталоге, два перехода подряд, 404, `contentDocument`
родителя и `target=_parent` из глубины 1. Регрессионно зелены пробы срезов
12–18 (`verify_frame_layout`, `verify_frame_images`, `verify_frame_click`,
`verify_frame_hit_test`, `verify_frame_scroll`, `verify_frame_forms`).

Не входит (очередь): отправка формы ребёнка, session history фреймов, фон
под-документа ниже его содержимого, именованные `target`, оверлеи форм внутри
фрейма, ввод текста и фокус внутри фрейма, плюс всё из очередей срезов 14–18.

## Срез 20 (P3, 2026-08-29) — отправка формы ребёнка

Срез 18 довёл до ребёнка собственное поведение элементов управления и отклонил
отправку явно («это навигация фрейма, отдельный пункт очереди»), срез 19 дал
саму навигацию. Здесь две половины соединяются. Измерено ДО правки
(`tests/wpt/verify_frame_form_submit.py`): клик доходит до самой кнопки
(`child-click id=go`), а события `submit` нет, сервер пробы не видит ни одного
запроса и пиксели фрейма не меняются — при том что КОНТРОЛЬ на той же
странице (её собственная форма) событие получает.

**Правила отправки переиспользуются целиком, ново только «куда идёт
результат».** Валидация, сбор полей, enctype, кодирование и отменяемое событие
`submit` — вопросы про ФОРМУ, а не про то, в каком документе она лежит:
[`lumen_dom::submit_form`], [`forms::enctype_of_form`],
[`forms::encode_form_fields`] принимают любой `&Document`.
[`Lumen::run_frame_form_submission`] повторяет сигнатуру
[`Lumen::run_form_submission`] аргумент в аргумент, включая
`fire_submit_event`: `requestSubmit()` уже разослал событие на JS-стороне, а
`submit()` пропускает его по спеке (§4.10.21.3), и это единственное, чем
скриптовый вход отличается от нативного.

* **Событие `submit` уходит в контекст РЕБЁНКА**, прямым `eval_js_value` по
  хэндлу фрейма: `route_query_js` знает только `self.js_ctx` — контекст
  СТРАНИЦЫ. Иначе отменить отправку мог бы кто угодно, кроме той страницы,
  которой форма принадлежит.
* **Адресат берётся у среза 19**, а не выводится заново: [`LinkTarget`] и
  [`Lumen::link_destination`] открыты до `pub(crate)`, потому что §4.10.21.4
  шаг 15 ссылается ровно на тот §4.6.3, по которому выбирает адресата `<a
  target>`. Двух разных ответов на «куда ведёт `_parent`» внутри одного
  движка быть не может.
* **Фрагментной ветки в отправке нет намеренно:** GET заменяет query целиком,
  поэтому её адрес не бывает «тем же документом с другим `#id`» — путь
  страницы по той же причине зовёт `navigate_to` напрямую, минуя
  `fragment_only`.
* **Подсказка о непройденной валидации** — единственное, что рисуется ПОВЕРХ
  СТРАНИЦЫ, а привязано к узлу ребёнка. Для неё появился
  [`frames::frame_page_origin`]: та же сумма host-боксов, которой
  [`splice_one_frame`] везёт пиксели, только сложенная по цепочке хозяев и с
  вычетом собственной `scroll_y` на КАЖДОМ звене. Возвращает `None`, а не
  `(0, 0)`, если у звена ещё нет host-бокса: начало координат страницы
  нарисовало бы подсказку в углу окна, как будто она принадлежит странице.
* **Оба входа страницы есть и здесь.** Скриптовый (`form.submit()` /
  `requestSubmit()`) приходит тем же `NavigateRequest::SubmitForm`, который
  памп фреймов в `about_to_wait` до сих пор дренировал и ВЫБРАСЫВАЛ вместе со
  всякой скриптовой навигацией фрейма (срез 1).

**Отклонения, и все три — «не расходиться со страницей».** POST по-прежнему
только пишет строку в лог: сетевой половины нет и у страницы, и заводить
«во фрейме умеем, на странице нет» нельзя. `formaction`/`formmethod`/
`formenctype`/`formtarget` на кнопке игнорируются — страница не учитывает их
нигде. Session history не пишется, поэтому back/forward не возвращает к
документу до отправки.

**Что нашла проба, кроме своего субъекта.** Пиксельная проверка подсказки
валидации оказалась невозможной, и это установил КОНТРОЛЬ, а не догадка:
снимок `resource://screenshot` не содержит оверлеев вовсе — подсказка САМОЙ
СТРАНИЦЫ, чей отказ виден в логе строкой `forms: submit blocked`, даёт на
снимке ровно ноль пикселей так же, как фреймовая. Перевод координат поэтому
проверяется юнит-тестом (`frame_page_origin_*`), а не пикселями; сама проба
это состояние фиксирует проверкой, чтобы появление оверлеев на снимке
однажды покраснело и было замечено.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1644+2 ok (+2 новых: сумма по цепочке хозяев с вычетом прокрутки на каждом
звене, и `None` без host-бокса); `dump_golden.py` 12/12; живая проба
`verify_frame_form_submit.py` в двух вариантах — обычная отправка, отмена
через `preventDefault`, `target=_top`, контроль формы страницы; и
`form.submit()`/`requestSubmit()` из скрипта ребёнка, относительный `action`
из подкаталога, событие РОВНО один раз, блокировка непройденной валидацией.
Регрессионно зелены пробы срезов 12–19 (`verify_frame_layout`,
`verify_frame_images`, `verify_frame_click`, `verify_frame_hit_test`,
`verify_frame_scroll`, `verify_frame_forms`, `verify_frame_links`).

Не входит (очередь): session history фреймов, фон под-документа ниже его
содержимого, именованные `target`, оверлеи форм внутри фрейма, ввод текста и
фокус внутри фрейма, плюс всё из очередей срезов 14–19.

## Срез 21 (P3, 2026-08-29) — фон под-документа на весь его вьюпорт

Residual среза 14, найденный пробой среза 19: содержимое ребёнка короче
вьюпорта фрейма — под ним видно фон СТРАНИЦЫ, а не фон ребёнка. Измерено ДО
правки (`tests/wpt/verify_frame_background.py`, разные цвета у страницы и у
ребёнка, содержимое ребёнка короче фрейма на 280px): полоса ниже содержимого
целиком в цвете страницы, 0 px цвета ребёнка.

**Причина — не в наличии подложки, а в её размере.** `paint_ordered` кладёт
заливку фона `<html>`-бокса ребёнка только в его СОБСТВЕННОМ прямоугольнике
(высота по содержимому), а «канвас»-заливка на весь вьюпорт (CSS Backgrounds
§3.11.1, тем же приёмом, каким `redraw_requested.rs` чистит всё окно
СТРАНИЦЫ) существует только у страницы и о вклеенных фреймах ничего не
знает. [`rebuild_frame_display_lists`] теперь сама кладёт такую заливку
первой командой списка ребёнка — размером в `FrameHandle::viewport`, а не в
контентный бокс — цветом [`lumen_layout::canvas_background_color`] или белым
UA-дефолтом, если у ребёнка фона нет вовсе (тем же значением, которым эта
функция документирует свой `None`).

Вставка ничего не ломает у соседних приёмов: [`rekey_frame_images`] и
[`splice_children_of`]/[`splice_one_frame`] ищут команды по содержимому
(`DrawImage.src`, пара src+rect), а не по индексу, и [`splice_one_frame`]
вклеивает ВЕСЬ `content_dl` ребёнка целиком внутрь своего клипа — новая
заливка идёт первой и красится ровно в окно фрейма на странице.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1635+2 ok (+2 новых: подложка на весь вьюпорт при коротком содержимом с
цветом ребёнка, и белый UA-дефолт без фона у ребёнка); `dump_golden.py`
12/12 (страниц с фреймами в наборе нет); живая проба
`verify_frame_background.py`: полоса ниже содержимого ребёнка целиком в его
цвете (56000/56000 px), 0 px цвета страницы. Регрессионно зелена проба
среза 15 (`verify_frame_images`): картинки фрейма и страницы по-прежнему не
путаются.

Не входит (очередь): session history фреймов, именованные `target`, оверлеи
форм внутри фрейма, ввод текста и фокус внутри фрейма, плюс всё из очередей
срезов 14–19.

## Срез 22 (P3, 2026-08-29) — ввод текста в поле внутри фрейма

Срез 18 довёл до ребёнка собственное поведение флажков, радио, `<summary>` и
ползунка, но текстовые поля отклонил в очередь. Измерено ДО правки
(`tests/wpt/verify_frame_text_input.py`): движок отказывает вводу ЯВНО —
MCP `type` в поле фрейма отвечает `Element is not a mutable text field`, ни
одного события `input` в ребёнке нет, при том что КОНТРОЛЬ на той же
странице (её собственное текстовое поле) наполняется.

**Отказ был честен: печатать было некуда.** `self.focused_node` после клика
внутрь фрейма указывает на host-элемент `<iframe>` (срез 16 — с точки зрения
СТРАНИЦЫ клик внутрь фрейма фокусирует контейнер), а `Self::typeable_field`
читает исключительно `self.layout_source`, документ страницы. Поле ребёнка
не мог адресовать никто: `NodeId` уникален лишь внутри своего документа —
та же причина, по которой у `:hover` есть отдельное [`Lumen::hovered_frame`]
рядом с `hovered_nid`. Появилась парная ему [`Lumen::focused_frame`] —
`(индекс фрейма, узел ЕГО документа)`, которая `focused_node` не заменяет, а
адресует ИМЕННО ввод текста внутрь под-документа.

* **Классификация поля и запись значения переиспользуются, ново только «в
  каком документе».** [`Lumen::frame_typeable_field`] — зеркало
  `Self::typeable_field` против документа ребёнка (тот же набор типов
  `<input>`, тот же отказ для `disabled`/`readonly` по §4.10.19.2, то же
  чтение РЕНДЕРЯЩЕГОСЯ значения через `Document::control_value`), а сама
  запись идёт штатными `forms::set_value`/`set_textarea_text` под коротким
  локом [`Lumen::with_frame_doc`] — тем же, которым срез 18 переключает
  флажок; он ради этого открыт до `pub(crate)`.
* **События клавиатуры уходят в контекст РЕБЁНКА**, прямым `eval_js` по его
  хэндлу, а не через `route_eval_js`: последний знает только контекст
  страницы — ровно то же соображение, по которому срез 20 отправляет туда
  событие `submit`. Иначе `keydown`/`input`/`keyup` слушал бы кто угодно,
  кроме документа, которому поле принадлежит.
* **Фокус фрейма сбрасывается на КАЖДОМ клике**, тремя ветками: клик по не
  typeable-узлу ребёнка, клик мимо любого бокса ребёнка и клик вне фрейма
  вовсе. Страница ведёт себя так же (`focused_node` переустанавливается
  безусловно), и без этого ввод после ухода из фрейма продолжал бы попадать
  в его поле — что проба и проверяет контролем ПОСЛЕ фрейма.
* **Оба входа, как у страницы.** Нативная клавиатура (`keyboard.rs`) и
  автоматизационный `type` (`about_to_wait.rs`) выбирают путь одинаково — по
  непустому `focused_frame`. В `keyboard.rs` ветка фрейма стоит ПЕРВОЙ, но
  порядок здесь только ради ясности: `focused_node` в этот момент указывает
  на host-`<iframe>`, который не typeable, так что страничная ветка всё
  равно не сработала бы.
...
Каретка и outline внутри фрейма — отдельный, больший пункт очереди.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1635 ok + 2 интеграционных (новых юнит-тестов нет: и `frame_typeable_field`,
и обе инъекции — методы `Lumen`, который в тестах shell не конструируется
нигде, поэтому доказательством служит живая проба); `dump_golden.py` 12/12;
A/B той же пробой `verify_frame_text_input.py` — ДО правки КРАСНЫЙ (2 отказа
`type`, ноль событий `input` у ребёнка, контроль страницы зелёный), ПОСЛЕ
ЗЕЛЁНЫЙ (`abc` в пустом поле фрейма, дозапись `6` к предзаполненному
`12345`, поле страницы после фрейма наполняется независимо). Регрессионно
зелена проба среза 18 (`verify_frame_forms`).

**Проба по ходу дела была починена, и это часть результата.** В первой
версии отказ `type` поднимался исключением и обрывал прогон трейсбеком —
то есть измерение «ДО» невозможно было ни повторить, ни прочитать. Отказ
теперь записывается маркером и разбирается вместе с прочими проверками, так
что несделанный ввод виден как отдельная строка `[ФЕЙЛ]` с текстом самого
движка, а не как загадочно пустое поле.

Не входит (очередь): видимый `:focus`/каретка внутри фрейма, перевод фокуса
клавишей Tab внутри ребёнка, выделение и позиция курсора (ввод всегда
дописывает в конец — как и у страницы), session history фреймов, именованные
`target`, оверлеи форм внутри фрейма, плюс всё из очередей срезов 14–19.

## Срез 23 (P3, 2026-08-31) — видимый `:focus`/`:hover`/`:active` внутри фрейма

Срез 22 записал в очередь: «видимого `:focus` внутри фрейма это не даёт —
`frames::layout_frame_document` не зовёт `set_interactive_state` вовсе, так
что для CSS ребёнок остаётся интерактивно-слепым, ровно как и для `:hover`».
Измерено ДО правки (`tests/wpt/verify_frame_focus_style.py`): заявленный
дефект подтверждён (поле фрейма не расширяется под `:focus`), и рядом с ним
проба нашла ещё два, которых заявка не называла — `getComputedStyle` внутри
фрейма пуст для ЛЮБОГО узла (`layout_frame_document` публикует ребёнку
только `update_layout_rects`/`update_viewport_size`, но не
`update_computed_styles`), и `document.activeElement` ребёнка остаётся
`BODY` после клика в его поле (страница узнаёт о фокусе фрейма — срез 22 —
но не сообщает об этом самому под-документу).

**Причина одна и та же для всех трёх: под-документ считается layout'ом, для
которого интерактивное состояние всегда пусто.** У страницы каждый проход
`lumen_layout::layout_measured` обёрнут в `set_interactive_state`/
`clear_interactive_state` вокруг ЕДИНСТВЕННОГО набора thread-local (`hover`/
`focus`/`active` каскада, `style/env.rs`), и тот же прогон публикует
вычисленные стили через `collect_computed_styles`. Ребёнок эти два вызова не
делал вовсе — не потому что для него они не нужны, а потому что
`layout_frame_document` эволюционировал (срезы 12→22) как чистый пересчёт
геометрии по вьюпорту, и добавлять в него интерактив было незачем, пока
кликом внутрь фрейма ничего, кроме ввода текста, не адресовалось.

* **Три поля `Lumen` (`hovered_frame`, `focused_frame`, новое
  `active_frame`) — источник истины**, `(индекс фрейма, узел ЕГО документа)`
  ровно как раньше. Новая [`crate::lumen::Lumen::frame_interactive`] сводит
  их в одно значение [`crate::frames::FrameInteractive`], а его метод
  `for_frame(idx)` возвращает состояние ОДНОГО фрейма — узел соседнего
  фрейма для этого прохода не «чужой `NodeId` с совпавшим индексом», а
  честное «ничего».
* **`layout_frame_document` принимает состояние параметром**, ставит его
  вокруг своего прохода layout теми же `set_interactive_state`/
  `clear_interactive_state`, что и страница, и публикует
  `update_computed_styles` рядом с `update_layout_rects` — тем самым второй
  найденный пробой дефект (пустой `getComputedStyle`) закрывается тем же
  изменением, что и заявленный: `:focus` во фрейме не может матчиться без
  косвенного продукта того же прохода.
* **Гейт «размер не изменился — не пересчитывать» в `sync_frame_viewports`
  сравнивает теперь ДВА входа, а не один.** Интерактивное состояние — такой
  же вход layout, как размер вьюпорта: без второй половины сравнения клик
  внутрь фрейма менял бы `:focus`, но `FrameHandle::viewport` остался бы тем
  же, и пересчёта не было бы вовсе. Прошлое состояние хранится в новом поле
  `FrameHandle::interactive`, рядом с `viewport` — тем же способом, каким
  тот уже был гейтом.
* **`document.activeElement` ребёнка обновляется отдельным вызовом**,
  `notify_frame_focus`, симметричным тому, что страница уже делает себе
  через `notify_focus_changed` — только адресован он контексту РЕБЁНКА
  прямым `eval_js` по его хэндлу, а не через `route_task_js` (тот знает
  только контекст страницы, та же причина, что у клавиатурных событий среза
  22 и `submit` среза 20). Уведомляются ОБА фрейма при переходе фокуса между
  ними — покинутый получает `None`, иначе его `activeElement` продолжал бы
  указывать на свой узел, и два под-документа одновременно считали бы себя
  сфокусированными.
* **`:hover`/`:active` проведены той же трубой**, хотя проба их не
  измеряет — ни одна автоматизационная поверхность не может их вызвать
  (`InputCommand::Click` жмёт и отпускает кнопку одним действием, движения
  мыши в протоколе нет). `hovered_frame` меняется в `cursor_moved.rs` —
  пересчёт вызывается в ОБЕИХ `#[cfg(feature = "v8")]`-ветках, поскольку вид
  не должен зависеть от того, собран ли движок со скриптами.
  `active_frame` — зеркало страничного `active_nid`, ставится/снимается в
  `mouse_input.rs` на press/release и обнуляется вместе с `hovered_frame`
  при уходе курсора из окна (`app/mod.rs`) — иначе фрейм остался бы
  подсвеченным после того, как отпустить стало некому.
* **Фокус и нажатие фрейма сбрасываются там же, где раньше сбрасывался
  только `hovered_frame`** — при выбросе устаревшего хэндла в
  `frame_links.rs` (навигация фрейма среза 19): оставленная пара нашла бы в
  новом дереве чужой узел с совпавшим индексом, `NodeId` уникален лишь
  внутри своего документа.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1655 ok (новых юнит-тестов нет по той же причине, что у среза 22: логика,
которую стоило бы тестировать изолированно, либо метод `Lumen` — который в
тестах shell не конструируется нигде, — либо `FrameInteractive::for_frame`,
слишком тонкая обёртка над структурным сравнением, чтобы отдельный тест
доказывал больше живой пробы; доказательством служит она). A/B той же
пробой `verify_frame_focus_style.py` — ДО правки КРАСНЫЙ (заявленный дефект
+ пустой `getComputedStyle` + `activeElement` не обновляется), ПОСЛЕ
ЗЕЛЁНЫЙ: поле фрейма 102px→202px под `:focus` (доказывает пересчёт
геометрии, а не только публикацию стиля отдельным каналом), `getComputedStyle`
внутри фрейма видит и статический фон, и фон `:focus`, `document.activeElement`
ребёнка — его поле, а поле СТРАНИЦЫ после клика во фрейм возвращается к
102px (два фокуса не сосуществуют). Регрессионно зелены пробы срезов 15–20
(`verify_frame_click`, `verify_frame_scroll`, `verify_frame_forms` —
последняя падает на двух проверках, но обе это уже заведённые BUG-927/928,
не связанные со срезом 23 — `verify_frame_text_input`).

Не входит (очередь): каретка внутри фрейма (дефект СТРАНИЦЫ — `caret_color`
не читается ни одним потребителем в `lumen-paint`, единственная каретка в
движке нарисована руками для омнибокса хрома — идёт отдельным номером, а не
частью этой очереди), перевод фокуса клавишей Tab внутри ребёнка, выделение
и позиция курсора, session history фреймов, оверлеи формы внутри фрейма,
плюс всё из очередей срезов 14–19.

## Срез 24 (P3, 2026-08-31) — именованный `target` адресует ЖИВОЙ фрейм

Срезы 19/20 разобрали `_self`/`_top`/`_parent` у ссылки и отправки формы
РЕБЁНКА, а любое другое имя — включая имя, СОВПАВШЕЕ с `name` реального
`<iframe>` — уходило в одну ветку с `_blank`: «вспомогательных browsing
context движок не создаёт вовсе» ([BUG-883](BUG-883-OPEN.md)). Дыра была
шире заявленной: `Lumen::link_destination` (ссылка ребёнка) `target` уже
читал, а вот клик по ссылке СТРАНИЦЫ (`click.rs`) его не читал вовсе —
`target="имя"` на обычном `<a>` страницы, указывающий на живой `<iframe
name="имя">`, тихо навигировал страницу целиком, как будто атрибута не
было. Измерено пробой ДО правки (`tests/wpt/verify_frame_named_target.py`):
клик по такой ссылке страницы увёл страницу на адрес ссылки вместо того,
чтобы сменить только названный фрейм.

* **Один источник истины для «есть ли живой фрейм с таким именем»** —
  новый [`Lumen::find_frame_by_name`] (`frame_links.rs`): линейный проход по
  `self.frames`, имя читается с host-узла `<iframe>`/`<frame>` в
  документе, где тот стоит (страница для глубины 0, документ
  фрейма-родителя для вложенного — та же пара источников, что уже различает
  срез 19 у `_parent`), сравнение регистрозависимое и точное, как у `id`.
  Обоим существующим вызовам НЕ пришлось меняться, чтобы получить эту
  проверку: и ссылка ребёнка (`Lumen::link_destination`), и его же отправка
  формы (срез 20, `run_frame_form_submission`) уже сводятся к одной точке
  разбора `target` — добавление одной ветки перед `NewWindow` дало обоим
  сразу.
* **`_blank` проверяется явным образом, а не «что не совпало — то окно».**
  Спека резервирует это имя; было бы неверно, если бы гипотетический живой
  фрейм с `name="_blank"` (спека такое имя запрещает, но проверка на входе
  этого не знает) вдруг начал матчиться раньше зарезервированного смысла.
* **Для СТРАНИЦЫ путь новый: `click.rs` раньше искал только href
  (`links::find_link_href`), теперь — сам `<a>` (`links::find_link`, уже
  существовавший ради того же в `frame_links.rs`) и его `target` в ОДНОМ
  локе документа — раздельные обходы дали бы разойтись адресу и имени
  ссылки, которая мутировала между ними.** Найденное имя ищется тем же
  `find_frame_by_name`; при совпадении навигирует фрейм
  (`Lumen::navigate_frame_to`) с базой СТРАНИЦЫ (`frame_env.page_base`) —
  ссылка написана в странице, а не в ребёнке, поэтому её относительный адрес
  резолвится страничной базой, а не базой целевого фрейма (тот же принцип,
  что у `_parent` в `link_destination`).
* **Поведение для НЕсовпавшего имени умышленно не тронуто ни на одном из
  двух путей.** До среза 24 страница `target` игнорировала целиком — ссылка
  с любым `target`, включая несуществующее имя, вела страницу как обычная;
  после среза 24 это же самое остаётся верным ТОЛЬКО когда имя ни на что не
  указывает (проверено пробой как отдельный контроль, «до» и «после»
  совпадают побайтово по числу запросов). Расширение не сузило и не
  изменило ни одного прежде работавшего исхода — оно добавило один НОВЫЙ,
  когда имя резолвится.

Проверка: clippy `-p lumen-shell --all-targets` -D warnings; тесты shell
1655 ok (новых юнит-тестов нет по той же причине, что у срезов 22/23:
`find_frame_by_name` — метод `Lumen`, который в тестах shell не
конструируется нигде; доказательством служит живая проба).
`dump_golden.py` 12/12 (изменение — чистая навигационная логика, дисплей-лист
не трогает). A/B пробой `verify_frame_named_target.py` — ДО правки КРАСНЫЙ
(ссылка ребёнка на именованного соседа не находила его — уходила в
`NewWindow`; ссылка страницы на именованный фрейм вела страницу целиком,
роняя заодно и контрольный клик по несовпавшему имени, потому что страница
успевала уйти с исходного адреса раньше него), ПОСЛЕ ЗЕЛЁНЫЙ по всем трём
сценариям: ссылка ребёнка → именованный сосед (сосед навигирует, сам
кликнутый фрейм — нет), ссылка страницы → именованный фрейм (фрейм
навигирует, страница-хозяин не перезагружается), контроль — несовпавшее имя
по-прежнему ведёт страницу.

Не входит (очередь): каретка внутри фрейма, перевод фокуса клавишей Tab
внутри ребёнка (на странице этой функциональности тоже нет — больший,
отдельный пункт, не часть этой очереди), выделение и позиция курсора,
session history фреймов, `target` формы СТРАНИЦЫ (страница `target`
формы не учитывает нигде — то же самое отклонение, что уже задокументировано
у отправки формы ребёнка среза 20, не расширено этим срезом), оверлеи форм
внутри фрейма, плюс всё из очередей срезов 14–19.

## Срез 25 (P3, 2026-08-31) — relayout ребёнка при мутации ЕГО DOM

Очередь срезов 14/15 записывала: «relayout ребёнка при мутациях (его
собственных или пришедших от родителя, срез 5) … не входит». Измерено ДО
правки (`tests/wpt/verify_frame_mutation_relayout.py`): мутация СВОИМ
скриптом ребёнка (`el.style.width = '300px'`) и мутация фасадом РОДИТЕЛЯ
(`contentDocument.getElementById(...).setAttribute('style', …)`) обе видны
дереву (срез 5 это уже доказал), но ни одна не доходит до
`getBoundingClientRect()` самого ребёнка — оба снимка геометрии стоят на
100px до конца прогона независимо от источника мутации.

**Два разных источника, два разных механизма сигнала.** У каждого фрейма —
своя `V8JsRuntime`, поэтому обычный `dom_dirty` (флаг, который любой
DOM-мутирующий натив уже поднимает — тот же путь, которым страница узнаёт
про собственные мутации) корректно ловит **свою** мутацию ребёнка: натив
общий, рантайм тот же. Он не может поймать **мостовую**: `contentDocument`/
фасады (срез 5) пишут в дерево ребёнка нативами `_lumen_f_*`
(`crates/js/src/frame_bridge.rs`), которые исполняются в изоляте
РОДИТЕЛЯ — Rust-уровневая правка чужого дерева мимо нативов ребёнка, так что
его собственный `dom_dirty` эту запись не видит вовсе.

* **Мостовая мутация метит новый глобальный реестр**
  (`frame_bridge::frame_dom_dirty`, `Mutex<HashSet<usize>>`), ключ — тот же
  `Arc::as_ptr(&doc) as usize`, которым уже адресуются `FRAME_OUTBOX`/
  `self_key` (срез 4/8). Единственная точка записи —
  `with_accessible_doc_mut`, общая для всех восьми мутирующих нативов
  (create_element/create_text/set_attr/remove_attr/append_child/
  insert_before/remove_node/set_text): она уже резолвит биндинг и держит
  его `Arc` в момент вызова, поэтому пометка стоит там одним местом, а не в
  каждом нативе по отдельности. Чтение — `V8JsRuntime::take_frame_dom_dirty`,
  зеркало уже существующего `frame_transport_pending` (срез 8): оба читают
  СВОЙ `self_doc_key` (тот же `Arc::as_ptr`, что кладёт `install_dom`) и
  спрашивают у процесс-глобального реестра «есть что-то для меня?».
* **Своя мутация не получила нового кода вовсе** — драйвер `take_dom_dirty()`
  уже существовал на `PersistentJs`, шелл просто начал спрашивать его у
  КАЖДОГО фрейма, а не только у страницы.
* **Дренаж — в `about_to_wait.rs`**, рядом с уже существующим циклом по
  `frame_js_handles` (пумп сообщений/событий/скролла со срезов 4/6/17):
  оба флага читаются БЕЗУСЛОВНО (не `else if`) на каждый фрейм каждый тик —
  своя и мостовая мутация могли прийти в один и тот же тик, и пропуск
  второго флага оставил бы его висеть до следующей случайной мутации того
  же фрейма. Пересчёт — `Lumen::refresh_frames(Some(idx))` (тот самый метод,
  которым срез 18 применяет нативное переключение элемента формы): полный
  layout ЭТОГО фрейма без гейта «вьюпорт/интерактив не менялись» (тот гейт
  — для другого входа, здесь вьюпорт не тронут) и пересборка display list
  страницы.

Отклонения среза: мостовая пометка консервативна — `with_accessible_doc_mut`
метит документ грязным, как только биндинг найден и доступен, даже если
переданный `f` ничего не изменил (плохой `nid`, цикл вставки и т.п.); лишний
относительно-дешёвый `relayout_frame_content` не портит корректность, а
пропуск настоящей мутации испортил бы видимый результат. `create_element`/
`create_text` тоже метят — узел ещё не вставлен, видимого эффекта нет, но
единая точка (`with_accessible_doc_mut`) важнее точности этих двух.

Проверка: clippy `-p lumen-js --features v8-backend` / `-p lumen-shell`
`--all-targets` -D warnings; тесты js lib (+3: мостовая мутация метит и
дренируется РОВНО один раз, недоступный биндинг не метит, чтение не метит);
shell 1610+2 ok (без изменений — новый путь не имеет юнит-теста: логика
живёт в `Lumen`, который в тестах shell не конструируется, доказательство —
живая проба); `dump_golden.py` 12/12. A/B пробой
`verify_frame_mutation_relayout.py` — ДО правки КРАСНЫЙ (оба канала мутации
видны дереву, но `getBoundingClientRect()` стоит на 100px до конца
прогона), ПОСЛЕ ЗЕЛЁНЫЙ (own 100→300 после мутации ребёнка, bridge 100→300
после мутации родителя, оба отдельными снимками). Регрессионно зелены пробы
срезов 5 (`verify_frame_mutation`) и 6 (`verify_frame_click`);
`verify_frame_forms` (срез 18) падает на тех же двух проверках, что и до
этого среза — заведены как [BUG-927](BUG-927-FIXED.md)/[BUG-928](BUG-928-FIXED.md),
не связаны с этой правкой.

Не входит (очередь): событие `resize` ребёнку при смене вьюпорта (слайс 14
называл его тем же пунктом, что и relayout при мутации — сигнал теперь
есть, событие ещё нет), каретка внутри фрейма, перевод фокуса клавишей Tab
