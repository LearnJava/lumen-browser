...
**Статус:** FIXED 2026-09-13 (GAP-NAVCTX срез 16)
...
[BUG-797](BUG-797-FIXED.md) (нет канала `postMessage` к настоящему опенеру).
Фактическое закрытие вкладки/browsing context из скрипта — отдельная
архитектурная работа, не в этом срезе.

## Срез 4 (GAP-NAVCTX срез 16, 2026-09-13, `p1-gap-navctx-srez16`) — закрытие

Оба барьера, названные в срезе 3, сняты по цепочке: BUG-797 закрыт
([BUG-797](BUG-797-FIXED.md), тот же срез), BUG-883 закрыт раньше
([BUG-883](BUG-883-FIXED.md), срез 15). Правки самого этого бага в кодовой
базе не требуется — минимум («`closed`/`name` как свойства окна,
`close()` через путь выгрузки») уже был сделан срезом 3; заявленный id
ждал только исчезновения второго барьера.

**Живая проверка** (`tests/wpt/run_report.py --root
html/browsers/browsing-the-web/unloading-documents --recursive --all`,
dev-release, обходной `.gitignore`-фикс тулинга ниже): единственный id этого
бага, `prompt-and-unload-script-closeable.html`, — **OK, 1/1** (`beforeunload
and unload events fire after window.close() in script-closeable browsing
context`). Соседний `prompt-and-unload-script-uncloseable.html` — тоже OK,
не регрессия (контроль другого пути).

`RemoteContext`-семейство тестов, упомянутое в карточке как второй
потребитель `closed`, не переизмерялось целиком в этом срезе — карточка
привязывала его к BUG-797, который теперь закрыт, но отдельного WPT-id за
самим BUG-887 в этой части нет (RemoteContext используется десятками
тестов по всему дереву вне какой-либо одной директории); если конкретные id
останутся TIMEOUT после этого среза, это уже не BUG-887/797, а
самостоятельная находка.

Попутно обнаружен и не связан с этим багом: `run_report.py`/`run_smoke.py`
детерминированно падают `PermissionError` на `tests/wpt/.heavy.lock` —
тулинг сам держит на нём мандаторный Windows-лок на время прогона
(`heavy_lock.py`), а сканер манифеста (`tools/manifest/vcs.py`) ищет
`.gitignore` только внутри `tests_root` (`tests/wpt/.gitignore`, которого не
было — исключение `/tests/wpt/.heavy.lock` жило только в корневом
`.gitignore`), поэтому читает файл как обычный тестовый источник и
натыкается на собственный лок. Без файла `.gitignore` внутри `tests/wpt/`
(добавлен этим срезом, две строки) `run_report.py`/`run_suite.py` не
работают вообще, гарантированно, а не изредка — это ломало гейт для любой
WPT-проверки на этой машине, не только для этого бага.

Гейт: изменений в `crates/` нет; `tests/wpt/.gitignore` — новый файл, не
затрагивает вендоренные `tools/`; `python scripts/check_doc_links.py`
чисто после переименования файлов бага.
