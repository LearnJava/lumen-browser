# BUG-982: the final URL after an HTTP redirect never reaches JS — `Response.url`/`.redirected`, `XMLHttpRequest.responseURL` and `WorkerLocation` (dedicated/shared worker) all report the *pre-redirect* request URL
