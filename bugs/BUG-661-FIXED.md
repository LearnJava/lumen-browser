...
**Статус:** FIXED 2026-08-23 (P1) — остаток по вычитанию полосы прокрутки и по не-px-паддингам вынесен в раздел «Остаток» ниже
...
[BUG-852](BUG-852-FIXED.md).

## Исправление (2026-08-23, P1)

Всё в шиме страницы, `crates/js/src/dom.rs`, блок «ResizeObserver» — ни одного
изменения в шелле.

**Находка 1 — гарантированная первая доставка.** Ключ в том, что доставка не
обязана ждать relayout: она может встать в очередь событийного цикла сама.
`observe()` теперь ставит задачу `_ro_initial_pass` прямо в `_lumen_timers`
(`nesting: 0` + `_lumen_request_wakeup`, шаблон [BUG-842](BUG-842-FIXED.md)) —
`setTimeout` не годится, потому что клэмп §8.6 в 4 мс лёг бы на каждую попытку.
Проход идемпотентен: один поставленный таск покрывает любое число `observe()`
до своего запуска, и он сам себя снимает, когда неотчитавшихся наблюдений не
осталось. Отдельная тонкость — `observe()` из скрипта времени парсинга: снимок
геометрии шелл ещё не опубликовал, у **любого** элемента «нет бокса», и отчёт
0×0 был бы не пропущенным, а **неверным** первым entry. Поэтому проход сначала
проверяет `_ro_layout_published()` (есть ли rect у `documentElement`) и до
`_RO_INITIAL_MAX_ATTEMPTS` = 120 оборотов цикла перепланирует себя вместо
доставки — граница нужна, чтобы страница без layout не крутила таск вечно.

**Находка 2 — валидация аргумента.** `observe()` бросает `TypeError` на всё,
что не `Element` (`nodeType !== 1` ловит и `document`, и текстовый узел, где
проверки `__nid__` не хватало); конструктор бросает `TypeError` на не-функцию.
Повторный `observe()` того же таргета, как требует §3.1 шаг 2, заменяет
наблюдение — то есть сбрасывает `lastW/lastH` и заново планирует доставку.

**Находка 3 — различие боксов.** `contentBoxSize`/`contentRect` теперь считают
контентный бокс: border-box минус ширины рамок и паддинги, прочитанные из
опубликованных computed-стилей. `contentRect.x/y` — смещение контентного бокса
внутри border-box (левый/верхний паддинг), как требует §5.1, а не позиция
элемента во вьюпорте, которая стояла там раньше. `devicePixelContentBoxSize`
масштабируется на `devicePixelRatio` и округляется. Добавлена поддержка
`observe(target, {box: 'border-box'})`. Девять чтений computed-стиля на
наблюдение платятся лениво: наблюдение с `border-box` считает контентную
геометрию только когда у него реально появился entry.

**Находка 4 — reparenting.** Открепление наблюдаемого элемента разрушает его
бокс, и это наблюдаемое изменение размера, даже если элемент вернули той же
геометрией на том же обороте (классическая пара `remove()` + `appendChild()`,
между которыми ни один проход доставки не встаёт). Нативный
`_lumen_remove_child` обёрнут **один раз** присваиванием (не объявлением
функции — оно бы перезаписало натив до взятия алиаса и ушло в рекурсию), и
обёртка сбрасывает `lastW/lastH` у наблюдений, чей таргет — сам удаляемый узел
или его потомок. Через этот единственный натив проходят все удаления шима,
включая неявное внутри переносящего `appendChild`/`insertBefore`.

### Замер

Живая проба `tests/wpt/verify_frame_load_media_gaps.py --variant ro-basic`
(dev-release, Windows, `--seconds 5`): было — единственный
`ro-callback n=1 h=120` уже **после** того, как проба сама поменяла высоту;
стало — `ro-callback n=1 h=50` до всякого ресайза, затем `ro-resized` и
`ro-callback n=1 h=120`.

Категория целиком (`run_report.py --all --root resize-observer --recursive`,
две сборки подряд на одной машине; baseline — сборка `main` из корневого
чекаута):

```
baseline (main):  tests 5/18 harness OK; subtests 21/44 passed
исправлено:       tests 9/18 harness OK; subtests 33/56 passed
```

Знаменатель сабтестов вырос (44 → 56) не случайно: тест, который раньше
останавливался на первом же ожидании колбэка, теперь доходит до следующих
проверок, поэтому сравнивать надо число прошедших, а не долю.

## Остаток (не исправлено)

- **Вычитание полосы прокрутки** из контентного бокса (`scrollbars.html`,
  `scrollbars-2.html`). Геометрии полосы в снимке `layout_rects` нет вообще,
  а `getComputedStyle` о ней ничего не сообщает — нужна публикация used-value
  геометрии бокса со стороны layout, а не арифметика в шиме.
- **Паддинг не в px.** `computed_style_to_map` публикует паддинг в *заданных*
  единицах (`length_to_css`), поэтому `_ro_len` разрешает px/em/rem точно, а
  `%`/`calc()`/вьюпортные единицы даёт нулём — то есть для них контентный бокс
  остаётся равен border-box, как было до этого исправления. Рамки публикуются
  через `px_str` и разрешаются всегда.
- **Фрагментация** (`fragments.html`): массивы `contentBoxSize`/`borderBoxSize`
  по-прежнему одноэлементные, по фрагменту на элемент multicol не строятся.
- `iframe-same-origin.html` и `devicepixel.html` — reftest-ы, упирающиеся в
  [BUG-480](BUG-480-OPEN.md) и в отсутствие настоящего DPR, а не в наблюдатель.
