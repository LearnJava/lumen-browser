...
(см. [BUG-449](BUG-449-OPEN.md) — контекст является обычным объектным литералом,
все члены — собственные свойства экземпляра). `save`/`restore` просто зовут натив
и о JS-копии ничего не знают.

Тот же корень даёт и вторую половину [BUG-451](BUG-451-OPEN.md): геттер
...
Правильное — общее с [BUG-449](BUG-449-OPEN.md): завести настоящий
