# BUG-819 — Streams: `tee()` закрывает исходный поток и теряет чанки, BYOB-ридер подменяется обычным, async-итерации нет, `TextDecoderStream` не закрывает свою читаемую сторону

**Статус:** OPEN
**Заведён:** 2026-08-21 (WPT-RUN-6, срез 19 — общий с [BUG-818](BUG-818-OPEN.md) маркер `streams-promise-unsettled`, 40 id)
...
Пофайловый прогон (см. [BUG-818](BUG-818-OPEN.md), та же обвязка) сажает на
...
с [BUG-818](BUG-818-OPEN.md) (40 id), потому что по исходнику теста эти две
