...
[BUG-979](BUG-979-OPEN.md) (срез 58) — тот же `winFacade`'s `w = {}`, но
шире: фасад вообще не проксирует к реальному глобальному объекту фрейма,
поэтому любой global, который фрейм объявил сам (не только методы
`EventTarget`), недостижим через `contentWindow`/именованный доступ.
