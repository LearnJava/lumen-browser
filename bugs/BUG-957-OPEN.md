...
## Смежная находка (не эта запись)

[BUG-979](BUG-979-FIXED.md) (срез 58, закрыт 2026-09-19) — тот же
`winFacade`'s `w = {}`, но шире: фасад не проксировал к реальному
глобальному объекту фрейма для ЗНАЧЕНИЙ/ВЫЗОВОВ (любой global, который фрейм
объявил сам). Эта запись (BUG-957) остаётся открытой — она о колбэке,
пересекающем границу изолятов (`addEventListener`), а не о значении;
`JsValue` (на чём стоит фикс BUG-979) функцию не переносит.
Заведено отдельным номером — починка структурно другая (живой мост к
глобалу фрейма, а не дописывание ещё нескольких IDL-имён).
