# BUG-348: `restyle_root_set_for_node_change` doesn't account for `:has()` — a DOM mutation can leave an ancestor's `:has()`-dependent style stale under the incremental restyle path
