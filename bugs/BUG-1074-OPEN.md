# BUG-1074 — `navigation.navigate('#frag' | '?q')` передаёт оболочке относительный URL без разбора по базе документа: загрузка падает `invalid url: "#frag": relative URL without a base`
