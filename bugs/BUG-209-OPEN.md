...
## Ревизия P3 2026-08-31

Живой прогон `--only 90 --no-cache` дважды подряд стабильно даёт **1.29%** вместо
задокументированных 1.71% — ратчет, не деградация. Диагноз остатка не изменился
(chrome-иконка сломанного AVIF-изображения, которую Edge рисует, а Lumen — нет);
снижение диффа — следствие независимой правки где-то на `main` (font-metrics/layout),
затронувшей соседние элементы страницы. `KNOWN_DEBTORS['90']` в `run.py` обновлён на
1.29.

## Воспроизведение

`python graphic_tests/run.py --only 90` → 1.29% (KNOWN_DEBTOR, в пределах храповика).
