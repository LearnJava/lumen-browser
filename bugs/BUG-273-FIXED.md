...
**Статус:** FIXED 2026-07-21 — срез 1 (viewport-culling офscreen-групп) влит 2026-07-16; срез 2 (межкадровый кэш bbox-filter-слоёв) влит 2026-07-20; срез 3 (GPU-композитинг) верифицирован закрытым 2026-07-21 — задним числом реализован миграцией на wgpu-дефолт (ADR-017, 2026-07-13), не отдельным фиксом
**Компонент:** paint (femtovg fallback backend, offscreen-композитинг blend/filter-слоёв); wgpu (текущий дефолт) не затронут корнем этого бага
...
backdrop-зависимы) и первое появление каждой группы на femtovg — остаётся
направлению 3 (GPU-композитинг шейдерами), которое, как выяснилось при
верификации ниже, уже закрыто на дефолтном бэкенде отдельной задачей.

## Срез 3 — верификация на дефолтном бэкенде (закрыт 2026-07-21, P1)

Направление 3 «GPU-композитинг (Phase 3)» было сформулировано 2026-07-08,
**до** флипа дефолтного бэкенда на wgpu (`P1-wgpu-flip`, 2026-07-13, ADR-017).
Ревизия кода `crates/engine/paint/src/renderer.rs` (wgpu `Renderer`,
обёрнутый тонким `WgpuBackend`) показала, что программируемый
GPU-композитинг для всех трёх примитивов там **уже реализован** —
не CPU-readback, а честный шейдерный render-plan с offscreen render-таргетами:
WGSL blend-шейдер на все 16 CSS blend-режимов (`BLEND_SHADER_SRC`,
`blend_mode_to_u32`) с отдельным `blend`-проходом (`CompositePlan`); цветоматричный
`filter`-пайплайн + двухпроходный сепарабельный Gaussian blur; отдельный
`backdrop-filter`-пайплайн (REPLACE-блендинг) с собственным result-кэшем
(`backdrop_cache.rs`). Иными словами, срез 3 не требовал новой реализации —
он был готов и в проде ещё до того, как эта диагностика началась.

**Эмпирическая верификация** (`scripts/scroll_perf.py graphic_tests/1000000-final.html`,
dev-release, без `LUMEN_BACKEND=` — т.е. на живом дефолте):

```
кадров: 283 (50+50 щелчков)
[frame] total:  avg 4.52 мс   p50 2.41 мс   max 176.24 мс
эффективный FPS по среднему кадру: ≈221
```

Для сравнения тот же прогон с явным `LUMEN_BACKEND=femtovg` (fallback-путь):

```
кадров: 133
[frame] total:  avg 124.87 мс   p50 7.43 мс   max 2156.93 мс
топ выбросов: 2156.93 / 1959.34 / 1938.47 / 1858.88 / 1303.27 мс
```

На femtovg остаются катастрофические одиночные кадры (>1.9 с) — это ровно
случай «первое появление группы», прямо названный невылеченным в конце
среза 2 (CPU-readback композит всё ещё платится полностью при первом входе
blend/filter/backdrop-слоя в вьюпорт, кэш среза 2 не покрывает blur/backdrop/blend).
На дефолтном wgpu таких выбросов нет — GPU render-pass не требует
`glReadPixels`+CPU-цикл ни для первого, ни для последующих кадров.

**Вывод.** Исходный симптом (слайдшоу ~660 мс/кадр, 1.5 FPS) физически
воспроизводим только под `LUMEN_BACKEND=femtovg`; femtovg с 2026-07-13 —
опциональный fallback-путь (не дефолт). На актуальном дефолтном бэкенде
проблема отсутствует. Закрываю FIXED; остаточная медлительность
first-appearance-кадров на femtovg-fallback не переоткрывается отдельным
багом — это архитектурная граница CPU-readback-пути, а не регрессия, и не
затрагивает пользователей дефолтной сборки.
