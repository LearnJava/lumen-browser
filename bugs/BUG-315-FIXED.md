...
**Renumbered 2026-07-18** from `BUG-295`, then `BUG-310` — assigned twice more by independent parallel sessions to different bugs (six BiDi commands with no live-window effect, see [BUG-295](BUG-295-FIXED.md); ElementTraversal/`ParentNode.children` gap, see [BUG-310](BUG-310-OPEN.md)); this bug kept its content but moved to the next free number each time.
