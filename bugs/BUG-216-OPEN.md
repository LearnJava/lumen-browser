...
> **Ревизия 2026-08-31 (P3):** живой прогон `--only 117 --no-cache` дважды подряд
> стабильно даёт **1.13%** (ратчет 2.23%→1.13%). Diff-картинка — только глиф-контуры
> четырёх строк текста (serif Edge vs Inter sans Lumen), без структурных отличий:
> кавычки/гильеметы примыкают вплотную, вложенность и `quotes: none` совпадают.
> Точечного P3-дефекта нет, диагноз стабилен. Новый baseline KNOWN_DEBTORS 1.13% (TEST-117).
