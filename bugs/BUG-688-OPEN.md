...
- `expose-legacy-touch-event-apis.html` (16/16 subtests actually pass, but harness itself ERRORs on "4 duplicate test names: `'ontouchstart' in [object Object]`" — the four probed objects (`window`, `document`, `HTMLElement.prototype`, `SVGElement.prototype`) all stringify to the generic `[object Object]` instead of distinct tags, a symptom of the already-open [BUG-367](bugs/BUG-367-FIXED.md) point (4)/toStringTag class, not a new finding, and orthogonal to the Touch gap itself since the *values* being compared are consistent).
