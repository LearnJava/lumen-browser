...
- `lumen::SelectDropdownSnapshot` (`crates/shell/src/lumen/select_dropdown_snapshot.rs`):
  раз за кадр `refresh` берёт документ через `try_lock` и читает опции
  (`collect_select_options`) и, для base-select, стили строк
  (`forms::resolve_option_row_styles`). Занятый или отравленный лок — рисуется
  прошлое чтение того же документа и `<select>`, для чужого — ничего.
  Геометрия (якорь, скролл, вьюпорт) применяется каждый кадр в `build`.
- `forms::build_base_select_dropdown` больше не принимает `&Document`: вся
  работа с документом вынесена в `resolve_option_row_styles`, построитель
  чистый.
- Один снимок обслуживает и дропдаун страницы, и дропдаун фрейма (FRAME-6):
  фреймовый путь тоже брал `handle.doc.lock()` блокирующе; ключ по адресу
  `Arc` документа разводит их чтения.
- Тесты: `select_dropdown_snapshot::tests` — свободный лок, base-select
  стили, занятый лок возвращается сразу с прошлым чтением, отравленный лок
  не паникует, чужой документ не наследует чтение.

Не тронуто сознательно: обработчик клика по опции (`lumen/click.rs`) берёт
документ блокирующе — выбор должен лечь в текущий DOM, как ввод с
клавиатуры в BUG-1108.
