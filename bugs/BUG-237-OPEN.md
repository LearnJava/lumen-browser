...
Живой прогон (`--build`, dev-release, два чистых прогона подряд) стабильно даёт **9.69%**
(baseline был 11.19%). Ратчет `KNOWN_DEBTORS['122']` в `graphic_tests/run.py` подтянут
11.19→9.69 — правдоподобно тот же класс правок, что подтянул соседние DEBTOR-ратчеты в тот
же день (BUG-451/BUG-452).

Diff-картинка (`122-line-height-step-diff.png`) новых структурных дефектов не показала —
ровно та же картина, что в расследовании 2026-06-23: stepped-колонки (зелёные/жёлтые/розовые
полосы) залиты 48/60/40px line-box против 24px natural у Edge, плюс сдвиг переноса
natural-колонки от font-parity (Inter шире Edge-шрифта → 3 строки вместо 2). Точечного
P3-дефекта не найдено, остаток по-прежнему целиком в домене «Lumen корректнее эталона».
