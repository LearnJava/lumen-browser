...
Взята опция (a) из вывода среза 10, но в узкой, дёшево доказуемой форме
вместо полного `nth-of-type(N)`: `A + subject`, когда `A` — голый `*`
(без `Type`/`Class`/`Id`/`Attribute`/псевдоклассов), сводится к «есть ли
у subject ЛЮБОЙ предшествующий element-sibling» — это ровно отрицание
`is_first_element_child`, уже закреплённого в ключе полем
`ShareKey::is_first_child` для самого subject-узла (срез 4). Никакого
нового поля не потребовалось: `NextSibling`-комбинатор перед SUBJECT
теперь допускается в `complex_is_share_safe`
(`share_safety.rs::compound_is_bare_universal`), когда левый компаунд —
буквально `[Universal]`; на ANCESTOR-позиции `NextSibling` остаётся
запрещён (ключ не хранит sibling-факты предков). Гейт
`sheet_has_position_dependent_subject` (`share_cache.rs`) расширен —
детектирует и эту форму, включая `track_position`, иначе `is_first_child`
остался бы `false`/`false`-заглушкой и результат был бы несостоятелен.

Добавлены 3 теста-регрессии в `style/tests/share_cache.rs`: позитивный
(`* + *` не мешает шарить, когда правило вообще недостижимо), корректность
(`* + *` даёт разный, верный результат первому/второму ребёнку) и
граница (`:not(label) + *` — НЕ голый `*` слева — по-прежнему запрещает
шаринг, что и должно быть, т.к. ключ не знает тега соседа).

Живой перемер (тот же метод, бинарник собран на коде этого среза):

- **github.com:** `insert=0 miss=1780 (key_none=1389 key_some_unshareable=391)`
  — бит-в-бит как срез 10.
- **lenta.ru:** `insert=0 miss=1498 (key_none=1415 key_some_unshareable=83)`
  — бит-в-бит как срез 10.

Ожидаемо (см. срез 10's разбор зонда): доминирующие блокеры на обоих
сайтах — НЕ `* + *`, а селекторы, где левый компаунд несёт реальное
ограничение (`:not(label) + *`, `h1 + *`/`h2 + *`, `.radio-input:disabled
+ .radio-label`, `~` внутри `:is(...)`), которые срез 11 сознательно НЕ
рескьюит (ключ не может дёшево нести произвольный тег/атрибуты соседа).
`compute_style_shareable`'s `shareable &= ... .all(selector_is_share_safe)`
требует безопасности КАЖДОГО кандидата одновременно — частичное решение
одного из нескольких one­временно действующих sibling-правил не двигает
`insert`, пока хотя бы одно из остальных остаётся блокирующим для тех же
391/83 узлов.

**Вывод:** срез 11 — корректное, протестированное и архитектурно чистое
расширение (снимает часть уже сформулированной опции (a), причём
дешевле, чем `nth-of-type(N)` — без нового поля ключа), полезное для
сайтов, где `* + *`/`lobotomized owl`-паттерн — единственный
sibling-блокер. НЕ прогресс на github.com/lenta.ru: оба сайта
одновременно используют сложные sibling-селекторы (конкретный тег/класс
соседа или `~` внутри `:is()`), которые опция (a) в общем виде
(`nth-of-type(N)`) тоже не рескьюила бы — `nth-of-type` кодирует
позицию, а не тег/состояние соседа, так что даже полная реализация (a)
не закрыла бы найденные срезом 10 доминанты. Полное решение требует
поля вроде «тег+атрибуты непосредственного предыдущего sibling»
— на порядок дороже (дублирует часть стоимости `attrs` на КАЖДЫЙ узел,
не только SVG-presentational, потому что предыдущий sibling сам может не
быть presentational) и не гарантированно окупается тем же классом
soundness-доказательства, что `is_first_child`.

**Рекомендация по очереди:** ветка BUG-1112 «расширить `ShareKey`
дёшево доказуемыми фактами» исчерпана для github.com/lenta.ru после 11
срезов (4 независимых механизма реализованы и подтверждены тестами:
атрибуты/`:root`/`:where`/`:is`/`:not`/`::pseudo-element` на предке и
subject, `:first-child`/`:last-child`/`:only-child`, динамический
`:hover`-фингерпринт предка и subject, и теперь голый `* + *`) — каждый
корректен и потенциально полезен другим сайтам, ни один не сдвинул
измеренную цель на этих двух. Следующий срез НЕ должен продолжать в том
же направлении без новой гипотезы: либо признать `share_insert=0` на
github.com/lenta.ru архитектурным пределом текущего `ShareKey` (закрыть
BUG-1112 как ДОРАБОТКА/DEBTOR, снять его как блокер BUG-935's M4-свопа
по умолчанию отдельным решением), либо спроектировать новый механизм
уровня «tag+attrs предыдущего sibling» с отдельной оценкой стоимости.

Гейты: `cargo clippy -p lumen-layout --all-targets -- -D warnings` чист;
`cargo test -p lumen-layout --lib style::tests::share_cache` — 23/23;
`cargo test -p lumen-layout --lib style::` — зелёный.

## Срез 12 (P3, 2026-09-24) — переклассификация в DEBTOR

По итогам среза 11 (ветка «дёшево доказуемые поля `ShareKey`» исчерпана
для github.com/lenta.ru — 4 независимых механизма реализованы и
подтверждены тестами, ни один не сдвинул `share_insert` с нуля) вынесено
пользователю решение по направлению: закрыть остаток как архитектурный
предел или спроектировать существенно более дорогой механизм. Решение
пользователя (2026-09-24) — принять `share_insert=0` на этих двух сайтах
как архитектурный предел текущего `ShareKey` и переклассифицировать
запись в `OPEN (DEBTOR)`.

Это НЕ означает, что все реализованные срезами 1-11 механизмы бесполезны:
каждый (атрибуты/`:root`/`:where`/`:is`/`:not`/`::pseudo-element` на
предке и subject, `:first-child`/`:last-child`/`:only-child`,
динамический `:hover`-фингерпринт, голый `* + *`) корректен и потенциально
включает шаринг на сайтах с более простыми селекторными паттернами —
только на github.com/lenta.ru они одновременно перекрыты более сложными
sibling-селекторами (конкретный тег/класс соседа, `~` внутри `:is()`),
которые вне зоны «дёшево доказуемого» факта.

**Статус:** `OPEN (DEBTOR)` — P3 не берёт эту ветку без новой гипотезы
уровня «поле для факта о непосредственном предыдущем sibling» с отдельной
оценкой стоимости (дублирует часть стоимости `attrs` на каждый узел, не
только SVG-presentational — см. срез 11). Файл остаётся на месте, ссылки
из BUGS.md/STATUS-файлов не меняются.

**Разделение с BUG-935:** этот срез закрывает только вопрос ShareCache;
судьба `LUMEN_BUG935_M4_SWAP` по умолчанию (отклонён срезом 47 BUG-935
именно из-за `cascade_reused=0`, который объясняет этот баг) — отдельное
решение, не принятое здесь. Следующая сессия, разбирающая BUG-935, должна
явно решить: держать `swap` измерительным флагом бессрочно (текущее
состояние) или найти путь A/B, не зависящий от починки ShareCache.

Без изменений кода в этом срезе — только реклассификация статуса и записи.
