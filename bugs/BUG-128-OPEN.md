...
- chrome-документ (`crates/chrome`, рендерится тем же движком, поэтому
  тоже стартует с `ComputedStyle::root()`) не уехал на serif: его
  `UA_DEFAULTS` теперь явно называет `html{font-family:'Golos Text'}` —
  зарезервированное имя, резолвится в bundled face без системного
  `FontProvider`, поэтому типографика chrome прежняя и не зависит от
