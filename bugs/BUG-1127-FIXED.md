...
Сразу после загрузки внешнего листа (`<link>` и каждый `@import`) его относительные `url()`
переписываются в абсолютные от URL самого листа — `crates/shell/src/css_url_rebase.rs`,
вызов в `stylesheets.rs::fetch_stylesheet_for_cascade`, до склейки листов. Сканер пропускает
комментарии и строки; не трогает абсолютные адреса, `url(#id)` (ссылка на документ), пустые и
экранированные аргументы. Файловая база переписывается в `file:`-URL, который резолвится
обратно в путь. Реестр CSSOM (`build_stylesheet_node_registry`) берёт сырой текст — `cssText`
сохраняет запись автора, как в Chrome.

Проверка: репро `g5/fontbase.html` в окне запрашивает `/g5/css/fonts/f.ttf` → 200
(`FontLoaded: «G5Font»`) и `/g5/css/bg.svg`. Живой apple.com за 100 с не дошёл до стадии
`@font-face` (медленная загрузка страницы, не этот дефект); лист футера использует тот же
`url("../assets/…")`, что покрыт тестом. Не покрыто: строковые аргументы `image-set("a.png")`
и `src()` (только `url()`).
