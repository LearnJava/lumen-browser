...
- Репро положено в `fuzz/regressions/fuzz_image-gif-lzw-hang` и
  `fuzz/corpus/fuzz_image/gif-lzw-ends-early.gif`; шаг replay в
  `.github/workflows/fuzz.yml` гоняет это репро на каждом прогоне и он
  **блокирующий** — `KNOWN_FAILING` его не покрывает.
- **Таргет `fuzz_image` пришлось вернуть в `KNOWN_FAILING` в тот же день** —
  но не из-за этого бага. Первый же прогон CI, где таргет стал блокирующим,
  нашёл [BUG-790](BUG-790-OPEN.md): 78-байтный GIF с заявленным логическим
  экраном 65281×57098 просит у аллокатора 14.9 ГБ. Дефект предсуществующий
  (тот же ввод даёт байт в байт тот же отказ на коде до этого фикса) и просто
  маскировался: пока `frame_image` вис на LZW, до него не доходили. Репро
  BUG-787 при этом остаётся блокирующим — шаг replay `KNOWN_FAILING` не
  покрывает.
