...
Первая строка очереди STATUS-P3.md (BUGS.md:146), проверена top-down.
Свежий build (dev-release) + два чистых `graphic_tests/run.py --only 77`
подряд стабильно дают **12.95%** — в пределах `_DEBTOR_TOL` (±2.0%) от
документированных 12.94%, ратчет не требуется.

Diff-картинка (`77-anchor-positioning-diff.png`) содержит ровно две области,
совпадающие с описанным выше остатком: одиночный оранжевый квадрат сверху
слева + 3×3 сетка (п.1, `inset-area` vs `position-area`) и два span-бара
внизу (п.2). Никаких посторонних/структурных расхождений вне этих зон не
найдено — отдельного нового дефекта, в отличие от BUG-124/BUG-931, здесь нет.
Диагноз 2026-06-19 остаётся в силе.
