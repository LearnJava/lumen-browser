...
**Статус:** FIXED 2026-09-25 — дрейф трекера: закрыт GAP-REFERRER срезом 9 (2026-09-22); строка BUGS.md не была перенесена при закрытии задачи (сверка с кодом 2026-09-25)
...
## Срез 9 (2026-09-22, P6, финал `p6-gap-referrer-srez9`) — GAP-REFERRER ЗАКРЫТА

Срезы 2-8 (не расписаны здесь по отдельности — сводка в `ROADMAP.md`'s
GAP-REFERRER row) довели `document_referrer_policy` до всех оставшихся
producer-ов из таблицы измерения (`<img>` во всех вариантах, `<script src>`,
`<link>`/`@import`, `<iframe src>`, `@font-face`/видео/`fetch_vtt_text`) и
добавили источники политики (`<meta name=referrer>`, заголовок ответа
`Referrer-Policy`, атрибут `referrerpolicy` на `a`/`area`/`iframe`/`img`/
`link`/`script` по HTML LS §6.6).

Единственный оставшийся пункт — preload-сканер — закрыт этим срезом явным
решением, без изменения кода: `lumen-html-parser::scan_preload_hints`/
`PreloadScanner` и их caller-ы (`page_pipeline.rs::dispatch_preload_hints`,
`page_source.rs::emit_early_hints`) не делают сетевого запроса вообще — они
резолвят URL и эмитят `Event::SubresourceHintFound`, стерр-строку лога
(доккомент `dispatch_preload_hints` и `page_source.rs:489` это фиксируют
явно; `grep -rn SubresourceHintFound crates/` показывает единственного
потребителя — `event_sink.rs`). Реальный fetch по каждому найденному
сканером ресурсу идёт отдельным путём с `&Document` в скоупе, который срезы
3-8 уже перевели на политику документа. Значит preload-сканер структурно не
может нести неверный `Referer` — исключение из объёма не компромисс, а факт
архитектуры (не «остаток, который решили не делать», а «здесь нечего
делать»).

Задача закрыта: все измеренные в постановке источники (`fetch()`/XHR/
`sendBeacon`/`<img>`/`<script src>`/`<link>`/`@import`/`<iframe src>`/
шрифты/медиа) несут `Referer`/`Origin` по политике документа с
per-элементным `referrerpolicy`-override там, где спека его определяет.
Живой WPT-повтор (`referrer-policy`/`beacon` категории) этим срезом
по-прежнему не прогонялся — задел для отдельной проверки, но не блокер
закрытия объёма задачи, которая была про генерацию заголовков, а не про
процент прохождения конкретной WPT-категории.
