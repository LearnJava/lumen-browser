...
**Компонент:** js (`crates/js/src/v8_runtime.rs` — перехватчик именованных
свойств на шаблоне глобального объекта; `window` при этом уже является
настоящим глобальным объектом — `globalThis === window` истинно,
[BUG-280](BUG-280-FIXED.md) исправлен)
...
* [BUG-381](BUG-381-FIXED.md) — измерение которого этот баг закрывает на трёх
...
## Как исправлено (P3, 2026-08-10)

Перехватчик именованных свойств V8 на шаблоне глобального объекта —
`window_named_properties_template` в `crates/js/src/v8_runtime.rs`, подключён
через `v8::ContextOptions { global_template }` в `v8_thread_main`. Ровно то
место, которое называл раздел «Как чинить»; дешёвый промежуточный вариант
(заводить свойство на каждый элемент с `id`) не понадобился.

**Почему `NON_MASKING` — это и есть весь порядок разрешения.** V8 зовёт
перехватчик с этим флагом *только* для имён, которые не разрешились больше
нигде. Поэтому собственные свойства `Window` и `var`/`function` страницы
побеждают элемент без единой строки бухгалтерии с нашей стороны, а до элемента
доходит ровно то, что иначе было бы `ReferenceError` — тот порядок, который
требует HTML LS §7.3.3 (объект именованных свойств стоит *позади* глобала).
`ONLY_INTERCEPT_STRINGS` убирает с пути символьные обращения
(`Symbol.toStringTag`, `Symbol.unscopables` и прочие), которые иначе шли бы
через перехватчик на каждом промахе.

Пара `getter` + `query`: без `query` проверка `'x' in window` заставляла бы V8
строить обёртку элемента только ради ответа «свойство есть» и тут же её
выбрасывать.

**Что резолвится:** любой элемент с совпадающим `id`, плюс
`img`/`form`/`iframe`/`embed`/`object` с совпадающим `name`. Три осознанных
упрощения против спеки, все в сторону «разрешить во что-то полезное вместо
`ReferenceError`»: несколько совпадений дают первое в порядке дерева, а не
`HTMLCollection`; совпавший `iframe` даёт сам элемент, а не его
`contentWindow` (браузингового контекста у `<iframe>` в движке всё равно нет);
индекс имён не поддерживается — на промах идёт обход дерева.

**Жизненный цикл документа.** Перехватчик ставится при создании контекста, за
много до того, как появится хоть какой-то документ; сам документ публикуется в
thread-local из `install_dom` (то есть заново на каждую навигацию). До первой
публикации — и навсегда в worker-изолятах, которые `install_dom` не зовут —
`named_access_lookup` отвечает `None`, то есть механизм инертен, а не
отсутствует.

**`try_lock`, а не `lock`.** Перехватчик срабатывает на *любом* промахе имени,
включая промах из JS, который натив вызвал, уже держа замок документа.
Блокирующий захват там встал бы в дедлок JS-потока с самим собой; отказ стоит
одного неразрешённого имени — ровно то поведение, что было до этого фикса.

**Идентичность обёртки.** Значение строится вызовом шимового
`_lumen_make_element`, который кэширует обёртки по узлу, поэтому
`window.probe === document.getElementById('probe')`. Своя обёртка ломала бы
код, завязанный на идентичность (учёт слушателей, ключи `Map`), причём
незаметнее, чем ломало отсутствие свойства.

### Цена

A/B на одном и том же билде (перехватчик снят подменой `global_template` на
`Default::default()`), минимум из 5 прогонов:

| Сценарий | С перехватчиком | Без |
|---|---|---|
| 4 млн чтений существующего глобала (`Math.PI`) | 52.7 мс | 54.4 мс |
| 20 тыс. промахов (`typeof zzzNope`) | 15.0 мс | 0.4 мс |

Главное здесь — первая строка: перехватчик на глобале **не** деоптимизирует
обычный доступ к глобальным переменным (разница в пределах шума). Промах стоит
~0.75 мкс против ~0.02 мкс на документе из 8 узлов; поиск O(n) по дереву, так
что на большом документе дороже. Приемлемо, потому что промахи по глобальному
имени в нормальном коде редки (фиче-детект на старте, а не в цикле), но если
когда-нибудь понадобится — лечится поддерживаемым индексом имён.

### Проверка

* 7 юнит-тестов в `v8_runtime.rs` (`element_id_is_a_named_property_of_window`,
  `named_access_yields_the_same_wrapper_as_get_element_by_id`,
  `name_attribute_is_named_property_only_on_eligible_elements`,
  `unmatched_name_stays_undefined`, `real_globals_win_over_named_elements`,
  `page_variable_wins_over_named_element`,
  `named_access_follows_live_dom_mutations`).
* Негативный контроль: при снятом перехватчике 4 из 7 краснеют, а 3
  «не сломай» — остаются зелёными, то есть тесты меряют именно этот механизм.
* E2E через шелл: `--dump-layout` на странице с `<div id="probe">`,
  `<img name="logo">` и проверками из раздела «Как чинить» — в дампе
  `NAMED-ACCESS-OK`.

### Что этот фикс разблокировал, но не починил

Все перечисленные выше срезы WPT-RUN-3 (9–43) ждали именно этого. Их `.ini`
теперь подлежат перезамеру — но два предупреждения из среза 10 остаются в
силе: `variable-cycles.html` после этого фикса всё равно упадёт, уже через
[BUG-499](BUG-499-OPEN.md), а три сабтеста
`variables-substitute-guaranteed-invalid.html` позеленеют *по совпадению*, не
потому что движок научился различать невалидность. Перезамер — за P2.
