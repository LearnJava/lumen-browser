...
Вся категория тем же A/B (`--root workers --recursive`, тот же слот, два
бинарника подряд):

| | harness OK | сабтесты | время |
|---|---|---|---|
| до | 165/249 | 221/881 | 20:41 |
| после | **193/249** | **279/894** | **15:55** |

Знаменатель сабтестов вырос (881 → 894), потому что перестававший висеть
файл успевает объявить больше подтестов — сравнивать надо числитель, а не
долю (урок «сравнивать сабтесты, а не знаменатель harness»).
...
- **`credentials`** читается и проверяется, но на поведение не влияет:
  ...
  ([BUG-859](BUG-859-FIXED.md)), поэтому разницы между `omit` и
