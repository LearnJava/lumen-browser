...
## Срез 21 — wgpu `TexturePool`: байтовый бюджет + LRU-вытеснение; первый замер симптома на дефолтном бэкенде (влит 2026-07-29)

**Почему этот срез вообще появился.** Все срезы 1–20 делались по femtovg, который перестал быть дефолтным
бэкендом ещё 2026-07-13 (ADR-017). Ни разу за 20 срезов симптом бага — «~1 ГБ на lenta.ru» — не был
перезамерен на том бэкенде, который у пользователя работает по умолчанию. Ревизия дефолтного пути дала
две находки.

**Находка 1 — у wgpu-бэкенда не было `debug_mem_report`.** Срез 16 дал пофакторную GPU-разбивку
`FemtovgBackend`, а `WgpuBackend` пользовался пустой заглушкой из `RenderBackend` (`backend.rs`), поэтому
`LUMEN_MEM_REPORT=1` на дефолтном бэкенде не печатал вообще ничего про GPU-хранилища. Добавлен
`WgpuBackend::debug_mem_report` → `Renderer::texture_pool_report()`: свободные текстуры, число классов
размеров, объём свободного списка против бюджета, счётчик вытеснений и уже существовавшие
`TEXTURE_POOL_HITS`/`MISSES`.

**Находка 2 — пул offscreen-слоёв рос без ограничения.** `TexturePool` (`texture_pool.rs`, ADR-008 Ф2)
хранит свободные текстуры в `HashMap<TextureKey, Vec<PooledTexture>>`; `release` всегда клал текстуру в
список своего класса размеров, и освобождался список только целиком — на `clear` (ресайз окна). Ни капа,
ни вытеснения не было, в отличие от femtovg-аналогов (`layer_pool` кап 8, срез 1; `resized_variants` LRU
128, срез 18). Класс размеров, востребованный один раз, держал свою текстуру до конца жизни окна.

**Фикс.** Свободный список ограничен байтовым бюджетом (по умолчанию 64 МиБ, переменная
`LUMEN_TEXTURE_POOL_MB`; `0` — снять ограничение и получить досрезовое поведение на том же бинарнике).
Вытеснение — LRU по классам размеров, выполняется в `TexturePool::trim()`, которую `Renderer::render`
вызывает **после `queue.submit`**: в этой точке wgpu удерживает ресурсы отправленных команд сам (тот же
довод, что у уже существовавшего возврата `temp_grad_layers` строкой выше), поэтому drop свободной
текстуры безопасен. `TexturePool` стал генериком по полезной нагрузке (`PoolTexture`-трейт: только
`width`/`height`) — иначе учёт байтов и LRU нечем проверить юнит-тестами, `PooledTexture` не
сконструировать без живого `wgpu::Device`.

**Замеры (dev-release, дефолтный бэкенд, один бинарник, переключение через `LUMEN_TEXTURE_POOL_MB`).**

* **Симптом бага на дефолтном бэкенде, впервые: lenta.ru = 629 МБ WS / 601 МБ private, плоско за 48 с
  простоя** (выборка RSS раз в 2 с: 621 МБ на первой выборке, дальше 629 МБ без движения). Против
  открывшего баг замера 2026-07-07 на femtovg — 1371 МБ WS / 1124 МБ private — и против Edge ~530 МБ:
  отрыв от Edge упал с ~840 МБ до ~100 МБ, роста в простое нет.
* **Пул на lenta.ru не задействован вообще**: `free 0 tex / 0 size-classes / 0.0 MiB | hits 0 misses 0` —
  wgpu-путь не открывает там ни одного offscreen-уровня.
* **Синтетика под пул** (`.tmp/pool-stress.html`: 48 карточек `backdrop-filter: blur+saturate` разных
  размеров + `opacity`, страница 18992 px, `LUMEN_BENCH=scroll:2500` — 2500 полных перерисовок со
  скроллом): пул выходит на плато **8 классов / 16 свободных текстур / 17.0 МиБ, 20 аллокаций за всю
  сессию**, дальше только попадания (hits 6→18→30 при неизменных misses=20). Бюджет 64 МиБ и снятый
  бюджет дают **побайтово одинаковую картину и 0 вытеснений**.

**Что это значит.** Неограниченность пула была структурным риском, а не наблюдаемой утечкой: на реальной
и на специально враждебной странице рабочий набор классов мал и стабилен. Срез поэтому НЕ даёт выигрыша
памяти — он ставит потолок там, где его не было, и добавляет наблюдаемость, которой на дефолтном
бэкенде не существовало. Заявлять экономию по этому срезу нельзя.

**Проверка живого пути вытеснения** (`LUMEN_TEXTURE_POOL_MB=4`, тот же прогон на 1200 кадров): 13 текстур
вытеснено, свободный список удержан на 3.4 МиБ при бюджете 4 МиБ, 1200 кадров отрисованы, ни паник, ни
ошибок валидации wgpu; медиана кадра 14.8 мс против 14.2/14.6 мс в прогонах A/B — то есть drop текстур
после сабмита безопасен и не стоит времени.

**Пиксельная нейтральность.** При дефолтном бюджете вытеснений не происходит (замерено), а остальная
семантика `acquire`/`release` не менялась, поэтому дефолтный путь бит-в-бит прежний. Графтесты на
wgpu-пути: TEST-00 (калибровка) PASS 0.00%, TEST-102 (opacity × stacking) PASS 0.00%, TEST-30
(backdrop-filter + фильтры) DEBTOR 10.64% при базлайне 10.24%, TEST-104 (mask × radius) DEBTOR 32.28%
при базлайне 32.78% — все в пределах известных чисел BUG-277.

**Тесты:** 11 юнитов `texture_pool::tests` (в т.ч. `unbounded_pool_never_evicts` —
поведение до среза, `trim_evicts_down_to_budget`, `trim_evicts_least_recently_used_class`,
`trim_drops_empty_size_classes`, `clear_resets_byte_accounting`) + 5 прежних
`cases::texture_pool_integration` — зелёные; `clippy -p lumen-paint --features backend-wgpu
--all-targets -D warnings` зелёный.
...
(image-кэши — срезы 6/17/18/19/20). Срез 21 добавил недостающий потолок и наблюдаемость на дефолтном
бэкенде и — главное — впервые перезамерил **сам симптом** там, где он у пользователя и проявляется:

**lenta.ru на дефолтном (wgpu) бэкенде — 629 МБ WS / 601 МБ private, без роста за 48 с простоя**, против
1371 МБ WS / 1124 МБ private в открывшем баг замере 2026-07-07 (femtovg) и против Edge ~530 МБ. Отрыв от
Edge — ~100 МБ вместо ~840 МБ. Заголовок бага («~1 ГБ на lenta.ru при одной загруженной картинке») больше
не описывает поведение движка, поэтому баг закрыт.

Два направления остаются **окончательно открытыми с обоснованием** и намеренно НЕ переводятся в
отдельный баг — по ним нечего чинить на уровне движка Lumen:
...
* **Пункт 2 — baseline пустого окна 224 МБ GPU + ~285 МБ неатрибуцированного GPU** (срез 16 счётчиком
  ...
  драйвер/swapchain/метод измерения). Не адресуемо без смены графического стека. Числа относятся к
  femtovg-пути; дефолтный wgpu-путь после среза 21 наблюдается через `LUMEN_MEM_REPORT`.
* **Пункт 3 (хвост) — cull `PushMaskLayer` (SVG-`<mask>` content)** — семантически возможен, но эмиттера
  такого контента в продакшене нет (срез 15), писать и проверять cull не над чем.

Если появится реальный эмиттер SVG-`<mask>` или сменится графический стек — заводить новый баг с
актуальными числами, а не переоткрывать этот.
