...
## Исправлено 2026-06-23 — D2-1 (per-pixel CPU fill + ellipse)

Per-box разбор TEST-39 (порог 16) показал: «256-тексельная квантизация» была НЕ
доминирующим фактором. Реальный состав диффа 1.62%:
- `.rg-ellipse` рисовался кругом — **69%** диффа (8927 px) → вынесено в **BUG-238 FIXED**.
- `.rep-radial`/`.rep-linear` hard-stop AA — ~28%.
- smooth radial интерполяция — устранялась квантизацией LUT.

Фикс квантизации: repeating linear/radial и **все** radial теперь рисуются
**per-pixel в device-разрешении** (`fill_gradient_texture` + `sample_fvg_stops`
в `femtovg_backend.rs`), минуя 256-тексельный LUT femtovg. Texture блитится через
`Paint::image`, удаляется после `flush` (`gradient_pending_delete`). Smooth-radial
теперь совпадают с Edge пиксель-в-пиксель; круги остаются кругами (BUG-221).
Суперсэмплинг опробован и **отвергнут** — блюрил тонкие кольца `.rep-radial`.

Проверено: TEST-39 1.62% → **1.18%**.

## Остаток (DEBTOR, 1.18%, KNOWN_DEBTORS '39')

1. **AA hard-stop повторяющихся градиентов**: `.rep-linear` (45° полосы) и
   `.rep-radial` (концентрические кольца) — Edge их сглаживает, Lumen рисует
   резко. Совпасть = AA-паритет (суперсэмплинг блюрит кольца → не годится без
   точного match Edge-кернела).
2. **gdigrab crop-edge шум** (~0.4%): верх/низ кадра (y0–40, y680–720) дают
   полноширинное расхождение магента-рамки — артефакт захвата, не градиент.
3. ≤0.5% по gdigrab недостижимо без (1) + font-parity (класс BUG-128).
