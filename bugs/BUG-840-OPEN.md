...
`step_func`-колбэка через событие `error` (см. [BUG-591](BUG-591-OPEN.md)).
Пока исключение съедено, любой провалившийся ассерт внутри колбэка
наблюдателя — не FAIL, а молчаливый TIMEOUT: тест просто перестаёт подавать
признаки жизни.
