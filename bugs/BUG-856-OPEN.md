...
**Родственный:** [BUG-772](BUG-772-OPEN.md) — тот же синхронный `connect`, увиденный через список «заблокированных портов» (92 порта × ~2.5 с реального `ECONNREFUSED` = ~230 с на файл). Здесь измерена **неограниченная** половина: если порт принимает соединение и не отвечает, ожидание не кончается никогда.
