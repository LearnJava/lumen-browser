...
**Статус:** OPEN — корень найден, срез 1 (пул offscreen-слоёв femtovg) влит 2026-07-07, срез 2 (blend-mode слой в пул) влит 2026-07-08; остаточные направления ниже
...
1. ~~Blend-слои (PREMULTIPLIED) вне пула~~ — закрыто срезом 2 (src-слой). Glyph atlas на тексте страницы — GPU на lenta всё ещё ~509 МБ против ~224 МБ baseline (~285 МБ страничных); требует того же счётчика GPU-памяти, срез 2 его не менял (lenta не использует blend-mode).
...
5. (Новое, срез 2) Blend-mode CPU-композитный `result_id` — one-off на каждый Pop, не пулябелен текущим механизмом; возможный источник неограниченного роста WS на blend-heavy страницах при периодических перерисовках. См. «Новое наблюдение» выше и BUG-273.
