...
Корректность: `cargo test -p lumen-paint --features backend-femtovg` 929+29 passed, 0 failed (+3 юнит-теста на `crop_region_rgba`: базовая вырезка, пустой/вырожденный регион, клампинг к границам буфера); `cargo clippy -p lumen-paint --all-targets --features backend-femtovg -- -D warnings` чист. Оконный `graphic_tests/run.py --only 30 --build` (LUMEN_BACKEND=femtovg) не завершён в рамках сессии (полная workspace-пересборка с нуля, включая V8, не уложилась в доступное время) — визуальная приёмка на TEST-30/103 (backdrop-filter) осталась не подтверждена gdigrab-прогоном, следующая сессия должна это сделать первым делом при продолжении этого бага. femtovg больше не дефолтный оконный бэкенд (им стал wgpu, `P1-wgpu-flip`), так что риск этого среза ограничен `LUMEN_BACKEND=femtovg`-сессиями.
...
3. Слои по bounding box вместо full-frame — **срез 5 (влит) сделал backdrop-filter's `filtered_backdrop_id` bbox-сайзингом**; остаётся: (a) визуально подтвердить срез 5 (gdigrab TEST-30/103, не сделано в рамках этой сессии — см. срез 5 выше), (b) `elem_image_id` у backdrop-filter (всё ещё full-framebuffer через `layer_pool`), (c) `PushOpacity`/`PushFilter`/`PushClipRoundedRect`/`PushClipPath`/`PushMask*`/`PushBlendMode` — все ещё full-framebuffer; `PushOpacity` первым делом требует добавить `bounds` в `DisplayCommand` (сейчас несёт только `alpha`), у остальных `bounds`/`rect` уже есть с BUG-273 среза 1.
