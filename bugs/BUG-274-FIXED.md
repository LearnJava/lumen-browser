...
**Статус:** FIXED 2026-07-08 (ветка p1-exp-wgpu-only) — корень диагностирован пофазным профилем, дефолт переключён на Vulkan
...
Фикс: `renderer.rs` — на Windows адаптер запрашивается сначала под Vulkan, при
неудаче — fallback DX12 (windowed и headless пути); `WGPU_BACKEND` env-var
по-прежнему переопределяет выбор. Диагностика оставлена: `LUMEN_FRAME_LOG=1`
печатает адаптер, `=2` — пофазную разбивку кадра + разбивку encode по типам
RenderPlanItem + счётчик созданных текстур.
