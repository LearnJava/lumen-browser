# BUG-834 — `history.back()` через границу документа замораживает страницу: обхода нет, запроса нет, `pageshow` нет, и таймеры текущего документа больше не срабатывают
