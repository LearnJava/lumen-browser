...
табов/переводов строки — это [BUG-693](BUG-693-FIXED.md), отдельная заявка.
...
- Смежное, но отдельное: у `location` те же компоненты объявлены как **обычные
  data-свойства** и присваивание им не навигирует — [BUG-376](BUG-376-FIXED.md),
  закрыт 2026-08-10 поверх этого среза: сеттеры `Location` делегируют запись
  временному объекту `URL`, то есть переиспользуют ре-сериализацию отсюда.
