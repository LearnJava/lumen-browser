...
не «больше времени всем»). На 48 случайных id из 1 173 отдаваемых напрямую:
восстановился **один** (`/fetch/range/non-matching-range-response.html`,
10.2 с TIMEOUT → 30.1 с `OK`) и **на нулевой балл** — 3 сабтеста, 0 PASS в
обоих армах. Сабтестов суммарно 122 против 122, PASS 5 против 5: за вшестеро
большее время гарнесс не добрал ничего. Верхняя граница занижения по правилу
трёх — **≤ 0.11 п.п.** Причина: 842 из 1 173 таких id грузят хелпер,
которого в чекауте нет (чаще всего `/common/subset-tests.js`), то есть ждут
не времени, а файла.

Обратная сторона осталась в силе и стала числом: после починки корпусный
прогон подорожает — те же 48 id стоили 11.6 мин в арме A и 48.4 мин в арме B
(**×4.2**), в пересчёте на 1 745 id это **+24.2 ч** тестового времени на
полный проход (≈4 ч настенных при `--processes 6`). Бюджет шарда, выведенный
