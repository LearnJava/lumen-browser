...
**Область:** `crates/js/src/dom.rs:9275` (`_mo_notify` — ровно 6 call site'ов, все внутри собственных обёрток шима: `setAttribute` :9347, `innerHTML` :9361, `appendChild` :9369, `removeChild` :9378, `textContent` :9393/:9398; вызывающих из `crates/shell/` нет ни одного), `crates/shell/src/main.rs` (парсерный путь построения дерева — JS-стороне о вставках не сообщает; та же граница, что у [BUG-804](BUG-804-FIXED.md))
...
`load`/`error` ([BUG-804](BUG-804-FIXED.md)). Отсюда и симметрия симптомов:
