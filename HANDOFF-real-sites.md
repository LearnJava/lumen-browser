...
### Баг A — ИСПРАВЛЕН 2026-06-15 (коммит d7c16d6f)
**Гипотеза про preloader оказалась неверной.** Реальная причина: контейнер-обёртка
страницы — `display:flex; flex-direction:column`, и `lay_out_flex` включал
`position:absolute/fixed` детей в flex-items. Они сдвигали курсор главной оси
(absolute-шапка 90px + fixed-оверлеи 380+174px), а ветка flex в `lay_out` делала
ранний `return` до обработки `abs_deferred`, поэтому inset'ы abs-детей вообще
игнорировались. Контент уезжал на y≈1004.
Фикс (`crates/engine/layout/src/box_tree.rs`): `lay_out_flex` исключает abs/fixed из
`item_idxs`; ветка flex после раскладки зовёт `lay_out_abs_children`. Теперь контент
с y=340 (шапка 90 + рекламный баннер 250), новости видны над сгибом 720px
(подтверждено скриншотом). Регресс-тест `flex_abs_child_does_not_advance_main_axis`.
`--dump-layout` теперь печатает `position=` (диагностика).

Остаётся (низкий приоритет): рекламный баннер 1200×250 на y=90 — пустое место
(трекер-домен заблокирован), как в браузере при блокировке рекламы.

### (исходное описание, для истории) контент начинается с y≈1004, ниже сгиба 720px
